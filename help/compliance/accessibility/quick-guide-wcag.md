...
「[How To Meet WCAG 2.1](https://www.w3.org/WAI/WCAG21/quickref/)」ページには、ガイドラインに準拠するための方法を示す節があります。この節では、WCAG が別の形式で示されており、読者が自身の関心や環境に合わせてガイドラインの内容を絞り込むことができます。Readerは、カスケーディングスタイルシートやスクリプティングなどの特定の Web コンテンツテクノロジーを指定したり、特定の優先度を指定したりすることで、表示する達成基準手法をフィルタリングできます。
