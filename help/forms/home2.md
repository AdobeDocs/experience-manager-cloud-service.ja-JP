...
**ドキュメントストレージ**
生成されたドキュメントとフォーム送信をAEMの Digital Asset Management System に格納するか、[SharePoint](/help/forms/configure-submit-action-sharepoint.md)、{OneDrive[、{6](/help/forms/configure-submit-action-onedrive.md)Azure Blob Storage[ などの外部ドキュメントリポジトリと統合します。](/help/forms/configure-submit-action-azure-blob-storage.md)
