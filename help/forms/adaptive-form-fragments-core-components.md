...
1. フラグメントのタイトル、名前、説明、タグを指定します。フラグメントには一意の名前を指定してください。同じ名前の別のフラグメントが存在する場合、そのフラグメントは作成できません。
...
アダプティブフォーム内で、なしベースのフォームフラグメントとスキーマベースのフォームフラグメントを複数回使用して、各フォームフラグメントフィールドに対して一意にデータを保存できます。 例えば、住所フォームフラグメントを使用して、住所の詳細を、ローン申し込みフォームで永住住所、コミュニケーションおよび現住所に関する情報を収集できます。
