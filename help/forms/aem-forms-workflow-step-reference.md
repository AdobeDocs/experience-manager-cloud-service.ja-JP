...
* **[!UICONTROL 優先度]**：選択した優先度が AEM インボックスに表示されます。使用可能なオプションは、「高」、「中」および「低」です。デフォルト値は「中」です。
...
* **[!UICONTROL 時間]**：タスクを完了するまでの時間数。この時間数は、タスクがユーザーに割り当てられた後にカウントされます。タスクが完了せず、「時間」フィールドに指定された時間数を過ぎると、期限後にタイムアウトハンドラー（選択した場合）が呼び出されます。
...
* **[!UICONTROL ルートタイトル]**：ルートのタイトルを指定します。タイトルは AEM インボックスに表示されます。
* **[!UICONTROL Coral アイコン：]** Coral アイコンの HTML 属性を指定します。Adobe CoralUI ライブラリでは、多数のタッチファーストなアイコンを提供します。ルートのアイコンを選択して使用することができます。アイコンは、タイトルと共に AEM インボックスに表示されます。変数にルートを格納した場合、ルートにはデフォルトの「タグ」Coral アイコンが使用されます。
...
PDF/A は、フォントを埋め込み、ファイルを圧縮解除することで、ドキュメントのコンテンツを長期保存するためのアーカイブ形式です。 その結果、通常、PDF/A ドキュメントは標準の PDF ドキュメントよりも大きくなります。以下を使用して、 ***PDF/A に変換*** AEM Workflow のステップを使用して、PDFドキュメントをPDF/A 形式に変換します。

「PDF/A に変換」の手順には、次のプロパティがあります。

**[!UICONTROL 入力ドキュメント]**:入力ドキュメントは、ペイロードを基準とした相対パス、絶対パス、ペイロードとして提供したり、Document データ型の変数に格納したりできます。

**[!UICONTROL 変換オプション]**:このプロパティを使用して、PDFドキュメントをPDF/A ドキュメントに変換する設定を指定します。 このタブで使用できる様々なオプションを次に示します。
* **[!UICONTROL 準拠]**:出力PDF/A ドキュメントが準拠する必要がある標準を指定します。
* **[!UICONTROL 結果レベル]**:変換出力の結果レベルを PassFail、Summary、または Detailed と指定します。
* **[!UICONTROL カラースペース]**:出力PDF/A ファイルに使用する事前定義済みのカラースペースを指定します。
* **[!UICONTROL オプションコンテンツ]**:指定した基準セットを満たした場合にのみ、特定のグラフィックオブジェクトや注釈を出力PDF/A ドキュメントに表示できます。
...
Document Description XML (DDX) は、宣言的なマークアップ言語です。この言語の要素は、ドキュメントの構築ブロックを表します。 これらの構築ブロックには、PDFおよび XDP ドキュメントと、コメント、しおり、スタイル設定されたテキストなどのその他の要素が含まれます。 DDX は一連の操作を定義します。1 つ以上の入力ドキュメントに適用して、1 つ以上の出力ドキュメントを生成することができます。  1 つの DDX を様々なソースドキュメントと共に使用できます。 以下を使用して、 ***DDX を起動ステップ*** AEM Workflow での様々な操作 ( ドキュメントの分解、Acrobatと XFA Formsの作成と変更、およびその他の説明 ( [DDX リファレンスドキュメント](https://helpx.adobe.com/content/dam/help/en/experience-manager/forms-cloud-service/ddxRef.pdf).

「Invoke DDX」ステップには次のプロパティがあります。

**[!UICONTROL 入力ドキュメント]**:入力ドキュメントのプロパティを設定するために使用します。 このタブで使用できる様々なオプションを次に示します。
* **[!UICONTROL 次を使用して DDX を指定]**:ペイロードを基準とした入力ドキュメント、絶対パス、ペイロードとして提供する、または Document データ型の変数に格納できる入力ドキュメントを指定します。
* **[!UICONTROL ペイロードからマップを作成]**:Assembler での呼び出し API 用に、ペイロードフォルダーの下にあるすべてのドキュメントを Input Document の Map に追加します。 各ドキュメントのノード名は、マップのキーとして使用されます。
* **[!UICONTROL 入力ドキュメントのマップ]**:オプションは、 **[!UICONTROL 追加]** 」ボタンをクリックします。 各エントリは、マップ内のドキュメントのキーとドキュメントのソースを表します。

**[!UICONTROL 環境オプション]**:このオプションは、呼び出し API の処理設定を設定するために使用されます。 このタブで使用できる様々なオプションを次に示します。
* **[!UICONTROL 検証のみ]**:入力 DDX ドキュメントの有効性を確認します。
* **[!UICONTROL エラー時に失敗]**:エラーが発生した場合に、呼び出し API サービスが失敗するかどうかを確認します。 デフォルトでは、この値は False に設定されています。
* **[!UICONTROL 最初の通し番号]**:自己増分する数を指定します。 この自己増分値は、連続する各ページに自動的に表示されます。
* **[!UICONTROL デフォルトのスタイル]**:出力ファイルのデフォルトのスタイルを設定します。

>[!NOTE]
>
>環境オプションは、HTTP API と同期が維持されます。

**[!UICONTROL 出力ドキュメント]**:出力ファイルを保存する場所を指定します。 このタブで使用できる様々なオプションを次に示します。
* **[!UICONTROL 出力をペイロードに保存]**:ペイロードフォルダーの下に出力ドキュメントを保存するか、ペイロードがファイルの場合はペイロードを上書きします。
...
**[!UICONTROL サンプル CustomerDetails テーブル]**
