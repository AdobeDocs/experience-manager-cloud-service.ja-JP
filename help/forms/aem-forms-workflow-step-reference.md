...
* **[!UICONTROL 準拠]**:出力PDF/A ドキュメントが準拠する必要がある標準を指定します。 PDF/A-1b、PDF/A-2b、PDF/A-3b など、様々なPDF標準をサポートします。
* **[!UICONTROL 結果レベル]**:変換出力の結果レベルを PassFail、Summary、または Detailed と指定します。
* **[!UICONTROL カラースペース]**:事前に定義されたカラースペースを、出力PDF/A ファイルに使用できる S_RGB、COATED_FOGRA27、JAPAN_COLOR_COATED、SWOP のいずれかとして指定します。
* **[!UICONTROL オプションコンテンツ]**:指定した基準のセットを満たした場合にのみ、特定のグラフィックオブジェクトや注釈を出力PDF/A ドキュメントに表示できるようにします。
...
* **[!UICONTROL 次を使用して DDX を指定]**:ペイロードを基準とした入力ドキュメントを指定します。絶対パスを持つか、ペイロードとして提供するか、Document データ型の変数に格納できます。
...
* **[!UICONTROL エラー時に失敗]**:エラーの場合に呼び出し API サービスが失敗するかどうかを示すブール値。 デフォルトでは、この値は False に設定されています。
