...
## フォームデータモデル（FDM）サービスを呼び出しステップ {#invoke-form-data-model-service-step}

[[!DNL AEM Forms]  のデータ統合](data-integration.md)機能により、複数の異なるデータソースを設定して接続することができます。これらのデータソースには、web サービス、REST サービス、OData サービス、CRM ソリューションがあります。[!DNL AEM Forms] データ統合を使用すると、様々なサービスを実行するフォームデータモデル（FDM）を作成できます。例えば、構成されたデータベースに対して、データの取得、追加、更新を実行できます。 **[!UICONTROL データモデルサービスの呼び出しステップ]**&#x200B;を使用して、フォームデータモデル（FDM）を選択し、FDM のサービスを使用できます。例えば、各種データソースの取得、更新、追加を行うことができます。
...
フォームデータモデル（FDM）の呼び出しサービス手順には、フォームデータモデル（FDM）操作を容易にする以下のフィールドがあります。
...
* **[!UICONTROL サービス]**：選択したフォームデータモデル（FDM）のサービスのリストです。
* **[!UICONTROL サービスの入力]**／**[!UICONTROL リテラル、変数またはワークフローメタデータおよび JSON ファイルを使用して入力データを指定]**：サービスには複数の引数を持たせることができます。ワークフローメタデータプロパティ、JSON オブジェクト、変数からサービス引数の値を取得するか、用意されたテキストボックスに直接値を入力するには、このオプションを選択します。
   ...
   * **[!UICONTROL 入力 JSON からのマップ入力フィールド]**：JSON ファイルのパスを指定して、その JSON ファイルから一部のサービスの引数の入力値を取得します。JSON ファイルのパスは、ペイロードとの相対パス、絶対パスにするか、JSON またはフォームデータモデル（FDM）タイプの変数を使用して入力 JSON ドキュメントを選択できます。
...
* **[!UICONTROL 以下のオプションを使用して出力 JSON ドキュメントを保存する]**：出力 JSON ファイルを保存します。出力 JSON ファイルのパスは、ペイロードに対する相対パスまたは絶対パスのどちらでもかまいません。また、JSON またはフォームデータモデル（FDM）データタイプの変数を使用して出力 JSON ファイルを保存することもできます。
