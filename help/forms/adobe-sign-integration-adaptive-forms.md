...
例えば、毎日午前 00:00 に設定サービスを実行するには、**Adobe Acrobat Sign 設定サービス**&#x200B;設定の **sign.status.exp** プロパティを `0 0 0 1/1 * ? *` に指定します。次の JSON ファイルは、設定サービスを毎日午前 00:00 に実行するサンプルが表示されています。
