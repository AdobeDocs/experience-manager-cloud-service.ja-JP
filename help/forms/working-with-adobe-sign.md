...
1. [Adobe Sign契約書のドキュメント有効期限の設定](#set-document-expiration-for-an-adobe-sign-agreement)
...
![文書の有効期限（日数）と受信者の設定を含む電子サインの設定](assets/signer_details_new.png)
...
1. 「**[!UICONTROL 受信者の署名順序]**」ドロップダウンから、署名順序を選択します。 受信者は、アダプティブフォームを&#x200B;**[!UICONTROL 連続]**&#x200B;して（指定順に）署名することも、**[!UICONTROL 同時]**&#x200B;に（順不同で）署名することもできます。
   ...
   順番に署名する場合は、Adobe Sign 契約書は一度に 1 人の受信者に送られます。 割り当てられたアクションを受信者が完了すると、契約書が次の受信者に送信されます。以下、同様の処理が繰り返されます。
...
1. コンテンツブラウザーで「**[!UICONTROL フォームコンテナ]**」を選択し、「**[!UICONTROL 設定]**」![設定](assets/Smock_Wrench_18_N.svg) アイコンを選択します。
1. このプロパティブラウザーで「**[!UICONTROL 電子サイン]**」アコーディオンを展開し、「**[!UICONTROL Adobe Sign を有効にする]**」オプションを選択します。
1. 「**[!UICONTROL ドキュメント有効期限（日数）]**」フィールドで、受信者が署名を完了する必要がある日数を指定します。

1. 「完了![保存](assets/save_icon.svg)」アイコンを選択して、変更を保存します。

>[!NOTE]
>
> **[!UICONTROL ドキュメント有効期限（日数）]** フィールドはオプションです。 空白のままにすると、契約書の有効期限は切れません。
...
1. このプロパティブラウザーで「**[!UICONTROL 電子サイン]**」アコーディオンを展開し、「**[!UICONTROL Adobe Sign を有効にする]**」オプションを選択します。 この操作により、アダプティブフォームに対して [!DNL Adobe Sign] が有効になります。
1. 「**[!UICONTROL 受信者を追加]**」を選択します。アダプティブフォームに受信者が追加されます。複数の受信者をアダプティブフォームに追加できます。アダプティブフォームの送信時に、すべての受信者がAdobe Sign契約書を受け取ります。
...
1. **[!UICONTROL 編集]**&#x200B;アイコン（![編集](assets/Smock_Edit_18_N.svg)）をクリックして、受信者に関する以下の情報を指定します。
   ...
   * **[!UICONTROL 受信者とフォームに入力する人は同じですか？]:** フォームの入力者と最初の受信者が同じ人の場合は、**[!UICONTROL はい]**&#x200B;を選択します。<!-- If the option is set to **No,** then do not use the signature step component in the Adaptive Form. If the form contains a Signature Step component, then the field is automatically set to Yes. -->
...
これで、フォームに署名する準備が整いました。 フォームのプレビューを表示して、署名エクスペリエンスを確認できます。 受信者がメールで署名用のフォームを受信すると、公開済みフォーム上に [!DNL Adobe Sign] ブロックのフィールドが表示されます。 **[!UICONTROL いつ受信者とフォームに入力する人は同じですか？]** オプションが「はい」とマークされ、条件が満たされると、ユーザーは送信後にAdobe Sign契約書にリダイレクトされ、ユーザーは契約書が電子メールに表示されるのを待つのではなく、すぐに文書に署名できます。
...
**Q:** アダプティブフォームを別のアダプティブフォームに埋め込むことができます。埋め込みアダプティブフォームを[!DNL Adobe Sign]有効にできますか？
**Ans:**&#x200B;いいえ、[!DNL Adobe Sign]が有効なアダプティブフォームを埋め込むアダプティブフォームの使用は、Experience Manager Formsではサポートされていません

**Q:**&#x200B;高度なテンプレートを使用してアダプティブフォームを作成し、編集のために開くと、「電子サインまたは受信者が正しく設定されていません」というエラーメッセージが表示されます。エラーメッセージを解決するには？
詳細テンプレートを使用して作成された&#x200B;**Ans:** アダプティブフォームは、[!DNL Adobe Sign]を使用するように設定されています。このエラーを解決するには、[!DNL Adobe Sign] クラウド設定を作成して選択し、アダプティブフォームの[!DNL Adobe Sign]受信者を設定します。

**Q:** アダプティブフォームの静的テキストコンポーネントで[!DNL Adobe Sign] テキストタグを使用できますか？
**Ans:**&#x200B;はい、テキストコンポーネントでテキストタグを使用して、レコードのドキュメントに[!DNL Adobe Sign] フィールドを追加できます（自動生成されたレコードのドキュメントのオプションのみ）有効なアダプティブフォーム。テキストタグを作成する手順とルールについて詳しくは、[Adobe Sign ドキュメント &#x200B;](https://helpx.adobe.com/jp/sign/using/text-tag.html)を参照してください。また、アダプティブFormsでは、テキストタグのサポートが限定的です。テキストタグを使用して、[Adobe Sign Block](working-with-adobe-sign.md#configure-cloud-signatures-for-an-adaptive-form)がサポートするフィールドのみを作成できます。
