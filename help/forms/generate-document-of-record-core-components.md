...
**基本テンプレート：** Forms Designer または Acrobat フォーム（AcroForm）で作成された XFA テンプレート（XDP ファイル）。[基本テンプレート](#base-template-of-a-document-of-record)は、レコードのドキュメントのスタイル設定とブランディング情報を指定するために使用されます。以前に、XFA テンプレート（XDP ファイル）をAEM Formsインスタンスにアップロードします。
...
これで、XDP ファイルをレコードのドキュメントのテンプレートとして使用するように、アダプティブフォームが設定されました。次の手順は、 [アダプティブフォームコンポーネントを、対応するテンプレートフィールドにバインドする](#bind-adaptive-form-components-with-template-fields).
...
これで、AcroForm をレコードのドキュメントのテンプレートとして使用するように、アダプティブフォームが設定されました。次の手順は、 [アダプティブフォームコンポーネントを、対応するテンプレートフィールドにバインドする](#bind-adaptive-form-components-with-template-fields).
...
「レコードのドキュメント」テンプレートを変更し、変更したレコードのドキュメントテンプレートをAEM Formsにアップロードすると、アダプティブFormsエディターによって変更されたバインディングが自動的に検出され、新しいバインディングが必要なアダプティブフォームコンポーネントが通知されます。 これにより、レコードのドキュメントテンプレートを増分更新できます。
...
* レコードのドキュメントのテンプレートは、リッチテキストをサポートしません。したがって、静的アダプティブフォーム内またはユーザーが入力した情報内のリッチテキストは、レコードのドキュメント内にプレーンテキストとして表示されます。
