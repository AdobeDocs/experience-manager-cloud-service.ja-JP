...
はい。AEM Formsでは、レコードのドキュメント（DoR）の生成がサポートされており、保険会社は送信されたフォームデータに基づいて PDF とレコードを生成できます。
...
* **レコードのドキュメントから非表示フィールドを除外**：このプロパティを選択すると、レコードのドキュメントから非表示フィールドが除外されます。すべてのフォームフィールドに適用されます。デフォルトでは、「**レコードのドキュメントから非表示フィールドを除外**」オプションは選択されていません。
