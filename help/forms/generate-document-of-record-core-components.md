---
title: アダプティブForms用の送信PDF（以前のレコードのドキュメント）の生成
description: アダプティブPDFコアコンポーネントのフォーム送信から送信Formsを生成する方法を説明します。 アーカイブまたは参照用に、送信されたフォームのPDFを作成します。
...
「メールの送信」、「AEM ワークフローの呼び出し」、「Power Automate フローの呼び出し」、およびその他の [&#x200B; 送信アクション &#x200B;](configuring-submit-actions.md) などの送信アクションを使用して、送信PDFを受け取ることができます。
![送信アクションの画像](/help/forms/assets/submit-actions-img.png)


>[!NOTE]
>
> **[!UICONTROL レコードのドキュメント連結参照フィールド]** プロパティを使用すると、任意のフォームデータモデルの送信PDFを保存できます。
...
1. デフォルトのテンプレートとカスタムのテンプレートのどちらを選択するかに応じて、以下のプロパティの一部またはすべてのプロパティが「レコードのドキュメント」タブに表示されます。以下のプロパティを指定して、送信PDFの外観を定義します。
   ...
   1. **フォームフィールドのプロパティ**:
      ...
      **複数選択ドロップダウンのラベルを表示**
      ...
      送信PDFに、内部格納された値ではなく、複数選択ドロップダウンコンポーネント用に選択された表示ラベルが表示されるようになりました。 例えば、ユーザーがドロップダウンから「California」と「New York」を選択すると、送信PDFには、`CA` や `NY` などの内部値ではなく、選択されたラベルが表示されます。 選択した各オプションは、[&#x200B; 基盤コンポーネントベースのアダプティブForms](/help/forms/generate-document-of-record-for-non-xfa-based-adaptive-forms.md) の動作と一致するように、コンマ区切り値ではなく別の行に表示されます。
