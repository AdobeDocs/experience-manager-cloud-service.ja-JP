...
* [Submission PDFにロケール固有のカスタム XDP テンプレートを使用する](#locale-specific-custom-xdp-templates-for-document-of-record)
...
* [AEM Sites ページに埋め込まれたフォームに対する送信PDFの設定](#configure-document-of-record-for-forms-embedded-in-aem-sites)
...
<span class="preview"> AEM Sites ページに埋め込まれたフォーム用の送信PDF設定と、レコードのドキュメント用のロケール固有のカスタム XDP テンプレートは、アーリーアダプタープログラムの下にあります。 公式メール IDからaem-forms-ea@adobe.comにメールを送信して、アーリーアダプタープログラムに参加し、これらの機能へのアクセスをリクエストできます。</span>

カスタム XFA テンプレート（XDP ファイル）を送信PDF テンプレートとして関連付ける場合は、テンプレートのロケール固有のバージョンを指定できます。 AEM Formsは、送信PDFを生成する際に、フォームロケールに基づいて適切なXDPを自動的に選択します。

### ロケール固有のテンプレート選択の仕組み

1. デフォルトのXDP テンプレートをAEM Forms インスタンスにアップロードします。 （例：`a.xdp`）。
1. ローカライズされたテンプレートを提供するには、命名規則`basename.<locale>.xdp`を使用して、ロケール固有のXDP ファイルを作成し、同じフォルダーにアップロードします。 次に例を示します。

   * `a.xdp` — デフォルトテンプレート
   * `a.fr.xdp` — フランス語のロケールテンプレート
   * `a.de.xdp` — ドイツ語のロケールテンプレート

1. アダプティブフォームを&#x200B;**レコードのドキュメントのテンプレートとしてフォームテンプレートを関連付ける**&#x200B;ように設定し、デフォルトのXDP ファイル （例：`a.xdp`）を選択します。
1. 送信PDFが生成されると、AEM Formsはフォームのロケールを使用してテンプレートを解決します。 フランス語のロケールフォームの場合、システムは存在する場合は`a.fr.xdp`を使用します。それ以外の場合は、デフォルトの`a.xdp`にフォールバックします。

### ワークフローの例

1. **[!UICONTROL Forms]** > **[!UICONTROL Formsとドキュメント]**&#x200B;で、`a.xdp`と`a.fr.xdp`を同じフォルダー（例：`/content/dam/formsanddocuments/testlang/`）にアップロードします。
1. フォームの&#x200B;**[!UICONTROL レコードのドキュメント]**&#x200B;設定を設定して、`a.xdp`をテンプレートとして関連付けます。
1. ユーザーがフランス語版のフォームに入力し、送信PDFを生成またはダウンロードすると、PDFはフランス語のXDP テンプレートからコンテンツを使用します（例えば、テンプレートで定義されたローカライズされたヘッダーテキストやフィールドラベル）。
1. 送信PDFの添付ファイル（**[!UICONTROL メールを送信]**&#x200B;など）を含む送信アクションには、ロケールに適したPDFも含まれます。

>[!NOTE]
>
> ロケール固有のカスタム XDP テンプレートのサポートは、フォームテンプレートをレコードのドキュメント テンプレートとして関連付ける場合に適用されます。 自動生成された送信PDFのローカライズについては、[機械翻訳または人による翻訳を使用して、コアコンポーネント ベースのアダプティブフォームを翻訳するを参照してください](/help/forms/using-aem-translation-workflow-to-localize-adaptive-forms-core-components.md)。
...
![画像の送信アクション &#x200B;](/help/forms/assets/submit-actions-img.png)
...
**[!UICONTROL アダプティブフォームコンテナ]** コンポーネントを使用して、AEM Sites ページに埋め込まれたアダプティブ Formsの送信PDF（レコードのドキュメント）を設定して生成できます。 作成者は、Forms エディターでフォームを開くことなく、サイトページで直接DoR設定を行うことができます。

### 事前準備

* アダプティブフォームは、AEM Sites ページまたはエクスペリエンスフラグメントの&#x200B;**[!UICONTROL アダプティブフォームコンテナ]** コンポーネントを使用して作成されます。 詳しくは、[AEM Sites ページまたはエクスペリエンスフラグメントへのアダプティブフォームの追加](/help/forms/create-or-add-an-adaptive-form-to-aem-sites-page.md)を参照してください。
* アダプティブ Forms クライアントライブラリがSites ページテンプレートに追加されます。

### Sites ページでの送信PDFの設定

1. 埋め込まれたフォームを含むAEM Sites ページまたはエクスペリエンスフラグメントを編集モードで開きます。
1. ページの&#x200B;**[!UICONTROL アダプティブフォームコンテナ]** コンポーネントを選択し、![設定](assets/configure-icon.svg) アイコンをクリックします。
1. 「**[!UICONTROL レコードのドキュメント]**」タブを開きます。
1. 次のいずれかのオプションを選択します。

   * **なし** – 送信PDFを生成しません。
   * **レコードのドキュメントを生成** – 送信PDFを自動的に生成します。
   * **フォームテンプレートをレコードのドキュメントのテンプレートとして関連付ける** — カスタム XDPまたはAcroForm テンプレートを使用します。 AEM Forms インスタンスからテンプレートを参照して選択します。

1. （オプション）「**レコードのドキュメントからファイル添付ファイルを除外**」を選択して、送信PDFからアップロードされた添付ファイルを除外します。
1. 「**[!UICONTROL 完了]**」をクリックします。

### レコードのドキュメントのプロパティのカスタマイズ

Submission PDF generationを有効にすると、Sites ページでブランディングとレイアウトをカスタマイズできるようになります。

1. **[!UICONTROL アダプティブフォームコンテナ]**&#x200B;で、埋め込まれたフォームのルートパネルを選択します。
1. プロパティのサイドバーで、「**[!UICONTROL レコードのドキュメント]**」タブを開きます。
1. **[!UICONTROL 基本]**、**[!UICONTROL フォームフィールドのプロパティ]**&#x200B;および&#x200B;**[!UICONTROL マスターページのプロパティ]** タブでオプションを設定します。 各プロパティについて詳しくは、[Submission PDF](#customize-the-branding-information-in-document-of-record)の「ブランディング情報のカスタマイズ」を参照してください。
1. 「**[!UICONTROL 完了]**」をクリックします。

### 送信PDFのプレビューとダウンロード

* サイトページでフォームをプレビューして送信するか、[&#x200B; ルールエディター](/help/forms/rule-editor-enhancements-use-cases.md#download-document-of-record)を使用して&#x200B;**DoR**&#x200B;をダウンロード ボタンを追加して、オンデマンドで送信PDFを生成します。
* 埋め込みフォームで設定された送信アクション（**[!UICONTROL メール送信]**&#x200B;または&#x200B;**[!UICONTROL AEM ワークフローの呼び出し]**&#x200B;など）には、生成された送信PDFを含めることができます。

>[!NOTE]
>
> Sites ページに埋め込まれたフォームの送信PDFの生成は、**[!UICONTROL アダプティブフォームコンテナ]** コンポーネントのコアコンポーネントで作成されたアダプティブ Formsでサポートされています。 **[!UICONTROL アダプティブForms – 埋め込み（v2）]** コンポーネントを使用して追加されたフォームの場合は、Forms エディターでDoRを設定します。
...
* 送信PDFのフィールドにスタイルを適用するには、基本テンプレートで、`AF_METATEMPLATE` ルートサブフォームの下の`AF_FIELDSSUBFORM` サブフォームにあるフィールドが提供されます。
...
1. デフォルトのテンプレートとカスタムのテンプレートのどちらを選択するかに応じて、以下のプロパティの一部またはすべてのプロパティが「レコードのドキュメント」タブに表示されます。 以下のプロパティを指定して、送信PDFの外観を定義します。
   ...
   1. **フォームフィールドのプロパティ**:
      ...
      **複数選択ドロップダウンのラベルを表示**
      ...
      <span class="preview">この機能は、早期アクセスプログラムを通じて使用できます。 アクセス権をリクエストするには、公式アドレスから [aem-forms-ea@adobe.com](mailto:aem-forms-ea@adobe.com) にメールを送信してください。</span>
...
* **レコードのドキュメントからタイトルを除外：** プロパティを設定すると、パネル/テーブルのタイトルが送信PDFから除外されます。 パネルおよびテーブルに対してのみ適用できます。
* **レコードのドキュメントから説明を除外：** プロパティを設定すると、パネル/テーブルの説明が送信PDFから除外されます。 パネルおよびテーブルに対してのみ適用できます。
