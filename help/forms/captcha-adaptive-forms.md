...
| 適用先 | 基礎コンポーネントに基づくアダプティブフォーム。<br> コアコンポーネントに基づくアダプティブフォームについては、[ここをクリック ](/help/forms/captcha-adaptive-forms-core-components.md)してください。 |
...
## OSGiでのreCAPTCHA クラウド設定の上書き {#override-recaptcha-osgi}

環境ごとに異なるプロジェクト ID、サイトキー、またはシークレットを使用するには、**[!UICONTROL Apache Sling Context-Aware Configuration Override Provider: OSGi configuration]**&#x200B;のOSGi設定を追加します。

### プロジェクトにOSGi オーバーライドを追加する {#override-recaptcha-steps}

1. AEM プロジェクト用にCloud Manager Git リポジトリを複製します。

   ```shell
   git clone <your-cloud-manager-repository-url>
   ```

1. 複製したリポジトリをテキストエディターで開きます。

1. アプリケーションの下の`ui.config` フォルダーに移動します（`<your-application-folder>`をプロジェクトの`/apps`の下のフォルダー名に置き換えます）。

   * **作成者：** `ui.config/src/main/content/jcr_root/apps/<your-application-folder>/osgiconfig/config.author`
   * **公開用：** `ui.config/src/main/content/jcr_root/apps/<your-application-folder>/osgiconfig/config.publish`

   >[!NOTE]
   >
   > フォルダー`osgiconfig`、`config.author`、`config.publish`が存在しない場合は、それらを作成します。

1. OSGi オーバーライドファイルを両方の実行モードフォルダーに作成し、それぞれに同じファイル名を使用します。

   * **作成者：**&#x200B;の`config.author` フォルダーで、`org.apache.sling.caconfig.impl.override.OsgiConfigurationOverrideProvider-integrationTest.cfg.json`を作成します。
   * **パブリッシュの場合：**、`config.publish` フォルダーに、`org.apache.sling.caconfig.impl.override.OsgiConfigurationOverrideProvider-integrationTest.cfg.json`を作成します。

1. 次のJSONを各ファイルに貼り付けます（または、必要に応じてオーサーコンテンツと公開コンテンツを別々に調整します）。 `overrides`の下のすべてのパスで、`<environment-name>`をreCAPTCHA Enterprise クラウド設定の名前に置き換えます。 [Sling オーバーライド構文](https://sling.apache.org/documentation/bundles/context-aware-configuration/context-aware-configuration-override.html#override-syntax)を使用します。
   ...
   >[!NOTE]
   >
   >`projectId`、`siteKey`および`secretKey`をCloud Managerの環境変数およびシークレットとして追加して、**作成者**、**プレビュー**、**公開**&#x200B;に適用します（**環境設定** ダイアログで&#x200B;**ステップ適用**&#x200B;を使用）。 Cloud Manager](/help/implementing/cloud-manager/environment-variables.md)の[環境変数を参照してください。

   ![Recaptcha オーバーライド ](/help/forms/assets/recaptcha-override.png)
...
1. このリポジトリをデプロイするCloud Manager パイプラインを実行します。 デプロイメント後、オーバーライドは、その環境に定義された変数とシークレットから`projectId`、`siteKey`、`secretKey`を提供します。
