...
* **ナビゲーション（イベント）:** ユーザーがナビゲーションオブジェクトをクリックすると、true が返されます。 ナビゲーションオブジェクトは、パネル間の移動に使用します。
...
切り替えボタンをタップすると、フォームオブジェクトと関数パネルが切り替わります。

### D. ビジュアルルールエディター {#visual-rule-editor}

ルールを記述するビジュアルエディターは、ルールエディターのユーザーインターフェイスをビジュアルエディターモードに切り替えると表示されます。ルールタイプを選択し、それに応じて条件とアクションを定義できます。 ルール内の条件とアクションを定義する際は、フォームオブジェクトと関数パネルから、フォームオブジェクトと関数をドラッグ＆ドロップすることができます。
...
ルールを保存するには、「**[!UICONTROL 完了]**」ボタンを押します。不完全なルールでも保存できます。ただし、不完全なルールは無効なため、実行されることはありません。フォームオブジェクトに保存されたルールは、同じフォームオブジェクトから次回ルールエディターを起動すると一覧表示されます。このビューでは、既存のルールを管理できます。詳しくは、[ルール管理](rule-editor.md#p-manage-rules-p)を参照してください。
...
ローン申し込みフォームの例の「ローン要件」セクションでは、配偶者の有無、給与、および配偶者の給与（既婚の場合）を申請者が指定する必要があります。ユーザー入力に基づいて、ルールはローン適格額を計算し、「貸付資格」フィールドに表示します。次のルールを適用して、シナリオを実装します。

* 「配偶者の給与」フィールドは、配偶者の有無が「既婚」の場合にのみ表示されます。
...
1. 式フィールドで、以下の操作を行います。
   ...
   * 最初の「**[!UICONTROL オブジェクトをドロップまたは次から選択]**」フィールドの「**[!UICONTROL 給与]**」フィールドで、「フォームオブジェクト」タブから選択またはドラッグ＆ドロップします。
...
1. 次に、式フィールドの周りの強調表示された領域をタップし、「**[!UICONTROL 拡張式]**」をタップします。
   ...
   >[!NOTE]
   >
   >「オプションを選択」フィールドから、コンポーネント、関数、数式、およびプロパティ値を使用して、複雑な式を作成することもできます。
...
>[!NOTE]
>
>代わりに、「When」ルール内で貸付資格を計算するために「次の値を設定」ルールを使用することもできます。このルールは、「配偶者の給与」フィールドの表示／非表示を切り替えるために作成したものです。「Marital Status （配偶者の有無）」が「Single （独身）」の場合、ルールエディターでは組み合わせルールが次のように表示されます。
>
...
* **プライベート**
構文：`@private`
プライベート関数は、カスタム関数に含まれていません。

* **名前**
構文：`@name funcName <Function Name>`
あるいは、`@function funcName <Function Name>` **または** `@func` `funcName <Function Name>` を使用できます。`,`
...
あるいは、`@argument` `{type} name <Parameter Description>` **または** `@arg` `{type}` `name <Parameter Description>` を使用できます。
...
あるフィールドから別の類似したフィールドにルールをコピー＆ペーストすると、時間を節約できます。
...
ルールエディターでは、複数の AND および OR 演算子を使用してネストされたルールを作成できます。 ルール内では複数の AND 演算子と OR 演算子を混在させることができます。
...
* 最初の行は反復可能です。このため、ユーザーは複数の製品を注文し、それぞれ異なる量を指定することができます。この要素の名前は、「`Row1`」です。
