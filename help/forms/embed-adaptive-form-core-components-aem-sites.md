...
* **既存のアダプティブフォームを埋め込む**:この [アダプティブForms — 埋め込み](#embed-existing-af) コンポーネントを使用すると、既存のアダプティブフォームをAEM Sites内のページに簡単に組み込むことができます。 例えば、 [!UICONTROL アダプティブForms — 埋め込み] コンポーネントをサイトのページに追加します（次のビデオを参照）。
...
* **アダプティブFormsウィザードを使用したフォームの作成**:
   ...
   以下を使用： [アダプティブForms — 埋め込み](#embed-new-af) フォーム作成ウィザードを使用してAEM Sitesエディター内でアダプティブフォームを作成するためのコンポーネントです。 フォームは外部エンティティとして保存されます。 このフォームは、他の Sites ページやスタンドアロンフォームでも再利用できます。
...
1. 次をクリック： [!UICONTROL ポリシー] をクリックし、 **コアコンポーネントの例 — アダプティブフォーム** チェックボックス。
...
## 既存のアダプティブフォームを埋め込む  {#aem-container-component}

使用 **[!UICONTROL アダプティブForms — 埋め込み]** コンポーネントを使用する場合は、新しいアダプティブフォームを埋め込むか、既存のアダプティブフォームをサイトのページに埋め込むことができます。 この [!UICONTROL アダプティブForms — 埋め込み] コンポーネントでは次のことが可能です。
...
有効にするには [!UICONTROL アダプティブForms — 埋め込み] テンプレートのポリシーのコンポーネントで、次の手順を実行します。
1. 次に移動： [!UICONTROL ページ情報] > [!UICONTROL テンプレートを編集]
1. 次をクリック： [!UICONTROL ポリシー] をクリックし、 **コアコンテンツ** チェックボックス。
1. 「[!UICONTROL 完了]」をクリックします。
...
この [!UICONTROL アダプティブフォーム — 埋め込み] コンポーネント。 内 [!UICONTROL アダプティブFormsを編集 — 埋め込み] ダイアログでは、次の項目を指定できます。
