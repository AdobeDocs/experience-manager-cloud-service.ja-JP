...
1. デフォルトのテンプレートとカスタムのテンプレートのどちらを選択するかにより異なりますが、以下のプロパティの一部または全部が「レコードのドキュメント」タブに表示されます。以下のプロパティを指定して、レコードのドキュメントの外観を定義します。

   1. **基本のプロパティ**:
      * **テンプレート**:カスタムテンプレートを選択する場合は、 [!DNL AEM Forms] サーバー。 使用する [!DNL AEM Forms] サーバーにないテンプレートを使用する場合は、最初に XDP を [!DNL AEM Forms] サーバーにアップロードする必要があります。
      ...
      * **レコードのドキュメントから非表示のフィールドを除外**:このプロパティを設定すると、レコードのドキュメントから除外する非表示フィールドが識別されます。
      * **パネルの説明を非表示にする**:このプロパティを設定すると、パネル/テーブルの説明がレコードのドキュメントから除外されます。 パネルおよびテーブルに適用できます。

      ![基本のプロパティ](/help/forms/assets/basicpropertiesdor.png)

   1. **フォームフィールドのプロパティ**:
      * **チェックボックスおよびラジオボタンのコンポーネントには、選択した値のみが表示されます**:このプロパティを設定すると、 [!UICONTROL レコードのドキュメント].
      * **複数の値の区切り記号**:複数の値を表示する場合は、カンマや改行などの区切り文字を選択できます。
      * **オプションの整列**:目的の配置（水平、垂直、アダプティブフォームと同じ）を選択して、表示するチェックボックスやラジオボタンなどのフィールドの配置を設定できます [!UICONTROL レコードのドキュメント]. デフォルトでは、 [!UICONTROL レコードのドキュメント]. 次の場所からプロパティを設定： [!UICONTROL フォームフィールドプロパティ] が DoR の場合、 [!UICONTROL 項目の整列] アダプティブフォームのフィールドの 例えば、 [!UICONTROL アダプティブフォームと同じ] 」オプションを使用する場合、アダプティブフォームのオーサーインスタンスで設定された整列が [!UICONTROL レコードのドキュメント] フィールド。
      * **水平方向の整列のオプション数**：レコードのドキュメントに表示する水平方向の位置揃えのオプションの数を設定できます。
...
次の項目を設定できます。 [!UICONTROL レコードのドキュメント] テンプレートを、アダプティブフォームエディターまたはアダプティブフォームテンプレートエディターから直接作成します。
...
1. を開きます。 **[!UICONTROL レコードのドキュメントテンプレート]** 「 」タブで、次のオプションを選択します。
   * **[!UICONTROL なし]**:このオプションを選択した場合、次のいずれもおこなわれません。 [!UICONTROL レコードのドキュメント] アダプティブフォーム用に作成されたテンプレート。

   * **[!UICONTROL フォームテンプレートをレコードのドキュメントテンプレートとして関連付ける]**：このオプションを選択すると、XFA フォームがレコードのドキュメントのテンプレートとして使用されます。

   * **[!UICONTROL レコードのドキュメントを生成]**:このオプションを選択した場合、 [!UICONTROL レコードのドキュメント] テンプレートがアダプティブフォーム用に自動的に生成されます。

1. 「![保存](/help/forms/assets/check-button.png)」をタップして、プロパティを保存します。

![レコードのドキュメントテンプレートのサポート](/help/forms/assets/dor-templatesupport.png)

>[!NOTE]
>
>条件 [!UICONTROL レコードのドキュメント] テンプレートがアダプティブフォームテンプレートエディターを使用して作成された場合、以下で使用できるオプションは 2 つだけです。 [!UICONTROL レコードのドキュメントテンプレート] タブとして [!UICONTROL なし] および [!UICONTROL レコードのドキュメントを生成].
