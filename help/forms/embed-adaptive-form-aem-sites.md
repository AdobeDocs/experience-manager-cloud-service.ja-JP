...
**[!UICONTROL アダプティブ Forms – 埋め込み（v2）]**&#x200B;を使用して、新しいアダプティブフォームまたは既存のアダプティブフォームの埋め込みを開始する前に、**アダプティブ Forms クライアントライブラリ**&#x200B;をAEM Sites ページに追加します。


### AEM Sites ページまたはエクスペリエンスフラグメントに、アダプティブフォームクライアントライブラリを追加

**[!UICONTROL フォームがページの幅全体をカバーする場合]** オプションが&#x200B;**[!UICONTROL フォームコンテナ]**&#x200B;設定ダイアログボックスで選択され、アダプティブFormsが使用されている場合、対応するサイトのページにクライアントライブラリを含める必要があります。

![&#x200B; ページオプションの幅全体をフォームでカバーし、アダプティブフォームが使用されている場合](/help/forms/assets/overlaycorecomponent.gif)
...
* **フォームはフレームの幅全体をカバーします**：インラインフレーム (iframe) は、アダプティブフォームをAEM Sitesページに読み込むHTML要素です。
   ...
   * 次の場合、 **[!UICONTROL フォームはフレームの幅全体をカバーします]** チェックボックスがオンの場合、アダプティブフォームは配置されるコンテナの全幅を占有します。 この場合、iframe はフォームのレンダリングには使用されません。 アダプティブフォームのレイアウトとデザインは、コンテナの幅全体に適応し、レスポンシブで、様々な画面サイズに調整できます。 このオプションを使用すると、AEM Sites ページ内に複数のアダプティブフォームを埋め込むことができます。
     ...
     **     >[!NOTE]
         >
         > AEM Sites ページに複数のフォームを埋め込むには、「[!UICONTROL &#x200B; フォームはフレームの幅全体をカバーする]**」チェックボックスを選択します。
   ...
   * 次の場合、 **[!UICONTROL フォームはフレームの幅全体をカバーします]** チェックボックスがオフの場合、アダプティブフォームはコンテナの幅全体をカバーしません。 代わりに、iframe を使用してフォームをレンダリングします。このフォームは、特定の幅を超えて拡張することはできません。 この方法は、アダプティブフォームに明確な境界があり、コンテナ内でアダプティブフォームの隣にある他の AEM コンポーネントと共存する必要がある場合に役立ちます。このオプションを選択しない場合、AEM Sitesページで iframe を使用せずに埋め込むアダプティブFormsは 1 つだけになります。
         ...
         >[!NOTE]
         >
         > AEM Sites ページでサポートされるアダプティブフォームは、iframeを使用しないアダプティブフォームは1つだけです。 **[!UICONTROL &#x200B; アダプティブForms – 埋め込み]** **&#x200B; コンポーネントを使用してさらにアダプティブFormsを追加するには、「[!UICONTROL &#x200B; フォームはフレームの幅全体をカバー]**」オプションを選択します。
...
* 編集モードで開いたサイトページ内からアダプティブフォームを選択し、「**[!UICONTROL 新しいウィンドウで編集]**」をクリックします。元のフォームは、修正可能な編集モードで開きます。
...
* [スタンドアロンフォームの作成](/help/forms/creating-adaptive-form-core-components.md)
* [AEM Sites ページでフォームを直接作成する](/help/forms/create-or-add-an-adaptive-form-to-aem-sites-page.md)
