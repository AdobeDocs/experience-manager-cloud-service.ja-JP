---
title: Forms Experience Builderのデプロイと設定
...
# Forms Experience Builderのデプロイと設定

>[!NOTE]
>
> Forms Experience Builderは、早期アクセスプログラムで利用できます。 開始する前に、アクセスをリクエストし、許可されていることを確認してください。 詳細な手順については、[ オンボーディング ](product-overview.md#onboarding)情報を参照してください。
...
### フォームが有効になっていることを確認する

Forms Experience Builder を使用する前に、[お使いの環境で AEM Forms が有効になっていること](/help/forms/setup-forms-cloud-service.md)を確認します。

### 環境の設定

設定プロセスは、AEM Formsの実装によって異なります。 プロジェクトに合ったパスを選択します。

**Edge Delivery Services用**

Edge Delivery Services Formsを使用しており、主にユニバーサルエディターを使用している場合。 [Edge Delivery Services Forms](/help/edge/docs/forms/universal-editor/getting-started-universal-editor.md)用にプロジェクトを準備します。 これは、Forms Experience Builderを有効にするための1回限りの設定です。

コアコンポーネントベースのフォームの&#x200B;****

AEM オーサー環境でコアコンポーネントに基づくアダプティブ Formsを使用している場合は、[ アダプティブ Forms コアコンポーネントが有効になっていることを確認してください](/help/forms/enable-adaptive-forms-core-components.md)。



## クイックスタート

### Forms Experience Builderへのアクセス

ワークフローやフォームの種類に応じて、3つの主要な場所からForms Experience Builderにアクセスできます。


**1. アダプティブ Forms エディター（コアコンポーネント用）**

特定のフォームの編集中にビルダーを直接起動できます。

1. **AEM/Forms/Formsとドキュメント**&#x200B;に移動します。
1. [ コアコンポーネントテンプレート ](/help/forms/creating-adaptive-form-core-components.md)を使用して新しいフォームを作成するか、既存のフォームを開きます。
1. エディターのツールバーの&#x200B;**Forms Experience Builder** アイコンを選択して、対話型インターフェイスを開きます。

   ![AI アシスタントアイコン*](/help/edge/docs/forms/assets/adaptive-forms-editor.gif){width="75%"}

**1. ユニバーサルエディター（Edge Delivery Services Forms用）**

Edge Delivery Services経由で配信されるフォームの場合、ビルダーはユニバーサルエディターに統合されます。

1. ユニバーサルエディターでEdge Delivery Services フォームを開きます。
2. 右側のパネルで&#x200B;**Forms Experience Builder** アイコンを選択して、対話型インターフェイスを起動します。
