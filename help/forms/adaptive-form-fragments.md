...
>[!NOTE]
>
> * アダプティブフォーム内で何度もベースでないフォームフラグメントを使用している場合、フラグメントのフィールド間でデータ同期がおこなわれます。 1 つの [フォームフラグメント（コアコンポーネントに基づく）](/help/forms/adaptive-form-fragments-core-components.md)  フォーム内で複数回呼び出します。 データの同期に関する問題を発生させることなく、なしベースのフォームフラグメントとスキーマベースのフォームフラグメントの両方をサポートします。
