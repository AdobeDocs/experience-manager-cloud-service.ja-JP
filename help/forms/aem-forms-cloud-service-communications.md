...
* **基本認証**：基本認証は、HTTP プロトコルに組み込まれたシンプルな認証スキームです。クライアントは、Basicという単語の後にスペースとbase64でエンコードされた文字列username:passwordが続くAuthorization ヘッダーでHTTP リクエストを送信します。 例えば、管理者／管理者として認証するために、クライアントは Basic [base64 エンコードされたユーザー名文字列]: [base64 でエンコードされたパスワード文字列]を送信します。
...
>[!NOTE]
>
> AEM Forms Communication APIを呼び出すための詳細な手順については、「[OAuth サーバー間の認証を使用したAEM Forms Communications APIの呼び出し](/help/forms/aem-forms-cloud-service-communications-on-demand-processing.md)」を参照してください。
