...
# AEM AssetsとAEM mediaLibraryのFAQ {#aem-assets-vs-aem-medialibrary}
...
AEM Assets は、AEM プラットフォーム上のアプリケーションです。お客様はこのアプリケーションを使用して、Web ベースのリポジトリ内でデジタルアセット（画像、ビデオ、ドキュメントおよびオーディオクリップ）を管理できます。AEM Assetsには、メタデータのサポート、レンディション、デジタルアセット管理ファインダー、およびユーザーインターフェイスを介した管理が含まれています。
...
1. デジタルアセット管理に関連するすべてのワークフローステップ（AEM Assets 取得、AEM Assets 削除、AEM Assets サブアセット操作、AEM Assets メタデータ抽出）.
1. パッケージ領域の「dam」に含まれるライブラリ.
