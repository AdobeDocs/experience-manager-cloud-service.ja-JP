---
title: 検索ファセット。
description: この記事では、Experience Managerで検索ファセットを作成、変更、使用する方法について説明します。
feature: 検索、メタデータ
role: User,Admin
...
Adobe Experience Manager Assetsの企業全体のデプロイメントには、多数のアセットを格納できます。 Experience Managerの一般的な検索機能のみを使用する場合は、適切なアセットの検索が困難で時間がかかることがあります。
...
1. Experience Managerのロゴをクリックし、**[!UICONTROL ツール]** / **[!UICONTROL 一般]** / **[!UICONTROL Formsを検索]**&#x200B;に移動します。
...
1. Experience Managerのロゴをタップし、**[!UICONTROL ツール/一般/Formsを検索]**&#x200B;に移動します。
...
`Multi Value Property` の述語では、複数の値でアセットを検索できます。[!DNL Assets]に複数の製品の画像があり、各画像のメタデータに製品に関連付けられたSKU番号が含まれているシナリオを考えてみましょう。 この述語を利用すれば、複数の SKU 番号で製品の画像を検索できます。
...
`Tags` の述語では、タグに基づいたアセットの検索が実行できます。デフォルトでは、[!DNL Assets]は指定したタグに基づいて、1つ以上のタグに一致するアセットを検索します。 言い換えれば、検索クエリは指定したタグを用いて OR 演算を実行します。ただし、「すべてのタグに一致」オプションを使用すれば、すべての指定したタグを含むアセットを検索することも可能です。
