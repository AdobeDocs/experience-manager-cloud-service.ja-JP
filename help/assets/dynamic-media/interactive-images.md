...
>* Experience Manager の実装で、製品データを e コマースソリューションから Experience Manager に取り込む際に、e コマース統合フレームワークを&#x200B;*使用していない*。このようなソリューションには、IBM WebSphere® Commerce、Elastic Path、SAP Hybris、Intershop などがあります。
>
...
* Firefox で F12 キーを押して、Firebug プラグインをアクティブ化し、「ネット」タブを使用します。または、ビルトインのインスペクターツールと「ネットワーク」タブを使用できます。
...
>
>ただし、カルーセルバナーでは、ホットスポットを含む画像の画像マップをサポートしていますが、インタラクティブ画像ではサポートしていません。同じ画像を使用するインタラクティブ画像またはカルーセルバナーを作成する場合には、このことに注意してください。同じ画像の別々のコピーを使用してインタラクティブ画像とカルーセルバナーを作成することもできます。
>
...
ショッパブルインタラクティブ画像によって手順 1 と、手順 2 の一部が置き換えられる場合、ユーザーはショッパブル画像内のホットスポットをタップします。このようなユーザー操作はビューアで処理されます。ビューアは、Adobe Experience Manager Assets に以前に追加されたすべてのホットスポットデータを含むイベントを web ページに返します。
