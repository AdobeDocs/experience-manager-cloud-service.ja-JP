---
title: 新しいビデオビューア
description: Dynamic Media の新しいビデオビューアでは、パフォーマンス、アクセシビリティ、設定機能が向上し、ビデオ再生のエクスペリエンスが強化されています。
role: User
...
| 領域 | 説明 |
|------|-------------|
| ビューアの可用性 | **ビデオ（新規）** という名前の新しいオプションとして表示されます。 |
...
5. **URL** をクリックして、プレビューリンクをコピーします。
   ![URL をコピー ](/help/assets/assets/Copy-url1.jpg)
...
![Dynamic Media コンポーネントをドラッグ ](/help/assets/assets/drag-component.jpeg)
...
2. コンポーネントツールバーで **設定** をクリックします。
   ![Dynamic Media 設定を開く ](/help/assets/assets/configure-asset.png)

3. **Dynamic Media 設定ダイアログ** で、「**ビューアプリセット**」ドロップダウンリストから「**ビデオ （新規）**」を選択します。
   ![ ビデオ（新規）ビューアプリセットを選択 ](/help/assets/assets/viewer-preset.jpeg)

4. 必要な修飾子を「**ビューア修飾子**」フィールドに入力します（例：`autoplay=true&muted=true`）。
   ![ ビューア修飾子 ](/help/assets/assets/additional-modifiers.jpeg)
...
| 修飾子 | 説明 |
|--------|-------------|
| `asset` | ビデオまたはアダプティブビデオセットのアセット ID を指定します。 |
| `posterimage` | 再生を開始する前に表示する画像を指定します。 |
| `serverurl` | 画像サービングルートパスを指定します。 |
| `contenturl` | コンテンツルートパスを指定します。 |
| `videoserverurl` | ビデオサーバーのルートパスを指定します。 |
| `sources.dash` | 再生する DASH マニフェスト URL を指定します。 |
| `sources.hls` | 再生するHLS マニフェスト URL を指定します。 |
| `autoplay=true` | ビデオの読み込み時に再生を自動的に開始します。 |
...
| `isletterboxed=true/false` | ビデオをレターボックスとストレッチのどちらにするかを制御します。 |
| `customcss` | ビューアのスタイル設定用のカスタム CSS ファイルを指定します。 |
| `transition` | ビューアコントロールのトランジションの表示/非表示の動作を指定します。 |
...
| `video_watched_seconds` | すべての再生イベントを監視した合計秒数。 | すべての再生イベントを監視した合計秒 |
| `play_rate` | ビデオの読み込みに対するビデオ再生の割合。 | （`video_views` ÷ `video_impressions`） × 100 |
| `avg_time_watched_in_seconds` | 表示ごとの平均視聴秒数。 | `video_watched_seconds` ÷ `video_views` |
| `avg_completion_rate` | 完全なビデオ完了に達したビューの割合。 | （`video_views`÷完了ビュー） × 100 |
