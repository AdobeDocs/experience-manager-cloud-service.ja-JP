...
例えば、Dynamic Media イメージプロファイルを作成してフォルダーに割り当てたとします。フォルダーにアップロードした画像アセットには、イメージプロファイルが自動的にアセットに適用されます。ただし、後でイメージプロファイルに新しいスマート切り抜き率を追加することにします。アセットを選択してフォルダーに再度アップロードする代わりに、次を実行するだけで済みます。 *Scene7:アセットを再処理* ワークフロー。
