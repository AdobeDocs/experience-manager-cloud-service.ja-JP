...
>[!NOTE]
>
>*定義済みの、標準搭載のビューアプリセット*&#x200B;の編集はサポートされていません。 標準提供ビューアプリセットを編集しようとすると、そのビューアプリセットを新しい名前で保存するように指示されます。
...
Adobe Experience Managerで&#x200B;**[!UICONTROL ツール]**（ハンマーのアイコン）/**[!UICONTROL アセット]/[!UICONTROL ビューアプリセット]**&#x200B;をタップして、ビューアプリセットを追加、編集、削除、公開、非公開およびプレビューできます。
...
>[!NOTE]
>
>あらかじめ用意されているすべてのビューアを公開してから、最初のビューアに使用します。
...
* Android™スマートフォン
* Android™タブレット
* ビデオの場合、[BlackBerry®](https://developer.blackberry.com/devzone/develop/supported_media/bb_media_support_at_a_glance.html#kba1328730952678)および[Windows® Phone](https://msdn.microsoft.com/library/windows/apps/ff462087%28v=vs.105%29.aspx)では、MP4再生が追加でサポートされます。

### ビューアプリセットのリッチメディアタイプ {#rich-media-types-for-viewer-presets}

管理者は、ビューアプリセットの作成時に、次のリッチメディアタイプを追加およびカスタマイズできます。

<table>
 <tbody>
  <tr>
   <td><strong>カルーセルセット</strong><br /> </td>
   <td><p>ホットスポットや画像マップ、またはその両方が 2 つ以上の一連の画像に追加されます。画像を左右にパンし、画像上でホットスポットをクリックすると、その画像の詳細を確認したり、Webサイトのランディング、カテゴリまたはホームページから直接購入したりできます。</p> </td>
  </tr>
    <tr>
  ...
  <tr>
   ...
   <td>画像の一部にホットスポットが追加されます。これをクリックすると、画像の詳細情報や、Webサイトのランディング、カテゴリ、ホームページから直接購入できます。</td>
  </tr>
  <tr>
   <td><strong>インタラクティブビデオ</strong></td>
   <td>ビデオのタイムラインセグメントにサムネールが追加されます。このサムネールをクリックすると、詳細を確認したり、Webサイトのランディング、カテゴリまたはホームページから直接購入することができます。</td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td><p>360/VR ビデオビューアを使用すると、エクイレクタングラー形式でビデオをレンダリングして、室内、物件、場所、風景、医療処置などの臨場感あふれる表示を実現することができます。</p> <p>フラットディスプレイでの再生中、ユーザは視野角を制御する。 モバイルデバイスでの再生には、組み込みのジャイロコントロールが使用されます。</p> <p>ビューアでは、360 ビデオアセットの配信をネイティブサポートしています。デフォルトでは、表示または再生するための追加設定は不要です。360 ビデオは、.mp4、.mkv、.mov といった標準のビデオ拡張子を使用して配信されます。最も一般的なコーデックは H.264 です。</p> </td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td><p>垂直方向ズームビューアを使用すると、製品画像の閲覧エクスペリエンスを最大限に高めて、ユーザーにとって最適な製品の表示を実現できます。スウォッチを垂直方向に配置することで、次のような利点があります。</p>
    <ul>
     <li>スウォッチが「見開きの上に」表示されます。<br/> 水平スウォッチの場合、ユーザのデスクトップ画面のサイズに応じて、ユーザがページを下にスクロールするまで、スウォッチは表示されません。ビューア内に垂直方向にスウォッチを配置することで、ユーザーの画面サイズに関係なく、確実にスウォッチが表示されるようになります。</li>
...
>[!NOTE]
>
>Dynamic Mediaのすべての標準搭載ビューアプリセットはアクティブ化（オン）されますが、公開する必要があります。
>[ビューアプリセットの公開](#publishing-viewer-presets)を参照してください。
>
...
次の表に、iOS、Android™ 2.xおよびAndroid™ 3.xデバイスでサポートされるモバイルビューアのジェスチャを示します。
...
Experience Managerは、**[!UICONTROL 詳細表示/ビューア]**&#x200B;からアセットを表示する場合に、様々なビューアプリセットを表示します。 表示されるビューアの数を増減できます。

**表示するビューアプリセットの数を増やすには：**
...
ビューアプリセットを作成しておくと、アセットの表示やアセットとの対話のための様々な設定を適用できます。ただし、ビューアプリセットを作成する必要はありません。 必要に応じて、Experience Managerアセットに付属している初期設定のそのまま使用できるビューアプリセットを使用できます。

ビューアプリセットを作成する場合、保存した後、ビューアプリセットページでビューアのステートが自動的にアクティブ化されます（**[!UICONTROL On]**&#x200B;に設定）。 この状態は、画像やビデオをプレビューするときは常に Dynamic Media コンポーネントとインタラクティブメディアコンポーネントに表示されることを意味します。

一部のビューアプリセットには、ビューアの全体的な動作に影響する専用の設定があります。作成するビューアプリセットに応じて、次の点に注意する必要があります。
...
1. Experience Managerの左上隅にあるExperience Managerのロゴをタップし、左のレールで&#x200B;**[!UICONTROL ツール]**（ハンマーのアイコン）/**[!UICONTROL アセット]/[!UICONTROL ビューアプリセット]**&#x200B;をタップします。
...
1. 次のいずれかの操作をおこないます。
   ...
   * 別のCSSファイルにスタイル設定を定義した場合は、CSSファイルをExperience Managerアセットにアップロードできます。 アップロードしたCSSファイルを見つけて、ビューアプリセットと関連付けるには、**[!UICONTROL 選択したタイプ]**&#x200B;のプルダウンメニューの下の&#x200B;**[!UICONTROL CSS]**&#x200B;を読み込みをタップします（必要に応じて、ビジュアルエディターを上にスクロールして表示します）。
   ...
   >ビジュアルエディターと同様に、CSSでプロパティを直接変更すると、ビューアサンプルに対する効果をすぐに確認できます。 また、同時に、Visual Editor でもその同じプロパティが自動的に更新されます。このことから、RAW である CSS エディターと、Visual Editor のどちらを使用しても、また交互に使用してもかまいません。

   >[!NOTE]
   >
   >ボタンのアートワークについては、2倍の画像を選択し、高解像度のアートワークをアップロードします。 インタラクティブな画像や買い物バナーを使用する場合は、あらかじめ用意されている様々なホットスポットボタンから選択することもできます。
...
1. 新しいビューアプリセットを公開します。プリセットは、Webサイトで使用する前に公開する必要があります。
...
インタラクティブビデオビューアプリセットを作成または編集する場合は、どの表示モード設定を使用するかを選択できます。 この選択は、**[!UICONTROL 「動作]**」タブの「**[!UICONTROL 選択されたコンポーネント]**」プルダウンメニューから`InteractiveSwatches`を選択した場合に発生します。 選択するディスプレイモードは、ビデオの再生中にサムネールを表示する方法とタイミングに影響します。`segment` ディスプレイモード（デフォルト）または `continuous` ディスプレイモードのいずれかを選択できます。
...
<table>
 <tbody>
  ...
  <tr>
   <td>セグメント</td>
   <td><p><code>Segment </code>は、既製のインタラクティブビデオビューアプリセット <code>Shoppable_Video_light</code> と <code>Shoppable_Video_dark</code> および自身で作成するすべてのインタラクティブビデオビューアプリセットのデフォルトのディスプレイモードです。</p> <p>このモードでは、ビデオセグメントに割り当てられるサムネールの数が、表示パネル内の表示されるスポットの数よりも少ないとします。 そのような場合、次または前のサブセグメントのサムネールは、パネル内の空白部分を埋めるために<i>取り込まれません</i>。 つまり、特定のビデオセグメントに割り当てられたスウォッチの表示を保持します。</p> </td>
  </tr>
  <tr>
   <td>連続</td>
   <td><p><code>continuous </code>表示モードで、セグメント内のサムネールの数が、パネルに表示される数より少ないとします。 その場合、ビューアには、次のセグメントまたは前のセグメント（最後のサムネールが表示される）からのサムネールの表示が自動的に含まれます。</p> <p><a href="/help/assets/dynamic-media/interactive-videos.md">このトピックの</a>ビデオは <code>continuous </code> ディスプレイモードの一例です。</p> </td>
  </tr>
 </tbody>
</table>
...
例えば、30秒のビデオセグメントが1つあるとします。 30 秒の間に表示されるサムネールは合計で 9 個です。ブラウザーは 4 つのサムネールがディスプレイパネルに表示できるようにサイズ設定されています。30秒のビデオ時間セグメントは3つのサブセグメントに分割されます。 次の表に、特定の時間サブセグメントに対してどのサムネールが表示されるかを示します。
...
ビデオサブセグメント3は、割り当てられたサムネールを越えてはいません。 また、サムネール 4、6、7 は、他のサムネールの 2 倍の時間パネルに表示されます。

ビューアでは次のロジックに従い、サイドパネルに表示できる数に基づいて表示するサムネールの数を決めています。

* サブセグメントの数=次のサブセグメントに切り上げ(サムネールの数/サムネールパネルに表示されるスロットの数（ブラウザーウィンドウのサイズに基づく）。
上の表の例を使用した場合、9個のサムネール/ 4個のスロット= 2.25;ビューアのロジックでは、3つのサブセグメントに切り上げられます。

* サムネールの数=次のサムネールに切り上げます（サムネールの数/ビデオサブセグメントの数）。
上の表の例を使用した場合、9つのサムネール/3つのビデオサブセグメント= 3つのサムネールが表示されます。

* サブセグメントの長さ=ビデオの合計時間/ビデオサブセグメントの数。
上の表の例を使用して、30秒/3ビデオサブセグメント= 10秒で各ビデオサブセグメントが表示されます。
...
オーサーモードでビューアプリセットがアクティベートされているかどうかで、ユーザーインターフェイスに表示されるビューアプリセットが変わります。デフォルトでは、ビューアプリセットは作成後に「オン」になります。プリセットをオフに切り替えると、作成者モードで表示されません。 プリセットが公開されている場合は、オン/オフが切り替えられているかどうかに関係なく、常に公開されます。 リストが不安定になったり、ビューアプリセットを使用可能にしたくない場合は、ビューアプリセットを非アクティブにします。
...
1. ビューアプリセットページの「**[!UICONTROL 状態]**」列ヘッダーの下で、ビューアプリセットのアクティベートとアクティベート解除の切り替えアイコンをタップします。
   ...
   アクティブ化されたビューアプリセットの右側、青いボックス内に切り替えが表示されます。非アクティブなビューアプリセットは、左側のライトグレーのボックス内に表示されます。
...
ただし、*ビューアプリセットを使用して*&#x200B;アセットを配信するには、ビューアプリセットも公開する必要があります。 アセットのURLまたは埋め込みコードを取得するには、すべてのビューアプリセットを&#x200B;*および*&#x200B;公開する必要があります。 Dynamic Mediaに付属の標準搭載されているすべてのビューアプリセットをアクティブ化して公開します。 自分で作成して追加したカスタムビューアプリセットは自動的にアクティベートされますが、やはり手動で公開する必要があります。
...
*定義済みの、標準搭載のビューアプリセット*&#x200B;の編集はサポートされていません。 標準提供ビューアプリセットを編集すると、新しい名前で保存するように指示されます。
...
1. **[!UICONTROL ビューアプリセットエディター]**&#x200B;ページで、「**[!UICONTROL 外観]**」タブと「**[!UICONTROL 動作]**」タブにあるオプションを使用して、ビューアプリセットに変更を加えます。
...
WCMとしてExperience Managerを使用している場合は、ビューアプリセットを使用してアセットをページ上に直接追加できます。 [ページへの Dynamic Media アセットの追加](/help/assets/dynamic-media/adding-dynamic-media-assets-to-pages.md)を参照してください。
