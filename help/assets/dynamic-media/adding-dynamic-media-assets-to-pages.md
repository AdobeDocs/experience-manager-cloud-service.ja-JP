...
* **[!UICONTROL ビューア修飾子]** - ビューア修飾子は、名前=値の組み合わせで &amp; を区切り文字とした形式です。ビューア修飾子を使用すると、ビューアリファレンスガイドに概略が記されているとおり、ビューアを変更することができます。はビューア修飾子の一例です。 `posterimage=img.jpg&caption=text.vtt,1` ビデオのサムネールに異なる画像を設定し、ビデオにクローズドキャプションファイルを関連付けます。
