...
1. インストール後、ビデオファイル（Windows®のみ）を右クリックしてMediaInfoを選択するか、MediaInfoを開いてビデオファイルをアプリケーションにドラッグします。 幅、高さ、fpsなど、ビデオファイルに関連付けられたすべてのメタデータが表示されます。
...
* **[!UICONTROL 一定ビットレートエンコーディング]** （CBR） - CBR エンコーディング中、ビットレート、または1秒あたりのビット数は、エンコーディングプロセス全体を通じて同じに保たれます。CBR エンコーディングは、設定したデータレートをビデオ全体にわたって設定に保持します。また、CBR エンコーディングはメディアファイルの品質を最適化しませんが、ストレージスペースを節約します。
ビデオ全体に同様のモーションレベルが含まれている場合は、CBRを使用します。CBRは、ビデオコンテンツのストリーミングに最もよく使用されます。[&#x200B; カスタム追加のビデオエンコーディングパラメーターの使用](/help/assets/dynamic-media/video-profiles.md#using-custom-added-video-encoding-parameters)も参照してください。

* **[!UICONTROL 可変ビットレートエンコーディング]** （VBR） - VBR エンコーディングは、コンプレッサーで必要なデータに基づいて、設定したデータレートを下げ、上限まで調整します。この機能は、VBR エンコーディングプロセス中に、メディアファイルのビットレートのニーズに応じて、メディアファイルのビットレートが動的に増減することを意味します。
VBRはエンコードに時間がかかりますが、最も良好な結果が得られます。メディアファイルの品質は優れています。VBRは、ビデオコンテンツのhttp プログレッシブ配信に最もよく使用されます。

VBRとCRBの違い？
VBRとCBRを選択する場合、ほとんどの場合、メディアファイルにVBRを使用することをお勧めします。VBRは、競争力のあるビットレートで高品質のファイルを提供します。VBRを使用する場合は、必ず2 パスエンコーディングで使用し、最大ビットレートをターゲットビデオビットレートの1.5倍に設定してください。
...
デフォルトでは、ビデオレポートを初めて入力すると、レポートには現在の月の最初から始まるビデオデータが表示され、現在の月の日付で終了します。 ただし、独自の日付範囲を指定することで、デフォルトの日付範囲を上書きできます。 次回ビデオレポートを開くと、指定した日付範囲が使用されます。
...
1. ビデオレポートページで、次のいずれかの操作を行います。
   ...
   * 右上隅付近にある「**[!UICONTROL ビデオレポートを更新]**」アイコンを選択します。
「更新」を使用できるのは、レポートの終了日が現在の日付の場合のみです。この機能を使用すると、前回レポートを実行した後に発生したビデオトラッキングを確認できます。

   * 右上隅付近にある&#x200B;**[!UICONTROL 日付選択]** アイコンを選択します。
ビデオデータの開始日と終了日の範囲を指定し、**[!UICONTROL レポートを実行]**&#x200B;を選択します。
...
See [Adobe Developer Connection](https://help.adobe.com/ja_JP/scene7/using/WSef8d5860223939e2-43dedf7012b792fc1d5-8000.html).
...
## Dynamic Media におけるビデオの複数のキャプションとオーディオトラックのサポートについて
...
1. 通常どおりビデオを公開します。
キャプション修飾子を使用した既存のURLで、複数のキャプションを読み込めるようになりました。
...
「[Web ページへのビデオビューアまたは画像ビューアの埋め込み](/help/assets/dynamic-media/embed-code.md)」を参照してください。
「[Web アプリケーションへのURLのリンク &#x200B;](/help/assets/dynamic-media/linking-urls-to-yourwebapplication.md)」を参照してください。インタラクティブコンテンツに相対URL （特にExperience Manager Sites ページへのリンク）を含むリンクがある場合、URL ベースのリンクは使用できません。
...
1. 再生を開始するには、![PLay アイコン &#x200B;](https://spectrum.adobe.com/static/icons/workflow_22/Smock_PlayCircle_22_N.svg)をクリックします。
必要に応じて、![最大化アイコン &#x200B;](https://spectrum.adobe.com/static/icons/workflow_22/Smock_Maximize_22_N.svg)をクリックして、表示ウィンドウを最大化します。
ページの左下隅付近にある「**[!UICONTROL URL]**」と「**[!UICONTROL 埋め込み]**」ボタンに注意してください。これらのボタンを使用して、ビデオのURLを[&#128279;](/help/assets/dynamic-media/linking-urls-to-yourwebapplication.md)web アプリケーションにリンクするか、[Web ページにビデオを埋め込むか](/help/assets/dynamic-media/embed-code.md)、それぞれします。
...
ビデオのチャプターリストを作成する方法は、キャプションを作成する方法とほとんど同じです。 つまり、WebVTT ファイルを作成します。 ただし、このファイルは、WebVTT キャプションファイルとは別にする必要があります。 キャプションとチャプターを1つのWebVTT ファイルに結合することはできません。
