...
>[!IMPORTANT]
>
>複数のアセットのスマート切り抜きウィンドウの再調整またはサイズ変更を手動でおこなう場合、後でアセットを再処理する場合でも、それらの編集内容は保持され、保持されます。 ただし、画像プロファイルの「**[!UICONTROL レスポンシブ画像の切り抜き]**」で幅、高さ、またはその両方を編集した場合、それらのアセットは再処理の対象となります。
>詳しくは、 [フォルダー内のDynamic Mediaアセットの再処理](/help/assets/dynamic-media/about-image-video-profiles.md#reprocessing-assets).
