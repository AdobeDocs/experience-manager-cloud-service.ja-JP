...
## Dynamic Media証明書の更新に必要な1回限りのDNS更新 {#dns-update-dynamic-media-certificate-renewals}

ドメインでCAA （Certification Authority Authorization） DNS レコードを使用している場合は、Dynamic Media ホスト名で使用されるTLS/SSL証明書の継続的な更新を許可するようにDigiCertを認証する必要があります。

ドメインのルート（apex）に次のCAA レコードを追加します。
...
CAA レコードが存在し、DigiCertが認証されていない場合、現在の証明書の有効期限が切れると証明書の更新が失敗し、画像およびビデオ配信のダウンタイムが発生する可能性があります。 ドメインにCAA レコードが存在しない場合、アクションは必要ありません。
