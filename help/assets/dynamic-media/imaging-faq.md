---
title: スマートイメージング
description: Adobe Sensei AI を使用したスマートイメージングが、各ユーザー固有の閲覧特性を適用して、ユーザーのエクスペリエンス用に最適化された適切な画像を自動的に提供することで、パフォーマンスとエンゲージメントの向上を実現している仕組みを説明します。
...
Adobe Sensei AI を使用したスマートイメージングが、各ユーザー固有の閲覧特性を適用して、ユーザーのエクスペリエンス用に最適化された適切な画像を自動的に提供することで、パフォーマンスとエンゲージメントの向上を実現している仕組みを説明します。
...
また、AVIF と WebP の両方のサポートに伴うスマートイメージングの改善により、LCP （Largest Contentful Paint）のGoogle Core Web Vital スコアが改善されました。
...
>
>スナップショットは、最適化された動的な画像配信における Dynamic Media のパワーをわかりやすく伝えるために作られた、視覚的なデモツールです。テスト画像やDynamic Mediaの URL を試して、様々なDynamic Media画像修飾子の出力を視覚的に確認し、次の項目に対するスマートイメージング最適化を確認します。
>
...
上記と同様に、Adobeでも、より大きなサンプルセットを使用してテストを実行しました。 AVIF 形式は WebP よりもサイズ削減率が 20％向上しました（WebP では JPEG に対して 27％の削減を実現）。視覚的な品質はすべて同じです。 全体的に見て、AVIF は JPEG よりも最大で平均 41％のサイズ削減を実現しています。
...
スマートイメージングは、ユーザーのブラウザー、デバイスのディスプレイ、ネットワーク状況に応じてファイルサイズを自動的に最適化することで、画像配信を強化します。 このアプローチにより、読み込み時間が短縮され、様々な環境での視聴エクスペリエンスが向上します。 画像はページの読み込み時間のほとんどを占めるので、パフォーマンス向上は次のようなビジネス KPI に大きな影響を与える可能性があります。
...
画像の URL に `bfc=off` を追加することで、スマートイメージングをオフにできます。

Dynamic Media 画像サービングおよび画像レンダリング API の [bfc](https://experienceleague.adobe.com/en/docs/dynamic-media-developer-resources/image-serving-api/image-serving-api/http-protocol-reference/command-reference/r-bfc) も参照してください。
...
ネットワーク帯域幅をオンにすると、提供される画質が、実際のネットワーク帯域幅に基づいて自動的に調整されます。 ネットワーク帯域幅が不十分な場合は、DPR（デバイスピクセル比）の最適化がオンになっていても、自動的にオフになります。

会社は、画像の URL に `network=off` を追加することで、個々の画像のネットワーク帯域幅の最適化を無効にすることができます。
...
* ブラウザーがの形式をサポートしている場合は、自動的に AVIF に変換します
...
スマートイメージングは、新しい形式に変換する際に、JPEGの画像ファイル形式の画質を再計算します。
...
調整は、画像の形式、画質、またはその両方に関係するだけです。 形式変換中、スマートイメージングはプリセット設定に従って完全な視覚的忠実性を維持しますが、ファイルサイズは小さくなります。 既存の URL またはプリセットに `bfc=on`、`dpr=on,dprValue`、`network=on` またはこれら 3 つのパラメーター設定を追加して有効にするだけです。

例えば、画像プリセットで、500 × 500 ピクセルのJPEG形式（`quality=85` と `unsharp mask=0.1,1,5` を含む）を指定しているとします。 ユーザーがChrome ブラウザーを使用しているかどうかをスマートイメージングが検出します。 次に、画像が、同じサイズ（500 × 500）で、JPEGの設定に一致するアンシャープマスクを持つ WebP に変換されます。 次に、WebP とJPEGのファイルサイズを比較し、小さい方をユーザーに提供します。
+++
...
スマートイメージングは、すべてのお客様に対してすぐに使用する準備が整っています。 その利点を活用するには、既存の URL またはプリセットに `bfc=on`、`dpr=on,dprValue`、`network=on` またはこれら 3 つのパラメーター設定をすべて追加します。

スマートイメージングをアクティブ化するには、会社のDynamic Media ClassicまたはExperience Manager版Dynamic Media アカウントに、ライセンスの一部としてAdobeバンドルの CDN （Content Delivery Network）が含まれている必要があります。
+++
...
サポートケースを作成する際に、アカウントでアクティベートするスマートイメージング機能を指定します。
...
>[!NOTE]
>
>DPR では、正しい `dprValue` を判断するためにクライアントサイドの調整が必要です。 そのため、Adobeでは、URL に `dpr=on,dprValue` を追加して、DPR を有効にすることをお勧めします。
...
1. サポートケースには、次の情報を記入してください。
   ...
   * **有効にするスマートイメージング機能：**
      ...
      * アカウントに必要な機能をリストします。
         ...
         * ブラウザーフォーマット変換：WebP または AVIF
         * ネットワーク帯域幅の最適化
         * DPR:DPR では、正しい `dprValue` を判断するためのクライアントサイドの調整が必要です。 そのため、Adobeでは、URL に `dpr=on,dprValue` を追加して、DPR を有効にすることをお勧めします。

   * **スマートイメージングのドメイン：**

      * *`company.com`* や *`mycompany.scene7.com`* など、関連するすべてのドメインを一覧表示します。
      * スマートイメージングは、汎用ドメインとカスタムドメインの両方をサポートします。
      * ドメインを特定するには、[Dynamic Media Classic デスクトップアプリケーションを開き ](https://experienceleague.adobe.com/en/docs/dynamic-media-classic/using/getting-started/signing-out#getting-started) 会社アカウントにログインします。

         1. **[!UICONTROL 設定]**／**[!UICONTROL アプリケーション設定]**／**[!UICONTROL 一般設定]**&#x200B;に移動します。
         1. 「**[!UICONTROL 公開先サーバー名]**」フィールドを探して、ドメインを確認します。
         1. 別のプロバイダーで管理されているのではなく、Adobeの CDN を使用していることを確認します。

   * **HTTP/2 サポートを示します：**

      * スマートイメージングが HTTP/2 上で機能する必要がある場合は、を指定します。

1. Adobeカスタマーサポートでは、リクエストされたスマートイメージング機能をデフォルトで有効にするので、URL にパラメーターを手動で追加する必要はありません。
1. Adobeでは、キャッシュを通じてパフォーマンスを最大限に高めるために、有効期間（TTL）を 24 時間以上に設定することをお勧めします。
TTL を調整するには：

   1. **Dynamic Media Classicの場合：**
      1. **[!UICONTROL 設定]**/**[!UICONTROL アプリケーション設定]**/**[!UICONTROL Publish設定]**/**[!UICONTROL Image Server]** に移動します。
      1. **[!UICONTROL クライアントキャッシュのデフォルトの有効期限]** 値を 24 時間以上に設定します。
   1. **Adobe Experience ManagerのDynamic Mediaの場合：**
      1. [ 次の手順 ](/help/assets/dynamic-media/config-dm.md) に従います。
...
1. デベロッパーツールを開いたときにキャッシュが無効化されるようにします。
   ...
   * macOSのデベロッパーパネルの「**[!UICONTROL ネットワーク]**」タブで、「**[!UICONTROL キャッシュを無効にする]**」を選択します。
...
>
>`X-Adobe-Smart-Imaging` の値が–1 で WebP がまだ配信中の場合、スマートイメージングがアクティブになります。 ただし、キャッシュが古いので、サイズのメリットは計算されませんでした。 画像の URL で `cache=update` を（1 回だけ）使用して、この問題を修正できます。
>この修飾子の使用例を次に示します。
...
**Chrome web ブラウザーに fmt=tif を含む URL がある場合、リクエストが失敗するのはなぜですか？**
...
はい。スマートイメージングでは、画質（％単位）を自動的に調整します。この品質は、Adobeが開発した機械学習アルゴリズムを使用して決定されます。 このパーセントは、範囲固有のものではありません。
+++
...
はい。スマートイメージングでは、画質レベルに応じた非可逆変換をサポートするようになりまし。会社の設定を使用するか、画像の URL パスに `qlt=100` を追加して、画質を 100 に設定することで、可逆変換を引き続き使用できます。
+++
