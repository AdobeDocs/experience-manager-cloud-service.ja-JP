...
# スマートイメージングに関する FAQ {#smart-imaging}
...
+++**URL の変更や、画像プリセットの変更、またはサイトへの新しいコードのデプロイが必要ですか。**
...
はい、両方の質問に対して。 スマートイメージングは、HTTP または HTTPS で配信された画像に対して機能します。また、HTTP/2 上でも機能します。
...
+++**自分のアカウントでスマートイメージングを有効にすることはできますか？**
...
+++**自分のアカウントでスマートイメージングが有効になっているのはいつですか？**
...
最初の切り替え中、キャッシュが再構築されるまでの間は、アドビの起点サーバーにあるキャッシュされていない画像が直接ヒットします。このため、元のサーバーからリクエストをプルするときに許容できるパフォーマンスが維持されるように、アドビでは一度に少数の顧客の移行を処理するよう計画します。ほとんどのお客様にとって、CDN でキャッシュが完全に再構築されるまでに要する時間は約 1 ～ 2 日です。
...
+++**スマートイメージングで AVIF の最適化を無効にすることはできますか？**

はい。WebP をデフォルトで提供する状態に戻す場合は、同様にサポートケースを作成します。通常どおり、画像の URL に `bfc=off` パラメーターを追加して、スマートイメージングをオフにできます。ただし、スマートイメージングの URL 修飾子で WebP または AVIF を選択することはできません。この機能は、会社のアカウントレベルで維持されます。

+++

+++**Chrome Web ブラウザーで fmt=tif の URL を使用している場合、要求が失敗するのはなぜですか。**
...
+++**JPEGと PNG のみがスマートイメージングに置き換えられますか？**

はい。この機能は、JPEG と PNG でのみ機能します。

+++

+++**JPEGが WebP ではなく Chrome に戻ることがあるのはなぜですか。**

スマートイメージングは、変換が有益かどうかを判断します。変換が有益な場合にのみ、新しい画像を返します。

+++

+++**デバイスのピクセル比 (dpr) が複合画像で機能しないのはなぜですか？**
...
+++**スマートイメージング PNG が可逆 WebP/AVIF に変換されるのはなぜですか？**
...
+++**スマートイメージングで PNG を使用して可逆変換を続けることはできますか？**
