...
eコマースエクスペリエンスでは、デフォルトのクイックビューが使用され、ポップアップに購入を促す製品情報が表示されます。 ただし、このようなポップアップにカスタムコンテンツが表示されるように設定できます。使用するビューアに応じて、顧客はホットスポット、サムネール画像、画像マップをタップして、情報や関連コンテンツを表示できます。
...
各ビューアの機能は異なりますが、サポートされる3つすべてのビューアでクイックビューの作成手順は同じです。

**クイックビューを使用してカスタムポップアップWindows®を作成するには：**

1. アップロードしたアセットのクイックビューを作成する。

   クイックビューは、通常、使用しているビューアで使用するアセットを編集するのと同時に作成します。
...
1. ビューアの埋め込みコードを取得し、Web サイトにビューアを統合します。
   ...
   <table>
    ...
    <td><strong>ビューアをWebサイトに統合するには、次の手順を実行します</strong></td>
    </tr>
    <tr>
    <td>インタラクティブ画像</td>
...
1. 使用するビューアは、クイックビューの使用方法を知っている必要があります。
   ...
   **上記のサンプル埋め込みコードの例を使用すると、次のコードが得られます。**
...
1. 次に、`quickViewActivate`ハンドラを設定します。
   ...
   `quickViewActivate`ハンドラーは、ビューアのクイックビューを制御します。 ハンドラーには、クイックビューで使用する変数のリストと関数呼び出しが含まれます。 埋め込みコードは、クイックビューのSKU変数セット用のマッピングを提供します。 また、サンプルの`loadQuickView`関数呼び出しを行います。

   **Webペー**
ジで使用する変数mappingMap変数を、クイックビューに含まれるSKU値と汎用変数にマッピングします。
   ...
   **関数呼び**
出しハンドラーには、クイックビューを機能させるための関数呼び出しも必要です。この関数は、ホストページからアクセスできることが前提となります。埋め込みコードは、サンプル関数呼び出しを提供します。
...
1. 以下の操作を実行してください。
   ...
   * ビューア外でページにシンプルな `loadQuickView` () 関数を作成します。
      ...
      例えば、次の例では、SKUの値をブラウザーコンソールに書き込みます。
   ...
   * Web サーバーにテスト HTML ページをアップロードし、開きます。
      ...
      クイックビューの変数がマッピングされます。 関数呼び出しが実行された。 また、ブラウザーコンソールは、変数値をブラウザーコンソールに書き込みます。 これは、提供されたサンプル関数を使用しておこないます。



1. 関数を使用して、クイックビューでシンプルなポップアップを呼び出せるようになりました。 以下の例では、ポップアップに `DIV` を使用しています。
1. ポップアップの `DIV` を以下のようなスタイルにします。必要に応じて、追加のスタイルを追加します。
...
1. HTML ページのボディにポップアップの `DIV` を配置します。
   ...
   要素の1つにはIDが設定され、ユーザーがクイックビューを起動するとSKU値で更新されます。 この例にはこれに加え、ポップアップを表示後に再び隠すための単純なボタンも含まれています。
...
1. ポップアップウィンドウのSKU値を更新するには、関数を追加します。 手順5で作成したシンプルな関数を次のように置き換えて、ポップアップウィンドウを表示します。
...
1. Web サーバーにテスト HTML ページをアップロードし、開きます。ユーザーがクイックビューを起動すると、ビューアにポップアップ`DIV`が表示されます。
1. **全画面表示モードでカスタムポップアップウィンドウを表示する方法**

   インタラクティブビデオビューアなどの一部のビューアでは、全画面表示モードでの表示をサポートしています。ただし、前の手順で説明したポップアップを使用すると、全画面表示モード中はビューアの背後に表示されるようになります。

   標準モードとフルスクリーンモードでポップアップウィンドウを表示するには、ビューアのコンテナにポップアップウィンドウをアタッチします。 この場合は、2つ目のハンドラーメソッド`initComplete`を使用します。
...
1. 前の手順で言及したように、ビューアにポップアップをアタッチするには、以下のコードを使用します。
   ...
   * カスタムポップアップウィンドウを特定しました。
   * DOM からの削除
   * ビューアコンテナの指定
   * ビューアコンテナへのポップアップのアタッチ

1. setHandlersコード全体は次のようになります（インタラクティブビデオビューアを使用していました）。
...
1. ハンドラーを読み込んだ後で、ビューアを初期化します。
   ...
   **例**：この例では、インタラクティブ画像ビューアを使用します。
   ...
   ホストページにビューアを埋め込んだ後で、ビューアインスタンスが作成されていることを確認します。 また、`init()`を使用してビューアを呼び出す前に、ハンドラーが読み込まれていることを確認してください。
