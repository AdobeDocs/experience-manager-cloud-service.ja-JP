...
1. **フィールドを追加するには**
   ...
   * フィールドがモデルに追加されると、その特定のデータタイプに対して定義できる&#x200B;**プロパティ**&#x200B;が右側のパネルに表示されます。ここで、そのフィールドに必要な項目を定義することができます。
      ...
      * 「**フィールドラベル**」を入力すると、「**プロパティ名**」が空欄の場合は自動入力され、後で手動で更新できます。
         ...
         >[!CAUTION]
         プロパティを手動で更新する場合 **プロパティ名** データ型の場合、名前には、ラテン文字、数字、アンダースコア「_」のみを特殊文字として含める必要があります。
         以前のバージョンのAEMで作成されたモデルに不正な文字が含まれている場合は、それらの文字を削除または更新してください。
      次に例を示します。
...
* **プロパティ名**
   ...
   データ型のこのプロパティを手動で更新する場合は、名前に注意してください。 **必須** 次を含む *のみ* ラテン文字、数字、特殊文字としてのアンダースコア「_」。
...
メッセージに、 [参照](/help/sites-cloud/authoring/getting-started/basic-handling.md#references) 詳細を調べるパネル：
...
* コンテンツフラグメントモデルは、AEMの GraphQL クエリのスキーマを決定します。
   ...
   * 公開上のスキーマは、JSON 形式でのコンテンツフラグメントコンテンツのライブ配信の基盤となるので、最も重要です。

* 問題は、コンテンツフラグメントモデルが変更された場合、つまり編集された場合に発生する可能性があります。 つまり、スキーマの変更が既存の GraphQL クエリに影響を与える可能性があります。

* コンテンツフラグメントモデルに新しいフィールドを追加する場合、（通常は）有害な効果はありません。 ただし、既存のデータフィールド（名前など）を変更したり、フィールド定義を削除したりすると、これらのフィールドをリクエストする際に、既存の GraphQL クエリが壊れます。

### 要件 {#the-requirements}

* ライブコンテンツ配信に既に使用されているモデル（つまり、公開されたモデル）を編集する際のリスクをユーザーに知らせるため。

* また、意図しない変更を避けるために、を使用します。

変更されたモデルが再公開されると、どちらのモデルでもクエリが壊れる場合があります。

### 解決策 {#the-solution}

これらの問題に対処するために、コンテンツフラグメントモデルは、 *ロック済み* 作成者の読み取り専用モードに切り替えます。 これは、 **ロック済み**:

![ロックされたコンテンツフラグメントモデルのカード](assets/cfm-model-locked.png)

モデルが **ロック済み** （読み取り専用モードで）モデルの内容と構造は表示されますが、編集はできません。

次の項目を管理できます。 **ロック済み** モデルは、コンソールまたはモデルエディターから次の操作を実行します。

* コンソール

   コンソールから、 **ロック解除** および **ロック** ツールバーのアクション：

   ![ロックされたコンテンツフラグメントモデルのツールバー](assets/cfm-model-locked.png)

   * 以下が可能です。 **ロック解除** 編集を可能にするモデル。
   ...
   * また、 **ロック** その後、モデルが
   * モデルを再公開すると、すぐにに戻されます。 **ロック済み** （読み取り専用）モード。

* モデルエディター

   * ロックされているモデルを開くと、警告が表示され、次の 3 つのアクションが表示されます。 **キャンセル**, **読み取り専用を表示**, **編集**:

      ![ロックされたコンテンツフラグメントモデルを表示する際のメッセージ](assets/cfm-model-editor-lock-message.png)

   * 次を選択した場合、 **読み取り専用を表示** モデルの内容と構造を確認できます。

      ![読み取り専用を表示 — ロックされたコンテンツフラグメントモデル](assets/cfm-model-editor-locked-view-only.png)

   * 次を選択した場合、 **編集** 更新を編集して保存できます。

      ![編集 — ロックされたコンテンツフラグメントモデル](assets/cfm-model-editor-locked-edit.png)

      >[!NOTE]
      上部に警告が表示される場合がありますが、その場合は、モデルが既存のコンテンツフラグメントで既に使用されています。
