...
description: フラグメントのコンテンツを作成し、目的に応じてそのコンテンツのバリエーションを作成することで、AEMでのバリエーションによるヘッドレスコンテンツの柔軟性をさらに高める方法を説明します。
feature: コンテンツフラグメント
...
[](/help/assets/content-fragments/content-fragments.md#constituent-parts-of-a-content-fragment) バリエーションは、特定のチャネルやシナリオで使用するマスターコンテンツのコピーを作成および編集でき、ヘッドレスなコンテンツ配信の柔軟性を高める、AEMコンテンツフラグメントの重要な機能です。
...
![全画面表示エディ](assets/cfm-variations-02.png)
ター：次の操作が可能です。
...
元のタブを再度選択すると（例えば、**Little Pony Inc.**）、このセカンダリタブが閉じます（この場合は&#x200B;**Adam Smith**）。
...
**プライマリ**&#x200B;はコンテンツフラグメントの必須の部分であり、定義に従ってコンテンツのプライマリコピーを保持します。一方、バリエーションはそのコンテンツの更新および適合された個別のバージョンを保持します。マスターが更新されると、これらの変更がバリエーションにも関連する可能性があるので、バリエーションに反映する必要があります。
