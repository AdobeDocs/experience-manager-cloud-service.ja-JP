...
次の場合、 [authoring](/help/assets/content-fragments/content-fragments-variations.md#authoring-your-content)を使用する場合、コンテンツフラグメントエディターは *markdown* ヘッドレス配信またはページオーサリング用にコンテンツを簡単に書き込むための構文：
...
ハッシュタグ（#）を前に付けてヘッダーを作成します。H1 には 1 つのハッシュタグ (#) が使用され、H2 には 2 つのハッシュタグ (##) が使用されます。 最大で 6 個のハッシュタグを使用できます。例：
