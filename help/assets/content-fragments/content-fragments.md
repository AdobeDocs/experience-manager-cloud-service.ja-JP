...
description: Adobe Experience Manager(AEM) のコンテンツフラグメントをas a Cloud Service的に使用して、ページに依存しないコンテンツをデザイン、作成、キュレーションおよび使用する方法を説明します。これは、ページオーサリングおよびヘッドレス配信に最適です。 また、MSM と共に使用する方法も示します。
...
* 構造の範囲は次のとおりです。
   * 基本
      ...
      * これを使用して、ページオーサリングで使用する簡単なコンテンツを準備できます。
   * 複合
      * テキスト、数値、ブール値、データ、時間など、様々なデータタイプの多くのフィールドの組み合わせ。
      * これは、より構造化されたコンテンツをページオーサリング用に準備する場合、またはアプリケーションに配信する場合に使用できます。
   * 入れ子
...
以下のページでは、コンテンツフラグメントを作成、設定、管理および使用するためのタスクについて説明します。

* [インスタンスに対するコンテンツフラグメント機能を有効化します。](/help/assets/content-fragments/content-fragments-configuration-browser.md)
* [コンテンツフラグメントモデル](/help/assets/content-fragments/content-fragments-models.md)  — モデルの有効化、作成、定義
...
* 用途 [コンテンツフラグメントとGraphQL（コンテンツを配信可能）](/help/assets/content-fragments/content-fragments-graphql.md) アプリケーションで使用する場合。 その一助として、[JSON 出力](/help/assets/content-fragments/content-fragments-json-preview.md)をプレビューできます。
* [MSM for Assets を使用したコンテンツフラグメントの再利用](#reusing-content-fragments-with-msm-assets)

>[!NOTE]
>
>これらのページは、次の情報で読み取ることができます。
>
...
ただし、すべてのチャネルに同じコンテンツを使用するのは避けたい（たぶん）ので、特定のチャネルに従ってコンテンツを最適化する必要があります。
...
* データの複雑さを反映させるために、ネストされたコンテンツを作成します。

次に、これらのコンテンツフラグメントを組み立てて、様々なチャネルをまたいでエクスペリエンスを提供できます。
...
AEM コンテンツフラグメントを使用して構造化コンテンツを記述し、管理できます。構造化コンテンツは、テキスト、数値データ、ブール値、日時など、様々なコンテンツタイプを含むモデルで定義されます。
...
を通じてアクセスする場合 **Assets** コンソールで、MSM を使用してフラグメントのライブコピーを作成できます。

詳しくは、 [MSM for Assets を使用したコンテンツフラグメントの再利用](/help/assets/reuse-assets-using-msm.md). これにより、 [継承](/help/assets/content-fragments/content-fragments-variations.md#inheritance) フラグメントのバリエーションと個々のフィールドの両方に対して
...
* Adobe Analytics で使用されている [コンテンツフラグメントコンポーネント別のページエディター](/help/sites-cloud/authoring/fundamentals/content-fragments.md) （参照コンポーネント）:
...
* [画像を含める、または画像に接続できる](#fragments-with-visual-assets).
* 使用済み [中間コンテンツ](#in-between-content-when-page-authoring-with-content-fragments) （ページで参照された場合）
* 配信メカニズム（ページやチャネル）とは独立しています。
...
* フラグメントに&#x200B;**アセットを挿入**（混在メディアフラグメント）
   ...
   * これらはフラグメントの一部です ( [コンテンツフラグメントの構成要素](#constituent-parts-of-a-content-fragment)) をクリックします。
   * アセットの位置を定義します。
  ...
  >[!NOTE]
  >
  >コンテンツフラグメント自体に挿入したビジュアルアセットは、前の段落に添付されます。フラグメントをページに追加すると、中間コンテンツが追加されたときに、これらのアセットはその段落との関連で移動します。

* **関連コンテンツ**

   * フラグメントに接続しているが、フラグメントの固定部分ではない ( [コンテンツフラグメントの構成要素](#constituent-parts-of-a-content-fragment)) をクリックします。
   * 位置決めの柔軟性がある。
   * ページでフラグメントを使用する際に、（中間コンテンツのように）容易に使用できます。
...
* **フラグメントの段落**
   ...
   * 個々のエンティティとして区切られたテキストのブロック（多くの場合、複数行）。
...
* **フラグメントに挿入したアセット（混在メディアフラグメント）**
   ...
   * フラグメントへの追加、フラグメントからの削除、またはフラグメント内での移動は、 [フラグメントエディターでのリッチテキスト形式](/help/assets/content-fragments/content-fragments-variations.md#inserting-assets-into-your-fragment).
...
* **関連コンテンツ**
   ...
   * これはフラグメント外部のコンテンツですが、編集に関連します。通常、画像、ビデオ、またはその他のフラグメントです。
   * コレクション内の個々のアセットは、ページエディターでページにフラグメントを追加するときに、フラグメントと共に使用できます。つまり、コレクション内の個々のアセットは任意であり、特定のチャネルの要件に応じて使用します。
   * アセットは [コレクションを介してフラグメントに関連付けられる](/help/assets/content-fragments/content-fragments-assoc-content.md)関連付けられたコレクションを使用すると、作成者は、ページ作成時に使用するアセットを決定できます。
...
* **バリエーション**
   ...
   * 編集目的に固有のフラグメントテキストのレンディション。チャネルに関連付けることができますが、強制的ではありません。ローカルでアドホックに変更する場合もあります。
   * のコピーとして作成 **マスター**&#x200B;必要に応じて編集できます。 バリエーション自体にはコンテンツの重複があります。
...
* コンテンツフラグメントを操作する際にページエディターで使用できます。
* [フラグメントのフロー内に追加される追加のコンテンツ](/help/sites-cloud/authoring/fundamentals/content-fragments.md#adding-in-between-content) ページ上で使用または参照された後。
* 使用可能な [コンテンツフラグメントを操作する際のページエディター](/help/sites-cloud/authoring/fundamentals/content-fragments.md).
...
コンテンツフラグメントをページオーサリングに使用するには、次も必要です。
...
フラグメントと、その要素およびバリエーションを一緒に使用して、複数のチャネルに対応した一貫性のあるコンテンツを作成できます。フラグメントをデザインする際は、何を使用し、どこで使用するかを考慮します。
