...
例えば制作のプロフェッショナルは、アドビアプリケーションのビルトインの XMP サポートを使用して、複数のファイル形式に情報を渡します。 [!DNL Assets] リポジトリーでは、XMP メタデータを抽出し、そのデータをコンテンツのライフサイクルの管理に使用します。自動化ワークフローを作成することもできます。
...
[!DNL Adobe Experience Manager Assets]のこのXMP書き戻し機能は、元のアセットのレンディションに対するメタデータの変更をレプリケートします。
アセットのメタデータを[!DNL Assets]内から変更する場合、またはアセットのアップロード中に変更を行う場合、変更内容は最初にアセット階層のメタデータノードに保存されます。書き戻し機能を使用すると、メタデータの変更をアセットのすべてのレンディションまたは特定のレンディションに反映できます。この機能は、`jcr`名前空間を使用するメタデータプロパティのみを書き戻します。つまり、`dc:title`という名前のプロパティは書き戻されますが、`mytitle`という名前のプロパティは書き戻されません。
