...
# [!DNL Adobe Experience Manager Assets] 開発者向けの使用例、API、リファレンス資料  {#assets-cloud-service-apis}
...
>[!CAUTION]
>
>一部のAPIは引き続き存在しますが、アクティブにサポートされません(×で示されます)。 可能な限り、これらのAPIは使用しないでください。
...
[!DNL Experience Manager]では、HTTP APIを使用して、アセットをクラウドストレージに直接アップロードできます。 [!DNL Cloud Service]バイナリファイルをアップロードする手順を以下に示します。 [!DNL Experience Manager] JVM内ではなく、外部アプリケーションでこれらの手順を実行します。
...
* `minPartSize` （数値）:複数のURIがある場合にいずれかのに提供されるデータの最小長(バ `uploadURIs`イト単位)。
* `maxPartSize` （数値）:複数のURIがある場合にいずれかのに提供されるデータの最大長(バ `uploadURIs`イト単位)。
...
>[!MORELIKETHIS]
* [[!DNL Experience Cloud] as a [!DNL Cloud Service] SDK](/help/implementing/developing/introduction/aem-as-a-cloud-service-sdk.md)を参照してください。
