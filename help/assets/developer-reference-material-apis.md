...
>[!CAUTION]
>
>一部のAPIは引き続き存在しますが、アクティブにはサポートされていません（「×」を付けて示します）。 これらのAPIは、可能な限り使用しないでください。
...
[!DNL Experience Manager]に[!DNL Cloud Service]として、HTTP APIを使用して、アセットをクラウドストレージに直接アップロードできます。 バイナリファイルをアップロードする手順は次のとおりです。
...
* `minPartSize` （数）:複数のURIが存在する場合に、いずれかのURIに対して提供される可能性があるデータの最小長（バイト単位） `uploadURIs`です。
* `maxPartSize` （数）:複数のURIが存在する場合に、いずれかのURIに対して提供されるデータの最大長（バイト単位） `uploadURIs`です。
