...
## OpenAPI ベースのAEM Assets APIの設定 {#setup-openapi-aem-assets}

2024年11月より前に作成された実稼動環境、開発環境または下位環境に`AEM Assets API Users`製品プロファイルサービスが表示されない場合は、OpenAPI ベースのAEM Assets APIを設定する必要があります。

OpenAPI ベースのAEM Assets APIの設定には、AEM as a Cloud Service環境の最新化とAEM Assets API アクセスの有効化が含まれます。 近代化は、環境（実稼動、開発、ステージ）ごとに実行する必要があります。 詳しくは、[OpenAPI ベースのAEM APIの設定](https://experienceleague.adobe.com/ja/docs/experience-manager-learn/cloud-service/aem-apis/openapis/setup)を参照してください。
