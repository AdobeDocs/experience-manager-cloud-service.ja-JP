...
アップロードを開始した場合の出力には、1 つ以上のアップロード URI 値が含まれています。複数の URI を指定した場合、クライアントはバイナリを複数の部分に分割し、指定されたアップロード URI に対して、各部分のPUTリクエストを順におこなうことができます。 バイナリを部分に分割する場合は、次のガイドラインに従ってください。
* 各部品は、最後の部品を除き、以上のサイズである必要があります `minPartSize`.
* 各パーツのサイズは以下でなければなりません `maxPartSize`.
* バイナリのサイズがを超えた場合 `maxPartSize`を使用する場合は、バイナリを複数の部分に分割してアップロードする必要があります。
* すべての URI を使用する必要はありません。

バイナリのサイズが以下の場合 `maxPartSize`の代わりに、バイナリ全体を単一のアップロード URI にアップロードすることもできます。 複数のアップロード URI を指定する場合は、最初のアップロード URI を使用し、残りのアップロード URI は無視します。 すべての URI を使用する必要はありません。

CDN エッジノードを使用すると、要求されたバイナリアップロードを高速化できます。

これをおこなう最も簡単な方法は、 `maxPartSize` パーツサイズと同じにします。 API コントラクトでは、この値を部分サイズとして使用する場合、バイナリをアップロードするのに十分なアップロード URI があることを保証します。 これをおこなうには、バイナリをサイズの一部に分割します `maxPartSize`各パーツに対して 1 つの URI を順に使用します。 最後の部分は、以下の任意のサイズにすることができます `maxPartSize`. 例えば、バイナリの合計サイズが 20,000 バイトで、 `minPartSize` は 5,000 バイトです。 `maxPartSize` は 8,000 バイトで、アップロード URI の数は 5 です。 この場合は次の手順に従います。
...
* 最後の 2 つのアップロード URI を使用する必要はありません。 無視して。

一般的な誤りは、API が提供するアップロード URI の数に基づいて部分のサイズを計算することです。 API 契約では、この方法が機能するとは保証されず、実際には次の範囲外のサイズのパーツが生じる可能性があります： `minPartSize` および `maxPartSize`. その結果、バイナリのアップロードに失敗する可能性があります。
