...
>[!IMPORTANT]
>
>[AEM Governance Agent](/help/ai-in-aem/agents/governance/overview.md#permission-management-in-content-hub)を使用して、属性ベースのアクセス制御ルールを設定することもできます。
...
管理者は、ブランドや地域などのアセットのメタデータにもとづいて条件を作成し、特定のユーザーグループ IDにリンクすることで、アクセスルールを定義できます。 これらのルールでは、論理（AND、OR）演算子と比較（等号ではなく等号）演算子を使用して、どのユーザーグループに対して表示されるアセットを正確に指定します。
...
いいえ。現時点では、管理者はContent Hub インターフェイスでABAC ルールを直接作成することはできません。 代わりに、テンプレートのスプレッドシート（この記事に記載されているリンクをダウンロード）をダウンロードし、ルールを定義し、実装のサポートチケットを介してAdobe サポートに送信する必要があります。

### AEM Assets Content HubでABAC ルールを設定する際に、どのような種類のメタデータ条件を使用できますか？

AEM ASSETS Content HubのABAC ルールでは、ANDやORなどの論理演算子や、equalsやnot equalsなどの比較演算子を使用できます。 ルールで使用されるメタデータプロパティは、AEM メタデータスキーマで正しく定義され、使用可能である必要があり、リージョン、ブランド、公開ステータスなどのフィールドを含めることができます。

### AEM Assets Content Hub ABACが、大規模なチームと多様なアセットニーズを抱える企業にとって特に有用なのはなぜですか？

ABACは、ユーザーの役割、地域、ブランドにもとづいて、アセットへの詳細なルールベースのアクセスを可能にするため、大規模なチームを抱える組織にとって有用です。 これにより、ユーザーは手作業による権限の割り当てやアセットの過度な重複なしに、責任に関連するアセットのみを表示することができます。

### Adobe サポートに提出する前に、AEM Assets Content Hub用のABAC スプレッドシートを準備する方法を教えてください。

管理者は、Adobe Admin Consoleでユーザーグループを作成し、グループ IDをメモし、スプレッドシートの各グループの権限と条件を明確に定義する必要があります。 すべてのメタデータプロパティが適切なスキーマに正しくマッピングされていることを確認し、「コメント」列を使用して各ルールのビジネスの意図を明確にし、Adobeでルールを検証および実装しやすくします。
