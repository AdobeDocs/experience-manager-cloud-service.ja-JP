...
### ユーザーはContent Hubにどのようにアクセスでき、どのような権限を割り当てることができますか？

ユーザーをContent HubAdobe Admin Consoleに追加するには、Content Hubの関連製品プロファイルにユーザーを割り当てます。
...
### Content Hubのユーザーのタイプごとに使用できる様々な製品プロファイルには何がありますか？

製品プロファイルは、Content Hubを使用する様々なタイプのユーザーが使用できます。
...
### 管理者が組織でContent Hubを有効にするにはどうすればよいですか？

管理者は、Cloud Managerにログインしてプログラムを選択（または作成）し、「ソリューションとアドオン」タブで「AssetsとContent Hub」を有効にして、プログラムを更新する必要があります。 これにより、Adobe Admin ConsoleにContent Hub インスタンスが作成され、ユーザーアクセスを管理できます。
...
### Content Hub クレジットはどのように使用されますか？
...
はい。Content Hub クレジットが利用可能な場合は、開発やステージなどの下位環境でContent Hubを有効にすることができます。 有効な下位環境ごとに 1 つのクレジットを消費します。

### Content Hubで承認済みアセットにアクセスする権限を持つにはどうすればよいですか？

Content Hub ユーザーは、Content Hub ポータルでブランド承認済みアセットにアクセスできます。 Content Hub ユーザーになるには、AEM Limited Users 製品プロファイルに追加されている必要があります。

### Content Hubにアセットをアップロードする権限を付与するにはどうすればよいですか？

アセットを追加する権限を持つContent Hub ユーザーは、Content Hub ポータルで brand approved assets にアクセスするだけでなく、Content Hubにアセットをアップロードすることもできます。 アセットを追加する権限を持つContent Hub ユーザーになるには、AEM Limited Users 製品プロファイルおよびAEM Users 製品プロファイルに追加されている必要があります。

### Content Hubの設定ユーザーインターフェイスにアクセスする権限を持つにはどうすればよいですか？

Content Hub管理者は、ブランド承認済みアセットへのアクセス、Content Hubへのアセットのアップロード、画像を編集するAdobe Express統合に加えて、Content Hubの設定ユーザーインターフェイスにもアクセスできます（Adobe Expressの使用権限がある場合）。 Content Hub管理者になるには、AEM Limited Users およびAEM Administrators 製品プロファイルに追加されている必要があります。

### Content HubでAdobe Expressを使用して画像を編集する権限を付与するにはどうすればよいですか？

アセットをリミックスする権限を持つContent Hub ユーザーは、Content Hub ポータルで brand approved assets にアクセスできるだけでなく、Adobe Expressにアクセスできます（Adobe Expressの権限がある場合）。 アセットをリミックスする権限を持つContent Hub ユーザーにするには、AEM Limited Users 製品プロファイルおよびAEM Users 製品プロファイルに追加されている必要があります。
