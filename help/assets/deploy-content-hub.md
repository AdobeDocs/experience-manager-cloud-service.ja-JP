...
### ユーザーはAEM Assets Content Hubにどのようにアクセスでき、どのような権限を割り当てることができますか？

ユーザーをAEM Assets Content Hubに追加するには、Content Hubの関連製品プロファイルにユーザーを割り当てることでAdobe Admin Consoleを利用できます。

ユーザーは次の権限を利用できます。

* Content Hub ユーザーは、Content Hub ポータルでブランド承認済みアセットにアクセスできます。

* Content Hub管理者は、ブランド承認済みアセットへのアクセス、Content Hubへのアセットのアップロード、画像を編集するAdobe Express統合に加えて、Content Hubの設定ユーザーインターフェイスにもアクセスできます（Adobe Expressの使用権限がある場合）。

* アセットを追加する権限を持つContent Hub ユーザーは、Content Hub ポータルで brand approved assets にアクセスするだけでなく、Content Hubにアセットをアップロードすることもできます。

* アセットをリミックスする権限を持つContent Hub ユーザーは、Content Hub ポータルで brand approved assets にアクセスできるだけでなく、Adobe Expressにアクセスできます（Adobe Expressの権限がある場合）。

### AEM Assets Content Hubでは、様々なタイプのユーザーが使用できる様々な製品プロファイルにはどのようなものがありますか？

製品プロファイルは、AEM Assets Content Hubを使用する様々なタイプのユーザーが利用できます。

* Content Hub ユーザー：AEM Assetsの制限付きユーザー

* Content Hub管理者：AEM Assetsの制限付きユーザー+ AEM管理者

* アセットを追加する権限を持つContent Hub ユーザー：AEM Assetsの限定ユーザー+ AEM ユーザー

* アセットをリミックスする権限を持つContent Hub ユーザー：AEM Assetsの限定ユーザー+ AEM ユーザー

### 管理者が組織のAEM Assets Content Hubを有効にするにはどうすればよいですか？

AEM Assets Content Hubを有効にするには、管理者がCloud Managerにログインしてプログラムを選択（または作成）し、「ソリューションとアドオン」タブで「AssetsとContent Hub」を有効にしてから、プログラムを更新する必要があります。 これにより、Adobe Admin ConsoleにContent Hub インスタンスが作成され、ユーザーアクセスを管理できます。
...
### AEM Assets ライセンスで利用できるContent Hub クレジットはいくつですか？

使用可能なContent Hub クレジットの数は、AEM Assets ライセンスによって異なります。

* Assets Ultimateには 3 つのContent Hub クレジットが含まれています。

* Assets Primeには、1 つのContent Hub クレジットが含まれます。

* 既存のAssets as a Cloud Serviceのお客様は、1 つのContent Hub クレジットを受け取ります。

### AEM Assets Content Hubのクレジットはどのように使用されますか。

Content Hubが有効になっている環境ごとに 1 つのContent Hub クレジットが使用されます。 例えば、実稼動環境、開発環境、ステージ環境でContent Hubを有効にするには、3 つのクレジットが必要です。

### 下位環境でContent Hubを有効にできますか？

はい。Content Hub クレジットが利用可能な場合は、開発やステージなどの下位環境でAEM Assets Content Hubを有効にすることができます。 有効な下位環境ごとに 1 つのクレジットを消費します。

### AEM Assets Content Hubで承認済みアセットにアクセスする権限を持つにはどうすればよいですか？

AEM Assets Content Hubのユーザーは、Content Hub ポータルでブランド承認済みアセットにアクセスできます。 Content Hub ユーザーになるには、AEM Limited Users 製品プロファイルに追加されている必要があります。

### AEM Assets Content Hubにアセットをアップロードする権限を付与するにはどうすればよいですか？

アセットを追加する権限を持つAEM Assets Content Hub ユーザーは、Content Hub ポータルで brand approved assets にアクセスするだけでなく、Content Hubにアセットをアップロードすることもできます。 アセットを追加する権限を持つContent Hub ユーザーになるには、AEM Limited Users 製品プロファイルおよびAEM Users 製品プロファイルに追加されている必要があります。

### AEM Assets Content Hubの設定ユーザーインターフェイスにアクセスする権限を持つにはどうすればよいですか？

AEM Assets Content Hub管理者は、Content Hubの設定ユーザーインターフェイスにアクセスできます。また、ブランド承認済みアセットへのアクセス、Content Hubへのアセットのアップロード、Adobe Express統合による画像の編集もできます（Adobe Expressの使用権限がある場合）。 Content Hub管理者になるには、AEM Limited Users およびAEM Administrators 製品プロファイルに追加されている必要があります。

### AEM Assets Content HubでAdobe Expressを使用して画像を編集する権限を付与するにはどうすればよいですか？

アセットをリミックスする権限を持つAEM Assets Content Hub ユーザーは、Content Hub ポータルでブランド承認済みアセットにアクセスできるだけでなく、Adobe Expressにアクセスできます（Adobe Expressの使用権限がある場合）。 アセットをリミックスする権限を持つContent Hub ユーザーにするには、AEM Limited Users 製品プロファイルおよびAEM Users 製品プロファイルに追加されている必要があります。
