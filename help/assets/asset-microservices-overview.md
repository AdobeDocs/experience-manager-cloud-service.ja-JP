...
* **直接バイナリアクセス**:Experience Manager環境用に設定されたアセットはCloud Binary Storeに転送（およびアップロード）され、その後、アセットマイクロサービスに [!DNL Experience Manager]よってクライアントは直接アクセスして作業を実行できます。これにより、ネットワークの負荷と、保存されるバイナリの重複を最小限に抑えることができます。
* **処理の外部化**[!DNL Experience Manager]：アセットの処理は 環境の外部でおこなわれるので、主要なデジタルアセット管理機能を提供しシステムでのエンドユーザーのインタラクティブな作業をサポートするためのリソース（CPU、メモリ）を節約できます。

## 直接バイナリアクセスを使用したアセットのアップロード {#asset-upload-with-direct-binary-access}

提供製品の一部として含まれている Adobe Experience Manager クライアントはすべて、直接バイナリアクセスを使用したアップロードをデフォルトでサポートしています。これには、Webインターフェイス、Adobeアセットリンク、[!DNL Experience Manager]デスクトップアプリを使用したアップロードが含まれます。

カスタムアップロードツールを使用できます。このツールは[!DNL Experience Manager] HTTP APIと直接連携します。 これらの API を直接使用することもできますし、アップロードプロトコルを実装している次のオープンソースプロジェクトを使用し拡張することもできます。
...
後処理ワークフローは、[!DNL Experience Manager]ワークフローエディターで作成および管理される、通常の[!DNL Experience Manager]ワークフローモデルです。 ユーザーは、あらかじめ用意されている使用可能なワークフローステップやカスタムワークフローの使用など、アセットに対して追加の処理ステップを実行するようにワークフローを設定できます。
