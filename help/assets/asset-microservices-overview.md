...
アセットマイクロサービスは、サードパーティのレンダリングツールとメソッド（[!DNL ImageMagick]やFFmpegトランスコーディングなど）が不要になり、設定が簡単になると同時に、一般的なファイル形式に対する基本的な機能をデフォルトで提供します。
...
* **外部化された処理**:アセットの処理は環境の外部でおこなわれ、アセットのリソース（CPU、メモリ）を節約して重要なDigital Asset Management(DAM)機能を提供し、エンドユーザー向けのシステムとの対話的な作業をサポートします [!DNL Experience Manager] 
