...
Content Hubのユーザーインターフェイス設定にアクセスして設定できるのは、Content Hub管理者権限を持つユーザーのみです。 設定インターフェイスにアクセスする前に、Adobe Admin Consoleを通じて管理者権限を付与する必要があります。

### 管理者は、AEM Assets Content Hubの UI で何を設定できますか？

管理者は、AEM Assets Content Hub設定ユーザーインターフェイスで以下の設定を行うことができます。

* メタデータフィールドの読み込み（必須フィールドおよび自動承認設定を含む）

* 検索ページに表示される検索フィルター

* インデックス作成に使用される検索可能なメタデータフィールド

* アセットカードのメタデータ（カードに最大 6 つのフィールドを表示）

* アセットの詳細表示プロパティ

* ブランディング要素（ロゴ、バナーテキスト、カラー、バナー画像）

* アセットの表示ルール（期限切れのアセットの動作を含む）

* レンディションおよびダウンロードオプション

* カスタムナビゲーションリンク（外部タブ）

* コレクションおよび共有設定（公開リンクと権限）

これらの設定は、エンドユーザーに対するContent Hubの視覚的なエクスペリエンスと機能的動作の両方を制御します。


### 管理者がAEM Assets Content Hubでアセットのアップロード用に新しいメタデータフィールドを追加する方法

管理者は、「**インポート**」タブの **メタデータを追加** をクリックし、ラベルを指定し、メタデータスキーマから適切なプロパティにマッピングし、入力タイプを選択し、フィールドが必須かオプションかを決定することで、アセットのアップロード用の新しいメタデータフィールドを追加できます。 確認して保存すると、アセットのアップロード中にユーザーが新しいフィールドを使用できるようになります。

### AEM Assets Content Hubのアセットの自動承認設定とは何ですか？また、どのように機能しますか？

自動承認設定を有効にすると、新しいアセットがアップロード時にContent Hubで直ちに使用可能になります。 デフォルトでは、この設定はオフになっています。AEMの作成者または管理者は、新しいアセットがContent Hub ポータルに表示される前に、手動で承認する必要があります。

### 管理者はAEM Assets Content Hubでアセット検索のフィルターを設定するにはどうすればよいですか？

管理者は、「**フィルターを追加** をクリックし、フィルターラベルを指定し、適切なメタデータプロパティにマッピングして、データタイプを選択し、追加を確認することで、アセット検索のフィルターを設定できます。 フィルター条件に一致するアセットが 1 つ以上ある場合は、フィルターがインターフェイスで使用できるようになります。 変更を適用するには、「**保存**」をクリックします。

### アセットの詳細表示に表示できるアセットプロパティと、AEM Assets Content Hubでの設定方法を教えてください。

ファイル名、タイトル、形式、サイズなどのアセットプロパティや、カテゴリなどのカスタムフィールドは、アセットの詳細表示に表示することができます。 管理者はこれらのプロパティを追加するには、**メタデータを追加** をクリックし、ラベルを指定して、それをメタデータスキーマにマッピングし、データタイプを選択して、確認して変更を保存します。

### 管理者は、AEM Assets Content Hubのアセットカードに表示されるメタデータの詳細をどのようにカスタマイズできますか。

管理者は、アセットカード設定の **メタデータを追加** をクリックし、ラベル（ファイル形式など）を指定し、メタデータスキーマ内の適切なプロパティにマッピングして、追加を確認することで、アセットカードに表示されるメタデータの詳細をカスタマイズできます。 アセットカードには最大 6 つのメタデータフィールドを表示できます。
