...
1. プロパティのラベルを指定し、**[!UICONTROL メタデータ]** フィールドを使用してプロパティにマッピングし、新しいアセットメタデータに適した入力タイプを選択します。 入力タイプラベルが更新され、Adobe Experience Manager（AEM）に合わせて調整されます。 使用可能な入力タイプには、テキスト、タグ、日付、番号、ドロップダウン、複数値テキスト、スマートタグ、スマートカラータグなどがあります。 特定のメタデータフィールド（xcm:colorDistribution、xcm:machineKeywords）の場合、入力タイプは自動的に割り当てられ、変更できません。
...
コンテンツハブを使用すると、管理者はアセットの検索中に表示されるフィルターを設定できます。 設定ユーザーインターフェイスを使用して、最大40個のフィルターを設定できます。

新しいフィルターを追加するには、次の手順を実行します。

1. [設定](#access-configuration-options-content-hub)ユーザーインターフェイスで、「**[!UICONTROL フィルター]**」をクリックします。
1. 「**[!UICONTROL フィルターを追加]**」をクリックします。
1. フィルターのラベルを指定し、**[!UICONTROL メタデータ]** フィールドを使用してプロパティにマッピングし、新しいフィルターに適した入力タイプを選択します。 入力タイプラベルが更新され、Adobe Experience Manager（AEM）に合わせて調整されます。 使用可能な入力タイプには、テキスト、タグ、日付、番号、ドロップダウン、複数値テキスト、スマートタグ、スマートカラータグなどがあります。 特定のメタデータフィールド（xcm:colorDistribution、xcm:machineKeywords）の場合、入力タイプは自動的に割り当てられ、変更できません。
...
同様に、使用可能な各フィルターの横にある ![編集アイコン](assets/do-not-localize/edit_icon.svg) をクリックしてラベルを編集したり、削除アイコンをクリックして既存のフィルターを削除したりできます。 すべての変更を行ったら、「**[!UICONTROL 保存]**」をクリックして、変更を適用します。![コンテンツハブの設定 UI のフィルター](assets/add-filters.png)
...
**アセットカード**&#x200B;に表示する必要がある主なメタデータプロパティを最大 6 フィールドまで設定することもできます。アセットカードの![ キーメタデータ](/help/assets/assets/asset-card-metadata.png)
...
>[!CAUTION]
>
>検索可能としてマークしたフィールドを慎重に選択することをお勧めします。 ユーザーが検索する可能性が高く、有意義な結果を返すのに役立つフィールドのみを含めます。 日付や数値などのフィールドは、検索可能なコンテンツではなく、フィルタリングや並べ替えに適しています。 検索可能またはフィルター可能として多くのフィールドをマークすると、検索結果の品質とパフォーマンスに影響を与える可能性があります。
> 
> 設定ユーザーインターフェイスを使用して、最大30個の検索フィールドを設定できます。
>
>タグプロパティはフィルタリングのみ有効になっており、検索できません。
...
同様に、使用可能な各メタデータプロパティの横にある ![編集アイコン](assets/do-not-localize/edit_icon.svg) をクリックしてプロパティを編集したり、削除アイコンをクリックして既存のプロパティを削除したりできます。 すべての変更を行ったら、「**[!UICONTROL 保存]**」をクリックして、変更を適用します。![コンテンツハブの設定 UI の検索](assets/configuration-search.png)

追加する検索フィールドのリストは、順序が検索結果に影響を与えるため、関連性の高い順序である必要があります。

>[!NOTE]
>
>使用するフィールドとContent Hub設定ページを同期させる必要があります。 UIを使用してフィールドを削除しないでください。特に、それらのフィールドがABAC ルールまたはダイレクト検索APIの使用で使用されている場合は、削除しないでください。

### ブランディング {#configure-branding-content-hub}

管理者は、ブランディング要件に合わせて [!DNL Content Hub] ポータルをカスタマイズできます。![既定をリセット](/help/assets/assets/reset-default-content-hub.png)
...
![ブランディング](/help/assets/assets/ColorPalette.svg) **[!UICONTROL ブランディング]** ページで、「**[!UICONTROL バナー]**」セクションの各フィールドを使用して、タイトルと本文をバナーに追加します。**[!UICONTROL バナーテキストのカラー]**&#x200B;の横にある正方形ボックスをクリックして、バナーテキストのカラーピッカーからテキストカラーを選択するか、カラーピッカーの正方形ボックスの横のフィールドにカラーの 16 進コードを指定します。![バナーテキストコンテンツハブ](/help/assets/assets/banner-text-content-hub.png)

#### プライマリカラーとセカンダリカラーの変更{#Change-the-primary-and-secondary-color}

![ ブランディング](/help/assets/assets/ColorPalette.svg) **[!UICONTROL ブランディング]** ページでは、「**[!UICONTROL カラー]**」セクションを使用して、カラーピッカーを使用して選択するか、カラーの 16 進数コードを定義することによって、プライマリカラーとセカンダリカラーを設定します。 これらのカラーは、UI 要素の背景、テキスト、アイコンのカラーを設定して、[!DNL Content Hub] UI をブランドのテーマに合わせます。![ プライマリカラーとセカンダリカラー](/help/assets/assets/primary-secondary-color-content-hub1.png)
...
1. [設定](#access-configuration-options-content-hub)ユーザーインターフェイスで、「**[!UICONTROL アセットカード]**」をクリックします。
...
1. **[!UICONTROL 配信の承認されたアセットの表示をユーザーに許可]**&#x200B;を有効にして、コンテンツハブで配信の承認されたすべてのアセットを表示するように切り替えます。
1. 「**[!UICONTROL 保存]**」をクリックして、変更内容を適用します。
...
同様に、各 URL の横にある ![編集アイコン](assets/do-not-localize/edit_icon.svg) をクリックしてリンクを編集したり、削除アイコンをクリックして既存の URL を削除したりできます。 すべての変更を行ったら、「**[!UICONTROL 保存]**」をクリックして、変更を適用します。![コンテンツハブの設定 UI のカスタムリンク](assets/configuration-custom-links1.png)

カスタムリンクは、コンテンツハブのホームページの「インサイト」タブの横に新規タブとして表示されます。![コンテンツハブの設定 UI の「カスタムリンク」タブ](assets/configuration-ui-custom-link-tab.png)
...
1. **[!UICONTROL 公開リンクを有効にする]**&#x200B;切替スイッチを有効にして、外部ユーザーがコンテンツハブにログインせずにアセットにアクセスしてダウンロードするために使用できる公開リンクの作成を許可します。

1. **[!UICONTROL 表示専用コレクション]**&#x200B;切替スイッチを有効にすると、すべてのユーザーがアクセス可能ですが、作成者と管理者のみが編集できるコレクションを許可します。

1. **[!UICONTROL 公開コレクション]**&#x200B;切替スイッチを有効にして、すべてのユーザーがアクセスおよび編集できるコレクションを許可します。 **[!UICONTROL コレクションのみを表示]**&#x200B;および&#x200B;**[!UICONTROL 公開コレクション]**&#x200B;の切替スイッチが無効になっている場合、デフォルトでは、管理者以外のユーザーが非公開コレクションのみを作成できます。
...
### AEM Assets Content Hubのユーザーインターフェイス設定にアクセスして設定できるのは誰ですか？

Content Hubの管理者権限を持つユーザーのみが、Content Hub ユーザーインターフェイス設定にアクセスして設定できます。 設定インターフェイスにアクセスする前に、Adobe Admin Consoleから管理者権限を付与する必要があります。

### AEM Assets Content Hub UIでは、管理者は何を設定できますか？

管理者は、AEM Assets Content Hub設定ユーザーインターフェイスで次の設定を行うことができます。

* メタデータフィールドの読み込み（必須フィールドと自動承認設定を含む）

* 検索ページに表示される検索フィルター

* インデックス作成に使用される検索可能なメタデータフィールド

* アセットカードのメタデータ（カードに表示される最大6つのフィールド）

* アセットの詳細ビューのプロパティ

* ブランディング要素（ロゴ、バナーテキスト、カラー、バナー画像）

* アセットの表示ルール（期限切れのアセット行動を含む）

* レンディションとダウンロードのオプション

* カスタムナビゲーションリンク（外部タブ）

* 収集と共有の設定（公開リンクと権限）

これらの設定は、エンドユーザーに対するContent Hubの視覚的なエクスペリエンスと機能的な動作の両方を制御します。


### AEM Assets Content Hubでアセットをアップロードする際に、管理者が新しいメタデータフィールドを追加する方法を教えてください。

管理者は、「**インポート**」タブの「**メタデータを追加**」をクリックし、ラベルを指定し、メタデータスキーマから適切なプロパティにマッピングし、入力タイプを選択し、フィールドが必須またはオプションであるかどうかを決定することで、アセットのアップロード用に新しいメタデータフィールドを追加できます。 確認して保存すると、新しいフィールドはアセットのアップロード中にユーザーが使用できるようになります。

### AEM Assets Content Hubのアセットの自動承認設定とは何ですか？その仕組みはどのようになっていますか？

自動承認設定を有効にすると、新しいアセットをアップロード時にすぐにContent Hubで利用できるようになります。 デフォルトでは、この設定はオフになっており、AEMの作成者または管理者は、新しいアセットがContent Hub ポータルに表示される前に、手動で承認する必要があります。

### AEM Assets Content Hubでアセットを検索する際に、管理者がフィルターを設定する方法を教えてください。

管理者は、**フィルターを追加**&#x200B;をクリックし、フィルターラベルを指定して、適切なメタデータプロパティにマッピングし、データタイプを選択して、追加を確認することで、アセット検索用のフィルターを設定できます。 フィルター条件に一致するアセットが少なくとも1つ存在すると、フィルターはインターフェイスで使用できるようになります。 **保存**&#x200B;をクリックすると、変更が適用されます。

### アセットの詳細ビューに表示できるアセットのプロパティと、AEM Assets Content Hubでの設定方法を教えてください。

ファイル名、タイトル、形式、サイズ、カテゴリーなどのカスタムフィールドなどのアセットプロパティは、アセットの詳細ビューに表示できます。 管理者は、**メタデータを追加**&#x200B;をクリックし、ラベルを指定してメタデータスキーマにマッピングし、データタイプを選択し、確認して、変更を保存することで、これらのプロパティを追加します。

### AEM Assets Content Hubのアセットカードに表示されるメタデータの詳細を管理者がカスタマイズするにはどうすればよいですか？

管理者は、アセットカード設定の「**メタデータを追加**」をクリックし、ラベル（ファイル形式など）を指定し、メタデータスキーマの適切なプロパティにマッピングし、追加を確認することで、アセットカードに表示されるメタデータの詳細をカスタマイズできます。 アセットカードには、最大6つのメタデータフィールドを表示できます。

### 管理者は、組織のブランディング要件に合わせてAEM Assets Content Hub ポータルをカスタマイズできますか？

はい。 AEM Assets Content Hubでは、メインバナーのタイトルや本文を編集し、カラーピッカーや16進数コードでテキストカラーを選択して、背景、テキスト、アイコンのセカンダリカラーとプライマリカラーを設定することで、ルックアンドフィールをパーソナライズできます。
