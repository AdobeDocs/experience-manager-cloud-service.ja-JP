...
* **Content Advisor**: Content Advisorは、Experience Manager Assets リポジトリと統合されたユーザーインターフェイスを提供し、リポジトリで使用可能なデジタルアセットを参照または検索できるようにします。 その後、アプリケーションのオーサリングエクスペリエンスで使用できます。
Content Advisorは、AdobeまたはAdobe以外のアプリケーションと統合できます。
...
* **AEM AssetsとEdge Delivery Servicesのドキュメントベースのオーサリングフローの統合**: [!DNL AEM Assets]が[!DNL Microsoft Word]や[!DNL Google Docs]などのドキュメントベースのオーサリングツールと統合すると、オーサリングツールにContent Advisorが提供されます。 コンテンツアドバイザーを使用して[!DNL AEM Assets]にアクセスし、承認済みアセットをコンテンツに挿入します。
