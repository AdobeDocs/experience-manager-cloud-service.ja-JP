...
### AEM Assets Content Hub内のAssets Insights に記載されている機能にアクセスできるのは誰ですか。

AEM Assets Content Hubのユーザーは、Assets Insights の節で説明しているアクションを実行し機能にアクセスすることができます。

### AEM Assets Content Hubの「インサイト」タブで使用できるアセットインサイトは何ですか？

リポジトリー内のアセット数、コレクション数、Assetsのアップロード（年、月、日ごと）、アクティブユーザー（日または月ごと）、AEM Assets Content Hub内のファイル形式に基づいたアセットの分類を表示できます。

### AEM Assets Content Hubにアップロードされたアセットの統計情報を表示するにはどうすればよいですか？

AEM Assets Content Hubでアップロードされたアセットとコレクションの統計情報を表示するには、「インサイト」タブに移動します。このタブで、アップロード履歴を年別、月別または日別にトラックできます。

### AEM Assets Content Hubのユーザーアクティビティに関して分析できる指標は何ですか。

アクティブユーザーの数を日別または月別に分析できます。この分析は、AEM Assets Content Hub内で折れ線グラフを使用して視覚的に表されます。
