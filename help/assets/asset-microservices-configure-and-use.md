...
特定の MIME タイプのアセットが処理される際は、まず、その MIME タイプがレンディション仕様の除外 MIME タイプの値と照合されます。そのリストと一致する場合、この特定のレンディションはアセット(ブロックリスト)に対して生成されません。

それ以外の場合は、MIMEタイプが、含まれているMIMEタイプと比較され、リストと一致する場合は、レンディションが生成されます(許可リスト)。
...
* アセットを処理するためのカスタムワークフロー手順です。
* 製品情報やプロセス情報など、外部システムからアセットにメタデータやプロパティを追加するための統合。
* 外部サービスによって行われた追加処理。

後処理ワークフロー設定を Adobe Experience Manager に追加する作業は、次の手順で構成されます。

* 1つ以上のワークフローモデルを作成します。 ドキュメントは、これを *後処理ワークフローモデルと呼びますが*、通常のExperience Managerワークフローモデルです。
* これらのモデルに対する追加特定のワークフロー手順を示します。 手順は、ワークフローモデルの設定に基づいてアセットに対して実行されます。
* 追加 [!UICONTROL DAM Update Asset Workflow Completed Process] stepを最後に実行します。 この手順を追加すると、Experience Managerは処理が終了し、アセットを処理済みとしてマークでき、 *新規* （アセットに対して表示）を確認できます。
* カスタムワークフローランナーサービスの設定を作成します。この設定を使用すると、後処理ワークフローモデルの実行をパス（フォルダーの場所）または通常の式で設定できます。
