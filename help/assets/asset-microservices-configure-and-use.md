...
# アセットマイクロサービスの使用を開始する {#get-started-using-asset-microservices}
...
特定のMIMEタイプを持つアセットが処理されると、まず、そのMIMEタイプがレンディション仕様の除外されたMIMEタイプの値に対して確認されます。 そのリストと一致する場合、この特定のレンディションはアセットに対して生成されません（「ブラックリスト」）。

それ以外の場合は、MIMEタイプが含まれているMIMEタイプと比較され、リストと一致する場合は、レンディションが生成されます（「ホワイトリスト」）。
...
* ユーザは、アセットユーザーインターフェイスでフォルダを選択し、 **[!UICONTROL Properties]** actionを使用してフォルダのプロパティ画面を開き、「 **[!UICONTROL Processing Profiles]** 」タブをクリックし、ドロップダウンでそのフォルダに適した処理プロファイルを選択できます。 選択内容は、「保存して閉じる」アク **[!UICONTROL ションの際に保存され]** ます。
