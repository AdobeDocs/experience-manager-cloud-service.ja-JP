...
多くの場合、デジタルアセットは、利用の条件と期間を明記したライセンスに関連付けられています。[!DNL Adobe Experience Manager Assets]は[!DNL Experience Manager]プラットフォームと完全に統合されているので、アセットの有効期限に関する情報やアセットの状態を効率的に管理できます。 ライセンス情報をアセットに関連付けることもできます。
...
1. [!DNL Assets] コンソールで、ツールバーの「**[!UICONTROL 検索]**」をクリックすると [!DNL Experience Manager] 検索ボックスが表示されます。
