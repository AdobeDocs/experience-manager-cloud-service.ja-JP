...
1. AEM as a Cloud Service プログラムで高度なネットワークを設定し、IP 許可リストへの登録を有効にしている場合は、Workfront IP をこの許可リストに追加して、イベント購読と様々な API 呼び出しが AEM に渡されるようにする必要があります。
   ...
   * [Workfront Event Subscription API IP](https://experienceleague.adobe.com/docs/workfront/using/adobe-workfront-api/event-subscriptions/event-subs-api.html?lang=ja)
