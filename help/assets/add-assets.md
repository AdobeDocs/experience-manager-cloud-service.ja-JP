...
* [一括取り込みツール](#bulk-ingestion-tool):大量のアセットを取り込む場合、場合によっては取り込むため、デプロイ時には最初に取り込むために使用 [!DNL Experience Manager]します。

### アセット取り込み一括ツール{#bulk-ingestion-tool}

追加次の詳細を参照してください。

* このメソッドを使用する場合は、次の場合に使用します。
* 該当する人物
* 設定手順
* 取り込みジョブの管理方法とステータスの確認方法。
* 取り込むアセットの管理やキュレーションに関する注意事項

ツールを設定するには、次の手順に従います。

1. 一括インポート設定を作成します。  ツール/アセット/一括インポートに移動し、「作成」ボタンを選択します。
...
## シナリオベースのアップロード方法{#upload-methods-comparison}

| アップロード方法 | 用途? | プライマリの個人（管理者、開発者、クリエイティブユーザー、マーケター） |
|---------------------|-------------------------------------------------------------------------------------------|------------|
| アセットコンソール/UI | 時折アップロード、プレスアンドドラッグの容易さ、ファインダーのアップロード。 大量のバルクインジメンションに対しては使用できません。 | すべての  |
| アップロードAPI | アップロード中のアセットの動的な判定 | デベロッパー |
| Desktop App | 容量の少ないアセットの取り込み、移行 | 管理者、マーケティング担当者 |
| アセットリンク | クリエイティブやマーケターが、サポートされるCreative Cloudーのデスクトップアプリ内でアセットに関して共同作業を行えるようにします。 | クリエイティブ、マーケティング担当者 |
| 一括取り込みツール | データストアからのバルクアセットの取り込み  移行や、時折行うバルクインジメンションに推奨。 | 管理者、開発者 |
