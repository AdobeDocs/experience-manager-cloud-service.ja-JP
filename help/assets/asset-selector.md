...
| `modalMode` | 認証フローをモーダル（ポップアップ）に表示するかどうかを示すブール値。`true` に設定すると、認証フローがポップアップで表示されます。`false` に設定すると、認証フローはページ全体をリロードして表示されます。_Note :_ユーザーがブラウザーポップアップを無効にしている場合、UX を向上させるために、この値を動的に制御できます。 |
