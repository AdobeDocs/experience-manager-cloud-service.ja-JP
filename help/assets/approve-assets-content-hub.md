...
### Experience Manager as a Cloud ServiceでContent Hubのアセットを承認する目的は何ですか？ {#approving-assets-content-hub}

アセットを承認すると、最新および承認済みのバージョンのみをContent Hubで使用できるようになり、すべてのチャネルとアプリケーションでブランドの厳密な一貫性が維持されます。 この制御されたプロセスにより、ブランドマネージャーやマーケターのアセット管理が合理化されます。

### Content Hub用のアセットを承認するための前提条件を教えてください。

アセットメタデータ（特にアセットプロパティの **ステータス** フィールド）を編集するには、AEM Assets as a Cloud Serviceへのアクセス権と書き込み権限が必要です。

### AEM as a Cloud ServiceでAssets ビューを使用して 1 つのアセットを承認するにはどうすればよいですか？

アセットを選択し、ツールバーの **詳細** をクリックして「**基本**」タブに移動し、「**ステータス**」ドロップダウンリストから **承認済み** を選択して、「**保存** をクリックします。 アセットがContent Hubで使用できるようになります。

### Content Hubのアセットを一括で承認することはできますか。承認する場合はその方法を教えてください。

はい、アセットは一括で承認できます。 Assets表示で、複数のアセットを選択し、「**一括メタデータ編集**」をクリックし、「プロパティ」の下の「**ステータス** フィールドで **承認済み** を選択し、「**保存**」をクリックします。 選択したアセットはすべてContent Hubで使用できます。
...
### 「承認ターゲット」フィールドとは何ですか？また、アセットの公開にどのような影響を与えますか？

アセットの詳細ページの「**承認ターゲット**」フィールドでは、承認されたアセットの公開先を選択できます。 オプションには、**配信** （OpenAPI を使用した Dynamic Media とContent Hubの両方に公開）または **Content Hub** のみ含まれます。 オプションを選択しない場合、Assets as a Cloud Service環境のデフォルト値が適用されます。 詳しくは [&#x200B; 承認済みアセットのデフォルトの承認ターゲットと公開宛先 &#x200B;](#default-approval-target-options-publish-destinations) を参照してください。


### Assetsのアセットの詳細を表示ページに「承認ターゲット」フィールドが表示されない場合はどうなりますか。

Assetsのアセットの詳細を表示ページに「**承認ターゲット**」フィールドがない場合は、メタデータフォームを編集し、使用可能なコンポーネントからフォームに **承認対象** フィールドをドラッグして、「**保存**」をクリックする必要があります。 アセットの承認ターゲットを設定できます。

### 管理者ビューで新しく取り込んだアセットの承認を自動化するにはどうすればよいですか？

オーサー環境でフォルダーを作成し、**ツール**/**Assets**/ **メタデータプロファイル** に移動して、メタデータプロファイルを作成および編集します。 1 行のテキストフィールドを追加し、「承認済みAssets **というラベルを付け** 「」にマッピングします。/jcr:content/metadata/dam:status&#39;を選択し、デフォルト値を `approved` に設定します。 メタデータプロファイルをフォルダーに適用します。 これにより、フォルダーに追加された新しいアセットが自動的に承認されます。

### Content Hubで承認済みアセットにアクセスできるのは誰ですか？また、どのような制御が行われますか？

承認済みアセットは、Content Hub内の同じ組織に属するユーザーが使用できます。 厳格な制御により、最新の承認済みバージョンのみがアクセスできるようになり、ブランドの一貫性とセキュリティが維持されます。
