...
アセットを承認すると、AEM Assets Content Hub内で最新および承認済みのバージョンのみを使用できるようになり、すべてのチャネルとアプリケーションでブランドの厳密な一貫性が維持されます。 この制御されたプロセスにより、ブランドマネージャーやマーケターのアセット管理が合理化されます。

### AEM Assets Content Hubのアセットを承認するための前提条件を教えてください。

アセットメタデータ（特にアセットプロパティの **ステータス** フィールド）を編集するには、AEM Assets as a Cloud Serviceへのアクセス権と書き込み権限が必要です。

### AEM Assets Content Hubで使用できるように、AEM as a Cloud ServiceのAssets ビューを使用して 1 つのアセットを承認する方法を教えてください。

アセットを選択し、ツールバーの **詳細** をクリックして「**基本**」タブに移動し、「**ステータス**」ドロップダウンリストから **承認済み** を選択して、「**保存** をクリックします。 アセットがAEM Assets Content Hubで使用できるようになります。

### AEM Assets Content Hubのアセットを一括で承認することはできますか。承認する場合はその方法を教えてください。

はい、アセットは一括で承認できます。 Assets表示で、複数のアセットを選択し、「**一括メタデータ編集**」をクリックし、「プロパティ」の下の「**ステータス** フィールドで **承認済み** を選択し、「**保存**」をクリックします。 選択したすべてのアセットは、AEM Assets Content Hubで使用できます。
...
### AEM Assetsのアセットの詳細を表示ページに「承認ターゲット」フィールドが表示されない場合はどうなりますか。
...
### AEM Assets管理者ビューで新しく取り込んだアセットの承認を自動化するにはどうすればよいですか？
...
### AEM Assets Content Hubで承認済みアセットにアクセスできるのは誰ですか？また、どのような制御が行われていますか？

承認済みアセットは、AEM Assets Content Hub内の同じ組織に属するユーザーが使用できます。 厳格な制御により、最新の承認済みバージョンのみがアクセスできるようになり、ブランドの一貫性とセキュリティが維持されます。
