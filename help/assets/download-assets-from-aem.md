...
* [デスクトップアプリ](https://experienceleague.adobe.com/docs/experience-manager-desktop-app/using/using.html?lang=ja#download-assets)
...
ユーザーがフォルダーまたはコレクションを含むダウンロードをリクエストした場合、AEMは、ダウンロードしたフォルダーまたはコレクションの下にあるアイテム数（Assets、フォルダーまたはレンディション）を簡単に見積もり、リクエストされたダウンロードがサポートされている制限内であることを確認します。 デフォルトでは、50,000個を超えるアイテムを含むダウンロードはブロックされ、AEMには`The selected items are larger than the configured maximum download limit` メッセージが表示されます。

次に示すOSGI設定をアプリケーションコードに追加し、[Cloud Manager パイプライン経由でデプロイすることで、ダウンロードサイズの制限を増やすことができます](https://experienceleague.adobe.com/ja/docs/experience-manager-cloud-service/content/implementing/deploying/configuring-osgi)。
