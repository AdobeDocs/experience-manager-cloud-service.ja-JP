...
AEM Assets Content Hubのアセットカードで「**Adobe Expressを使用して編集**」オプションを表示するには、アセットを新しいバリエーションに再構成する権限を持つ[&#128279;](#onboard-content-hub-users-add-assets)Content Hub ユーザーの権限に加えて、Adobe Express EnterpriseまたはTeamsの権限（[&#x200B; プラン &#x200B;](https://www.adobe.com/jp/express/pricing)を参照）が必要です。
...
Content Hubは、すべての`cq5dam.*` レンディションの中で最も幅の大きいレンディションを選択します。カスタムレンディションは、エンコードされた幅が既存の自動生成レンディションを超えた場合にのみ勝者となります。
