...
[](https://docs.adobe.com/content/help/en/experience-manager-core-components/using/introduction.html) AEM Core Componentsは、この目的で必要な読み取り操作を提供し、JSON出力をカスタマイズできる、非常に包括的で柔軟性の高いカスタマイズ可能なAPIを提供しますが、AEMの専用のAEMテンプレートに基づくページでホストする必要があるので、実装のノウハウが必要です。 すべてのSPA開発組織がこのような知識に直接アクセスできるわけではありません。
...
>
>* [CORS／AEM の説明](https://helpx.adobe.com/experience-manager/kt/platform-repository/using/cors-security-article-understand.html)
...
>[!NOTE]
>
>子アセットとフォルダーのアセットタイプに応じて、子エンティティのリストには、それぞれの子エンティティを定義するすべてのプロパティのセットが既に含まれている場合があります。 または、この子エンティティリストのエンティティに対して、一部のプロパティのみ公開される場合もあります。

### Assets {#assets}

アセットが要求された場合、応答はメタデータを返します。（それぞれのアセットスキーマで定義されるタイトル、名前、その他の情報など）。
