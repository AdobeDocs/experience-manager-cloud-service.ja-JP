...
上記は、アセットに対して [!DNL Experience Manager] が管理できる基本的なメタデータプロパティです。これにより、ユーザーはすべてのアセットを表示できます。例えば、最終変更日にアセットを並べ替えると、最近追加または変更されたアセットを検出する場合に便利です。
...
* アセットの新しいバージョンがアップロードされます
* アセットが再処理されます

最終変更日とユーザーは変更しません。

* アセットを移動または名前変更したとき
* アセットがチェックアウトされたとき、チェックインまたはバージョン
* アセットが公開または非公開にされたとき
* メタデータの更新時
* 参照またはコレクションの更新
