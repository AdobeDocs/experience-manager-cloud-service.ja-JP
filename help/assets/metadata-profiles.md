...
Experience Manager Assetsでのプロファイルの使用に関する重要な概念は、プロファイルがフォルダーに割り当てられる点です。 プロファイル内では、メタデータプロファイルの形式で、ビデオプロファイルまたはイメージプロファイルと共に設定されています。これらの設定は、フォルダーのコンテンツを（そのサブフォルダーコンテンツを含めて）処理します。したがって、ファイルやフォルダーの命名方法、サブフォルダーの配置方法、これらのフォルダー内のファイルの処理方法は、プロファイルによるそれらのアセットの処理方法に大きな影響を与えます。
一貫性のある適切なファイルおよびフォルダーの命名戦略と優れたメタデータプラクティスを使用することで、デジタルアセット収集を最大限に活用し、適切なファイルが適切なプロファイルで処理されるようにします。
