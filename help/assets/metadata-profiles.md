...
1. メタデータプロファイルのタイトル（「サンプルメタデータ」など）を入力し、「 」を選択します。 **[!UICONTROL 送信]**. メタデータプロファイルの「フォームを編集」が表示されます。
1. コンポーネントをクリックし、 **[!UICONTROL 設定]** タブをクリックします。 例えば、 **[!UICONTROL 説明]** コンポーネントを編集し、そのプロパティを編集します。
**[!UICONTROL 説明]**&#x200B;コンポーネントについて、次のプロパティを編集します。

   * **[!UICONTROL フィールドラベル]** - メタデータプロパティの表示名。ユーザーの参照用のみで使用します。
   * **[!UICONTROL プロパティにマッピング]**  — このプロパティの値は、リポジトリー内の保存先のアセットノードへの相対パスまたは名前を提供します。 この値は、パスがアセットのノードの下にあることを示すので、常に「`./`」から開始する必要があります。
...
1. クリック **[!UICONTROL フォルダーへのメタデータプロファイルの適用]** 新たにアップロードされたアセットを受け取る 1 つ以上のフォルダーを選択し、「 **[!UICONTROL 完了]**. 既にプロファイルが割り当てられているフォルダーには、フォルダー名のすぐ下にプロファイルの名前が表示されます。
...
1. チェックマークを選択して対象のフォルダーを選択し、「**プロパティ**」を選択します。
...
* CRXDE Lite で、`/content/dam/jcr:content` ノードに移動します。プロパティを追加します。 `metadataProfile:/etc/dam/metadata/dynamicmedia/<name of metadata profile>`. 「**すべて保存**」をクリックします。
...
1. クリック **[!UICONTROL フォルダーからのメタデータプロファイルの削除]** プロファイルを削除する 1 つまたは複数のフォルダーを選択し、「 」をクリックします。 **[!UICONTROL 完了]**.
