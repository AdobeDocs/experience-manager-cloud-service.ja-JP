...
* AEM オーサー層：
Web コンテンツを管理するためのweb ベースのインターフェイス（関連するAPI）を提供します。 これは、次の両方の方法で機能します。
...
* Assets Compute サービス：
   * アップロードされた画像、ビデオ、ドキュメント（PDF、Adobe Photoshop ファイルなど）の処理を担当します。 処理では、Adobe AIを使用して画像やビデオのメタデータ（説明的なタグやプライマリカラートーンなど）を抽出し、Adobe PhotoshopやAdobe Lightroom APIなどのAPIを使用してレンディション（様々なサイズやフォーマットなど）を生成できます。
...
* Edge 配信サービス：
   ...
   * [Edge Delivery Servicesを利用して、AEM as a Cloud Serviceの基盤となるアーキテクチャと重要な要素を確認しましょう](https://experienceleague.adobe.com/docs/experience-manager-learn/cloud-service/introduction/architecture.html?lang=ja)
