...
契約書に署名すると、Adobeが環境をプロビジョニングし、契約書で定義されたオンボーディング担当者にウェルカムメールが送信され、Adobeがオンボーディングコールを編成します。 このワークフローは、通常、今後のサービス開始日が設定されていない限り、契約成約から数日後に発生します。

## システムプロビジョニング {#provisioning}

契約が締結されると、契約の対象となるすべてのAEM クラウド環境がAdobeによって自動的にプロビジョニングされます。 これらの環境は、同様にCloud Managerのプログラムにリンクされています（詳しくは、後ほど説明します）。 各サブスクリプションには、少なくとも1つの実稼動環境、1つのステージング、オプションの開発またはテストのAEM クラウド環境が含まれます。

## オンボーディングコール {#onboarding-call}
...
* 必要に応じて、SME （分野の専門家）を追加します。

電話会議の目的は、質問したり、サポートプロセスを理解したり、場合によっては製品を初めて見たりする機会を提供することです。

## ウェルカムメール {#welcome-email}

環境プロビジョニングプロセスが完了すると、契約で定義されたシステム管理者は、Adobe Experience Cloudへのアクセス権が付与されたことを確認するウェルカムメールを受信します。 このドキュメントのジャーニーを読んでいるのであれば、おそらくあなたがその人でしょう。

このメールには、Experience Cloud サービスと Cloud Manager セルフサービスポータルの使用を開始する方法が詳しく記載されています。 さらに、このメールには、サポートリソース、フォーラム、FAQ などの参照先に関する重要な情報も含まれています。 メールで提供されるリソースのリストでは、AEM クラウド環境へのアクセス方法の詳細も確認できます。
...
これらの準備手順を理解できたので、自分自身でシステムにログインする準備がほぼ整いました。 システム管理者の視点から、AEM as a Cloud Serviceの概要を説明します。 次に、基本用語を説明します。 ジャーニーの次のパートである [AEM as a Cloud Service の用語](terminology.md)に進んでください。
