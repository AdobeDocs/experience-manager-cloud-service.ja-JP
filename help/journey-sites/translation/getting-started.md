...
このドキュメントでは、AEM で Sites コンテンツの翻訳にどのように取りかかればよいかを説明します。 読み終えると、次のことができるようになります。
...
Sites コンソールには、コンテンツの構造の概要が表示されるので、コンテンツの移動や、新しいページの作成、ページの移動とコピー、コンテンツの公開などの管理が容易になります。

Sites コンソールにアクセスするには：

1. グローバルナビゲーションメニューで、**ナビゲーション**／**Sites** を選択します。
...
1. 列内の項目のチェックボックスをタップまたはクリックすると、その項目を選択でき、選択した項目の詳細が右側の列に表示され、上のツールバーで選択した項目で使用できるいくつかのアクションが表示されます。
...
1. Sites コンソールを使用してページを探し、選択します。 個々のページを選択するには、そのページのチェックボックスを選択する必要があります。
...
コンテンツ固有のパスは、後で翻訳を設定する際に必要になるので、メモしておいてください。
