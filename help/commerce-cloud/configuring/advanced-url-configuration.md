...
編集者がサイトのトップレベルナビゲーションを完全に制御したい場合は、カタログの最上位カテゴリをレンダリングする単一のカタログページを使用する必要がない場合があります。 代わりに、編集者は複数のカタログページを作成できます。トップレベルナビゲーションに含めるカタログのカテゴリごとに 1 つずつ、カタログページを作成できます。

その使用例では、各カタログページに、カタログページ用に設定されたカテゴリに固有の商品ページやカテゴリページへの参照を含めることができます。 この `UrlProvider` これらを使用して、設定済みカテゴリのページおよびカテゴリへのリンクを作成します。 ただし、パフォーマンス上の理由から、サイトのナビゲーションルート/ランディングページの直接カタログページの子のみが考慮されます。

カタログページの製品ページとカテゴリページをそのカタログページの下位に配置することをお勧めします。そうしないと、ナビゲーションやパンくずなどのコンポーネントが正しく機能しない場合があります。

>[!NOTE]
>
> 複数のカタログページの完全なサポートが必要 [CIF コアコンポーネント2.10.0](https://github.com/adobe/aem-core-cif-components/releases/tag/core-cif-components-reactor-2.10.0) 以降
...
さらに、 [Sling マッピング](#sling-mapping) を使用して、SKU を製品 `url_key` と組み合わせます。ほとんどの e コマースシステムでは、SKU は特定の形式に従っており、受信リクエストのために SKU と `url_key` を分離することは簡単にできます。そのことを念頭に置くと、製品ページの URL を `/p/{{category}}/{{sku}}-{{url_key}}.html` に、カテゴリ URL を `/c/{{url_key}}.html` にそれぞれ書き換えることができます。この `/p` および `/c` 製品ページとカテゴリページを他のコンテンツページと区別するために、プレフィックスが引き続き必要になります。

### 新しい URL 形式への移行 {#migrate-url-formats}

デフォルトの URL 形式の多くは、互いに何らかの互換性があり、1 つの形式の URL が別の形式の URL で解析される場合があることを意味します。 これは、URL 形式間の移行に役立ちます。
