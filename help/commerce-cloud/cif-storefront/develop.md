...
>[!VIDEO](https://video.tv.adobe.com/v/347038/?captions=jpn&quality=12&learn=on)
...
>[!TIP]
>
>**AEM as a Cloud Serviceを初めて使用する場合は、**
>[AEM as a Cloud Service SDKを使用したローカル開発環境の設定に関する詳細なガイドをご覧ください。](https://experienceleague.adobe.com/docs/experience-manager-learn/cloud-service/local-development-environment-set-up/overview.html?lang=ja)
...
CIF アドオンは、[&#x200B; ソフトウェア配布ポータルからzip ファイルとしてダウンロードできます。](/help/implementing/developing/tools/package-manager.md) zip ファイルには、CIF アドオンが&#x200B;**Sling機能アーカイブ**&#x200B;として含まれています。これはAEM パッケージではありません。 SDK リストには、AEM as a Cloud Serviceライセンスでアクセスできます。
...
AEM as a Cloud Service SDKを使用したローカル CIF アドオン開発の場合は、次の手順に従います。

1. 最新のAEM as a Cloud Service SDKを入手しましょう。
1. AEM .jarを解凍して、`crx-quickstart` フォルダーを作成します。 次のコマンドを実行します。
...
1. CIF アドオンの正しい Sling 機能アーカイブファイルを `crx-quickstart/install` フォルダーにコピーします。
   ...
   * CIF アドオン zip ファイルには、2つのSling機能アーカイブ `.far` ファイルが含まれています。
...
1. Adobe Commerce GraphQL エンドポイントを保持する `COMMERCE_ENDPOINT` という名前のローカル OS 環境変数を作成します。
   ...
   * この変数は、AEM でコマースシステムへの接続に使用されます。CIF アドオンには、Commerce GraphQL エンドポイントをローカルで使用できるように、ローカルのリバースプロキシが含まれています。 このプロキシは、CIF オーサリングツール（製品コンソールおよびピッカー）で使用され、GraphQL の直接呼び出しを行う CIF クライアントサイドコンポーネントにも使用されます。

   * この変数は、AEM as a Cloud Service 環境に対しても設定する必要があります。変数について詳しくは、[AEM as a Cloud Service用OSGiの設定](/help/implementing/deploying/configuring-osgi.md#local-development)を参照してください。
...
1. （オプション）ステージング済みカタログ機能を有効にするには、Adobe Commerce インスタンスの統合トークンを作成する必要があります。[概要](/help/commerce-cloud/cif-storefront/getting-started.md#staging)の手順に従って、トークンを作成します。
   ...
   * シークレットについて詳しくは、[AEM as a Cloud Service用OSGiの設定](/help/implementing/deploying/configuring-osgi.md#local-development)を参照してください。

1. AEM as a Cloud Service SDKを起動します。
...
OSGi コンソールで設定を確認します：`http://localhost:4502/system/console/osgi-installer`。 リストには、機能モデルファイルで定義されているCIF アドオン関連のバンドル、コンテンツパッケージ、OSGi設定を含める必要があります。
...
CIF コアコンポーネントは、提供された`all` パッケージを含めるか、CIF コンテンツパッケージと関連するOSGi バンドルを個別に使用することで、任意のプロジェクトで使用できます。 CIF コアコンポーネントを手動でプロジェクトに追加するには、次の依存関係を使用します。
