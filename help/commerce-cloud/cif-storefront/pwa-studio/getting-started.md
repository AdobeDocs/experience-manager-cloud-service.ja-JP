...
PWA StudioをAEMのGraphQL エンドポイントに接続するには、[AEM Extension for PWA Studioを使用します。](https://github.com/adobe/aem-pwa-studio-extensions)
...
1. PWA Studio アプリケーションに Apollo Link ラッパーを追加します。`pwa-root/src/index.js`で、次の変更を行います。
   ...
   Apollo Client カスタマイズの詳細については、[linkWrapper.js.](https://github.com/adobe/aem-pwa-studio-extensions/blob/master/aem-cfm-blog-extension/extension/src/linkWrapper.js)を参照してください。

1. ナビゲーションコンポーネントをブログエントリで拡張するには、次のアダプテーションを`pwa-root/local-intercept.js`に追加します。
...
両方の設定の完全な例は、ここで[確認できます。](https://github.com/adobe/aem-pwa-studio-extensions/tree/master/aem-cfm-blog-extension/aem/config/src/main/content/jcr_root/apps/blog-demo/config)
