...
1. GraphQL クエリをアップデートし、`eco_friendly` 属性を部分クエリに含めます。
   ...
   >[!NOTE]
   >
   > `createdAt()` メソッドは、[Product インターフェイスの一部として実装されています。](https://github.com/adobe/commerce-cif-magento-graphql/blob/master/src/main/java/com/adobe/cq/commerce/magento/graphql/ProductInterface.java) 一般的に見られるスキーマ属性のほとんどは実装されているので、真のカスタム属性には `addCustomSimpleField` のみを使用します。
