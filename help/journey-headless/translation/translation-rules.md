...
>* The **翻訳可能** フラグは、翻訳統合フレームワーク設定が **[翻訳するコンテンツモデルフィールドを有効にする](/help/sites-cloud/administering/translation/integration-framework.md)**.
...
1. デフォルトでは、すべての設定が親パス（この例では `/content/dam`）から継承されます。「 」オプションの選択を解除します。 **継承元`/content/dam`** そのため、設定にフィールドを追加できます。
