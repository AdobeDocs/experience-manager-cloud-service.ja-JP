...
あらゆるチャネルにわたって一貫したエクスペリエンスを推進し、パーソナライズされたキャンペーンを管理するために、個々のチャネルとサーフェスを、配信時に準拠すべき独自のコンテンツ構造としてとらえることができます。ただし、各チャネルに独自のコンテンツモデルを持たせることは、維持が困難です。
...
ここでは、AEM を使用してコンテンツを配信する初めてのヘッドレスアプリを実装するために必要な事項の概要を説明します。これらの手順の実行方法については、ヘッドレスデベロッパージャーニーの後半で詳しく説明します。
...
* ルートフォルダーがモデル内に設定されている場合は、新しいコンテンツフラグメントをインラインコンテンツフラグメントエディターで簡単に作成できるようになります。その場合、開業者は場所を選択する必要はありませんが、名前を指定するだけで、新しい参照を編集できます。
