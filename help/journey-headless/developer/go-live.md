...
ただし、AEMのヘッドレスストアは、ここで停止する必要はありません。 [ジャーニーの「はじめに」](getting-started.md#integration-levels)では、AEMがヘッドレス配信と従来のフルスタックモデルをサポートするだけでなく、両方の利点を組み合わせたハイブリッドモデルもサポートする方法について簡単に説明しました。
