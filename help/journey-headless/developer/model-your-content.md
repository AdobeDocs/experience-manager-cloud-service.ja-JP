...
冒頭の「[CMS ヘッドレス開発について学ぶ](learn-about.md)」では、ヘッドレスコンテンツ配信と、それを使用すべき理由について説明しました。次に、「[AEM Headless as a Cloud Service - はじめに](getting-started.md)」では、独自のプロジェクトのコンテキストで AEM ヘッドレスを説明しました。。
