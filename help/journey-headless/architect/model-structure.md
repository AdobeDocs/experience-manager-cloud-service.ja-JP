...
「**モデルを有効化**」オプションはデフォルトで有効になっています。つまり、モデルは、保存されしだい、（コンテンツフラグメントの作成で）使用できます。 必要に応じて、この機能を無効にすることができます。後で既存のモデルを有効（または無効）にすることができます。
...
データ型を追加した後、 **プロパティ** そのフィールドに対して プロパティは、使用するタイプによって異なります。次に例を示します。
...
>[!NOTE]
>
>作成者が、モデルがまだ公開されていないコンテンツフラグメントを公開しようとすると、その旨が選択リストに表示され、モデルがフラグメントと共に公開されます。
