...
# AEM as a Headless CMS 用のオーサリング — はじめに {#architect-headless-introduction}
...
## コンテンツ管理システム（CMS） {#objective}

* **コンテンツ管理システムとは**
* **コンテンツ管理システム（CMS）は、その名のとおり、コンテンツの管理に使用されるコンピュータシステムです。もう少し正確に言えば、web サイトで公開するコンテンツの管理に（通常）使用されるものです。**

## ヘッドレス CMS {#full-stack}

ヘッドレスとは、コンテンツを web 上でのコンテンツの表示方法から効果的に切り離すシステムを表す用語です。

![従来は、CMS でコンテンツを管理し、web ページでのそのコンテンツのレンダリングするのも CMS でした。](/help/journey-headless/developer/assets/full-stack.png)
...
![第一に、AEM は、要件に合わせてカスタマイズ可能な幅広い機能を備えたコンテンツ管理システムです。](/help/journey-headless/developer/assets/adding-channel.png)
...
![コンテンツは、web サイト上でのコンテンツのレンダリング方法を定義する様々なコンポーネントを使用して、web ページ用に作成されます。ここでも AEM は、カスタマイズしたコンポーネントをプロジェクトチームが開発できるので、きわめて柔軟です。](/help/journey-headless/developer/assets/headless-cms.png)

コンテンツモデリング

もう 1 つの技術用語は、コンテンツモデリングです（データモデリングとも呼ばれます）。なぜ、これが作成者の関心事になるのでしょうか。

## ヘッドレスアプリケーションがコンテンツにアクセスして何らかの処理を行えるようにするには、事前に定義された構造がコンテンツに必要です。コンテンツを自由形式にすることも可能ですが、その場合は、アプリケーション側の処理が&#x200B;*非常に*&#x200B;複雑になります。 {#content-modeling}
...
### データモデリングについて詳しくは、「AEM ヘッドレスコンテンツアーキテクトジャーニー」を参照してください。 {#access-content}
...
## CMS ヘッドレス開発について {#whats-next}

コンテンツをモデル化する方法[](basics.md)

## AEM ヘッドレスコンテンツアーキテクトジャーニー {#additional-resources}
