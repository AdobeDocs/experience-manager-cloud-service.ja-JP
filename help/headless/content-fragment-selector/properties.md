...
|--- |--- |--- |--- |--- |
| `ref` | FragmentSelectorRef | いいえ | | `ContentFragmentSelector` インスタンスへの参照。`reload` などの提供された機能にアクセスできます。 |
...
| `allowedRepositoryIds` | 文字列[] | 不要 | | コンテンツフラグメントセレクターでリポジトリとコンテンツフラグメントをフィルタリングするためのリポジトリ ID のリスト。 リポジトリ ID が指定された場合、リポジトリセレクターに表示されるのは、これらのリポジトリのみです。 指定されていない場合や空の配列の場合は、ユーザーがアクセスできるすべてのリポジトリを使用できます。 |
...
| `getImsToken` | 現在ログインしているユーザーの認証 `imsToken` を取得します。これは、アセットレンディションの生成など、他のサービスへのリクエストを認証するために使用できます。 |
