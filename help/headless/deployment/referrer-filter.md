...
例えば、リファラーが `my.domain` のリクエストにアクセスを許可するには、次の操作を行います。

>[!CAUTION]
>
>これは、標準設定を上書きする基本的な例です。 製品のアップデートが常にカスタマイズに適用されるようにする必要があります。
...
>[!CAUTION]
>
>ユーザーは、次の点に完全に対処する必要があります。

データのセキュリティを確保するには、次の点を確認する必要があります。

* アクセスは信頼されたドメインに&#x200B;**のみ**&#x200B;許可する

* ワイルドカード [`*`] 構文は使用&#x200B;**しない**。ワイルドカードを使用すると、GraphQL エンドポイントへの認証済みアクセスが無効になり、エンドポイントが世界中に公開されることになります。

* 直接的にも間接的にも、**決して**&#x200B;機密情報を公開しない。

   * 例えば、すべての [GraphQL スキーマ](/help/headless/graphql-api/content-fragments.md#schema-generation)を次のようにします。
...
機密データをどの手段でも利用できないことを確認する必要があるので、そのような詳細については慎重に検討する必要があります。
