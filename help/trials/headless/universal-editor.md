...
>[!NOTE]
>
>この体験版でユニバーサルエディターを使用するには、Chrome ブラウザーをシークレットモードにせずに使用する必要があります。これは、ユニバーサルエディターではなく、体験版エクスペリエンスの制限です。
...
1. コンポーネントは、テキストコンポーネントであることを示すタブ付きの青い境界線で選択されています。カーソルは境界内にあり、テキスト入力を待っています。テキストを `Aloha Spirit in Lofoten` に変更します。
...
ユニバーサルエディターを使用して、コンテンツのコンテキスト内に留まったまま画像をスワップアウトすることもできます。
...
1. 次に、メイン記事の「**Read More**」リンクを選択します。

   ![プレビューモード](assets/do-not-localize/ue-preview-publish-1.png)

1. 記事を参照し、「**Back**」リンクをクリックして、メインページに戻ります。

   ![「Back」リンクを使用してメインページに戻る](assets/do-not-localize/ue-preview-publish-3.png)
...
1. ページを下にスクロールして、「**Adventure**」セクションに移動します。

1. **Bali Surf Camp** などの Adventure の 1 つを選択します。
