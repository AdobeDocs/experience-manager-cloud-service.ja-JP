...
1. 次の図に示すように、**「CSVにエクスポート**」をクリックして、レポートをコンマ区切り値(CSV)形式でダウンロードすることもできます。
...
#### ベストプラクティスアナライザレポートでのフィルターの使用{#bpa-filters}

[ACSコモンズ](https://adobe-consulting-services.github.io/acs-aem-commons/)に関連する結果を除外するには、次の手順に従います。

1. ページの左側にある左側のパネルアイコンをクリックします。 これは&#x200B;**ACSコモンズフィルタ**&#x200B;を表示します。 **ACS Commons Filter**&#x200B;をクリックして、次の画像に示すインタラクティブチェックボックスを表示します。

   ![画像](/help/move-to-cloud-service/best-practices-analyzer/assets/report_filter_1.png)

   >[!NOTE]
左側のレールアイコンは、BPAがACSコモンの使用を検出した場合にのみ表示されます。

1. ACSコモンに関連するすべての結果を除外するには、ボックスを選択解除します。 以下の図に示すように、レポートに&#x200B;**フィルター適用済みの検索カウント**&#x200B;が表示されます。 このフィルターは、カンマ区切り値(CSV)形式でエクスポートする場合にもレポートに適用されます。

   ![画像](/help/move-to-cloud-service/best-practices-analyzer/assets/report_filter_2.png)

   >[!NOTE]
ACSコモンズの結果は無視しないでください。AEMとの互換性をCloud Serviceとして確認するには、[ドキュメント](https://adobe-consulting-services.github.io/acs-aem-commons/pages/compatibility.html#aem-as-a-cloud-service-feature-incompatibility)を参照してください。
