...
1. ユーザーは、単一の AEM インスタンスのインデックスマネージャーにアクセスできなくなり、インデックス作成のデバッグ、設定または維持ができなくなります。ローカルデプロイメントおよびオンプレミスデプロイメントにのみ使用されます。
1. ユーザーは、1つのAEM インスタンスでインデックスを変更したり、整合性チェックやインデックス再作成について心配する必要はありません。
1. 一般的に、インデックスの変更は、Cloud Manager CI/CD パイプラインの品質ゲートウェイを回避しないように本番環境に移行する前に開始され、本番環境のビジネス KPIに影響を与えません。
1. 本番環境での検索パフォーマンスを含むすべての関連指標は、実行時に顧客が利用できるので、検索とインデックス作成のトピックの全体的な表示を提供します。
...
1. お客様は、Cloud Manager ビルドページでインデックスジョブが完了したかどうかを確認し、新しいバージョンがトラフィックを受け取る準備ができたときに通知を受け取ることができます。

制限事項：

* 現在、AEM as a Cloud Service でのインデックス管理は、`lucene` 型のインデックスに対してのみサポートされています。つまり、すべてのインデックスのカスタマイズは`lucene`型である必要があります。 `async` プロパティには、`[async]`、`[async,nrt]`、`[fulltext-async]` のいずれかを指定できます。
...
インデックス設定の変更が必要な場合は、設定が[ プロジェクト設定](#project-configuration) セクションで提供されているガイドラインに準拠していることを確認してください。 必要に応じて調整を行います。

## プロジェクト設定

プロジェクトに組み込む手順は次のとおりです。

1. Jackrabbit `filevault-package-maven-plugin` のバージョン >= `1.3.2` を使用することを強くお勧めします。必要に応じて、トップレベル `pom.xml`のバージョンを更新します。
...
## 差分インデックスを使用したシンプルなインデックス管理

ほとんどのAEM インデックスは、簡易インデックス管理を使用して設定できます。
これにより、すぐに使用できる（OOTB）インデックスをカスタマイズし、1つのJSON ファイルを使用してカスタムインデックスを定義する簡単な方法が提供されます。

>[!TIP]
>
>AEM インデックスの設定に役立つオンラインツールがあります：[Oak Indexing Tools](https://oak-indexing.github.io/oakTools/index.html)。 ここでは、カスタムインデックスをこの新しい形式に変換するのに役立つステップバイステップガイドと追加ツールを備えた簡易インデックス管理[に関する節を](https://oak-indexing.github.io/oakTools/simplified.html)用意しています。

制限事項：現在、簡易インデックス管理は、`/apps`、`/libs`を含むインデックスには使用できません。
例えば`includedPaths`のように`/content` プロパティを持つすべてのインデックスに使用できます。
`includedPaths` プロパティのないインデックス、または`includedPaths`に`/apps`または`/libs`が含まれるインデックスの場合，
クエリを変更するか、以下の従来のインデックス設定モードを使用することを検討してください。

簡易インデックス管理では、既存のすぐに使える（OOTB）インデックスをカスタマイズし、完全にカスタムなインデックスを追加できます。
簡易インデックス管理では、定義をコピーしたり、バージョンを明示的に定義したりする必要はありません。
インデックス定義のカスタマイズは、最新のすぐに使えるインデックス，
必要に応じて新しいインデックスバージョンを作成できます。

ほとんどのインデックスの場合、カスタムインデックスと既存のインデックスへのカスタマイズは、`diff.index` パッケージを使用して作成できます。
このようなインデックスを設定するには、次のステップバイステップガイドを使用します。
次の例では、`damAssetLucene` インデックスをカスタマイズします
完全カスタムインデックスを同時に導入します。
プロセスは次のとおりです。

1. `ui.apps`という名前の`ui.apps/src/main/content/jcr_root/_oak_index/diff.index` ディレクトリに新しいフォルダーを作成します。

2. 次のコンテンツを含む構成ファイル `.content.xml`を追加します（これは必要なプレースホルダー設定であり、通常のインデックス定義ではありません）: `ui.apps/src/main/content/jcr_root/_oak_index/diff.index/.content.xml`
...
3. 次の内容のテキストファイル `diff.json`を作成します。
この例では、標準のインデックス `damAssetLucene`をカスタマイズします
`test`という名前のプロパティをさらにインデックス化します。 また
`acme.testIndex` ノードのプロパティ `testing`をインデックス化する`nt:unstructured`という完全カスタムインデックス：
...
4. `ui.apps/src/main/content/META-INF/vault/filter.xml` で、FileVault フィルターにエントリを追加します。
...
変更を適用したら、Cloud Managerを使用して新しいアプリケーションをデプロイします。
このデプロイメントでは、追加（必要に応じて結合）する2つのジョブが開始されます
オーサーとパブリッシュのインデックス定義。
スイッチの前に、更新されたインデックス定義を使用して、基になるリポジトリのインデックスが再作成されます。

## 従来のインデックス設定

簡易インデックス管理を使用して設定できないインデックス
レガシー設定モードを使用する必要があります。

従来のインデックス設定は、`includedPaths` プロパティを持たないインデックスにのみ適用されます
または、`/apps`、`/libs`または`/`をカバーする必要があるプロパティがあるプロパティがあります。
すぐに使えるインデックスの中には、次のようなパスをカバーしているものもあります。

* `cqPageLucene`：このインデックスをカスタマイズする必要がある場合は、
代わりに`cqPageContent`を使用するようにクエリを移行することを検討してください。
`includedPaths`の値が`/content`で、タグが付いています。
* `ntBaseLucene`：ベストプラクティスは、このインデックスの変更を避けることです。
代わりに、`acme.`などのプレフィックスを持つ完全カスタムインデックスを使用して，
修飾子が適用されます。
詳しくは、「簡易インデックス管理」の節を参照してください。
...
3. 完全カスタムインデックス：まったく新しいインデックスをゼロから作成できます。 また、これらのインデックスは、`-custom-` およびバージョン番号で終わる必要があります。また、名前の競合を避けるために、インデックス名にプレフィックスを使用します。例：`/oak:index/acme.product-1-custom-2`。`acme.` はプレフィックスです。
...
制限事項の節で説明したように、パッケージマネージャーを使用して抽出されたインデックス定義が異なるタイプ（例：`type`）の場合でも、カスタマイズされたインデックス定義の`lucene`を常に`elasticsearch`に設定する必要があります。
...
インデックス管理とは、インデックスの追加、削除、変更を行うことです。インデックスの&#x200B;*定義*&#x200B;変更はすぐにできますが、変更を適用する（「インデックスの構築」、または既存インデックスの場合は「インデックスの再作成」と呼ばれる）には時間が必要です。これは即時には実行されません。インデックスを作成するデータをリポジトリでスキャンする必要があります。
...
最高のオペレーショナルパフォーマンスを得るには、インデックスを過度に大きくしないようにします。すべてのインデックスの合計サイズを目安にすることができます。カスタムインデックスを追加し、開発環境で標準インデックスを調整した後で、このサイズが100%以上増加する場合は、カスタムインデックス定義を調整する必要があります。 AEM as a Cloud Service を使用すると、システムの安定性とパフォーマンスに悪影響を与える可能性のあるインデックスを、デプロイされないようにしたり、削除したりすることができます。
...
エラーや、変更が不要になったなどの理由で、インデックス定義の変更を元に戻す必要が生じる場合があります。例えば、インデックス定義 `damAssetLucene-8-custom-3` に誤りがある場合は、以前の定義 `damAssetLucene-8-custom-2` に戻す必要があります。これを実現するには、前のインデックス `damAssetLucene-8-custom-4`のコピーである`damAssetLucene-8-custom-2`という名前の新しいインデックスを作成します。

### インデックスの削除 {#removing-an-index}

次の設定は、標準提供（OOTB）のインデックスのカスタマイズと、完全なカスタムインデックスにのみ適用されます。元のOOTB インデックスはAEMで使用されるため、削除できません。
