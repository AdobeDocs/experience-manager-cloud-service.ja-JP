...
**パブリッシュ配信** プロジェクトの場合、設定パイプラインは、Cloud Managerを介して、開発環境、ステージング環境、実稼動環境の各タイプにデプロイできます。 設定ファイルは、[&#x200B; コマンドラインツール &#x200B;](/help/implementing/developing/introduction/rapid-development-environments.md#deploy-config-pipeline) を使用して、迅速な開発環境（RDE）にデプロイできます。 パブリッシュ配信環境に接続されたドメインのトラフィックを設定する必要がある場合は、ターゲットデプロイメント [**パブリッシュ配信パイプライン**](/help/implementing/cloud-manager/configuring-pipelines/configuring-production-pipelines.md#targeted-deployment) [&#128279;](/help/implementing/cloud-manager/configuring-pipelines/configuring-production-pipelines.md#targeted-deployment) 実稼動 [&#x200B; または非実稼動 &#x200B;](/help/implementing/cloud-manager/configuring-pipelines/configuring-non-production-pipelines.md#targeted-deployment)）を使用します。

設定パイプラインは、**Edge Delivery** プロジェクトのCloud Managerを通じてデプロイすることもできます。 ドメインが [**Edge Delivery サイト**](/help/implementing/cloud-manager/configuring-pipelines/configuring-edge-delivery-pipeline.md) に接続されている場合は、**Edge Delivery パイプライン** を使用します。
...
| [&#x200B; ワークフローのパージのメンテナンスタスク &#x200B;](/help/operations/maintenance.md) | `MaintenanceTasks` | ワークフローインスタンスの数を最小限に抑えて、ワークフローエンジンのパフォーマンスを向上させます。<br><br>[&#x200B; ワークフローインスタンスの定期的なパージ &#x200B;](/help/sites-cloud/administering/workflows-administering.md#regular-purging-of-workflow-instances) も参照してください | X |  |
...
機密情報をソース管理に保存する必要がないように、設定ファイルは、設定パイプライン変数または環境変数から秘密鍵を参照することをサポートしています。 ログ転送などの一部の設定では、特定のプロパティにシークレット変数が必須です。 CDN 設定でのシークレットの使用について詳しくは、[CDN 資格情報と認証の設定 &#x200B;](/help/implementing/dispatcher/cdn-credentials-authentication.md) を参照してください。

次のスニペットは、シークレット変数 `${{SPLUNK_TOKEN}}` の設定での使用方法の例です。
...
**推奨される方法** は、**秘密鍵** 型のCloud Manager パイプライン変数を使用することです。そのため、機密情報をソース管理に保存する必要はありません。 「**適用されたステップ**」選択ボックスでは、「**デプロイ**」オプションを使用する必要があります。
...
環境ごとに異なるシークレット値を使用する場合は、シークレット環境変数を使用します。

環境変数の使用方法について詳しくは、[Cloud Manager環境変数 &#x200B;](/help/implementing/cloud-manager/environment-variables.md) を参照してください。

>[!NOTE]
>シークレットの環境変数を使用する方が、より面倒で、厳密な規律が必要になります。環境変数は、設定パイプラインと共にデプロイされません。 パイプラインを実行する前にデプロイする必要があります。また、パイプライン設定が参照している間は削除しないでください。 これが、パイプラインシークレットが推奨される理由です。
