...
パフォーマンスへの悪影響を低減するために、Adobe Experience Managerでは、特定の長時間実行中のリソースを大量に消費するオペレーションを、バックグラウンドオペレーションとして非同期で処理します。 非同期処理では複数のジョブがキューに入れられ、システムリソースの可用性に応じて順に実行されます。
...
>[!NOTE]
>
>[OSGi 設定](/help/implementing/deploying/configuring-osgi.md#creating-osgi-configurations)は可変コンテンツと見なされ、そのような設定は本番環境のコンテンツパッケージとしてデプロイする必要があります。

### 完了したジョブをパージ {#purging-completed-jobs}

AEMは、1日以上経過した完了した非同期ジョブを削除するために、毎日01:00にパージジョブを実行します。
...
1. AEM SDK Quickstart Jar の AEM web コンソール（`https://<host>:<port>/system/console`）に管理者ユーザーとしてログインします。
1. **OSGi**／**設定**&#x200B;に移動します。
1. Web コンソールで、**[!UICONTROL Async Process Default Queue Configuration]** を開きます。
...
1. 「**メール通知を有効にする**」オプションを選択すると、このジョブステータスに関するメール通知を受信できます
1. 変更を保存します。
...
1. AEM SDK Quickstart Jar の AEM web コンソール（`https://<host>:<port>/system/console`）に管理者ユーザーとしてログインします。
1. **OSGi**／**設定**&#x200B;に移動します。
1. Web コンソールで、**[!UICONTROL Async Move Operation Job Processing Configuration]** を開きます。
