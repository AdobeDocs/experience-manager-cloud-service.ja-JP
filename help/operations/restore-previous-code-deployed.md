...
AEM as a Cloud Serviceでのコンテンツの復元[も参照してください](/help/operations/restore.md)。
...
* この機能を保護する機能フラグが有効になっています（オン）。
* プログラムが AEM as a Cloud Service で実行されている。
* この環境の最後のパイプラインが正常に完了し、実行から **30 日未満**&#x200B;である。
* 環境のステータスが&#x200B;*実行中*&#x200B;で、進行中のパイプラインがない。

**デプロイ済みの以前のコードを復元**&#x200B;は、`Production`環境、`Development`環境、`Stage`に加えて、`Specialized Testing Environment`環境で機能します。 確認後、Cloud Managerはリストアを開始し、開始時および正常に完了した時点でプッシュ通知を送信します。

>[!IMPORTANT]
>
>初回使用の場合は、リスクを軽減して安定性を確保するために、`Stage`で使用する&#x200B;*前*&#x200B;に`Production`の手順を検証することをAdobeで強くお勧めします。
