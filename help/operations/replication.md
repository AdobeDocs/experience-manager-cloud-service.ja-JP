...
* レプリケートされたコンテンツの合計サイズは5 MB未満にする必要があります。 ノードとプロパティのみが含まれますが、バイナリは含まれません。バイナリには、ワークフローパッケージとコンテンツパッケージが含まれます。
...
**レプリケーションAPIのパスとサイズの制限**

レプリケートするパスは100個未満にすることをお勧めします。500個がハードリミットです。 ハードリミットを超えると、ReplicationExceptionがスローされます。 アプリケーションロジックにアトミックレプリケーションが必要ない場合、この制限は、 ReplicationOptions.setUseAtomicCallsをfalseに設定することで解決できます。この設定は、任意の数のパスを受け入れますが、内部的にバケットを作成して、この制限を下回ります。 レプリケーション呼び出しごとに送信されるコンテンツの量は、5 MB以下にする必要があります。ノードとプロパティは含まれますが、バイナリ（ワークフローパッケージとコンテンツパッケージはバイナリと見なされません）は含まれません。
