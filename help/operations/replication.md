...
* レプリケートされたコンテンツの合計サイズは 10 MB 未満にする必要があります。これには、ノードとプロパティのみが含まれ、バイナリ（ワークフローパッケージやコンテンツパッケージなど）は含まれません。
...
レプリケートするパスは 100 個未満にし、ハードリミットを 500 個にすることをお勧めします。ハードリミットを超えると、ReplicationException がスローされます。アプリケーションロジックにアトミックレプリケーションが必要ない場合は、ReplicationOptions.setUseAtomicCalls を false に設定することで、この制限を解除できます。この設定の場合は、任意の数のパスを扱えますが、内部的には、この制限内に収まるようにバケットが作成されます。レプリケーション呼び出しごとに送信されるコンテンツの量は、10 MB 以下にする必要があります。この中には、ノードとプロパティは含まれますが、バイナリは含まれません（ワークフローパッケージやコンテンツパッケージはバイナリと見なされます）。
