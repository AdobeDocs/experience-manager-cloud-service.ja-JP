...
<table style="table-layout:auto">
 <tbody>
  ...
  <tr>
    <td>バージョンのパージ</td>
    <td>アドビ</td>
    <td>既存の環境（2024 年 4 月 16 日より前に作成された環境）の場合、パージは無効になり、将来的にデフォルトで 7 年間有効になります。お客様は、より低いカスタム値（30 日など）で設定できます。<br><br> <!--Alexandru: leave the two line breaks in place, otherwise spacing won't render properly-->新しい環境（2024年4月15日（PT）以降に作成された環境）では、デフォルトで以下の値でパージが有効になり、顧客はカスタム値を使用して設定できます。
     <ol>
  ...
  <tr>
    <td>監査ログの削除</td>
    <td>アドビ</td>
    <td>既存の環境（2024 年 4 月 16 日より前に作成された環境）の場合、パージは無効になり、将来的にデフォルトで 7 年間有効になります。お客様は、より低いカスタム値（30 日など）で設定できます。<br><br> <!-- See above for the two line breaks -->新しい環境（2024 年 4 月に作成された環境）では、デフォルトで、 <code>/content</code> 次の動作に従ったリポジトリのノード：
     <ol>
