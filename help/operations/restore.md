...
>
>AEM as a Cloud Serviceの既存のバックアップおよび復元システムの詳細については、 [AEMでのバックアップと復元のas a Cloud Service](/help/operations/backup.md).
...
1. 次の日： **コンテンツを復元** 環境の詳細ページの「 」タブで、まず、「 **復元する時間** 」ドロップダウンリストから選択できます。
...
1. 情報アイコンを使用して、復元するバックアップを見つけ、そのバックアップに含まれるコードおよびAEMリリースのバージョンに関する情報を表示し、復元の影響を考慮します。 [バックアップを選択します。](#choosing-the-right-backup)
   ...
   * 復元オプションに表示されるタイムスタンプは、すべてユーザーのコンピューターのタイムゾーンに基づいています。

1. 次をクリック： **復元** リストアプロセスを開始するためにリストアするバックアップを表す行の右端にあるアイコン。
...
また、復元を行うと、実稼動環境とステージング環境が同期しなくなる場合があることに注意してください。 コンテンツの復元の結果については、ユーザーが責任を負います。
