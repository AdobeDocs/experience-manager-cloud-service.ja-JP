...
1. 次にこの **Adobe AEM リンクチェッカーサービスは通常のスケジュールで実行され** キャッシュ内のエントリの構文が有効かどうかを確認します。
1. 構文が検証されたリンクが、[外部リンクチェッカーウィンドウに表示されます。](#external-using)ただしこれらのリンクは、**保留**&#x200B;状態にとどまります。
1. この **Adobe AEM リンクチェッカータスクは定期的に実行され** GETを呼び出すことによりリンクを検証します。
1. 次に、**Adobe AEM リンクチェッカータスクは** GET呼び出しの結果を使用して [ 外部リンクチェッカーウィンドウ ](#external-using) のエントリを更新します。
...
* **Adobe AEM リンクチェッカー情報ストレージサービス** – このサービスは、リポジトリ内のリンクチェッカーキャッシュのサイズを定義します。
* **Adobe AEM リンクチェッカーサービス** – このサービスは外部リンクの構文の非同期チェックを実行します。
   * チェック期間と、チェッカーがスキップするリンクのタイプを他のオプションで定義できます。
* **Adobe AEM リンクチェッカータスク** – このサービスは外部リンクのGET検証を実行します。
   * 間隔を別々に定義することを可能にし、他のオプション間で不正なリンクや正常なリンクを確認します。
* **Adobe AEM リンクチェッカートランスフォーマー** – このサービスは、ユーザー定義のルールセットに基づいてリンクを変換します。
...
1. OSGi コンソールを開きます。
1. **Adobe AEM Link Checker Transformer を編集する**
1. 無効にするオプションを選択します。
