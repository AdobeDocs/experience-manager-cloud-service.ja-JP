...
また、分類を設計する際は、順序が重要かどうかを考慮することが重要です。 明確な順序が不要な場合および多数の兄弟ノードが予想される場合は、`sling:Folder` や `oak:Unstructured` などの順序がないノードタイプを使用することを推奨します。順序付けが必要な場合は、`nt:unstructured` および `sling:OrderedFolder` の方が適切です。
...
The [JCR クエリのチートシート](#jcr-query-cheatsheet) このドキュメントの節では、使用可能な制約と、インデックス定義が取得されるように見える方法の概要を説明します。 [クエリパフォーマンスツール](#query-performance-tool)を使用してクエリをテストし、適切なインデックスが使用され、クエリエンジンがインデックス外の制約を評価する必要がないことを確認します。
...
* クエリの「最適化を読み取り」スコア。
   * これは、クエリを実行するためにスキャンされた行/ノードの数と読み取られた一致結果の数の比率として計算されます。
...
クエリの説明を実行ツールを使用すると、開発者はクエリ実行計画を理解できます ( [クエリ実行プランの読み取り](#reading-query-execution-plan)) には、クエリの実行時に使用されるインデックスの詳細が含まれます。 これは、クエリがどの程度効果的にインデックス付けされ、そのパフォーマンスを遡及的に予測または分析するかを理解するために使用できます。
...
制限に一致するアセットがごく一部の場合でも `jcr:content/metadata/myProperty = "My Property Value"`に値を指定する場合、クエリは、要求された「ページ」の結果を満たすために、多数のノードを読み取る（試行する）必要があります。 その結果、クエリのパフォーマンスが低下し、クエリのパフォーマンスが低下する可能性があります `Read Optimization` スコアを割り当ててクエリパフォーマンスツールに表示 )、大量のノードがトラバースされていることを示す WARN メッセージが表示される場合があります ( [インデックストラバーサル](#index-traversal)) をクリックします。
...
1. クエリは、インデックス内で「ordered」とマークされていないプロパティで並べ替えられます。
   * この場合、インデックスから返されるすべての結果は、クエリエンジンで読み取られ、インメモリ内で並べ替えられる必要があります。
