...
このページには、CMKをサポートするすべてのプログラムが一覧表示されます。その後、「プログラムを有効にする」ボタンをクリックして、オンボーディングプロセスを開始できます。

![ プログラムでCMK](./assets/cmk/step0.png)を使用できるようにします

プログラムの作成/編集中にCloud Manager内でCMKが有効になっている場合は、「オンボーディングを開始」ボタンをクリックします。
...
## CMK設定のステージング {#stage-cmk-setup}

CMK UIで必要な情報を指定すると、AdobeはAEM as a Cloud Service Stage環境の設定プロセスを開始します。 このプロセスには時間がかかり、完了すると通知が送信されます。

![Adobeがステージング環境を設定するのを待ちます](./assets/cmk/step4.png)

## 製品CMK設定 {#prod-cmk-setup}

ステージの設定が完了したら、エンドツーエンドの徹底的な検証を行う必要があります。 すべてが正常に動作している場合は、それに応じて本番環境を設定できることを確認する必要があります。

![実稼動環境を確認](./assets/cmk/step5.png)

CMK UIで確認した後、AdobeはAEM as a Cloud Service Prod環境の設定プロセスを開始します。 このプロセスには時間がかかり、完了すると通知が送信されます。

![AdobeがProd環境を設定するのを待ちます](./assets/cmk/step6.png)


## CMK設定の完了 {#complete-the-cmk-setup}

設定プロセスが完了すると、UIでCMK設定のステータスを確認できます。Key Vaultと暗号化キーも確認できます。
![のプロセスが完了しました](./assets/cmk/step7.png)
