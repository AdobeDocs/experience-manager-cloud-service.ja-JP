...
デフォルトでは、OIDC 認証が成功すると、ユーザーは最初にリクエストした URL にリダイレクトされます。 ただし、`redirect` クエリパラメーターを使用して、この動作をカスタマイズできます。

### リダイレクトパラメーターの使用

認証を開始する際に、`redirect` パラメーターを認証リクエストに追加することで、カスタムリダイレクト URL を指定できます。

```
/content/wknd/us/en/adventures?redirect=/content/wknd/us/en/welcome
```

この例では、認証が成功すると、ユーザーは最初にリクエストされたページではなく `/content/wknd/us/en/welcome` にリダイレクトされます。

### セキュリティの制約

セキュリティ上の理由から、`redirect` パラメーターには次の制限があります。

* **相対パスである必要があります**：リダイレクト URL は `/` で始める必要があります（例：`/content/mysite/dashboard`）
* **クロスサイトリダイレクトなし**：絶対 URL （`https://external-site.com` など）は使用できません
* **プロトコルに関連する URL がありません**:`//` で始まる URL は、プロトコルに関連するリダイレクトを防ぐために拒否されます

無効なリダイレクト URL が指定された場合、認証はエラーで失敗します。

### 使用例

1. **ログイン後のウェルカムページ**：最初のログイン後に、パーソナライズされたウェルカムページにユーザーをリダイレクトする

   ```
   /content/mysite/secure-area?redirect=/content/mysite/welcome
   ```

2. **ダッシュボードのリダイレクト**：認証後に、ユーザーを特定のダッシュボードに誘導します

   ```
   /content/mysite/login?redirect=/content/mysite/user/dashboard
   ```

3. **ディープリンク**：ユーザーが認証を行ってから特定のリソースにアクセスできるようにします
...
AEMで SAML 認証ハンドラーが既に設定されていて、ユーザーが [data synchronization](https://experienceleague.adobe.com/ja/docs/experience-manager-cloud-service/content/sites/authoring/personalization/user-and-group-sync-for-publish-tier#data-synchronization) を有効にした状態でリポジトリに存在する場合、元の SAML ユーザーと新しい OIDC ユーザーの間で競合が発生する可能性があります。
...
>[!NOTE]
>SAML 認証ハンドラーが無効になり、OIDC 認証ハンドラーが有効になると、[&#x200B; データ同期 &#x200B;](https://experienceleague.adobe.com/ja/docs/experience-manager-cloud-service/content/sites/authoring/personalization/user-and-group-sync-for-publish-tier#data-synchronization) が無効になり、既存のセッションは無効になります。 ユーザーは再度認証する必要があるため、リポジトリ内に新しい OIDC ユーザーノードが作成されます。
