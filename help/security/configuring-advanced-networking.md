...
高度なネットワークを使用する場合、環境全体の安定性を確保し、下位の環境が使用可能な接続を使い果たさないようにするために、接続数が制限されます。

接続はAEM インスタンスあたり 1,000 に制限され、数が 750 に達するとアラートがユーザーに送信されます。

##### 接続制限は、非標準ポートからの送信トラフィックのみに適用されますか。それとも、すべての送信トラフィックに適用されますか。

制限は、高度なネットワーク （非標準ポートでのエグレス、専用エグレス IP または VPN を使用）を使用する接続に対してのみ適用されます。

##### 発信接続数に大きな違いはありません。 通知が届いたのはなぜですか？

顧客が動的に接続を作成する場合（リクエストごとに 1 つ以上など）、トラフィックが増加すると、接続がスパイクする可能性があります。

##### アラートを受けずに過去に同様の状況を経験することは可能ですか？

アラートは、ソフト制限に達した場合にのみ送信されます。

##### 上限に達するとどうなりますか。

ハードリミットに達すると、高度なネットワーク（専用のエグレス IP または VPN を使用した非標準ポートでのエグレス）を介したAEMからの新しいエグレス接続は、DoS 攻撃に対して保護するためにドロップされます。

##### 上限を引き上げることはできますか？

いいえ、多数の接続を持つと、パフォーマンスに大きな影響を与え、ポッドや環境全体で DoS の原因となる可能性があります。

##### 一定期間が経過すると、AEM システムによって接続が自動的に閉じられますか。

はい。接続は、JVM レベルで閉じられ、ネットワークインフラストラクチャの様々なポイントで閉じられます。 ただし、これは実稼動サービスでは遅すぎます。 接続が不要になった場合は明示的に閉じるか、接続プールを使用する場合はプールに戻す必要があります。 そうしないと、リソース消費が大きすぎて、リソースの枯渇を招く可能性があります。

##### 接続の上限に達した場合、ライセンスに影響が及び、追加コストが発生しますか？

いいえ、この制限に関連するライセンスやコストはありません。 これは技術的な制限です。

##### 限界までどれくらい近い？ 上限はどのくらいですか。

このアラートは、接続が 750 を超えるとトリガーされます。 AEM インスタンスあたりの最大接続数は 1,000 です。
...
はい、専用のエグレス IP を使用する場合にも制限は適用されます。
