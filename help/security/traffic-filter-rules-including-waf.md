...
トラフィック フィルタ ルールのサブカテゴリには、拡張セキュリティ ライセンスまたは WAF-DDoS 保護ライセンスが必要です。 これらの強力なルールは、WAF（web アプリケーションファイアウォール）トラフィックフィルタールール（または WAF ルール）と呼ばれ、この記事で後述される [WAF フラグ](#waf-flags-list)にアクセスできます。
...
>[!NOTE]
>リクエスト/応答の編集、リダイレクトの宣言、AEM以外のオリジンへのプロキシなど、CDN でトラフィックを設定する他のオプションに関心がありますか。 早期導入プログラムに参加して、[その方法を学び、試してみましょう](/help/implementing/dispatcher/cdn-configuring-traffic.md)。
...
* **ルール設定の推奨プロセス：** Web サイトを保護するための高度な手法をお読みください。
...
Adobeは、トラフィックフィルタールールに関するフィードバックの提供や質問を、電子メールで受け取るようにユーザーを招待します **aemcs-waf-adopter@adobe.com**.

## トラフィック保護の概要 {#traffic-protection-overview}

現在のデジタル環境では、悪意のあるトラフィックの脅威は常に存在します。Adobeは、リスクの重大さを認識しており、お客様のアプリケーションを保護し、攻撃が発生した場合に攻撃を軽減するためのいくつかのアプローチを提供しています。

エッジでは、アドビが管理する CDN は、フラッド攻撃やリフレクション攻撃／増幅攻撃を含む、ネットワーク
レイヤー（レイヤー 3 およびレイヤー 4）での DoS 攻撃を吸収します。

デフォルトでは、アドビは、特定のしきい値を超えた予期せぬ高トラフィックのバーストによるパフォーマンスの低下を防ぐための対策を講じています。サイトの可用性に影響を与える DoS 攻撃が発生した場合、Adobeのオペレーションチームにアラートが送信され、軽減策が講じられます。

お客様は、コンテンツ配信フローの様々なレイヤーでルールを設定することで、アプリケーションレイヤー攻撃（レイヤー 7）を回避するための積極的な対策を講じることができます。

例えば、Apache レイヤーでは、次のいずれかを設定できます。 [Dispatcher モジュール](https://experienceleague.adobe.com/en/docs/experience-manager-dispatcher/using/configuring/dispatcher-configuration#configuring-access-to-content-filter) または [ModSecurity](https://experienceleague.adobe.com/ja/docs/experience-manager-learn/foundation/security/modsecurity-crs-dos-attack-protection) 特定のコンテンツへのアクセスを制限します。

この記事で説明しているように、トラフィックフィルタールールは、Cloud Manager の設定パイプラインを使用して、Adobeが管理する CDN にデプロイできます。 IP アドレス、パス、ヘッダーなどのプロパティに基づくトラフィックフィルタールールおよびレート制限の設定に基づくルールに加えて、お客様は、WAF ルールと呼ばれる強力なトラフィックフィルタールールのサブカテゴリをライセンスできます。
...
1. 継続的にトラフィックを監視し、脅威の状況の変化に合わせてルールを変更します。
...
この `kind` パラメーターをに設定する必要があります `CDN` およびのバージョンは、スキーマのバージョンに設定する必要があります。これは次のとおりです `1`. 以下の例を参照してください。
...
| **プロパティ** | **タイプ** | **説明** |
|---|---|---|
| reqProperty | `string` | リクエストプロパティ。<br><br>次のいずれか：<br><ul><li>`path`：URL のフルパスをクエリパラメーターなしで返します。</li><li>`queryString`：URL のクエリ部分を返します</li><li>`method`：リクエストで使用される HTTP メソッドを返します。</li><li>`tier`：次のいずれかを返します `author`, `preview`、または `publish`.</li><li>`domain`：小文字のドメインプロパティ（`Host` ヘッダーで定義）を返します</li><li>`clientIp`：クライアント IP を返します。</li><li>`clientCountry`:2 文字のコードを返します（[地域インジケーターの記号](https://en.wikipedia.org/wiki/Regional_indicator_symbol)）を選択します。</li></ul> |
...
* Adobeでは、以下を使用することをお勧めします。 [正規表現 101](https://regex101.com/) および [Fastly Fiddle](https://fiddle.fastly.dev/) 正規表現を使用する場合。 また、Fastly による正規表現の処理方法について詳しくは、[こちらの記事](https://www.fastly.com/documentation/reference/vcl/regex/#best-practices-and-common-mistakes)を参照してください。
...
| **名前** | **許可されたプロパティ** | **意味** |
|---|---|---|
| **許可** | `wafFlags`（任意）、`alert`（任意、未リリース） | wafFlags が存在しない場合、それ以上のルール処理を停止し、サービング応答に進みます。wafFlags が存在する場合、指定された WAF 保護が無効になり、追加のルール処理に進みます。<br>アラートが指定されている場合、ルールが 5 分間に 10 回トリガーされると、アクションセンターの通知が送信されます。 この機能は未リリースです。早期導入プログラムに参加する方法については、[トラフィックフィルタールールアラート](#traffic-filter-rules-alerts)の節を参照してください。 |
| **block** | `status, wafFlags`（任意、相互に排他的）、`alert`（任意、未リリース） | wafFlags が存在しない場合、他のすべてのプロパティをバイパスする HTTP エラーを返します。エラーコードは status プロパティによって定義されるか、デフォルトの 406 となります。wafFlags が存在する場合は、指定された WAF 保護を有効にし、さらにルール処理を続行します。<br>アラートが指定されている場合、ルールが 5 分間に 10 回トリガーされると、アクションセンターの通知が送信されます。 この機能は未リリースです。早期導入プログラムに参加する方法については、[トラフィックフィルタールールアラート](#traffic-filter-rules-alerts)の節を参照してください。 |
| **log** | `wafFlags`（任意）、`alert`（任意、未リリース） | ルールがトリガーされたことをログに記録します。トリガーされていない場合、処理に影響は及びません。wafFlags は影響を受けません。<br>アラートが指定されている場合、ルールが 5 分間に 10 回トリガーされると、アクションセンターの通知が送信されます。 この機能は未リリースです。早期導入プログラムへの参加方法については、[トラフィックフィルタールールアラート](#traffic-filter-rules-alerts)の節を参照してください。 |
...
| CODEINJECTION | コードインジェクション | コードインジェクションとは、任意のアプリケーションコードコマンドをユーザー入力することで、ターゲットシステムを制御または損傷する試みです。 |
...
| SANS | 悪意のある IP トラフィック | [SANS インターネット ストーム センター](https://isc.sans.edu/) 悪意のあるアクティビティに関与した、報告された IP アドレスのリスト。 |
...
| PRIVATEFILE | プライベートファイル | プライベートファイルは、Apache のように、本質的に機密です `.htaccess` ファイル、または機密情報がリークする可能性のある設定ファイル |
...
* 2 つの競合する規則を作成すると、許可された規則は常にブロック規則よりも優先されます。 例えば、特定のパスをブロックするルールと、1 つの特定の IP アドレスを許可するルールを作成した場合、ブロックされたパス上のその IP アドレスからのリクエストは許可されます。
...
このルールはパスのリクエストをブロックします `/helloworld` chrome を含むユーザーエージェントを使用したパブリッシュ環境：
...
特定の条件に基づいて、一定の速度の受信リクエストを超えた場合は、トラフィックをブロックすることが望ましい場合があります。 `rateLimit` プロパティの値を設定すると、ルール条件に一致するリクエストのレートが制限されます。

レート制限ルールでは、WAF フラグを参照できません。Sites と Forms のすべてのお客様が使用できます。

レート制限は、CDN POP ごとに計算されます。例えば、モントリオール、マイアミ、ダブリンの POP で、それぞれ 1 秒あたり 80、90、120 件のリクエストのトラフィック率が発生したとします。 また、レート制限ルールは 100 の制限に設定されています。 その場合、ダブリンへのトラフィックのみがレート制限されます。
...
| window | 整数列挙：1、10 または 60 | 10 | リクエストレートが計算されるサンプリングウィンドウ（秒単位）です。カウンターの精度は、ウィンドウのサイズ（より大きなウィンドウの精度）によって異なります。 例えば、1 秒のウィンドウで 50% の精度、60 秒のウィンドウで 90% の精度を期待できます。 |
...
このルールは、過去 10 秒間に（CDN POP あたり）平均 60 回/秒を超えた場合、クライアントを 5 ミリ秒間ブロックします。
...
10 秒間の時間枠で（CDN POP ごとに）オリジンに対するリクエストの平均が 1 秒あたり 100 件を超えた場合、パス /critical/resource のリクエストを 60 秒間ブロックします。
...
>[!NOTE]
>
>この機能は未リリースです。早期導入プログラムを通じてアクセス権を取得するには、**aemcs-waf-adopter@adobe.com** にメールを送信してください。

ルールは、5 分以内に 10 回トリガーされた場合にアクションセンター通知を送信するように設定できます。 このようなルールにより、特定のトラフィックパターンが発生した場合にアラートが表示されるので、必要な対策を講じることができます。 の詳細情報 [アクション センター](/help/operations/actions-center.md)（メールを受信するために必要な通知プロファイルの設定方法を含む）。

![アクションセンターの通知](/help/security/assets/traffic-filter-rules-actions-center-alert.png)


alert プロパティ（機能が未リリースなので、現在は *experimental* というプレフィックスが付いています）は、すべてのアクションタイプ（allow、block、log）のアクションノードに適用できます。
...
* この `action` 属性は、ルールがブロック、許可またはログのいずれであるかを決定します。
* WAF のライセンスが取得され、WAF が有効になっている場合、 `waf` 属性は、検出された WAF フラグ（たとえば SQLI）を一覧表示します。 これは、WAF フラグがルール内にリストされているかどうかにかかわらず当てはまります。 これは、宣言する可能性のある新しいルールに関するインサイトを提供するためです。
* 一致する顧客宣言ルールがなく、一致する waf ルールがない場合、 `rules` プロパティが空白です。
...
ダッシュボードツールは、から直接複製できます [AEMCS-CDN-Log-Analysis-ELK-Tool](https://github.com/adobe/AEMCS-CDN-Log-Analysis-ELK-Tool) GitHub リポジトリ。
