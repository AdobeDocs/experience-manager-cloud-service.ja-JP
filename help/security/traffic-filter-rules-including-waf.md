...
* request プロパティ `clientIp` は、`equals`、`doesNotEqual`、`in`、`notIn` の述語でのみ使用できます。`clientIp` は、`in` および `notIn` 述語を使用する際に IP 範囲に対して比較することができます。次の例では、クライアント IP が 192.168.0.0/24 の IP 範囲（192.168.0.0 から 192.168.0.255）にあるかどうかを評価する条件を実装しています。
...
| **フラグ ID** | **フラグ名** | **説明** |
|---|---|---|
| 攻撃 | 攻撃 | その表にリストされている 1 つまたは複数の攻撃の種類を含むリクエストを識別するフラグ |
| ATTACK-FROM-BAD-IP | 不正 IP からの攻撃 | `BAD-IP` からのリクエストで、そのテーブルにリストされている 1 つまたは複数の攻撃の種類を含むリクエストを識別するためのフラグ |
...
| CVE | CVE | CVE を識別するフラグ。 常にフラグ `CVE-<CVE Number>` と組み合わされます。 Adobeが保護する CVE について詳しくは、Adobeにお問い合わせください。 |
...
| BAD-IP | 無効な IP | 悪意のあるソース （`SANS`、`TORNODE`）として識別された場合、または悪意のあるリクエストを大量に送信した後にWAFによって不正と識別された場合、IP からのリクエストを不正と識別するためのフラグ |
...
| 圧縮 | 圧縮が検出されました | POST リクエスト本文が圧縮されており、検査できません。 例えば、`Content-Encoding: gzip` リクエストヘッダーが指定され、POST 本文がプレーンテキストでない場合などです。 |
...
| OOB ドメイン | アウトオブバンドドメイン | アウトオブバンドドメインは、通常、侵入テスト中に、ネットワークアクセスが許可されている脆弱性を特定するために使用されます。 |
...
このルールは、クエリパラメーター `foo` を含むパブリッシュ上のリクエストをブロックしますが、IP 192.168.1.1 からのすべてのリクエストは許可します。
