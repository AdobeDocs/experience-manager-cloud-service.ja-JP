...
AEMは、Microsoft® 365の2つのOAuth2 ベースのトランスポート オプションをサポートしています。

* **SMTP + OAuth2** - OAuth2認証でSMTPを使用する標準パス。
* **Microsoft Graph API** – 組織がSMTP ベースの送信を許可していない場合（例えば、Microsoft® 365でテナント全体でSMTP AUTHが無効になっている場合）にMicrosoft Graphを介してメールを送信する代替パスで、SMTPとOAuth2の使用を禁止します。
...
`/apps/<my-project>/osgiconfig/config`の下に、次の構文が含まれています。 `smtp.host` および `smtp.port` の値は、[メールサービスのチュートリアル](https://experienceleague.adobe.com/ja/docs/experience-manager-learn/cloud-service/networking/examples/email-service)で説明するように、高度なネットワーク設定を反映します。
...
[Microsoft Outlook](#microsoft-outlook)で説明されているのと同じAzure アプリの登録手順に従います。手順6 （API権限）に次の違いがあります。 Outlook SMTP スコープの代わりに、Microsoft Graph `Mail.Send`のデリゲートされた権限を使用します。
...
### 更新トークンの生成 {#graph-generating-the-refresh-token}

認証URLおよびcURL リクエストでMicrosoft Graph スコープを使用して、[SMTP + OAuth2](#generating-the-refresh-token)と同じトークン生成手順に従います。

**認証URL** （`clientID`と`tenantID`を値に置き換えます）:
...
cURL トークンリクエストで、スコープを次のように置き換えます。

```
--data-urlencode 'scope=https://graph.microsoft.com/Mail.Send email openid profile offline_access'
```

### AEM as a Cloud Service との統合 {#graph-integration-with-aem-as-a-cloud-service}

1. Microsoft Graph スコープを使用して、`/apps/<my-project>/osgiconfig/config`の下に`com.day.cq.mailer.oauth.impl.OAuthConfigurationProviderImpl.cfg.json`という名前のOSGI プロパティ ファイルを作成します。
...
1. `oauth.flow`と`graph.flow`の両方が`true`に設定された`/apps/<my-project>/osgiconfig/config`の下にOSGI プロパティファイル `com.day.cq.mailer.DefaultMailService.cfg.json`を作成します。
...
1. 実行時に、`refreshToken` および `clientSecret` 秘密鍵を渡すには、[Cloud Manager 変数 API](/help/implementing/deploying/configuring-osgi.md#setting-values-via-api) を使用するか、[Cloud Manager を使用して変数を追加](/help/implementing/cloud-manager/environment-variables.md)します。 変数 `SECRET_SMTP_OAUTH_REFRESH_TOKEN` と `SECRET_SMTP_OAUTH_CLIENT_SECRET` の値を定義する必要があります。

### トラブルシューティング {#troubleshooting}

メールサービスが正しく機能しない場合は、`refreshToken`を再生成します。 SMTPとOAuth2を使用する場合は[更新トークンの生成](#generating-the-refresh-token)を使用し、Microsoft Graphを使用する場合は[更新トークンの生成](#graph-generating-the-refresh-token)を使用します。 Cloud Manager APIを介して新しい値を渡します。デプロイメントには数分かかることがあります。
