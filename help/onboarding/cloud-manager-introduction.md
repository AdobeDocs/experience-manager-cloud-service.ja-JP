...
次の手順に従って、Adobe CX EnterpriseのホームページからCloud Managerのプログラムと製品のページに移動することもできます。

1. [`https://experience.adobe.com`](https://experience.adobe.com)のAdobe CX Enterpriseに移動し、Adobe IDを使用してログインします。
...
| パイプライン実行 | パイプラインの開始 | x | x |  |  |
| パイプライン実行 | 重要な 3 層品質ゲートエラーの拒否/承認 | x | x | x |  |
| パイプライン実行 | 運用開始の承認の提供 | x | x | x |  |
| パイプライン実行 | 実稼動デプロイメントのスケジュール設定 | x | x | x |  |
| パイプラインの削除 | パイプラインの削除の許可 |  | x |  |  |
| 実行のキャンセル | 現在の実行のキャンセル |  | x |  |  |
| 個人用アクセストークンの生成 | Git へのアクセス |  | x |  | x |
| RDE を作成 | 高速開発環境の作成 | x |  |  | x |
| RDE をリセット | 高速開発環境のリセット | x |  |  | x |
