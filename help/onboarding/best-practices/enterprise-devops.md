...
クラウドサービスとしてのAdobe Experience Manager(AEM)は通常、複数の環境で構成されます。これらの要素は、様々なレベルで様々な目的に使用されます。
...
この環境は、品質保証チームが新しいシステムを包括的にテストするために使用します。 デザインと機能の両方 適切なコンテンツを持つオーサー環境とパブリッシュ環境の両方を含み、完全なテストを実施するために必要なすべてのサービスを提供する必要があります。
...
* コメントやその他のフォーム送信など、訪問者が生成したユーザーデータを保存します。
...
クラウドサービスプロジェクトとしてのAEMは、多くの場合、コードのデプロイメントをトリガーします。
...
* 異なるインスタンス間 — コンテンツを（HTTPまたはHTTPSを使用して）直接複製（クラウドサービスとしてのAEM）することで、
