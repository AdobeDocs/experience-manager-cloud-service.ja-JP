...
Edge Delivery Services で AEM オーサリングを行うプロジェクトは、コンテンツソースまたは[オーサリングメソッド](/help/edge/authoring.md)とは独立して、他の Edge Delivery Services プロジェクトの大半の仕組みを継承します。
...
ドキュメントには、ページの `<head>` でどの `<meta>` 要素がレンダリングされるかを定義するのに使用されるページ[メタデータブロック](/help/edge/authoring.md#metadata--seo)を含めることができます。AEM as a Cloud Service のページのプロパティは、`title`、`description`、`keywords` などの Edge 配信サービスですぐに使用できるプロパティにマッピングされます。
