...
* ドロップダウン、ラジオグループ、チェックボックスグループなどの特定のコンポーネントと一般的なコンポーネントを含め、アダプティブ Forms ブロックによって提供されるフォームコンポーネントのHTML構造について理解します。
...
* `.field-{Name}`：特定のフィールド名に基づいて、さらに要素を選択します。 例えば、`.field-first-name` は「名」テキストフィールドをターゲットにします。 このセレクターは field-{Name} クラスを持つ要素をターゲティングするために使用できますが、注意が必要です。 この特定の場合、入力自体だけでなくラベル要素と説明要素もターゲットにするので、入力フィールドのスタイル設定には役に立ちません。 テキスト入力フィールド（.text-wrapper input）をターゲティングする場合と同様に、より具体的なセレクターを使用することをお勧めします。
...
* 外側のラッパーのターゲット設定：これらのセレクターは、ラジオグループとチェックボックスグループの両方の最も外側のコンテナをターゲットにし、グループ構造全体に一般的なスタイルを適用できます。 これは、間隔、整列、その他のレイアウト関連のプロパティを設定する場合に便利です。
...
* フィールドセット内に、複数の.{Type}-wrapper 要素（この場合は .text-wrapper と .password-wrapper）がパネル内の個々のフォームフィールドを表します。
