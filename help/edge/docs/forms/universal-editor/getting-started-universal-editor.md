...
ユニバーサルエディターを使用してEdge Delivery Services for AEM Formsをスムーズに操作できるようにするには、先に進む前に次の前提条件を確認してください。

### アクセス要件

- **GitHub アカウント**：リポジトリを新規作成する権限を持つ GitHub アカウントが必要です。 これは、プロジェクトのソースコードを管理し、チームと共同作業を行う上で不可欠です。
- **AEM as a Cloud Service オーサリングアクセス**: AEM as a Cloud Service環境に対するオーサーレベルのアクセス権を持っていることを確認します。 このアクセス権は、フォームを作成、編集および公開するために必要です。

### 技術要件

- **Git に精通している**：リポジトリのクローン作成、変更のコミット、更新のプッシュなど、基本的な Git 操作を快適に実行できる必要があります。 これらのスキルは、ソース管理とプロジェクトの共同作業の基本です。
- **Web テクノロジーの理解**:HTML、CSS、JavaScriptに関する実用的な知識をお勧めします。 これらのテクノロジーは、フォームのカスタマイズとトラブルシューティングの基礎となります。
- **Node.js （バージョン 16 以降）**:ローカル開発およびビルドツールの実行には、Node.js が必要です。 システムにバージョン 16 以降がインストールされていることを確認します。
- **パッケージマネージャー（npm または yarn）**: プロジェクトの依存関係とスクリプトを管理するには、npm （ノードパッケージマネージャー）または yarn が必要です。

### 推奨される背景

- **AEM Sitesの概念**: サイト構造やコンテンツのオーサリングを含むAEM Sitesの基本的な理解は、フォームを効果的に移動および統合するのに役立ちます。
- **フォームデザインの原則**：使いやすさ、アクセシビリティ、データの検証など、フォームデザインのベストプラクティスに精通していると、効果的でユーザーにわかりやすいフォームを作成できます。
- **WYSIWYG エディターの使用経験**：以前にWhat You See Is What You Get（WYSIWYG）エディターを使用したことがある場合は、ユニバーサルエディターのビジュアルオーサリング機能をより効率的に活用できます。
...
**推奨：** 新規プロジェクト、パイロット、または概念実証イニシアティブ

AEM Forms ボイラープレートを活用して、プロジェクトの設定を迅速に行います。 このボイラープレートは、すぐに使用できるテンプレートを提供し、アダプティブ Forms ブロックをシームレスに統合して、AEM サイト内でフォームをすばやく構築およびデプロイできるようにします。

### 概要

統合フォームを使用して新しいプロジェクトを正常に起動するには、次の操作を行います。

1. AEM Forms ボイラープレートテンプレートを使用して、GitHub リポジトリを作成します。
2. AEMとリポジトリの間のコンテンツの同期を自動化するには、AEM コード同期を設定します。
3. GitHub プロジェクトとAEM環境の間の接続を設定します。
4. 新しいAEM サイトを確立して公開します。
5. ユニバーサルエディターを使用してフォームを追加および管理します。

以下の節では、各ステップを詳しく説明し、スムーズで効率的なプロジェクト設定エクスペリエンスを実現します。
...
フォームを既存のAEM プロジェクトに統合する場合は、次の前提条件を満たしていることを確認します。

- [AEM ボイラープレート XWalk](https://github.com/adobe-rnd/aem-boilerplate-xwalk) を使用して作成された既存のAEM プロジェクトがあります。
- [ローカル開発環境が設定されている ](#set-up-local-development-environment)
- プロジェクトリポジトリへの Git アクセス権を持っているので、必要に応じて変更をクローン、変更、プッシュできます。

>[!NOTE]
>
> プロジェクトが最初に [AEM Forms Boilerplate](https://github.com/adobe-rnd/aem-boilerplate-forms) を使用して設定された場合、フォーム機能は既に含まれています。 その場合は、「最初のフォームの作成 [ のセクションに進 ](#create-your-first-form) ます。

次のガイドでは、既存のプロジェクトにフォーム機能を追加するための構造化されたアプローチを提供します。 各ステップは、ユニバーサルエディター環境内でシームレスな統合と最適な機能を確保するように設計されています。

### 概要

次の大まかな手順を完了します。

1. アダプティブ Forms ブロックファイルをプロジェクトにコピーします。
2. フォームコンポーネントを認識してサポートするように、プロジェクトの設定を更新します。
3. ESLint ルールを調整して、新しいファイルとコーディング・パターンに対応します。
4. プロジェクトを構築し、変更をリポジトリにコミットします。
...
**このセクションのフォロワー：**\
このセクションは、パス A （新しいプロジェクト）またはパス B （既存のプロジェクト）のいずれかをフォローするユーザーに関連しています。

プロジェクトにフォーム作成の機能が備わったので、ユニバーサルエディターの直感的なWYSIWYG オーサリング環境を使用して最初のフォームを作成する準備が整いました。 次の手順では、AEM サイト内でフォームを構造化された方法でデザイン、設定および公開します。

### 概要

ユニバーサルエディターでフォームを作成するプロセスは、次のいくつかの主要な段階で構成されます。
...
4. **フォームをプレビューおよびテストする**\
   プレビュー機能を使用して、公開前にフォームの外観と動作を検証します。

5. **更新したページを公開**\
   確認したら、エンドユーザーがフォームを使用できるように、ページを公開します。

以下の節では、これらの各手順を詳しく説明し、フォームをスムーズかつ効果的に作成できるようにします。
