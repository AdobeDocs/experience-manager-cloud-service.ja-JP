...
# ユニバーサルエディターを使用したレスポンシブフォームの作成 - 完全なガイド
...
+++タブレットの最適化（768 px～1199 px）

**レイアウトとユーザビリティ：**

- 関連するフィールドを 2 列レイアウトにして、画面スペースを有効に活用します。
...
## メディアクエリブレークポイントを使用したカスタムレイアウトの設定

**ユニバーサルエディター**&#x200B;を使用してアダプティブフォームのコンポーネントのカスタムレイアウトを作成する際は、**CSS メディアクエリブレークポイント**&#x200B;を使用してレスポンシブ動作を定義する必要があります。これにより、様々なデバイスや画面サイズでフォームが正しくレンダリングされます。

**推奨されるブレークポイント（AEM コアコンポーネントに基づく）**

| **デバイスタイプ** | **推奨されるブレークポイント** |
|-----------------|---------------------------|
...
- これらのブレークポイントを使用して、様々なデバイス上でのコンポーネントのサイズ変更、積み重ね、非表示を行う方法を制御します。
- 一貫性のある UX を実現するには、組織のレスポンシブデザインガイドラインに従います。
- 複数のデバイスと方向でレイアウトをテストして、ユーザビリティとアクセシビリティを確保します。
...
>[!NOTE]
>
> ユニバーサルエディターでは、レスポンシブ動作を定義するための UI を提供しません。すべてのレイアウトのカスタマイズは、CSS を通じて処理する必要があります。
