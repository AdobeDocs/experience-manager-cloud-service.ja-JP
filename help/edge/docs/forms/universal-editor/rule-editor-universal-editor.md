...
ルールを記述するフォームコンポーネントを選択し、「![edit-rules](/help/forms/assets/edit-rules-icon.svg)」アイコンをクリックします。 ルールエディターのユーザーインターフェイスが表示されます。

![ルールエディターのユーザーインターフェイス](/help/edge/docs/forms/assets/rule-editor-for-field.png)

この記事では、`form object` と `form component` を交互に使用します。
...
![edit-rules](/help/forms/assets/edit-rules-icon.svg) アイコンをクリックすると、ルールエディターのエディターが開きます。
...
<table border="1">
  ...
  <tbody>
    <tr>
      <td>1. タイトル</td>
      <td>フォームコンポーネントのタイトルと、選択したルールタイプを表示します。 例えば、「Enter Gross Salary （総給与を入力）」は、「When」のルールタイプが選択されているテキストボックスコンポーネントです。 </td>
    </tr>
    <tr>
      <td>2. フォームオブジェクトと関数</td>
      <td>「<b>Forms オブジェクト </b>」タブには、フォームに含まれているすべてのコンポーネントの階層ビューが表示されます。 <b> 関数 </b> タブには、ルールエディターの一連の組み込み関数が含まれています。</td>
    </tr>
    <tr>
      <td>3. フォームオブジェクトと関数の切り替え</td>
      <td>切り替えボタンは、フォームオブジェクトと関数ペインを交互に表示または非表示にします。 </td>
    </tr>
    <tr>
...
フォームコンポーネントを選択すると、そのコンポーネントの既存のルールが一覧表示されます。 ルールエディターでルール概要のタイトルを表示してプレビューできます。 さらに、ルールの順序変更、ルールの編集、ルールの有効化／無効化、ルールの削除を行うことができます。
...
<table border="1">
  ...
  <tbody>
    <tr>
      <td>次の値を設定</td>
      <td>指定した条件に応じて、フォームコンポーネントの値を設定します。</td>
    </tr>
    <tr>
      <td>次の値をクリア</td>
      <td>指定したフォームコンポーネントの値をクリアします。</td>
    </tr>
    <tr>
      <td>非表示／表示</td>
      <td>条件を満たしているかどうかに基づいて、フォームコンポーネントの表示/非表示を切り替えます。</td>
    </tr>
    <tr>
      <td>有効／無効</td>
    ...
    <tr>
      <td>検証</td>
      <td>条件に基づいてフォームコンポーネントをチェックし、条件が満たされない場合はエラーを表示します。 </td>
    </tr>
    <tr>
      <td>条件</td>
      <td>条件を指定し、その条件が満たされた場合に、トリガーを実行するアクションを実行します。 <i> 条件 – アクション – 代替 </i> アクションルール構文または <i> 条件 – アクション </i> ルール構文に従います。 </td>
    </tr>
    <tr>
      <td>形式</td>
      <td> 値が変更された場合、指定された式を使用してフォームコンポーネントの表示値を変更します。</td>
    </tr>
    <tr>
      <td>サービスを呼び出す</td>
    ...
    <tr>
      <td>プロパティを設定</td>
      <td>条件に基づいて、指定されたフォームコンポーネントのプロパティの値を設定します。</td>
    </tr>
    <tr>
      <td>フォーカスを設定</td>
    ...
    <tr>
      <td>フォームを保存</td>
      <td>これにより、ユーザーはドラフトと送信Forms ポータルコンポーネントを使用して、フォームをドラフトとして保存できます。 </td>
    </tr>
    <tr>
      <td>フォームを送信</td>
      <td>フォームを送信します。</td>
...
1. 編集するフォームをユニバーサルエディターで開き、コンテンツツリーで **[!UICONTROL Gross Salary]** フィールドを選択して、![edit-rules](/help/forms/assets/edit-rules-icon.svg) を選択します。 または、**[!UICONTROL Forms オブジェクト**[!UICONTROL  ペインから直接 ]**Gross Salary]** フィールドを選択することもできます。
...
1. オーサリングモードで「**[!UICONTROL 課税所得]**」フィールドを選択し、![ルールを編集](/help/forms/assets/edit-rules-icon.svg) アイコンを選択します。または、{2 **[!UICONTROL Forms オブジェクト]** ペインから直接「課税所得 ]**フィールドを選択できます。**[!UICONTROL 
...
>[!NOTE]
>
> ユニバーサルエディターでは、静的インポートと動的インポートは、カスタム関数スクリプトではサポートされていません。 完全なコードを `../[blocks]/form/functions.js` ファイルに追加する必要があります。
