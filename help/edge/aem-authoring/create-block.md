...
これでブロックの作成方法がわかったので、無駄のない開発者エクスペリエンスを実現するために、コンテンツを意味論的な方法でモデル化する方法を理解する必要があります。

ドキュメントを参照してください [Edge Delivery Services プロジェクトを使用したAEM オーサリング用のコンテンツモデリング](/help/edge/aem-authoring/content-modeling.md) を参照して、Edge Delivery Services プロジェクトでのAEM オーサリングに対するコンテンツモデリングの仕組みを学びます。
