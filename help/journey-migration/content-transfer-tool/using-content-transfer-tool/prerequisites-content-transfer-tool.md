...
| Lucene インデックスの合計サイズ | 最大 25 GB の Lucene インデックスの合計サイズ（を除く） `/oak:index/lucene` および `/oak:index/damAssetLucene` は現在サポートされています。 この制限を超えるインデックスサイズのオプションについては、Adobe カスタマーケアでサポートチケットを作成してご相談ください。 |
