...
>abstract="コンテンツトランスファーツールを使用して、コンテンツを AEM as a Cloud Service (オーサー / パブリッシュ) に移行する方法について説明します。"
>additional-url="https://video.tv.adobe.com/v/327070/?captions=jpn&quality=12&learn=on" text=" デモを見る"
...
1. これで、リスト表示に移行リストが表示されます。 3つのドット記号（**...**）を選択します ドロップダウンを開き、**抽出キーをコピー**&#x200B;を選択します。 このキーは、抽出段階で必要になります。 この抽出キーをコピーします。
...
1. 次に、移行セットを作成するには、次のパラメータを選択します。
   ...
   1. **含めるパス**： パスブラウザーを使用して、移行する必要があるパスを選択します。 パスピッカーは、タイピングまたは選択による入力を受け付けます。 ユーザーは、UI から、または CSV ファイルをアップロードすることによって、パスを含めるオプションを 1 つだけ選択できます。
      ...
      1. パス選択のみが許可され、少なくとも1つのパスが存在する必要があります。パスが選択されていない場合、サーバーエラーが発生します。
