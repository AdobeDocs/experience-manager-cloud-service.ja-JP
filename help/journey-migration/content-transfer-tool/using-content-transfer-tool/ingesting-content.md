...
1. 取り込みを作成するために必要な情報を入力します。
   ...
   * **層：** 層を選択します。（オーサー／パブリッシュ）。
   ...
   >[!NOTE]
   > ターゲット層が `Publish` の場合、パブリッシュインスタンスは取り込み中も実行されたままになります。  ただし、取り込み中にコンパクションプロセスが実行されると、2 つのプロセス間で競合が発生する可能性が高くなります。  このため、取り込みプロセスは、1）コンパクションの時間指定スクリプトを無効にして、取り込み中にコンパクションが開始されないようにします。2）コンパクションが現在実行中かどうかを確認し、実行中の場合は、取り込みが続行する前にコンパクションの完了を待ちます。  パブリッシュの取り込みに予想より長い時間がかかる場合は、取り込みログで関連するログステートメントを確認してください。
