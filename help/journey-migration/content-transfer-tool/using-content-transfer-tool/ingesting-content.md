...
1. 取り込みの選択が選択されると、その期間の見積もりが表示されます。 これは、類似した取り込みの履歴データに基づくベストエフォートの予測です。

   * この見積もりは、抽出の「サイズを確認」値が収集され、使用可能な場合にのみ計算され、表示されます。
   * この値は推定値であり、インテリジェントに計算されますが、正確と見なすことはできません。 実際の期間は、様々な要因によって変わることがあります。
   * 取り込みの実行中、この値は期間ダイアログでも使用でき、「」からアクセスできます&#x200B;**期間の表示**&#x200B;取り込みの「アクション。

>[!CONTEXTUALHELP]
>id="aemcloud_cam_ingestion_estimate"
>title="取り込み期間推定"
>abstract="特定の取り込みの概算時間を表示して、取り込みに要する時間の一般的な感覚を提供できます。 その精度には限界があります。"
...
### 削除されたアセットが取り込みを再実行した後に存在しない

通常、取り込みの間にクラウド環境データを変更することはお勧めしません。

アセットタッチ UI を使用してCloud Service先からアセットを削除すると、ノードデータは削除されますが、画像付きのアセット Blob は、すぐには削除されません。 削除対象としてマークされ、UI に表示されなくなります。ただし、ガベージコレクションが発生して BLOB が削除されるまで、データストアに残ります。

以前に移行したアセットが削除され、ガベージコレクターがアセットの削除を完了する前に次の取り込みが実行されているシナリオでは、同じ移行セットを取り込んでも、削除されたアセットは復元されません。 取り込みがアセットのクラウド環境をチェックする場合、ノードデータはありません。そのため、取り込みはノードデータをクラウド環境にコピーします。 ただし、BLOB ストアをチェックすると、BLOB が存在することがわかり、BLOB のコピーをスキップします。 そのため、タッチ UI からアセットを表示すると、取得後にメタデータが表示されますが、画像は表示されません。 移行セットとコンテンツ取り込みは、この場合に対応するように設計されていません。 以前に移行したコンテンツを復元するのではなく、新しいコンテンツをクラウド環境に追加することを目的としています。
