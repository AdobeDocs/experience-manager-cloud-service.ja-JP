...
1. 取り込みを作成するために必要な情報を入力します。
   ...
   * **ワイプ：**`Wipe` 値を選択します。
      ...
      * 非ワイプ取り込みは、追加取り込みのユースケース向けに特別に設計されています。 これらの取り込みは、既存の移行セットの最後の取り込み以降に変更された新しいコンテンツを増分的に含むことを目的としています。 このユースケースの範囲外でワイプされていない取り込みを実行すると、取り込み時間が非常に長くなる可能性があります。
