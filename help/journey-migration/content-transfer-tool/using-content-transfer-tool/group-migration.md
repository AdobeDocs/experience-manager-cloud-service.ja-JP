...
移行されたグループは、「ローカルグループ」とは見なされなくなります。IMS グループです。AEMと IMS の間で同期できるように、IMS で再作成する必要があります。  グループは、Admin Consoleなどの方法を使用して、IMS で個別にまたは一括で作成できます。  Admin Consoleでグループを個別または一括で作成する方法について詳しくは、[ ユーザーグループの管理 ](https://helpx.adobe.com/ca/enterprise/using/user-groups.html) を参照してください。
...
ユーザーレポートは、プリンシパル移行レポートの最後（その一部）に追加されます（以下の [ 最終的な概要とレポート ](#final-summary-and-report) を参照）。  このレポートの情報（各ユーザーに対してレポートされるグループを含む）を使用して、ユーザーのバルクアップロードファイルを作成できます。このファイルは、Admin Consoleで使用すると、IMS 内の多数のユーザーを一括で作成できます。  既存の IMS ユーザーは一括で編集することもできます。

[ 複数ユーザーの管理」を参照してください。 | Admin Consoleを使用したユーザーの一括作成または編集の詳細については ](https://helpx.adobe.com/ca/enterprise/using/bulk-upload-users.html)CSV の一括アップロードを参照してください。
