...
新しいバージョン（v2.0.10）の使用を開始するには、コンテンツ転送ツールの古いバージョンをアンインストールする必要があります。新しいバージョンはアーキテクチャが大きく変更されているので、古いバージョンをアンインストールが必要になります。v2.0.10 では、新しい移行セットを作成し、その新しい移行セットで抽出と取り込みを再実行する必要があります。移行が既に進行中の場合は、移行が完了するまで、旧バージョンの CTT を引き続き使用することができます。
