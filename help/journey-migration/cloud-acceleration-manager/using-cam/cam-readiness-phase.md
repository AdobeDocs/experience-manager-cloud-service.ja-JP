...
Cloud Acceleration Manager でプロジェクトを作成したら、準備段階で、現在のAdobe Experience Manager(AEM) 実装の評価を開始できます。
...
1. プロジェクトのランディングページで、 **対応準備** を参照してください。
...
1. クリック **レビュー** から **ベストプラクティス分析** カード。
...
1. ベストプラクティスアナライザー (BPA) をダウンロードします。
   ...
   >[!NOTE]
   >ビジネスクリティカルなインスタンスへの影響を回避するため、Adobeでは、オーサー環境で BPA を実行することをお勧めします。 環境は、カスタマイズ、設定、コンテンツ、ユーザーアプリケーションの領域で、実稼動環境にできる限り近いものにする必要があります。 または、実稼動版のオーサー環境のクローンで実行することもできます。
...
1. クリック **新しいレポートをアップロード** BPA レポートを CAM にアップロードできます。
...
1. プロジェクトに移動し、「 **レビュー** から **ベストプラクティス分析** カード **対応準備** フェーズ。
...
1. クリック **トレンドラインを表示** レポートのトレンドライン表示を開きます。
   ...
   >
   >次の 2 つのグラフが表示され、このグラフの傾向が示されます。
   > 
   >1. **レポート結果のトレンド**
   >1. **カスタムコンポーネントとテンプレートのトレンド**
   >
   >次の図に示すように、ドロップダウンを使用して、グラフィック表示を追加または変更できます。
...
このレポートをクリックすると、 **レポート** タブをクリックします。
...
1. クリック **表示** から **計画とセットアップ** カード。 このカードには、AEM移行の計画と設定に役立つ、すべての関連コンテンツが記載されています。
