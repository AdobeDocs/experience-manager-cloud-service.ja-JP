...
アイコンは、エクスペリエンスの種類（Web サイト、モバイルアプリケーションまたは公開物）を示します。エクスペリエンスを追加するには、下向きの山形をタップまたはクリックし、「**エクスペリエンスを追加**」をタップして、エクスペリエンスのタイプを選択します。
...
* メディアプロジェクト - メディア関連アクティビティ用のリファレンスサンプルプロジェクトです。いくつかのメディア関連プロジェクトの役割（フォトグラファー、エディター、コピーライター、デザイナー、所有者、監視者）が含まれます。また、テキストをリクエストおよびレビューするコピーをリクエストワークフローも使用します。
...
| 編集者 | この役割のユーザーは、プロジェクトのコンテンツをアップロードおよび編集できます。 | プロジェクト、関連メタデータおよび関連アセットに対する読み取り/書き込みアクセス権撮影リストをアップロードし、アセットをレビューおよび承認する権限/etc/commerceへの書き込み権限特定のプロジェクトに対する権限の変更 | workflow-users グループ |
