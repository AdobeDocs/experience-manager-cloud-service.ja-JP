...
編集可能なコンテンツにはすべて、コンテンツのタイプを示すバッジが付きます。
...
#### リッチテキストのコンテキストメニュー {#rich-text-context-menu}
...
ユニバーサルエディターのリッチテキストエディター（RTE）を使用すると、作成者は標準のテキスト書式設定を適用できます。 次のオプションを使用できます。
...
* **取り消し線**
* **テキストの色**
   * 色を選択したり、16 進数の値を指定したりできるカラーパレットを開きます
   * モーダルエディターでのみ使用できます。コンテキスト内では使用できません
* **上付き文字**
* **添字**
* **箇条書き**
...
* **順序付きリスト**
   * Tab キーを使用してインデントを設定し、Shift + Tab キーを使用してインデントを解除します。
* **リンク**
   * URL を指定するか、コンテンツブラウザーを使用してAEM内のパスを選択します。
* **リンク解除**
   * 選択したテキストからリンクを削除します。
* **画像**
...
* **アウトデント**
* **テキストとして貼り付け**
   * ユニバーサルエディターに貼り付ける前に、クリップボードのテキストから書式を削除します。
* **すべての書式を削除**
...
コンテキストメニューまたはコンテンツツリーを使用して、コンポーネントを移動および並べ替えることができます。
...
>[!TIP]
>
>`Command-U` または `Shift-Command-U` のホットキーを使用して、それぞれ上または上に移動します。
