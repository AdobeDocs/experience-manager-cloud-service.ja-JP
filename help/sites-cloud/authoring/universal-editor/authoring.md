...
書式設定のオプションと詳細は、2 か所で利用できます。

* The **コンテキストメニュー** リッチテキストブロックの上にを開き、コンテキスト内の基本的な書式設定オプションを提供します。 スペース上の制限により、一部のオプションは省略記号ボタンの後ろに隠れる場合があります。
* The **プロパティパネル** に、使用可能なすべての書式設定オプションをテキストと共に表示します。

フォーカスが編集されたフィールドから離れると、変更内容は自動的に保存されます。
