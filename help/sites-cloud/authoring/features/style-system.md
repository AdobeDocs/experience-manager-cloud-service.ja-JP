...
テンプレート作成者は、コンテンツ作成者がコンポーネントを機能させる方法を設定する機能だけでなく、コンポーネントの別の視覚的バリエーションを設定する機能も必要です。
...
スタイルシステムを独自のコンポーネントに使用する場合は、次の操作を行います。
...
1. を選択します。 **スタイル** ボタンをクリックします。 **リスト** component ：スタイルメニューを開き、コンポーネントの外観を変更します。
...
1. WKND の英語のマスターホームページ ( ) の編集中 `http://<host>:<port>/sites.html/content/wknd/language-masters/en`、を使用してページのテンプレートを編集します。 **ページ情報/テンプレートを編集**.
...
例えば、レイアウトコンテナに次のようなセマンティクスを指定できます。 `<main>`, `<aside>`, `<nav>`など。
