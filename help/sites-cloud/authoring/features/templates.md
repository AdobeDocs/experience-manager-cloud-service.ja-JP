...
作業を開始する前に、テンプレートの作成には共同作業が必要と考えることが重要です。 この理由から、[ロール](#roles)がそれぞれのタスクに示されます。これは、実際にテンプレートを使用してページを作成する方法に影響を与えませんが、ページがテンプレートに関係する方法には影響を与えます。
...
* パブリッシュ環境で利用できるようにするために[テンプレートを公開します。](#publishing-a-template-template-author)
...
>[!TIP]
>
>国際化する必要がある情報は、テンプレートに含めないでください。 <!-- Never enter any information that must be [internationalized](/help/sites-developing/i18n.md) into a template.-->
>
...
1. これで、次の場合にテンプレートを使用できます。 [ページの作成](/help/sites-cloud/authoring/fundamentals/organizing-pages.md#creating-a-new-page)で、 [テンプレートの編集](#editing-templates-template-authors) 必要に応じて変更します。
...
![「追加」ボタン](/help/sites-cloud/authoring/assets/templates-add-button.png)
