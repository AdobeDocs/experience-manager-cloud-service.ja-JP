...
1. コメントフィールドの横の省略記号をクリックまたはタップして、オプションを表示します。
   ...
   ![タイムラインパネルのバージョン](/help/sites-cloud/authoring/assets/versions-timeline-rail.png)
...
1. 同じダイアログが開きます。 You can enter a **Label** and a **Comment** if required.
1. 「**作成**」で新しいバージョンを確定します。
...
1. ページバージョンがリストに表示されます。次のように、プレビューするバージョンを選択します。
   ...
   ![プレビューバージョン](/help/sites-cloud/authoring/assets/versions-revert.png)
...
コンテンツの作成は継続的で共同作業的なプロセスなので、Timewarpの目的は、コンテンツの変更を理解するために、発行されたWebサイトを発行者が時間の経過と共に追跡できるようにすることです。 この機能では、ページバージョンを使用して発行環境の状態を判断します。

次の手順を実行します。

* システムで、選択した時間にアクティブであったページバージョンが検索されます。
* つまり、Timewarp で選択した時点より前に作成またはアクティベートされたバージョンが表示されます。**
* 削除されたページに移動すると、そのページの古いバージョンがリポジトリで引き続き使用できる限り、そのページもレンダリングされます。
...
1. ダイアログで、目的の日時を設定して、「**日付を設定**」をクリックまたはタップします。時間を選択しない場合は、現在の時間がデフォルトになります。
   ...
   ![タイムワープのターゲット日](/help/sites-cloud/authoring/assets/versions-timewarp-target.png)

1. ページは、日付設定に基づいて表示されます。タイムワープモードは、ウィンドウの上部にある青いステータスバーで示されます。ステータスバーのリンクを使用して、新しい目的の日付を選択したり、タイムワープモードを終了したりします。

   ![Timewarpモード](/help/sites-cloud/authoring/assets/versions-timewarp.png)

### Timewarpの制限事項 {#timewarp-limitations}
...
* **Timewarpは読み取り専用** — 古いバージョンのページは編集できません。 古いバージョンは表示のみ可能です。古いバージョンを復元する場合は、[復元](#reverting-to-a-page-version)を使用して手動でおこなう必要があります。
* **Timewarpはページのコンテンツに基づいてのみ行われます** - webサイトのレンダリング用の要素（コード、CSS、アセット/画像など）が変更された場合、これらの項目はリポジトリでバージョン管理されないので、表示は元の表示と異なります。

>[!CAUTION]
>
>Timewarpは、作成者がコンテンツを理解し、作成するのを支援するツールとして設計されています。 監査ログや法的な目的のためのものではありません。
