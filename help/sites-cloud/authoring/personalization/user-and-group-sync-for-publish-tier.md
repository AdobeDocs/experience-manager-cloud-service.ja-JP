...
サーバーサイドのユーザープロファイルの永続性ロジックを正しく機能させるには、適切なプログラムと環境を示すリクエストをカスタマーサポートに送信して、 [データ同期](#data-synchronization-data-synchronization)を有効にしてください。
...
上記のロジックを正しく機能させるには、適切なプログラムと環境を示すリクエストをカスタマーサポートに送信して、 [データ同期](#data-synchronization-data-synchronization)を有効にしてください。
...
>[!IMPORTANT]
>
>本番環境でデータ同期を有効にする前に、実装を大規模にテストします。 ユースケースと永続化されるデータによっては、一貫性と待ち時間に関する問題が発生する可能性があります。

### カスタムコードと移行要件 {#custom-code-and-migration-requirements}

次の要件は、カスタムコードを使用してローカルユーザーまたはローカルグループを作成する場合にのみ適用されます。 データ同期が有効になっている場合、このようなカスタムコードを更新して、外部ユーザーと動的グループメンバーシップを持つ外部グループを作成する必要があります。

**必要な手順：**

* **カスタムコードの変更**：ユーザーまたはグループの作成を担当するカスタムロジックは、次のように更新する必要があります。

   * `rep:externalId` プロパティを設定して外部ユーザーを作成する
   * `rep:externalId` プロパティを設定して外部グループを作成する
   * ユーザーとグループの直接関係を使用するのではなく、`rep:externalPrincipalNames` プロパティを使用して動的グループメンバーシップを実装します

* **既存のデータの移行**: データ同期が実稼動環境で有効になる前に、既存のすべてのローカルユーザーとグループを外部ID モデルに移行する必要があります。

カスタム実装の更新と既存のユーザーとグループの移行に関する技術的なガイダンスについて詳しくは、[外部IDと動的グループメンバーシップへの移行](/help/security/migrating-to-external-identity.md)を参照してください。
