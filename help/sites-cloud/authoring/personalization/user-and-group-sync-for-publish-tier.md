...
カスタム登録コードは、ユーザーのユーザー名とパスワードを最小限に抑えて取得し、AEMでユーザーレコードを作成し、ログイン時に認証に使用できるように記述できます。 通常、この登録メカニズムの構築には次の手順を使用します。
...
顧客は、好みの SAML IdP を使用して SAML ベースの認証を使用できます。AEMで IdP を使用する場合、IdP は、SAML アサーションで説明するように、ユーザーの資格情報を認証し、AEMでユーザーの認証を仲介し、必要に応じてAEMでユーザーレコードを作成し、AEMでのユーザーのグループメンバーシップを管理します。
...
Web サイトのエンドユーザーは、別のブラウザーを使用してログインした場合、知らないうちに、パブリッシュ層インフラストラクチャの異なるサーバーノードに接続されたとしても,すべての Web ページリクエストで一貫したエクスペリエンスを期待しています。AEM as a Cloud Serviceは、すばやく同期することでこれを実現します `/home` パブリッシュ層のすべてのノードにわたるフォルダー階層（ユーザープロファイル情報、グループメンバーシップなど）。
