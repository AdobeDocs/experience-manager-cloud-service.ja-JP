...
### カスタムコードと移行の要件 {#custom-code-and-migration-requirements}

次の要件は、カスタムコードを使用してローカルユーザーまたはローカルグループを作成する場合にのみ適用されます。 データ同期が有効な場合、動的グループメンバーシップを持つ外部ユーザーと外部グループを作成するには、このようなカスタムコードを更新する必要があります。

**必要な手順：**

* **カスタムコードの変更**：ユーザーまたはグループの作成に責任を負うカスタムロジックは、次のように更新する必要があります。
...
* **既存データの移行**：実稼動環境でデータ同期を有効にする前に、既存のローカルユーザーとグループをすべて外部 ID モデルに移行する必要があります。

カスタム実装の更新と、既存のユーザーとグループの移行に関する技術的なガイダンスについて詳しくは、[ 外部 ID と動的グループメンバーシップへの移行 ](/help/security/migrating-to-external-identity.md) を参照してください。
