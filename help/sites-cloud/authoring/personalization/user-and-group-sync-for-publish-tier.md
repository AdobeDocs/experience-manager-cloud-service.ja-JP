...
>[!IMPORTANT]
>
>この記事で説明する機能を動作させるには、ユーザーデータ同期機能を有効にする必要があります。これには、現時点では、適切なプログラムと環境を示してカスタマーサポートにリクエストする必要があります。有効にしない場合、ユーザー情報は消えるまでの短い期間（1 ～ 24 時間）保持されます。
...
AEM as a Cloud Service では cookie ベースのスティッキーセッションが有効になっており、エンドユーザーはリクエストのたびに同じ公開ノードにルーティングされます。パフォーマンスを向上させるために、カプセル化されたトークン機能はデフォルトで有効になっているので、リポジトリ内のユーザーレコードを要求ごとに参照する必要はありません。 エンドユーザーが置き換えるアフィニティを持つパブリッシュノードが置き換えられた場合、そのユーザー ID レコードは、以下の「データ同期」の節で説明するように、新しいパブリッシュノードで使用できます。
...
とも呼ばれる）は、AEM オーサーでは[ここ](https://experienceleague.adobe.com/docs/experience-manager-65/administering/security/cug.html?lang=ja#applying-your-closed-user-group-to-content-pages)で説明されているとおりに定義されています。一部のユーザーが Web サイトの特定のセクションまたはページを制限するには、AEMオーサーを使用して、必要に応じて CUG を適用し、パブリッシュ層にレプリケートします。
