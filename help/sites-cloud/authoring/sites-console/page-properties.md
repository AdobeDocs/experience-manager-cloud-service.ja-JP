...
* **ログインページ** - ログインに使用されるページです
  ...
  >[!WARNING]
  >
  >指定されたログインページ **は、それ自体が**&#x200B;認証要件&#x200B;**によって保護されているページを** ポイントしてはなりません。 ログインページが保護されていない公開ページであることを確認する必要があります。 例えば、**認証要件**&#x200B;が有効になっていない`/content/<mysite>/en/login`などの専用ページがあります。
  >
  >設定されたログインページに&#x200B;**Enable**&#x200B;がチェックされている場合、そのページの認証要件は起動時にサイレントで削除され、一般にアクセスできるようになります。
...
* **構造化データ （JSON-LD）** – ここで定義されたコンテンツは、[&#x200B; ページコンポーネント &#x200B;](https://experienceleague.adobe.com/ja/docs/experience-manager-core-components/using/wcm-components/page#structured-data)によって、ページヘッドの`<script type="application/ld+json">` タグ内で発行されます。
