...
* デフォルトでは、 [ライブコピー](/help/sites-cloud/administering/msm/overview.md) ソースページの変更に応じて、ローンチページを自動的に更新します。 静的なコピーを作成して自動更新をおこなわないように指定できます。
...
1. **ソースを選択**&#x200B;のステップでは、**ページを追加**&#x200B;する必要があります。複数のページを選択することもできます。それぞれにパスを指定します。
   ...
   * ソースページ（複数可）を選択し、確定します（チェックマークを付けます）。
   ...
   >[!NOTE]
   >
   >ローンチにページやブランチを追加するには、サイト内に存在する必要があります。つまり、共通の最上位ルートの下に配置されます。
   >
...
1. ウィザードの&#x200B;**プロパティ**&#x200B;ステップでは、次の情報を指定できます。
   ...
   * **既存のコンテンツを使用**:元のコンテンツを使用してローンチが作成されます。
...
>[!CAUTION]
>
>別のテンプレートを使用する場合、新しいページは空になります。 ページ構造が異なるので、コンテンツはコピーされません。
>
>このメカニズムを使用して、 [既存ページ](/help/sites-cloud/authoring/fundamentals/organizing-pages.md#creating-a-new-page)  — コンテンツの損失を考慮する必要があります。
...
ローンチは、 [ローンチコンソール](/help/sites-cloud/authoring/launches/overview.md#the-launches-console):
