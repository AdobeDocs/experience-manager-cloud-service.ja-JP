...
静的なグラフィックの場合、そのグラフィックと同等の代替テキストを指定することが基本的な要件です。このメソッドは、 **代替テキスト** フィールドに入力します。 例えば、コアコンポーネントを参照してください。 **[画像](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/wcm-components/image.html)**.
...
1. 映像またはアニメーションの音声および視覚要素をテキストで適切に表現した字幕を提供します。これには、誰が話しているかを示す表示、設定の説明、視覚的に提示されるイベントや情報、声の表現などが必要です。 長さに応じて、トランスクリプトをビデオやアニメーションと同じページに配置したり、別のページに配置したりできます。後者のオプションを選択する場合は、ビデオまたはアニメーションの横にあるトランスクリプトへのリンクを指定します。
...
* 視覚的な位置に依存して情報を提供しないでください。例えば、ユーザーをページの右側にあるメニューに移動して詳細情報にアクセスしたい場合、 *右のメニュー*;代わりに、メニューに名前を付け（例えば、見出しを使用）、テキストでその名前を参照します。
...
AEM WKND デモプロジェクトでは、フォームコンポーネント ( **テキストフィールド**&#x200B;をページに追加します。 このデフォルトのタイトルは、コンポーネントタイプによって異なります。 独自のタイトルを **タイトルとテキスト** 」タブをクリックします。 各フォームコンポーネントに関連付けられているデータをユーザーが理解できるようなラベルを指定することが重要です。
...
機能が限られているシンプルなフォームでは、 `Submit` ボタンは、隣のフィールドのラベルとして機能できます ( 例： `Search`) をクリックします。 これは、ラベルテキストのスペースを見つけることが困難な可能性のある場合に便利です。
