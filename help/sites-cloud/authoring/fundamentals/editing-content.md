...
>
>ページをロックしたユーザーがページのロックを解除できない場合は、カスタマーサポートに連絡して、ロックを解除するためのオプションを評価してもらってください。
...
AEMには、実行したアクションの履歴と実行した順序が保存されます。これにより、複数のアクションを実行した順序で元に戻し、必要に応じて 1 つ以上のアクションを再適用できます。
