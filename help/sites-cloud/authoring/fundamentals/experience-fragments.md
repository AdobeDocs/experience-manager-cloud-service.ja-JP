...
## エクスペリエンスフラグメントのパーソナライゼーション {#personalization-experience-fragment}

エクスペリエンスフラグメントのパーソナライゼーションを使用すると、マーケターはエクスペリエンスフラグメントのターゲットオーディエンスを 1 回だけ定義し、任意のページでフラグメントを再利用できます。 この特徴は次のとおりです。

* フラグメントを使用するたびに、オーディエンスごとに必要なバリエーションを指定する必要がなくなりました。
* オファー全体のスタイル設定を維持

この単一のフラグメント内にグループ化された複数のコンポーネントを含むエクスペリエンスフラグメントを作成できます。 また、特定のオーディエンスセグメントごとにフラグメントのバリエーションを作成し、必要なチャネルでそれらのエクスペリエンスフラグメントを再利用することもできます。

パーソナライゼーションは、 **パーソナライズ** エクスペリエンスフラグメントまたはバリエーション、またはフラグメントを含むフォルダーのプロパティ。つまり、継承はパーソナライゼーションプロパティを上書きできます。

これらのプロパティを設定すると、 **ターゲット設定** モードを使用して、エクスペリエンスフラグメントエディターに切り替えることができます。

### エクスペリエンスフラグメントのパーソナライゼーションの定義 {#defining-personalization-experience-fragment}

フラグメントをパーソナライズするには：

1. で必要な場所に移動します。 **エクスペリエンスフラグメント** コンソール。

1. フォルダーまたはフラグメントを選択し、 **プロパティ** をクリックします。

   >[!NOTE]
   >
   >フォルダーに定義されたパーソナライゼーションプロパティは、サブツリー内のすべての子フォルダーと、そのサブツリー内のエクスペリエンスフラグメント（およびバリエーション）に継承されます。 継承を解除することで、上書きできます。

1. を開きます。 **パーソナライズ** タブをクリックし、設定を定義して保存します。 例えば、フォルダーの場合：
   ...
   >
   >フラグメント内のコンポーネントに対して実行されるターゲティングがページのレンダリングで機能するようにするには、次の条件を満たす必要があります。
   >
   >この **ContextHub のパス** 選択された **パーソナライズ** タブは次のいずれかにする必要があります。
   >
   ...
   >
   > 
この **セグメントパス** 選択された **パーソナライズ** タブは次のいずれかにする必要があります。
...
パーソナライゼーションプロパティを設定すると、フラグメントを編集用に開いたときに、ターゲティングモードを使用できるようになります。

![エクスペリエンスフラグメントエディター — ターゲットモード](/help/sites-cloud/authoring/assets/xf-targeting-mode.png)

このモードは、ページ編集の場合と同じ方法で動作します。 詳しくは、 [ページエディターのターゲットモード](/help/sites-cloud/authoring/personalization/targeted-content.md) を参照してください。
