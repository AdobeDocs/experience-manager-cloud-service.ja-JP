...
注釈は、色付きのスケッチまたはノート注釈をページに配置します。 注釈を使用すれば、そのユーザー（または他のユーザー）がコメントや質問を他の作成者またはレビュー担当者に残すことができます。
...
>[!CAUTION]
>
>リソース（コンポーネントなど）を削除すると、ページ全体での位置に関係なく、そのリソースにアタッチされたすべての注釈とスケッチが削除されます。
...
1. 注釈を追加するために必要なコンポーネント（注釈を追加できるコンポーネントは青いボーダーで強調表示されます）をクリックまたはタップして、ダイアログを開きます。
   ...
   * スケッチ（線と図形）を作成して、コンポーネントの特定の領域を強調表示します。
      ...
      ![注釈スケッチボタン](/help/sites-cloud/authoring/assets/annotation-sketch.png)

      スケッチを作成すると、カーソルが十字に変わります。 複数の異なる線を描くことができます。スケッチ線は注釈の色を反映し、矢印、円、楕円のいずれかになります。

   * 色を選択または変更します。

      ![注釈カラースウォッチボタン](/help/sites-cloud/authoring/assets/annotation-color-swatch.png)
...
1. ダイアログの外側をクリックまたはタップして、注釈ダイアログを閉じることができます。注釈の一部の表示（最初の単語）とスケッチが一緒に表示されます。
   ...
   ![注釈スケッチ](/help/sites-cloud/authoring/assets/annotation-sketches.png)
...
## その他のリソースへの注釈の作成 {#annotating-other-resources}

コンポーネント以外にも、次の様々なリソースに注釈を付けることができます。

* アセットの注釈の作 [成アセットの注釈の作成](/help/assets/manage-digital-assets.md#annotating)
* ビデオアセットの注釈の作 [成ビデオアセットの注釈の作成](/help/assets/manage-video-assets.md#annotate-video-assets)
