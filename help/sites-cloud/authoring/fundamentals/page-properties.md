...
* **タイトルとタグ**
   ...
   * **タグ** - この場所で、選択ボックスのリストを更新して、ページのタグの追加や削除を行うことができます。
      ...
      * タグについて詳しくは、 [タグの使用](/help/sites-cloud/authoring/features/tags.md).
...
* **設定**
   ...
   * **継承元 &lt;path>**  — 継承を有効/無効にします。～の可用性を切り替える **クラウド設定** （選択）
...
* **SEO**
   ...
   * **正規 URL**  — ページの正規 URL を上書きするために使用できます。空白にした場合、ページの URL は正規の URL になります
...
* **アイキャッチ画像**
   ...
   * **画像**
      ...
      以下が可能です。 **選択** アセットをアップロードするか、アップロードするファイルを参照してから、 **編集**&#x200B;または **クリア**.

   * **代替テキスト**  — 画像の意味や機能を表すために使用されるテキスト。例えば、スクリーンリーダーで使用する場合です。

   * **継承 — DAM アセットから取得した値**  — オンにすると、代替テキストに `dc:description`DAM のメタデータ
...
* **ContextHub 設定**
   ...
   * **継承元 &lt;path>**  — 継承を有効/無効にします。～の可用性を切り替える **ContextHub パス** および **セグメントパス** （選択）
...
* **インストール可能なエクスペリエンスを設定**
   ...
   * **「Enable」PWA**  — 機能を有効/無効にします。ユーザーがサイトをPWAとしてインストールできる
   * **StartupURL**  — 優先スタートアップ URL
   * **ディスプレイモード**  — ブラウザーを非表示にする方法、またはローカルデバイス上のユーザーに表示する方法
   * **画面の向き** -PWAがデバイスの向きを処理する方法
   * **テーマカラー**  — ローカルユーザーのオペレーティングシステムでのネイティブの UI ツールバーおよびナビゲーションコントロールの表示方法に影響するアプリの色
   * **背景色**  — アプリの背景色。アプリの読み込み時に表示されます。
   * **アイコン**  — ユーザーのデバイス上のアプリを表すアイコン

* **キャッシュ管理 (詳細)**

   * **キャッシュ戦略とコンテンツ更新の頻度** -PWAのキャッシュモデルを定義します
   * **オフラインで使用するようキャッシュするファイル**
      * **ファイルのプリキャッシュ（テクニカルプレビュー）** - AEMでホストされているファイルは、サービスワーカーがインストール中、使用前に、ローカルブラウザーのキャッシュに保存されます
      * **クライアント側ライブラリ**  — オフラインエクスペリエンス用にキャッシュするクライアント側ライブラリ
      * **パスを含む**  — 定義されたパスに対するネットワーク要求が傍受され、設定されたキャッシュ方法とコンテンツ更新の頻度に従って、キャッシュされたコンテンツが返されます
      * **パスの除外**  — これらのファイルは、ファイルのプリキャッシュおよびパスインクルージョンの設定に関係なく、キャッシュされません
...
>[!NOTE]
>
>また、アセットではプロパティの一括編集も可能です。 これは非常に似ていますが、いくつかの点で異なります。 詳しくは、複数のアセットのプロパティの編集を参照してください。
>
