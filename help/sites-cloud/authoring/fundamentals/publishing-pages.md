...
1. **公開を管理**&#x200B;ウィザードが起動します。最初のステップでは、**オプション**&#x200B;を使用すると、次のことが可能です。
   ...
   * **スケジュール設定**
     ...
     後で公開すると、選択したページ（または複数のページ）を指定した時間に公開するワークフローが開始されます。逆に、後で非公開することにより、特定の時間に選択したページ（または複数のページ）を非公開にするワークフローが開始されます。
...
ページを編集する際に、そのページを非公開にする場合、[ページを公開](#publishing-from-the-editor)する際と同じように、**ページ情報**&#x200B;メニューで「**ページを非公開**」を選択します。
