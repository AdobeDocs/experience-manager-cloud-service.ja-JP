...
>[!INFO]
>
>クライアントサイド監視は、AEM Cloud Service バージョン **2024.5.16461** 以降を使用しているお客様に対してのみ機能します。
...
1. **サイトで `/.rum` パスがブロックされています。修正する方法を教えてください。**
   ...
   RUM コレクションが機能するには、`/.rum` のパスが必要です。 AdobeがAEM as a Cloud Serviceの一部として提供するものを CDN の前におく場合は、`/.rum` パスを残りのAEM コンテンツと同じAEM オリジンに転送する必要があります。 また、いかなる方法でも調整されないようにしてください。
