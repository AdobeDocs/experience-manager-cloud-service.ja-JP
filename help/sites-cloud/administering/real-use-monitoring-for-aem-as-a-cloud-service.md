...
1. **この `/.rum` サイトでパスがブロックされています。修正方法を教えてください。**
   ...
   この `/.rum` rum コレクションが機能するにはパスが必要です。 AdobeがAEM as a Cloud Serviceの一部として提供する機能の前に CDN がある場合は、次のことを確認します。 `/.rum` パスは、AEM コンテンツの残りの部分と同じAEM オリジンに転送されます。 また、いかなる方法でも調整されないようにしてください。

1. **RUM コレクションは、契約上の目的でコンテンツリクエストにカウントされますか。**

   RUM ライブラリと RUM コレクションは、コンテンツリクエストとしてカウントされず、レポートされるページビュー数や API 呼び出し数を増やしません。 さらに、AEM as a Cloud Serviceに標準搭載の CDN を使用する場合、 [サーバーサイドコレクション](#serverside-collection) は、コンテンツリクエストの基礎です。
