...
実際の使用状況のモニタリングは、エンドユーザーがAEM サイトとどのようにやり取りしているかを顧客とAdobeが理解するのに役立ちます。 実際の使用モニタリングは、パフォーマンスの問題を診断し、実験の有効性を測定します。 リアルタイムモニタリングは、サンプリングを通じて訪問者のプライバシーを保持します。つまり、すべてのページビューのごく一部のみが監視され、個人を特定できる情報（PII）は収集されません。
...
実際の使用状況の監視では、クライアントサイドのトラフィックが自動的に監視されます。 Adobeのお客様は、このサービスが既存の設定にシームレスに統合されているので、追加の手順を実行する必要はありません。 Real Use Monitoring （RUM）サービスが一般提供されている場合は、この新機能のメリットが自動的に得られます。 実際の使用状況の監視サービスでは、現在顧客に表示されている監視対象の指標は公開されていません。 アドビでは、この機能をできる限り早くお客様に提供できるように取り組んでいます。
