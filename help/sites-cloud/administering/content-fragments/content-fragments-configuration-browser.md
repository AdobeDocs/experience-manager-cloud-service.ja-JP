...
>* 次の操作はできません [サイト設定を選択して関連するエンドポイントを作成します](/help/headless/graphql-api/graphql-endpoint.md).
...
他の設定（グローバル以外）を同等の Assets フォルダーで使用するには、接続を定義する必要があります。 この接続は、 **設定** （内） **Cloud Service** タブ **フォルダーのプロパティ** 」と入力します。
