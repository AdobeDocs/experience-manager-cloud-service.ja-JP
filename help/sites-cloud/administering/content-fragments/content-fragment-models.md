...
フラグメント内のフィールドを実現／レンダリングするための様々なオプション。多くの場合、これにより、作成者にフィールドの 1 つのインスタンスが表示されるか、作成者が複数のインスタンスを作成できるかを定義できます。条件 **複数のフィールド** を使用すると、項目の最小数と最大数を定義できます。詳しくは、 [検証](#validation) 詳しくは、を参照してください。
