...
>[!IMPORTANT]
>
>早期導入プログラムを通じて、コンテンツフラグメントエディターの様々な機能を利用できます。
>
...
* 右側のパネル：[プロパティ（メタデータ）とタグを表示](#view-properties-tags)するタブ、[バージョン履歴](#view-version-history)および[言語コピー](#view-language-copies)に関する情報を表示します。
   ...
   * 「**コメント**」タブでは、コメントを追加したり読み取ったりして、他の作成者との共同作業に役立てることができます
...
右側のパネルの「**コメント**」タブには、次の機能があります。

* 新しいコメントを追加
* コメント内の特定のユーザーにタグを付ける
   * フラグメントを直接開くためのリンクが記載された通知が届きます
* 既存のコメントに高評価を付ける
* コメントに返信する
* コメントの書式を設定します。基本的な書式を設定できます
* 既存のコメントを使用したテキスト検索の実行
* 既存のコメントを編集
* コメントの削除
...
>[!NOTE]
>
>これらのコメントは、[ 元のエディターの注釈 ](/help/assets/content-fragments/content-fragments-managing.md#annotating-a-content-fragment) や、[Assets コンソールのタイムライン ](/help/assets/content-fragments/content-fragments-managing.md#timeline-for-content-fragments) には表示されません。
