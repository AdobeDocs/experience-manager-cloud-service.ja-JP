...
1. [設定またはサブ設定](/help/sites-cloud/administering/content-fragments/content-fragments-configuration-browser.md)に適したフォルダーに移動します。
...
* **タブプレースホルダー**
   * コンテンツフラグメントコンテンツの編集時に使用するタブを導入可能モデルエディターで区切りとして表示され、コンテンツデータタイプのリストのセクションが分割されます。 各インスタンスは、新しいタブの開始を表します。
...
* **フィールドラベル**
の入力 **フィールドラベル** 自動生成します **プロパティ名**（必要に応じて手動で更新できます）
...
（特定のフィールドの）コンテンツは、現在のモデルから作成されたすべてのコンテンツフラグメントで一意である必要があります。
  ...
  >[!CAUTION]
  >
  >MSM（コンテンツフラグメントのコピーを作成する）を使用する場合は、 **ユニーク** 制約は、それぞれのコンテンツフラグメントモデルで使用されるデータタイプから削除する必要があります。 MSM for Content Fragments は、 **Assets** コンソール。
...
* コンソール
   ...
   * モデルを&#x200B;**ロック解除**&#x200B;して編集を有効にすることができます。
     ...
     次を選択した場合、 **ロック解除**&#x200B;を指定しない場合は、警告が表示され、 **ロック解除** アクション：
