...
Adobe Experience Manager（AEM）as a Cloud ServiceのLaunchでは、今後のリリースに向けてコンテンツを効率的に開発できます。
...
*ローンチ*&#x200B;を作成し、*ローンチ*&#x200B;のコンテンツフラグメントを編集および更新します。このフェーズで&#x200B;*Source* フラグメントに変更が加えられた場合、*リベース*&#x200B;操作で&#x200B;*起動*&#x200B;に&#x200B;**[Source](#rebase-a-launch-from-source)** （変更内容を含む）をコピーできます。 準備が整ったら、*昇格*&#x200B;により、ローンチコンテンツが複製され、ソースに戻されます。その後、ソースフラグメントを手動または自動でアクティブ化できます（ローンチの作成および編集時に設定されたフィールドに応じて異なります）。また、参照されているフラグメントをこのプロセスに含めるかどうかも指定できます。
...
* **プロモーション日**&#x200B;と時刻を定義：[&#x200B; ローンチを自動的にプロモーションする場合](#promote-automatically)

* 実行中の&#x200B;**[ジョブ](#jobs-history)**&#x200B;を表示し、さらにアクションを実行します（**LaunchをSource**&#x200B;と比較）
...
**[リベース](#rebase-a-launch-from-source)**&#x200B;または&#x200B;**[プロモーション](#promote-a-launch-to-source)** アクションの前に、常にソースとローンチを比較して、変更とコンテンツへの影響を確認することをお勧めします（両方のアクションでターゲットコンテンツが上書きされます）。
...
1. [昇格](#promote-a-launch-to-source)アクションと[リベース](#rebase-a-launch-from-source)アクションは右上から使用できます。

1. **戻る**&#x200B;でコンソールに戻ります。 これらの特定の違いを再度確認する場合は、**[ジョブ](#jobs-history)**&#x200B;のエントリを表示できます。

   ![ソースとローンチを比較](/help/sites-cloud/administering/content-fragments/assets/cf-launches-compare.png)

## ジョブ履歴 {#jobs-history}

過去の&#x200B;**実行されたSource** ジョブと比較の詳細を表示するには：

1. コンテンツフラグメントコンソールに移動します。

1. 「**ローンチ**」タブを開きます。

1. ローンチを選択すると、情報パネルが右側に開きます。

1. 「**ジョブ**」セクションには、実行された&#x200B;**Source** ジョブと比較するそれぞれの&#x200B;**launchDifferences** エントリが表示されます。

   ![&#x200B; ジョブ履歴](/help/sites-cloud/administering/content-fragments/assets/cf-launches-jobs.png)

1. 選択:

   * 特定のジョブの詳細を開くための虫眼鏡アイコン。
これにより、使用可能なアクションを含む&#x200B;**[LaunchとSource](#compare-launch-to-source)**&#x200B;の比較ビューが返されます。
   * **ログを表示**&#x200B;して、すべてのジョブの詳細の概要を表示します。
ここから、特定のジョブを選択して、**結果**&#x200B;を表示することもできます。 これにより、使用可能なアクションを含む&#x200B;**[LaunchとSource](#compare-launch-to-source)**&#x200B;の比較ビューに戻ります。

   これらの操作を実行すると、適切な&#x200B;**LaunchをSource** ジョブと比較できます。 ここから、**[リベース](#rebase-a-launch-from-source)**&#x200B;または&#x200B;**[プロモーション](#promote-a-launch-to-source)**&#x200B;が、その時点でのローンチと同じように表示されます。

## ローンチのリベース（ソースから） {#rebase-a-launch-from-source}

ソースフラグメントが更新された場合、**Rebase** アクションを使用して、ソース（変更を含む）をローンチにコピーできます。
...
>[!NOTE]
>
>**リベース**&#x200B;または&#x200B;**プロモーション** アクションの前に、ソースを常に[比較して](#compare-launch-to-source)起動し、変更とコンテンツへの影響を確認することをお勧めします。両方のアクションでターゲットコンテンツが上書きされます。
