...
1. Adobe Analytics の [グローバルナビゲーションメニュー](/help/sites-cloud/authoring/getting-started/basic-handling.md#global-navigation) 選択 **ツール/Cloud Serviceと翻訳のCloud Service**.
1. コンテンツ構造で設定を作成する場所に移動します。 この場所は多くの場合、特定のサイトに基づいている場合もあれば、グローバルな場合もあります。
1. フィールドに次の情報を入力し、「 」を選択します。 **作成**.:
...
1. Adobe Analytics の **設定を編集** ウィンドウで、 **Sites** および **Assets** タブをクリックし、 **保存して閉じる**.
...
1. サイトコンソールで、設定するページを選択し、「 」を選択します。 **プロパティを表示**.
1. 「**クラウドサービス**」タブを選択します。
1. Adobe Analytics の **設定を追加** 」ドロップダウンリストから、設定を選択します。
1. 「**保存して閉じる**」を選択します。
