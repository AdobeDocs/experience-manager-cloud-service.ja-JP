...
翻訳プロジェクトに追加する、または翻訳プロジェクトから除外するページ、コンポーネントおよびアセットの翻訳対象コンテンツは翻訳ルールによって特定されます。ページやアセットを翻訳する場合は、AEM がそのコンテンツを抽出して、翻訳サービスに送信できるようにします。
...
翻訳ルールを使用してコンテンツフラグメントを翻訳する場合は、 **翻訳するコンテンツモデルフィールドを有効にする** 翻訳統合フレームワーク設定のオプションを無効にする必要があります。また、ルールを作成するには、以下の手順に従う必要があります。
...
1. パスブラウザーを使用して必要なコンテキストを選択し、「 **確認** ボタンをクリックして保存します。
