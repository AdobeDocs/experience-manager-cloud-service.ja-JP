...
翻訳ジョブをスコーピングする場合、ジョブのステータスは「 **リクエストされた範囲**. 翻訳ベンダーがスコープを返すと、ステータスが「 」に変更されます。 **スコープ完了**. スコーピングが完了したら、「**範囲を表示**」コマンドを使用してスコーピング結果を確認できます。
...
翻訳ジョブをキャンセルして、翻訳プロセスを停止し、翻訳ベンダーがそれ以上の翻訳を実行しないようにします。 ジョブが **翻訳をコミット済み** または **翻訳中** ステータス。
