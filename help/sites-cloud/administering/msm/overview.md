...
MSM で使用される主な用語の概要を次の表に示します。 これらの詳細については、以降の節およびページで説明します。
...
| ライブの関係 | 特定のリソース（ソースとライブコピーの間の接続）に対する継承の有効な定義 | コピーを使用してソースに対する変更を同期できます |
...
| ロールアウト設定 | 同期するプロパティ、同期方法および同期のタイミングを決定するルール |  |
...
例えば、2 番目のブループリントから最初のブループリントを指すリンクは、ネストされた 2 番目のライブコピーから最初のライブコピーを指すリンクとして書き換えられます。

![ネストされたライブコピー](../assets/live-copy-nested.png)

>[!NOTE]
>
>ライブコピーブランチ内でページを移動または名前変更すると、AEMが関係を追跡できるように、ネストされたライブコピーとして扱われます。
