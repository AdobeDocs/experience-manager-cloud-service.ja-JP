...
これは、**Day CQ WCM Rollout Manager** の [OSGi 設定](/help/implementing/deploying/configuring-osgi.md)を使用しておこないます。ロールアウトマネージャーが、ブループリントに存在する名前を使用してライブコピーで作成されたページからの競合を処理する場合は、**手動で作成されたページで競合を処理**（`rolloutmgr.conflicthandling.enabled`）の値を true に設定します。
...
>[!CAUTION]
>
>競合処理が無効になっていても、AEM では、競合が無視されることを示す表示は一切おこなわれません。このような場合、この動作は明示的に設定する必要があるので、これが目的の動作であると見なされます。
