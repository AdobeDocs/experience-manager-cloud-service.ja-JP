...
>
>関連付けられたローカルページを作成する場合のベストプラクティスは、ソース内にローカルページを作成し、詳細なロールアウトを実行することです。これにより、ページがライブコピーとしてローカルに作成されます。
...
1. [ライブコピーの概要](live-copy-overview.md#using-the-live-copy-overview)を開き、休止されたライブコピーページを選択します。ページには、**INHERITANCE CANCELLED** と表示されます。
1. ツールバーの「**再開**」を選択します。
