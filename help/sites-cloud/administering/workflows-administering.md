...
description: ワークフローインスタンスの管理方法を学びます
...
ワークフローの管理用に、次の各種コンソールが用意されています。Use the [global navigation](/help/sites-cloud/authoring/getting-started/basic-handling.md#global-navigation) to open the **Tools** pane, then select **Workflow**:
...
* **自動割り当て**:テンプレートへのワークフローの自動割り当ての設定
...
## 検索ワークフローインスタンス {#search-workflow-instances}

1. Using Navigation select **Tools**, then **Workflow**.
1. 「**インスタンス**」を選択して現在進行中のワークフローインスタンスのリストを表示します。上部のレールの左隅で、「 **フィルター**」を選択します。 または、alt + 1キーを押します。 次のダイアログが表示されます。
...
1. フィルターダイアログで、ワークフローの検索条件を選択します。 次の入力に基づいて検索できます。
   ...
   * Where I Am:所有者と担当者、所有者のみ、担当者のみ
   * 開始日：指定した日付の前または後の開始日
   * 終了日：指定した日付の前または後の終了日
   * 期日：指定した日付の前または後の期日
   * 更新日：指定した日付の前または後に更新された日付
...
* **再試行手順** ：スクリプト手順コンポーネントインスタンスを再度実行します。 元のエラーの原因を修正したら、「ステップを再試行」コマンドを使用します。 例えば、プロセスステップが実行するスクリプトのバグを修正した後にステップを再試行します。
* **終了** ：エラーがワークフローに対して調整不能な状況を引き起こした場合は、ワークフローを終了します。 例えば、ワークフローは、リポジトリ内の情報など、ワークフローインスタンスに対して有効でなくなった環境条件に依存できます。
...
サービスを設定するには、OSGi設定ファイルを設定します。OSGi設定ファイルを参照して [ください](/help/implementing/deploying/configuring-osgi.md)。 次の表に、どちらの方法にも必要なプロパティを示します。
...
You can set the maximum size of the inbox by configuring the **Adobe Granite Workflow Service**, see [add an OSGi configuration to the repository](/help/implementing/deploying/configuring-osgi.md). 次の表に、設定するプロパティを示します。
