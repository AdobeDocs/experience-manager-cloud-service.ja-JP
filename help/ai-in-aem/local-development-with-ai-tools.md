...
### コード評価スキルの使用 {#use-the-code-assessment-skill}

**（Beta）** `code-assessment` スキルは、AEM as a Cloud Service プロジェクトのコード品質と正確性の問題をローカル ワークスペース内ですべて検出、レビュー、修正します。 問題を説明し、スキルはリクエストを適切な修正ワークフローにルーティングします。

サポートされているチェックには、Sling モデルの依存関係インジェクションの最新化、古いMaven依存関係の更新、送信HTTP呼び出しにタイムアウトが欠落しているタイムアウトの追加、バウンディングアンバウンドクエリ、Sling スケジューラー、リソース変更リスナー、レプリケーションおよびAssets API、JCRまたはOSGi イベント処理などが含まれ、時間の経過とともに追加されます。 問題に応じて、スキルは機械的な修正を直接適用するか、判断コールが必要な修正をガイドします。
