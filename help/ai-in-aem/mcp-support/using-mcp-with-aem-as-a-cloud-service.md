...
| MCP サーバー | エンドポイント | 説明 |
|---|---|---|
| **コンテンツ** | `/content` | ページとコンテンツフラグメントの作成、読み取り、更新、削除（CRUD）に加えて、アセットの読み込みとアセットの検索（最低限必要なAEM リリースバージョンは`26309`です）などのコンテンツ操作。 |
| **コンテンツ（読み取り専用）** | `/content-readonly` | ページとコンテンツフラグメントの読み取り専用コンテンツ操作（取得、リスト/検索）、アセット検索（最低限必要なAEM リリースバージョンは`26309`です）。 |
