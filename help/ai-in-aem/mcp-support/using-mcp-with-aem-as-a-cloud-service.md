...
| MCP サーバー | エンドポイント | 説明 |
|---|---|---|
| **コンテンツ** | `/content` | ページとコンテンツフラグメントの作成、読み取り、更新、削除（CRUD）に加えて、アセットの読み込みとアセットの検索（最低限必要なAEM リリースバージョンは`26309`です）などのコンテンツ操作。 [Cloud Connector](/help/ai-in-aem/mcp-support/setup-claude.md)では、メタデータの編集、参照の検査、未使用のアセットの識別のほか、アセットのアップロード、ダウンロード、移動/コピー/削除、アセットの公開/非公開もサポートされています。 |
| **コンテンツ（読み取り専用）** | `/content-readonly` | ページとコンテンツフラグメントの読み取り専用コンテンツ操作（取得、リスト/検索）、アセット検索（最低限必要なAEM リリースバージョンは`26309`です）。 [Cloud Connector](/help/ai-in-aem/mcp-support/setup-claude.md)では、アセットのダウンロード、参照の検査、未使用アセットの識別もサポートされています。 |
