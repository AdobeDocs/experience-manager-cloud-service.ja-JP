...
| MCP サーバー | エンドポイント | 説明 |
|---|---|---|
| **コンテンツ** | `/content` | ページとコンテンツフラグメントの作成、読み取り、更新、削除（CRUD）に加えて、アセットの読み込みとアセットの検索（最低限必要なAEM リリースバージョンは`26309`です）などのコンテンツ操作。 [Cloud Connector](/help/ai-in-aem/mcp-support/setup-claude.md)と[ChatGPT プラグイン &#x200B;](/help/ai-in-aem/mcp-support/setup-chatgpt.md)では、メタデータの編集、参照の検査、未使用のアセットの識別のほか、アセットのアップロード、ダウンロード、移動/コピー/削除、アセットの公開/非公開もサポートされています。 |
| **コンテンツ（読み取り専用）** | `/content-readonly` | ページとコンテンツフラグメントの読み取り専用コンテンツ操作（取得、リスト/検索）、アセット検索（最低限必要なAEM リリースバージョンは`26309`です）。 [Cloud Connector](/help/ai-in-aem/mcp-support/setup-claude.md)と[ChatGPT プラグイン &#x200B;](/help/ai-in-aem/mcp-support/setup-chatgpt.md)では、アセットのダウンロード、参照の検査、未使用アセットの識別もサポートされています。 |
| **Cloud Manager** | `/cloudmanager` | プログラム、環境、リポジトリ、パイプラインなど、トリガー可能なCloud Manager エンティティを管理します。 |
| **エクスペリエンスガバナンス** | `/experience-governance` | コンテンツ（テキスト、画像、ページ）をブランドガバナンスルールに照らして評価し、ブランドの設定とチェックをリストアップします。<br/>興味がある場合は、Experience Governance MCPにアクセスするには、[&#x200B; エージェント体験版にサインアップするか、有料ライセンス &#x200B;](https://experienceleague.adobe.com/ja/docs/experience-cloud-ai/experience-cloud-ai/agents/trial)を取得する必要があります。 |
...
| **AEM MCP サーバー** | `/aem` | 2026年6月の時点で新たに追加されたこのエンドポイントは、上記のAEM MCP サーバー（Cloud Migrationを除く）のツールを集約し、1つの接続を介して公開する単一のエンドポイントです。 これは、AEMへの接続に最適な方法です。 この集約サーバーに対してAdobeから要求できるアクセス制限は異なり、現時点では個々のサーバーに対してより制限されているので、要件を満たしていることを確認してください。 [MCP サーバーの制限](#restricting-mcp-servers)を参照してください。 |
...
* [OpenAI ChatGPT （MCP サーバーを手動で設定し、AEM ChatGPT プラグインもインストールする場合）](/help/ai-in-aem/mcp-support/setup-chatgpt.md)
...
デフォルトでは、すべてのMCP サーバが許可リストに加えるされます。 管理者は、組織、プログラム、または環境レベルで特定のMCP サーバーへのアクセスを制限するオプションがあります（AEM MCP Server以外 – 以下のメモを参照）。 この制限により、組織内のユーザーが利用できるMCP機能を詳細に制御できます。

>[!IMPORTANT]
>
>**AEM MCP サーバー** （`/aem`）は、1つのエンドポイントの背後にある複数のMCP サーバーを集約します。 Adobeからリクエストできる設定は異なり、オプションは&#x200B;**読み取り専用** （読み取り専用のツールのみ）または&#x200B;**アクセスなし** （ツールなし）です。 デフォルトでは、すべてのツール （**読み取り/書き込み**）にアクセスできます。 設定の変更をリクエストするには、Adobe（**`aemcs-mcp-feedback@adobe.com`**）にお問い合わせください。
...
* アセット検索
  ...
  >[!NOTE]
  >
  >MCP エンドポイントを介して&#x200B;**Assets Search**&#x200B;にアクセスするために必要なAEM リリースの最小バージョンは`26309`です。
