---
title: エクスペリエンス開発エージェントの概要
description: AEMの Experience Development Agent がCloud Managerの失敗したパイプラインを分析し、ログを作成してコード修正を提案し、デバッグを高速化する方法を説明します。
...
# エクスペリエンス開発エージェントの概要 {#development-agent-overview}

Experience Development Agent は、AEMの開発者と管理者がコードをより効率的に作成、デバッグ、デプロイおよび最適化するのに役立ちます。
...
## Cloud Managerから Experience Development Agent にアクセスする {#how-to-access-the-agent}

Experience Development Agent にアクセスするには、Cloud ManagerやExperience Hubなどのユーザーインターフェイスにある AI アシスタントを使用します。

**Cloud Managerから Experience Development Agent にアクセスするには：**
...
Experience Development Agent のパイプライントラブルシューティング ジョブには、Cloud Manager – 開発者ロールまたはCloud Manager - プログラムマネージャーロールが必要です。
...
| プロンプト | 結果 |
| --- | --- |
| *プログラムのメインプログラムの失敗したパイプラインをリストします。* | 結果は様々ですが、このプロンプトには、失敗したパイプラインのテーブルと、分析する特定のパイプラインを参照するためのフォローアップの提案が出力されます。 |
