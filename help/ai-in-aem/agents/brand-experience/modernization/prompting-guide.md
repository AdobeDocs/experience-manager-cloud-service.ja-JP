...
1. **スケールアップ。** – 結果が正しく表示されたら、URLの完全なリストを入力します。
...
* このスキルを実行すると、次の手順がホスト環境で自動的に実行されます。
   ...
   1. **Edge Delivery Services content generation** — エージェントは、EDS ブロック構造に従ってマークダウン ファイルを作成します
...
* 一般的な問題には既知のパターンがあります。
   ...
   * **ブロックがレンダリングされていません**: マークダウンでブロック名の形式を確認し、ブロックに`.js`で`.css`と`blocks/` ファイルの両方があることを確認します。
