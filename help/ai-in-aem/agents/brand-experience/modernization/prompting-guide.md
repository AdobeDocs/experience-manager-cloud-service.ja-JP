...
* Dynamic MediaとScene7の画像はソースで検出され、静的レンディションとして読み込まれるのではなく、DM URLとして保存されるので、レスポンシブでオンザフライの変換は維持されます。 これは、ドキュメントオーサリングとAEM Sites/ユニバーサルエディターの両方に適用されます。

### サイトカタログ {#site-catalog}

移行を開始する前に、このプロンプトを使用して既存のweb サイトを分析し、その規模、テンプレート、およびブロックのバリエーションを理解します。 この結果は、[一括読み込みに使用できます。](#bulk-import)

#### プロンプト例 {#example-prompts-site-catalog}

* 「スコープサイト https://example.com」
* 「https://example.comでテンプレートを検索」
* 「カタログサイト https://example.com」
...
* デザインの移行には、次の2つのフェーズがあります。
   1. フェーズ 1 （サイト全体）では、`styles/brand.css`と`styles/styles.css`が生成されます。
      * `brand.css`には、CSS カスタムプロパティ （フォント、色、間隔、見出しサイズ）としてデザイントークンが含まれています。
      * `styles.css`は`brand.css`をインポートし、トークンをデフォルトのコンテンツ（見出し、段落、ボタン、リンク、セクションの背景）に適用します。
      * プロジェクトに既に`brand.css`がある場合は、ソースから抽出する代わりにそれを使用するようにエージェントに促します。
   1. フェーズ 2では、個々のブロックを並行してスタイル設定し、`/blocks/{name}/{name}.css`でブロック固有のCSSを作成し、`brand.css`のトークンを参照します。
* ブロックのスタイル設定（フェーズ 2）では、サイト全体のデザイン（フェーズ 1）を最初に完了する必要があります。
* フェーズ 2では、1回のパスで80～90%のブロックレベルのスタイルの忠実度を達成します。
* 見積時間 :
   * フェーズ 1:5～10分
   * フェーズ 2:10～15分
...
* Block Critiqueは、移行されたブロックを元のソースと比較し、85%の類似性が達成されるか、3回の反復が完了するまで修正を適用します。
* このスキルでは、最初にページの移行によってブロックが作成されている必要があります。
* クリティークは、**コンテンツ/構造上の問題**&#x200B;と&#x200B;**スタイル設定の問題**&#x200B;の両方を検出して修正します。
...
このプロンプトを使用すると、サイト全体のすべての移行ブロックを1回のパスで検証できます。これは、複数ページの移行に適しています。

#### プロンプト例 {#example-site-critique}

* 「批判的な場所」
* 「移行されたサイトのすべてのブロックを検証する」

#### 知っておくべきこと {#wtk-site-critique}

* サイトのクリティクでは、テンプレートごとに1つずつ並行するサブエージェントを使用して、移行されたすべてのテンプレートのすべてのブロックを検証します。
* これは、ブロックのクリティク（グローバルスタイル→セクショントランスフォーマー→コンテンツ/構造パーサー→ブロック CSS）と同じ修正カスケードを、すべてのページで同時に適用します。
* 修正は重複排除されます。同じブロックを使用して同じ問題が複数のページに表示された場合、修正は1回適用されます。
* `brand.css`がまだ存在しない場合、コンテンツ構造のみのモードでクリティクが実行されます（スタイル設定なしでパーサーとトランスフォーマーを修正）。
* 複数ページプロジェクトのデザインを移行した後は、サイトのクリティクを使用することをお勧めします。
* 次のワークフローをお勧めします。
   1. ページの移行（単一または一括）。
   1. デザインの移行を実行します。
   1. `critique site`を実行して、すべてのテンプレートの残りのギャップを検証し、自動修正します。

### ページの評価 {#page-critique}

このプロンプトを使用して、移行された単一のページを元のweb サイトに対して視覚的に忠実性を持つように検証します。
...
* ページの批評は、85%の類似性または3回の反復まで、同じ修正カスケード（グローバル→セクション→コンテンツ→スタイル設定）を適用して、単一ページを検証します。
* ソースページ URLと移行されたパス （例：「/about」）が入力として必要です。
* サイト批判後のシングルページの改善や、ターゲットを絞った検証にページ批判を使用します。
* マルチページプロジェクトの場合は、[代わりにサイトの批判](#site-critique)を使用します。すべてのページを処理し、修正を自動的に重複排除します。
