...
フォーム作成ジョブは、自然言語プロンプトを使用してフォームを開発するように設計された [Brand Experience Agent](/help/ai-in-aem/agents/brand-experience/overview.md) の機能です。 このジョブは、適切なフォーム構造とフィールドタイプを自動的に生成します。 AI アシスタントを使用してジョブを表示します。
...
組織のフォーム作成ジョブを有効にするには、アクティベーションはAdobeから開始する必要があります。 次の方法で通知して、プロセスを開始します。
