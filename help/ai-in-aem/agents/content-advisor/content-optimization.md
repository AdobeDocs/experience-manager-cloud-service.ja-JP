...
description: コンテンツ最適化エージェントを使用して、自然言語の手順を適用してチャネル対応のバリエーションを作成することで、ユーザーがアセットを調整および適応する方法を変換する方法を説明します。
...
[AEMのコンテンツアドバイザーエージェントの一部として ](/help/ai-in-aem/agents/content-advisor/overview.md) コンテンツ最適化エージェントは、自然言語の手順を適用してチャネルに対応するバリエーションを作成することにより、アセットの調整および適応方法を変換します。 新しいレンディションの生成、ビジュアルプロパティの調整、背景の変更、特定のデジタルチャネル向けのアセットの準備など、エージェントはユーザーの意図を解釈し、複雑な編集タスクを自動的に実行します。 [ コンテンツ検出エージェント ](/help/ai-in-aem/agents/content-advisor/discovery.md) とシームレスに連携し、検出したアセットを取得して、コアの [Dynamic Media と OpenAPI 機能 ](/help/assets/dynamic-media-open-apis-overview.md) を使用して最適化されたバリエーションを生成します。これにより、手動での設計の手間をかけることなく、ブランド、チャネル、キャンペーンの要件を満たすことができます。

コンテンツ最適化エージェントの主なメリットには、次のようなものがあります。
...
* **[コンテンツ検出エージェントとのシームレスなコラボレーション](/help/ai-in-aem/agents/content-advisor/discovery.md)**：コンテンツ検出エージェントによって特定されたアセットに基づいて、自然な会話によるエンドツーエンドのアセット取得と最適化が可能になります。
...
* **マルチバリアントアセットの最適化**
  ...
  コンテンツ最適化エージェントは、単一の自然言語プロンプトを使用して、コンテンツ検出エージェントによって返されたアセットから、最適化された複数の画像のバリエーションを生成でき、ユーザーは、チャネルに対応するレンディションを迅速かつ効率的に作成できます。

## 登場人物 {#personas-content-optimization-agent}

コンテンツ最適化エージェントの主要なペルソナであるチャネルマーケターは、適切な高解像度ソースコンテンツを選択し、チャネルとオーディエンスセグメントに合わせて最適化された形式をリクエストできます。

また、地域のマーケターやエージェンシーワーカーは、コンテンツ最適化エージェントを使用して、チャネルに対応した画像バリエーションを迅速に生成し、迅速で一貫性のあるコンテンツ制作をサポートすることもできます。

## アクセス方法 {#access-content-optimization-agent}

AI アシスタントを使用して、AEMのコンテンツ最適化エージェントにアクセスできます。 [`experience.adobe.com`](https://experience.adobe.com) にログオンし、`Ask AI Assistant anything` フィールドを使用して自然言語でプロンプトを指定することで、AI アシスタントとのやり取りを開始できます。

![Access コンテンツ最適化エージェント ](/help/ai-in-aem/agents/content-advisor/assets/access-discovery-agent.png)

## 一般的なユースケースとサンプルプロンプト {#use-cases-prompts}

[ コンテンツ検出エージェントを使用して適切なアセットを検索することで、コンテンツ最適化エージェントを使用します。](/help/ai-in-aem/agents/content-advisor/discovery.md) 関連する画像が表示されると、ユーザーは、検索結果から直接、1 つまたは複数のアセットに対して、最適化された、またはチャネル固有のバリアントを生成できます。 また、ユーザーは、事前に検索を実行しなくても、プロンプトでアセット UUID またはアセットパスを指定して、バリアントを生成できます。 このワークフローにより、高品質の入力と一貫したより優れた最適化結果が保証されます。 [ 詳しくは、使用可能な最適化の完全なリストを参照 ](https://developer.adobe.com/experience-cloud/experience-manager-apis/api/stable/assets/delivery/) てください。
...
* **高解像度レンディションの作成**
  ...
  [ コンテンツ検出エージェント ](/help/ai-in-aem/agents/content-advisor/discovery.md) を使用して適切なアセットを検索し、検索結果が複数ある場合は次のプロンプトを使用します。
