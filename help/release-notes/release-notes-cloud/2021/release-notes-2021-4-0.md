...
* 多くのアセットのメタデータを、CSV ファイルを使用して一括で読み込んだり、CSV ファイルに書き出したりできます。デフォルトの日付形式は `yyyy-MM-dd'T'HH:mm:ss.SSSXXX` になりました。ユーザーは、列ヘッダーを更新して、別の形式を使用できます。 例えば、CSV ファイルの列ヘッダーとして `Date: DateFormat: yyyy-MM-dd'T'HH:mm:ssXXX` ではなく `Date` を追加します。
...
* 新しい環境を追加すると、環境が作成された後は別の地域に移動できないという通知がユーザーに送られます。
