...
[!DNL Cloud Service]の現在リリース（2021.7.0）のリリース日は 2021 年 7 月 29 日です。
次のリリース（2021.8.0）は 2021 年 8 月 26 日です。
...
* レプリケーションエージェントの「配布」タブにあるツリーレプリケーション UIは非推奨（廃止予定）と見なされ、2021年9月30日（PT）以降に削除されました。 代替レプリケーション戦略について[ご確認](/help/operations/tree-replication-workflows.md#tree-activation)ください。
...
* [!DNL Experience Manager Assets] には [!DNL Document Cloud] PDF ビューアが含まれており、PDF ドキュメントをネイティブにプレビューできます。 この機能を使用すると、ファイルの処理や変換をおこなわずに、複数ページの PDF ファイルをプレビューできます。 この機能は、[!DNL Experience Manager] 6.5とのパリティを向上させます。 ビューアで使用できるコントロールには、ズーム、ページへの移動、コントロールのドッキング解除、フルスクリーンでの表示などがあります。 また、ページやブックマークをプレビューして、移動することもできます。 ファイル自体へのコメントはサポートされています。 PDF ファイル内のコンテンツに対するコメントと注釈は、今後のリリースで予定されています。
