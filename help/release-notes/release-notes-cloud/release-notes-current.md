...
AEM Sites の[プログレッシブ web アプリ（PWA）機能](/help/sites-cloud/authoring/sites-console/enable-pwa.md)は、リリース 2025.1.0 以降の新しいプロジェクトでは廃止されます。この機能は、既存のプロジェクトでは引き続きサポートされますが、新しいプロジェクトには使用しないでください。
...
使いやすい WYSIWYG Dynamic Media テンプレートエディターを使用して URL を任意のファーストパーティまたはサードパーティのアプリケーションに埋め込んで、画像とテキストのバナーをその場でパーソナライズし、リアルタイムでバナーコンテンツが更新される非常に魅力的なエクスペリエンスを推進します。
...
「公開を管理」ワークフローを使用すると、環境（通常はオーサーインスタンスからパブリッシュインスタンスおよびプレビューインスタンスまで）をまたいでフォームを公開または非公開にすることができます。これにより、ユーザーは効率的な方法でコンテンツの公開または非公開したり、公開のスケジュールを設定したりできます。

#### コアコンポーネントベースのアダプティブフォーム用ドラフトの自動保存

一部入力したフォームを自動的にドラフトとして保存する、[自動保存機能](/help/forms/save-core-component-based-form-as-draft.md)を利用できるようになりました。後で戻って、同じデバイスまたは他のデバイスで入力を完了できます。 この機能により、ユーザーはフォームへの入力を最初からやり直す必要がなくなるので、フォームの放棄が減り、組織のコンバージョン率が向上します。

#### ルールエディターの機能強化

コアコンポーネントに基づくアダプティブフォームの場合、[呼び出しサービスの出力を使用してドロップダウンオプションに値を入力、または繰り返し可能なパネルや個別のパネルを設定](/help/forms/invoke-service-enhancements-rule-editor.md)できます。さらに、この出力は、他のフィールドを検証するためにも使用できます。

#### パネルレイアウトのナビゲーションボタンを使用したユーザーエクスペリエンスの向上

水平タブ、垂直タブ、アコーディオン、ウィザードなどのナビゲーションボタンをパネルレイアウトに追加できるようになりました。これらのボタンを使用すると、[選択したパネルに焦点を当てて、パネル間の切り替えを簡素化し、ユーザーエクスペリエンスを向上させることができます](/help/forms/rule-editor-core-components-usecases.md#navigating-among-panels-using-button)。
