...
**Adobe Express Embedded EditorでPhotoshop アセットを開く**

Assets ビューとContent Hubから、Adobe Expressの埋め込みエディターで、JPEGおよびPNG形式に加えてAdobe Photoshop（.psd）ファイルを開くことができるようになりました。 この機能強化により、クリエイティブ部門とマーケティング部門は、AEM Assetsから離れることなく、階層化されたデザインファイルを使用できるようになり、コンテンツ更新を効率化し、アプリケーションを切り替える必要性を減らすことができます。 PSDのサポートにより、ソースデザインアセットの柔軟性を維持しながら、コンテンツ制作ワークフローを迅速化できます。 作成されたリミックス作品は、チャンネル対応のアセットとしてAEMに保存できます。

**Adobe IllustratorおよびAdobe InDesign アセットをAEM AssetsからAdobe Expressに読み込む**

Adobe Expressでは、Assets プラグインを使用して、AEM AssetsからAdobe Illustrator（.ai）およびAdobe InDesign（.indd）ファイルを読み込めるようになりました。 Adobe Illustrator ファイルは、現在のドキュメントに読み込むことも、新しいExpress ドキュメントに読み込むこともできます。 Adobe InDesign ファイルは、新しいExpress ドキュメントに読み込むことができます。 この機能により、クリエイティブ部門とマーケティング部門は、承認済みのデザインアセットにより簡単にアクセスして再利用できるようになり、コンテンツ制作を迅速化し、チャネルをまたいで一貫性のあるブランドエクスペリエンスを確保できるようになります。
...
AEM Assetsでは、AEMから取得したアセットを使用してAdobe Expressで作成されたアセットのリネージュ情報が保持されるようになりました。 この機能では、ソースアセットと生成されたコンテンツとの関係を記録し、AEMにアセットのメタデータとして保存することで、承認されたアセットがクリエイティブワークフローをまたいで再利用される方法を追跡することができます。
...
AEM AI アシスタントを通じて、[休眠時間および無料期間](/help/ai-in-aem/agents/brand-experience/development/development.md#control-updates-job)を直接表示、作成、編集できるようになりました。主なメリットは、スケジュール設定のエラーが少ないことです。 リクエストを行う際に、アシスタントは可能な限り案内し、適用される制限、例えば3期間の上限、期間の間の必須1週間の間隔、スケジュールできない予定されているメンテナンス除外ウィンドウなどをフラグ付けします。 そのため、設定に失敗した後に制約を検出する代わりに、ビジネスオーナーとデプロイメントマネージャーは、同じ会話で有効なスケジュールに誘導されます。 これにより、重要なビジネスウィンドウを自動メンテナンス更新から保護し、行き違いや設定ミスを減らすことができます。
...
RDEは、コードとコンテンツの現在の状態のスナップショット ](/help/implementing/developing/introduction/rapid-development-environments.md#snapshots)を取得する機能[をサポートするようになりました。これは、後で復元できます。 これは、元に戻す必要がある場合のあるコードを同期する場合や、異なる機能の開発を切り替える場合に役立つことがあります。 また、テストの既知の開始点として、可変コンテンツのみを復元することもできます。
...
#### ASSETS ビューのUI拡張性 {#ui-extensibility-assets-view-beta}

Assets Viewは、開発者を第一に考えたUI拡張性に対応しています。この機能を利用すれば、顧客はすばやくエクスペリエンスをカスタマイズし、特定のビジネス要件に対応できます。Adobeの開発者向けドキュメントに従うことで、最小限の労力で拡張機能を構築、デプロイし、既存の安定した拡張ポイントを活用できます。 必要な拡張ポイントがまだ利用できないユースケースの場合、Adobeはお客様と直接連携して、要件を検討し、ニーズに合わせた新しい拡張性APIを提供することの技術的な実現可能性を評価します。また、**Beta リリース**&#x200B;などの新しいAPIを提供する場合があります。さらに、Adobeでは、現在、社内の早期導入フェーズで利用できる&#x200B;**生成AIを活用した拡張機能ジェネレーションツール**&#x200B;を開発しました。 このツールは、拡張機能の開発時間を大幅に短縮できます。 このベータプログラムに参加されるお客様には、このツールへのアクセス権が付与され、その進化を形作るためにフィードバックを共有することが推奨されます。参加または詳細を知りたい場合は、`GRP-ASSETSVIEWUIEXTENSIBILITY@adobe.com`さんにメールを送信してください。

#### ブランドに応じたメタデータ（BAM） {#brand-aware-metadata-beta}
...
お客様の組織がExperience Manager Assetsを初めて利用する場合は、**Assets オンボーディングエージェント Beta プログラム**&#x200B;にオプトインすると、オンボーディングに関する次のAEM Brand Experience AI スキルにアクセスできます。

* 対話型ワークフローを使用して、DAMの設定と移行計画を通じて、ビジネスニーズを把握し、AEM Assetsの導入をどのように構造化すべきかを提案します。

* フォルダー階層、タグ分類、メタデータフォームなどのAEM Assets設定アーティファクトを作成し、一括読み込みジョブを実行してオンボーディングを高速化できます。

**参加する理由？**

* AIを利用して、繰り返しの手作業を排除し、すぐに使用できるDAM環境を利用して、より迅速にコンテンツを制作できます。

* フォルダー、タグ、メタデータ、アセットのインポートを自動化して準備することで、運用オーバーヘッドを低減します。

* 企業向けにカスタマイズされたインテリジェントなレコメンデーションを使用して、厳選された設定エクスペリエンスを通じて、一貫性とガバナンスを向上させ、ベストプラクティスを遵守することができます。

参加または詳細を知りたい場合は、`GRP-AEM-ONBOARDING-AGENT@adobe.com`さんにメールを送信してください。
