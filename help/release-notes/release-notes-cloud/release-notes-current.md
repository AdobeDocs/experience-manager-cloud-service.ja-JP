...
「公開を管理」ワークフローを使用して、複数の環境（通常はオーサーインスタンスからパブリッシュインスタンスとプレビューインスタンス）にわたってフォームを公開または非公開にすることができます。 これにより、ユーザーは効率的な方法でコンテンツの公開、非公開、または公開のスケジュールを設定できます。

#### コアコンポーネントベースアダプティブフォームのドラフトの自動保存

ユーザーは、部分的に完了したフォームを自動的にドラフトとして保存する [ 自動保存機能 ](/help/forms/save-core-component-based-form-as-draft.md) を利用できるようになりました。 後で戻って、同じデバイスまたは他のデバイスで入力を完了できます。 この機能により、ユーザーはフォームへの入力を最初からやり直す必要がなくなるので、フォームの放棄が減り、組織のコンバージョン率が向上します。

#### ルールエディターの機能強化

コアコンポーネントに基づくアダプティブ Formsの場合は、[ 呼び出しサービスの出力 ](/help/forms/invoke-service-enhancements-rule-editor.md) を使用してドロップダウンオプションに値を入力し、繰り返し可能または個々のパネルを設定できます。 さらに、この出力は、他のフィールドの検証に使用できます。

#### パネルレイアウトのナビゲーションボタンによるユーザーエクスペリエンスの向上

水平タブ、垂直タブ、アコーディオン、ウィザードなどのナビゲーションボタンをパネルレイアウトに追加できるようになりました。 これらのボタンは [ 選択したパネルにフォーカスしてパネル間の切り替えを簡素化することで、ユーザーエクスペリエンスを向上 ](/help/forms/rule-editor-core-components-usecases.md#navigating-among-panels-using-button) させます。
...
AEM Forms Document Generation API で、[ 生成されたPDFドキュメントを Azure Blob ストレージに直接アップロード ](/help/forms/early-access-ea-features.md#doc-generation-api) できるようになりました。 この機能強化により、ストレージと取得が合理化され、効率が向上し、クラウドワークフローとの統合が促進されます。
