...
Experience Manager Assets の [Dynamic Media テンプレートにシェイプレイヤーを追加](/help/assets/dynamic-media/manage-dynamic-media-templates.md#add-shapes-to-the-canvas)できるようになりました。 画像レイヤーやテキストレイヤーと同様に、シェイプレイヤーは、テンプレート URL 経由でリアルタイム更新用のパラメーターをサポートしています。 また、テンプレート内のシェイプに CTA（コールトゥアクション）リンクを含めることもできます。
...
* **新しい環境** （今後作成され、個別に通知されます）：
   * 7日を超えるレプリケーション、DAM、ページの監査ログは、定期的に削除されます。
   * デフォルトでは、すべてのイベントがログに記録されます。

* **既存の環境** (この予定日より前に作成)：
   * 7年以上経過したレプリケーション、DAM、ページの監査ログは、定期的に削除されます。
   * デフォルトでは、すべてのイベントがログに記録されます。
   * このデフォルトの高いしきい値によって、最近のデータが意図せずに削除されるのを防ぎます。 ただし、リポジトリのパフォーマンスを最適化するには、小さい値を設定することをお勧めします。
