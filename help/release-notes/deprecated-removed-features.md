...
role: Admin
source-git-commit: c68d22d2ea12423d49e20a7e62cb5db5a48e131a
workflow-type: tm+mt
...
<td>org.apache.commons.lang<br>org.apache.commons.lang.enums<br>org.apache.commons.lang.builder<br>org.apache.commons.lang.exception<br>org.apache.commons.lang.math<br>org.apache.commons.lang.mutable<br>org.apache.commons.lang.reflect<br>org.apache.commons.lang.text<br>org.apache.commons.lang.time</td>
  ...
  <tr>
    ...
    <td>OSGi サービス HTTP APIは非推奨です。代わりにOSGi サーブレット ホワイトボードを使用してください。</td>
    <td>5/1/2024</td>
    <td>2027/12/31 （仮称）</td>
  </tr>
  </tbody>
...
コードを更新した後、コードの品質手順の結果を確認して、Cloud Managerに非推奨のAPI使用が残っていないことを確認します。

5月4日までにアップデートが行われない場合、AEM版のアップデートは提供されなくなります。 Adobe リリースアップデートを再度受け取るには、フルスタックパイプラインを正常に実行する必要があります。その後、更新は数日以内に適用されます。

### 一般的なガイドライン

現在非推奨APIを必要とするサードパーティライブラリを使用している場合は、そのサードパーティライブラリの新しいバージョンにアップデートしてみてください。
...
ACS AEM Commonsを使用している場合は、少なくともバージョン 6.11.0 （最新バージョンをお勧めします）を使用し、コンテンツパッケージに分類子`cloud`を指定して、Cloud Service](https://adobe-consulting-services.github.io/acs-aem-commons/pages/maven.html)のバージョンを[必ず含めてください。
...
`org.apache.sling.commons.auth`、`org.apache.sling.commons.auth.spi`、またはその両方を使用している場合、コードをそれぞれ `org.apache.sling.auth` または `org.apache.sling.auth.spi`. 古いバージョンの [ACS AEM Commons](https://adobe-consulting-services.github.io/acs-aem-commons/) を使用している場合は、最新バージョンに更新する必要があります。

アクションリスト：

* ACS AEM Commonsを使用している場合は、最新バージョン（少なくとも6.11.0）に更新し、`cloud`分類子を使用するようにしてください。
* `org.apache.sling.commons.auth`または`org.apache.sling.commons.auth.spi`から`org.apache.sling.auth`への移行を行います。 `org.apache.sling.auth.spi`.
...
* このリストから必要なバンドルを選択し、プロジェクトに追加します。 1つを追加する場合は、そのバンドルのパッケージインポートを確認してください。これらのインポートを完全にするために、これらのリストから他のバンドルを追加する必要がある場合があります。 これらのバンドルのバージョン 9.xを使用する場合は、以下からバージョンを正確にデプロイし、これらのパッケージを&#x200B;*および*&#x200B;書き出しを埋め込まないようにしてください。
...
* ACS AEM Commonsを使用している場合は、最新バージョン（少なくとも6.11.0）に更新し、`cloud`分類子を使用するようにしてください。
* `io.wcm:io.wcm.caconfig.extensions`を使用している場合は、これをバージョン 1.9.2以上に更新してください
...
* ACS AEM Commonsを使用している場合は、最新バージョン（少なくとも6.11.0）に更新し、`cloud`分類子を使用するようにしてください。
* Apache Kafka クライアントを使用しており、Apache ServiceMix （`org.apache.servicemix.bundles.kafka-clients`）からOSGi ラッパーバンドルを含める場合は、[AEM Apache Kafka Client Wrapper](https://repo.maven.apache.org/maven2/com/adobe/aem/osgi/com.adobe.aem.osgi.kafka-clients/4.0.0_1.0/)に置き換えます。 これは、Apache ServiceMixのバージョンと同じで、これら2つのパッケージの使用のみが削除されます。
* `com.adobe.aio.aem:aio-lib-osgi`を使用している場合は、最新バージョン（少なくとも2.0.12）に更新します。
