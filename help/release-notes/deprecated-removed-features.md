...
>* **2026 年 5 月 4 日（PT）**：非推奨（廃止予定）の API をまだ使用している環境 **Adobeの重要なリリースアップデートを受け取ることはありません** また、パフォーマンスと可用性に関するAdobeの標準コミットメントには従いません。 その結果、新機能やバグ修正を受け取ることができず、アプリケーションの安定性と稼動時間に悪影響が及ぶ可能性があり、セキュリティリスクの危険性がさらに高まる可能性があります。
>
>デプロイメントブロックを防ぐには、2026 年 3 月 30 日（PT）より前に、API の使用を削除します。
...
非推奨（廃止予定）の API の独自バージョン（独自バージョンの Guava など）をデプロイする場合は、この API を使用するすべてのバンドルが自分のバージョンに接続されていることを確認します。 現在Cloud Serviceに含まれているメジャーバージョンと同じメジャーバージョンをデプロイする場合は、何もする必要はありません。 ただし、推奨事項に従って最新バージョンをデプロイする場合は、Maven プロジェクトを調整し、そのライブラリを `aem-sdk-api` の前に新しい依存関係として含める必要があります。 これにより、コードが新しいバージョンにワイヤリングされます。 変更を加えたら、[AEM as a Cloud Service SDK Build Analyzer Maven プラグインを使用して ](https://experienceleague.adobe.com/ja/docs/experience-manager-core-components/using/developing/archetype/build-analyzer-maven-plugin) 非推奨の API の使用がフラグ付けされないことを確認します。
...
* ACS AEM Commons を使用している場合は、最新バージョン（少なくとも 6.11.0）にアップデートし、`cloud` の分類子を使用してください。
...
* このリストから必要なバンドルを選択して、プロジェクトに追加します。 追加する場合は、そのバンドルのパッケージの読み込みを確認してください。読み込みを完全に実行するために、これらのリストから他のパッケージを追加する必要が生じる場合があるからです。 これらのバンドルのバージョン 9.x を使用する場合は、以下のバージョンを正確にデプロイし、これらのパッケージを埋め込んで *および* エクスポートしないでください。
...
* ACS AEM Commons を使用している場合は、最新バージョン（少なくとも 6.11.0）にアップデートし、`cloud` の分類子を使用してください。
* `io.wcm:io.wcm.caconfig.extensions` を使用している場合は、をバージョン 1.9.2 以降に更新します
...
* `com.adobe.aio.aem:aio-lib-osgi` を使用している場合は、最新バージョン（少なくとも 2.0.12）にアップデートします。
