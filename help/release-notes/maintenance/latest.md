...
* SITES-24436：コンテンツフラグメント Rest API：重複参照の取得を高速化するために、ローカルキャッシュが導入されました。
...
* SITES-30126：コンテンツフラグメント Rest API:CF 公開パフォーマンスの向上：リソースが最小限のチェックを持つフラグメントかどうかをチェックします。
...
* SITES-31111：コンテンツフラグメント Rest API: バリエーションパッチ API がコンテンツフラグメントの起動内のコンテンツフラグメント参照を受け入れるようにします。
* SITES-31343：コンテンツフラグメント Rest API：バッチリクエストをリストするエンドポイントに、日付別のフィルタリングとページネーションを追加します。
* SITES-31472：ローンチが大量の場合、ローンチを削除するとリポジトリが一時停止する可能性があります。
* SITES-31641：コンテンツフラグメント Rest API: プロパティをモデルフィールドに追加して、拡張機能に関連する動的マップを保存します。
...
* SITES-26064：コンテンツフラグメント Rest API：フラグメントを作成し、バックエンドで `AccessDeniedException` を取得する際に返されるステータスコードを修正しました。
...
* SITES-31454：コンテンツフラグメント Rest API：フラグメント参照フィールドのリラックスパターンを使用して、UUID も受け入れます。
...
* SITES-31487：コンテンツフラグメント Rest API：ルートフォルダーに対して権限エンドポイントを呼び出すことを許可します。
