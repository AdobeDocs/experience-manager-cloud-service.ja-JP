...
aem-sdk-apiの新しいバージョンには、Java 11 ターゲットでコンパイルされたクラスが含まれており、これはCloud Manager Build environmentのデフォルト JDK バージョン 1.8と互換性がありません。 このアップデートでは、MavenがJDK 11を使用して実行される必要があります。
...
>[!NOTE]
>
>リリース 15787は4月11日に非公開になり、リリース 15860に置き換えられました。
...
* SITES-17767 - コンテンツフラグメント – 管理者 – jcr:content ノードを持たないフォルダーでも、許可されたcf-modelsをサポート
...
* SITES-16733 - MSM - ライブコピー – ブループリントインデックスページは、担当者:policyがノードに適用されたときにロールアウトできません
...
* SITES-17934 - コンテンツフラグメント - 大きなフラグメントツリーに対する DoS 保護が原因でプレビューに失敗する。 [&#x200B; ナレッジベース記事](https://experienceleague.adobe.com/ja/docs/experience-cloud-kcs/kbarticles/ka-23945)を参照してください。
...
* ASSETS-26549:「jcr:lastModifiedBy」を持つAssets / ノード：「workflow-process-service」がリストビューで「external user」として表示されます。
* ASSETS-26842：処理プロファイルの「App Builder」を読み取るために、「Firefly」テキストを更新します。
* ASSETS-28708：一部の IMS トークンリクエストの遅い応答。
* ASSETS-28767：フォルダーに多数のアセットが公開されている場合、アセットの公開状態が 一貫しません。
...
* ASSETS-31256:「jcr:lastModifiedBy」を持つAssets / ノード：「workflow-process-service」がリストビューで「external user」として表示されます。
...
* ASSETS-31330: ko_KR: キャプションおよびオーディオトラックのローカライズされていない文字列。
...
* ASSETS-35923：`aem-sdk-api` バージョンを `2024.2.15262.20240224T002940Z-231200` にアップグレードした後の CM パイプラインのビルドステップでの `UnsupportedClassVersionError`。 **CM Java バージョンを11**&#x200B;に設定するには、お客様の操作が必要です。[&#x200B; ビルド環境 / Maven JDK バージョンの設定](/help/implementing/cloud-manager/getting-access-to-aem-in-cloud/build-environment-details.md#alternate-maven-jdk-version)を参照してください。
