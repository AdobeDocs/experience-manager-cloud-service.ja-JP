...
* SITES-20149: RTC: `[cq-wcm-launches-core]` CF のローンチ用の新しい API をエクスポートします。
...
* SITES-20342：フォルダーレベルでの `[Backend]` 公開 – CF のみを公開するフィルターを追加。
...
* SITES-20495:`[BE]` フォルダーレベルで公開する権限を取得する機能。
...
* SITES-20601:CF Search API を `[OpenAPI]` 新して、ダイレクト子コンテンツフラグメントのみの取得を許可します。
* SITES-20653：実験の開始日と終了日を追加します。
* SITES-20656:`[BE]` 文字列を置き換える際に、大文字と小文字を区別するオプションを提供する。
* SITES-20666：オーサリング時に、デフォルトで実験のをオフにする必要があります。
* SITES-20752:CF のApple ローンチを `[cq-wcm-core]` 開します。
* SITES-20763：サイト統合の配信 API エンドポイントを更新します。
* SITES-20946：LIST モデルエンドポイントにプロパティとして etag を追加します。
* SITES-20947：コンテンツフラグメント ID ごとにサブタスクを取得で `[persistence]` ません。
* SITES-21012：モデルメタデータスキーマを製品に結合します。
* SITES-21043:Cloud Serviceのサ `[CF][launches]` ドポートのパフォーマンスが向上しました。
* SITES-21044:Cloud Serviceへの `[CF][launches]` Side-port 非同期編集ペイロード実装。
...
* SITES-21233:15860 へのアップグレード後、GS1 US の `[CoreCmp]` アコーディオンが破損する。
