...
* 一部のユーザーは、でフォームを作成する際に問題が発生する場合があります `Cloud Service version 2024.5.16461`ただし、ユーザーがフォームを作成すると、作成ダイアログボックスに次のエラーメッセージがポップアップ表示されます。

  `A server error occurred. Try again after sometime.`

  この問題は、フォームで使用されているテンプレートを作成者が最初に公開せずにフォームを公開したために発生します。その結果、が追加されます `jcr:uuid` 保護されたプロパティやシステム生成プロパティを以下に追加しました： `<template-path>/initial/jcr:content` ノード。 これにより、後続のフォーム作成でエラーが発生します。

  問題を解決するには、次の手順に従います。

   1. フォームで使用するテンプレートに、が含まれていないことを確認します `jcr:uuid` およびその他のシステム生成パスの保護されたプロパティ `<template-path>/initial/jcr:content` ノード。
