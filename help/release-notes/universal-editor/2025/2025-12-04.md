...
* ユニバーサルエディターは、新しい設定エンドポイントを使用して、エディターのそれらの機能のみを自動的に有効にするようになりました。これは、オーサリングモデルとコンテンツ永続性を組み合わせて使用できます。
   * ユースケースで使用できないオプションを非表示にするためにフィルターが必要なくなりました。
   * フィルターを使用して、ユースケースで使用できるが、ユーザーに公開したくないオプションを制限することもできます。
   * フィルターを使用できるのは、使用可能な機能を無効にする場合のみです。使用できない機能は表示されません。
