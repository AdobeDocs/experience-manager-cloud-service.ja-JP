...
Cloud Managerは、ドメイン名に対してDNS ルックアップを実行し、その[現在のステータス ](#statuses)を表示します。

カスタムドメイン名の 1 回目の検証と展開が正常に完了すると、Cloud Manager は自動的に DNS 検索をトリガーします。 以降の試行の場合は、ステータスの横にある「**再度解決**」アイコンを選択する必要があります。
