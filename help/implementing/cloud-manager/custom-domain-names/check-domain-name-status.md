...
**エラーの原因**
Fastlyは、最初に登録したアカウントにドメインをロックし、他のアカウントはサブドメインの登録に許可をリクエストする必要があります。 さらに、Fastlyでは、apex ドメインと関連するサブドメインを1つのFastly サービスとアカウントにのみ割り当てることができます。 AEM Cloud Service ドメインに使用されているのと同じapexとサブドメインをリンクする既存のFastly アカウントがある場合、次のエラーが表示される可能性があります。
