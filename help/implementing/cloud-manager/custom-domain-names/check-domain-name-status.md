...
Cloud Managerでドメイン名のステータスを確認する前に、[Add a customer-managed SSL certificate](/help/implementing/cloud-manager/managing-ssl-certifications/add-ssl-certificate.md#add-customer-managed-ssl-cert)の説明に従って、カスタムドメイン用のカスタマー管理（OV/EV） SSL証明書が既に追加されていることを確認します。
...
Cloud Managerでは、お客様管理（OV/EV） SSL証明書を使用してドメインの所有権を確認します。 完了すると、次のいずれかのステータスメッセージが表示されます。

| ステータス | 説明 |
| --- | --- |
| ドメイン検証失敗 | 顧客管理のEV/OV証明書がないか、エラーが発生して検出されました。<br> ステータスメッセージに記載されている手順に従って、問題を解決します。 準備が整ったら、ステータスの横にある「**再検証**」アイコンを選択する必要があります。 |
...
Cloud Managerでドメインマッピングを追加しようとすると、次のエラーメッセージが表示される可能性があります。

*ドメインは既に Fastly アカウントにインストールされています。 Cloud Serviceに追加する前に削除してください。*
...
**エラーの原因**
Fastlyは、ドメインを最初に登録するアカウントにドメインを制限し、他のアカウントはサブドメインの登録に許可をリクエストする必要があります。 さらに、Fastlyでは、apex ドメインと関連するサブドメインを1つのFastly サービスとアカウントにのみ割り当てることができます。 AEM as a Cloud Service ドメインに使用されている同じapexとサブドメインをリンクする既存のFastly アカウントがある場合、次のエラーが表示される可能性があります。
...
>[!TIP]
>
>Fastlyでドメインのデリゲーションの問題を解決するには、平均で1～2営業日かかります。 このため、Adobeでは、公開日よりも前にドメインをインストールすることをお勧めします。
...
UI を使用して既存の環境設定をすべて移行すると、このメッセージは表示されなくなります。 メッセージが消えるまでに1～2営業日かかります。
