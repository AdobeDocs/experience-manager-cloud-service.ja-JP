...
1. 次の日： **[マイプログラム](/help/implementing/cloud-manager/getting-access-to-aem-in-cloud/editing-programs.md#my-programs)** 画面で、コードをデプロイするプログラムをタップまたはクリックします。
...
UI で使用できる以外に、 [Cloud Manager API](https://developer.adobe.com/experience-cloud/cloud-manager/reference/api/#tag/Pipeline-Execution) ：再実行をトリガーし、再実行としてトリガーされた実行を識別します。
