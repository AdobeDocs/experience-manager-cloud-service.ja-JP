...
実稼動プログラム内の開発環境、ステージング環境、実稼動環境の更新は、クラウドネイティブなサービスなので、Adobeによって自動的に管理されます。
...
>[!NOTE]
> 2024 年の時点では、開発インスタンスと一部のサンドボックスプログラムは既に自動的に更新されているので、更新を手動で管理する必要はありません。 この移行の結果、で開発インスタンスの「環境を手動で更新」オプションを使用できない場合があります _一部_ /件のプログラム。

この **更新** オプションは、 **環境** 環境の省略記号ボタンをクリックして、一部の開発環境およびサンドボックスプログラム内の環境のカードを選択する。
...
* 適切なパイプラインが存在しない場合、 **更新** オプションを選択すると、ユーザーに作成を促すメッセージが表示されます。
