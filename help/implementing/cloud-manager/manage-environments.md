...
1. 選択した地域がドロップダウンの下に追加され、選択内容が示されます。
   * 選択した地域の隣にある `X` をタップまたはクリックすると、選択を解除できます。
1. 「**追加の公開地域**」ドロップダウンで別の地域を選択して、別の地域を追加します。
1. 環境を作成する準備が整ったら、「**保存**」をタップまたはクリックします。
...
1. お待ちください。 Cloud Manager は引き続き地域の復元を試みます。この地域はいつでも使用できるようになります。
1. 数時間後に問題が解決しない場合は、追加のパブリッシュ領域を削除し、（同じ地域または別の地域の）再度追加して、完全なデプロイメントをトリガーできます。

追加のアクションを実行する前にシステムが自身で回復するまでに待つ時間は、その領域の障害がシステムに与える影響によって異なります。

いずれの場合も注意してください。 [トラフィックは、常に、オンラインに最も近い他の地域にルーティングされます。](/help/operations/additional-publish-regions.md) 引き続き問題が発生する場合は、Adobeカスタマーケアにお問い合わせください。
...
1. IP 許可リストの表の上部にある入力フィールドを使用して、IP 許可リストと、その適用先となるオーサリングサービスまたはパブリッシュサービスを選択します。
