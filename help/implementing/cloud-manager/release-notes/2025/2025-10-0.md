...
* **専用のステージ専用および実稼動専用のデプロイメントパイプライン**

  Cloud Managerでは、専用のステージ専用および実稼動専用のデプロイメントパイプラインが提供され、ステージング環境と実稼動環境へのデプロイメントを個別に管理する柔軟性が向上しました。 [&#x200B; ステージのみのパイプラインと実稼動専用パイプラインの分割](/help/implementing/cloud-manager/configuring-pipelines/stage-prod-only.md)を参照してください。
