...
role: Admin
exl-id: 4ed6a439-d396-426a-b769-aa88d4fd18d0
...
* **Experience Hubへの簡単なアクセス**

  ユーザーの役割の選択が削除され、**プリセット**&#x200B;の選択用にガイドが追加されました（コンテンツ作成者、アセットライブラリ、管理者およびIT）。

* **お知らせ**&#x200B;および&#x200B;**製品の更新**

  利用可能な通知を切り替えて繰り返すことができますが、通知を却下することもできます。

* **最近**

  ページエディター、アセット、プログラム、パイプライン実行の詳細、セキュリティページなど、追加のページとリソースのサポートが追加されました。

* **プログラム リスト**

  Cloud Managerの詳細ページにすばやくアクセスできる、組織内のAEM Cloud Manager プログラムを表示します。

* **AEM Guides**

  AEM Guides アドオンが有効になっているオーサリング環境のクイックアクションとショートカット。

## 新機能 – Cloud Manager {#cloud-manager-whats-new}

* **パイプラインのデプロイ用のワンクリック ロールバック**

  最新の顧客ソースコードが期待どおりに機能しない場合は、以前のデプロイメントにすばやく戻します。 完全なパイプラインを再実行したり、手動でコミットを元に戻したりする必要はありません。<!--https://jira.corp.adobe.com/browse/CMGR-69556 -->
...
* **Edge Delivery ServicesのWAFのセルフサービス設定**

  Cloud ManagerでEdge Delivery Services プログラムを作成する場合は、Web Application Firewall （WAF）を有効にできます。 この設定により、悪意のあるトラフィックやDDoS攻撃からサイトをすぐに保護し、手動での設定作業を減らすことができます。
...
* よりシンプルで直感的なワークフローを使用して、実稼動以外のパイプラインで専用テスト環境を設定できるようになりました。 合理化されたセットアップにより、完了までの時間が短縮され、設定エラーが減少します。
...
2025年11月のCloud Manager リリースには、重大なバグ修正はありません。
