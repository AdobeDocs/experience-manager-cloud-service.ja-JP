...
* **（UI）ドメイン検証の機能強化**
  ...
  ドメインを追加すると、Fastly アカウントに既にインストールされている場合、Cloud Managerにエラーが表示されます。「*Domain already installed in Fastly. Cloud Serviceに追加する前に削除してください。*&quot;

## Private Beta プログラム {#private-beta-program}

一般リリースの前に、今後の機能に限定でアクセスするには、Cloud Managerのプライベートベータプログラムに参加してください。

次のプライベートベータ版の商談は現在利用可能です。
...
>[!NOTE]
>
>現在、デフォルトのプルリクエストコード品質チェックはGitHubでホストされているリポジトリに限定されていますが、この機能を他のGit ベンダーに拡張するためのアップデートが進行中です。
...
* **高度なネットワークポート転送名の一意性の適用**
  ...
  Cloud Manager では、高度なネットワークポート転送に一意の名前が適用されるようになりました。 以前は、名前の重複は許可されていましたが、対立につながっていました。 この修正により、ネットワーク設定の整合性に関するベストプラクティスに合わせて、各ポート転送エントリに個別の名前が付けられます。<!-- CMGR-67082 -->
