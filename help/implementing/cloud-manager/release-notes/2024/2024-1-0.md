...
description: AEM as a Cloud ServiceのCloud Manager 2024.1.0のリリースノート。
...
* Cloud Managerは、メインの[証明書](/help/implementing/cloud-manager/managing-ssl-certifications/introduction-to-ssl-certificates.md)だけでなく、中間証明書についても有効期限を検証するようになりました。
...
* 設定ファイルの場所が正しく設定されていない場合に、ビルドステップで設定パイプラインが失敗するエラーが修正されました。 新しいエラーメッセージは、ユーザーが設定ファイルの場所が正しいことを確認する必要があることを示します。
