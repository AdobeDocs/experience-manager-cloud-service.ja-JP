...
* **キー**: java:S1874
* **タイプ**:`Vulnerability` または `Bug`/Cloud Serviceの互換性
* **重大度**：情報、マイナー、メジャー
* **最初の対象バージョン**：バージョン 2026.1.0

Experience Manager API サーフェスは、使用を停止する必要がある API を特定するために、継続的に改訂されています。 これらの API は非推奨（廃止予定）で、削除日が示されています。

削除日が近いほど、このルールに違反する重大度が高くなります。 このような API を使用する場合は、安全な代替手段を使用する必要があります。

非推奨（廃止予定）の API の使用を削除する方法のガイダンスについては、[ この記事 ](/help/release-notes/deprecated-removed-features.md#aem-apis) を参照してください。
...
`Apache Sling` プロジェクトでは、（フィールドレベルまたはクラスレベルで） `@Inject` と組み合わせるとパフォーマンスが低下する可能性があるので、Sling モデルのコンテキストでは `DefaultInjectionStrategy.OPTIONAL` 注釈の使用はお勧めしません。 代わりに、より具体的なインジェクション（`@ValueMapValue` または `@OsgiInjector` 注釈など）を使用する必要があります。

推奨される注釈と [`Apache Sling` この推奨が最初に行われた理由について詳しくは、](https://sling.apache.org/documentation/bundles/models.html#discouraged-annotations-1) のドキュメントを参照してください。
...
Cloud Service に移行する前に、このプロパティを使用しているインデックス作成を更新する必要があります。詳しくは、[コンテンツの検索とインデックス作成](/help/operations/indexing.md#how-to-use)のドキュメントを参照してください。
