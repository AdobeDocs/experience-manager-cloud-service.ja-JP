...
Cloud Managerのエクスペリエンス監査は、サイトでのユーザーエクスペリエンスが最高水準であることを保証します。

監査結果は参考情報であり、デプロイメントマネージャーはスコアと、現在および以前のスコアとの間の変化を確認できます。 このinsightは、現在のデプロイメントによって発生したリグレッションがあるかどうかを判断するのに役立ちます。

Googleのオープンソースツールである[Google Lighthouse](https://developer.chrome.com/docs/lighthouse/overview/)は、エクスペリエンス監査を強化し、Cloud Managerはすべての実稼動パイプラインで有効にします。
...
実稼動パイプラインでは、エクスペリエンス監査をデフォルトで使用できます。 フルスタックパイプラインとフロントエンドパイプラインに対してオプションで有効にすることができます。 どのような場合でも、パイプライン実行中に評価されるコンテンツパスを定義する必要があります。
...
1. 実稼動および実稼動以外の両方のパイプラインでは、「**エクスペリエンス監査**」タブでエクスペリエンス監査に含めるパスを定義します。
   ...
   * 例えば、サイトが`wknd.site`で、エクスペリエンス監査に`https://wknd.site/us/en/about-us.html` （URLの例のみ）を含める場合は、パス `/us/en/about-us.html`を入力します。
...
1. 上記の 2 つの手順を繰り返して、必要なパスを追加します。
   ...
   * 最大 25 個のパスを追加できます。
   * パスを定義しない場合、Experience Auditにはデフォルトでサイトのホームページが含まれます。
...
**最も遅い5 ページ** ダイアログボックスを開くには、**最も遅いページを表示**&#x200B;をクリックします。 [監査対象として設定](#configuration)した、最もパフォーマンスの低い 5 つのページが表示されます。
...
グラフの上部にあるドロップダウン **Select**&#x200B;を使用してページ固有の詳細を選択し、下部にある&#x200B;**表示**&#x200B;と&#x200B;**トリガー**&#x200B;のドロップダウンを使用して、それぞれ異なる時間枠とトリガータイプを選択します。
...
ある時点でチャートをクリックすると、そのスキャンの詳細を示すポップアップが開きます。 **オープン エクスペリエンス監査スキャン**&#x200B;をクリックして、それらのスキャン結果を&#x200B;**[エクスペリエンス監査スキャン結果](#scan-results)** セクションに読み込みます。

![別のスキャンを選択](/help/implementing/cloud-manager/reports/assets/experience-audit-open-scan.png)

#### エクスペリエンス監査スキャン結果 {#scan-results}

「**エクスペリエンス監査スキャン結果**」セクションには、スキャンされたすべてのページのスコアの詳細が表示されます。 「**前**」と「**次**」ボタンを使用して、結果を移動し、表示する表示数を選択します。
...
ブラウザーで開いた新しいタブから`https://googlechrome.github.io/lighthouse/viewer/`に移動します。 選択したページの Lighthouse の生の JSON レポートを含んだ署名付き URL が自動的に読み込まれ、詳細な検査が可能になります。

![生レポートの表示](/help/implementing/cloud-manager/reports/assets/experience-audit-view-raw-report.png)

## オンデマンドスキャン監査レポート {#on-demand}

エクスペリエンス監査レポートは、パイプラインの実行中に実行するだけでなく、オンデマンドで生成することもできます。 このオプションは、パイプラインを実行することなく、ページをすばやくスキャンするための効果的なソリューションです。

オンデマンドスキャンを実行するには、「**レポート**」タブに移動して監査レポートの全容を確認し、「**スキャンを実行**」ボタンをクリックします。

![オンデマンドスキャン](/help/implementing/cloud-manager/reports/assets/experience-audit-on-demand.png)

**スキャンを実行** ボタンは使用できなくなり、オンデマンドスキャンが既に実行されている場合は、時計アイコンが表示されます。
...
パイプラインには、アクセスできない相対URL パスを表示する拡張可能なエラーセクションが表示されます。
