...
Health Assessmentは、AEM as a Cloud Service内のCloud Managerの実稼動環境とステージ環境を自動的に検出する非侵入型スキャンです。 コンテンツ、コード、設定を評価し、パターンに対する対策やベストプラクティスとの整合性を特定します。これにより、セキュリティとパフォーマンスを向上できます。
...
1. Cloud Managerで、ページの右上隅付近にある、使用する組織を選択します。 以下の画像は参照用です。組織を選択してください。
...
1. 「**レポートを実行**」をクリックします。
   ...
   選択した環境でレポートを実行する間、**実行レポート**&#x200B;は終了するまで無効のままです。
