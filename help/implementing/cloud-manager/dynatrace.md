...
Dynatrace を使用すると、すべての AEM アプリケーションのシームレスな可観測性を実現できます。 Dynatraceは、AEM アプリケーションを検出し、web サイトからコンテナ、Cloud Serviceまでのパスを表示して、ユーザーエクスペリエンスを明らかにします。 また、あらゆる顧客層をまたいだエンドツーエンドのトレース機能とリアルユーザーモニタリング機能を組み合わせることで、AEMのコンテンツ主導型のエクスペリエンスを格差や盲点なく向上させることができます。 異常が発生した場合は、Davis AI エンジンを利用して、Dynatraceがリアルタイムで診断します。 顧客に影響を与える前に、根本原因を破損したコードに特定することで、平均修復時間を最小限に抑えます。
