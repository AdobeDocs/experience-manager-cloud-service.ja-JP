...
Dynatrace を使用すると、すべての AEM アプリケーションのシームレスな可観測性を実現できます。 Dynatraceは、AEM アプリケーションを検出し、web サイトからコンテナ、クラウドサービスまでのパスを表示して、ユーザーエクスペリエンスを明らかにします。 すべての層にわたるエンドツーエンドのトレースと実際の使用のモニタリングを組み合わせることで、ギャップや盲点のない AEM コンテンツ主導のエクスペリエンスを次のレベルに引き上げます。 異常が発生した場合は、Davis AI エンジンを利用して、Dynatraceがリアルタイムで診断します。 顧客に影響を与える前に、根本原因を破損したコードに特定することで、平均修復時間を最小限に抑えます。
