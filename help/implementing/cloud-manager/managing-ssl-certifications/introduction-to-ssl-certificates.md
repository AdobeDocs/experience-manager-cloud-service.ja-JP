...
Adobeで管理されているDV証明書は、Let&#39;s Encryptに依存しています。 インストール済み証明書のCloud Manager制限に加えて、Let&#39;s Encryptは独自のレート制限を適用します。 1つのキー制限は、正確な識別子セットごとに&#x200B;**新しい証明書**&#x200B;です。7日間の期間内に、同じホスト名セットに対して最大5つの証明書を発行できます。 この制限に達すると、Cloud Managerにエラーが表示され、レート制限ウィンドウがリセットされるまで、そのホスト名セットに対して証明書を作成できません。 最新の値とその他の関連する制限については、[&#x200B; レート制限の暗号化に関するドキュメント &#x200B;](https://letsencrypt.org/docs/rate-limits/#new-certificates-per-exact-set-of-identifiers)を参照してください。
