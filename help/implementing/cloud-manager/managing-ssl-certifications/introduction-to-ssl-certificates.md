...
>title="SSL 証明書の管理"
>abstract="Cloud Managerには、SSL証明書をインストールおよび管理するためのセルフサービスツールが用意されています。 Platform TLS サービスを使用して、顧客が所有する証明書とサードパーティの証明機関からの秘密鍵を管理します。"
...
* サポートされている暗号化キーのタイプは次のとおりです。
   ...
   * RSA 2048 ビット、標準サポート。
2048 ビットを超えるRSA キー（3072 ビットや4096 ビットのRSA キーなど）は、現時点ではサポートされていません。
