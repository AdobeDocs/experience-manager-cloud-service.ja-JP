...
Cloudを使用して、独自のSSL証明書またはAdobe管理のDV （Domain Validation）証明書を追加する方法について説明します
...
* **顧客が管理する（OV/EV）SSL 証明書の場合：**
   ...
   * 証明書を取得したら、[Cloud Managerでカスタマー管理（OV/EV） SSL証明書](#add-customer-managed-ssl-cert)の詳細を追加します。
   * 追加すると、カスタムドメイン名は検証済みとマークされ、SSL 証明書が適用されます。

どちらの場合でも、証明書の検証とインストールが完了すると、カスタムドメインを環境で安全に使用できるようになります。 Cloud Manager インターフェイスで[&#x200B; ドメインのステータス &#x200B;](/help/implementing/cloud-manager/custom-domain-names/check-domain-name-status.md)を定期的に確認してください。 これにより、設定が期待どおりに機能していることが確認されます。
...
これで、プロジェクト用に有効なAdobe マネージド DV SSL証明書が追加されました。 多くの場合、この手順は、カスタムドメイン名を設定する最初の手順です。

これで、[CDN 設定](/help/implementing/cloud-manager/domain-mappings/add-domain-mapping.md)を追加する準備が整いました。

## 顧客管理（OV/EV） SSL証明書の追加 {#add-customer-managed-ssl-cert}
...
1. 「**証明書**」、「**秘密鍵**」、「**証明書チェーン**」フィールドで、OV または EV SSL 証明書から必要な値をコピーして、ダイアログボックスの各フィールドに貼り付けます。
   ...
   値で検出されたエラーが表示されます。 証明書を保存する前に、すべてのエラーに対処する必要があります。 一般的なエラーのトラブルシューティングについて詳しくは、[証明書エラー](#certificate-errors)を参照してください。
...
これで、プロジェクト用の有効なSSL証明書が追加されました。 多くの場合、このステップでカスタムドメイン名を設定します。
