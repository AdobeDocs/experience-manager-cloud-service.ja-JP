...
この設定は有効です。ただし、一方の証明書を更新しても、両方の証明書が同じ SAN エントリーをカバーします。 その結果、CDN は、該当するすべてのドメインに最新の証明書をインストールしますが、これは予期しないもののように見える場合があります。

このシナリオは予期しないものかもしれませんが、エラーではなく、基になる CDN の標準的な動作です。 同じ SAN ドメインエントリに対応する 2 つ以上の SAN 証明書がある場合、CDN はそのドメインの最新の証明書をインストールします。 この状況は、別の証明書が既に同じドメインエントリをカバーしている場合でも発生します。
