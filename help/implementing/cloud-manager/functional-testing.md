...
![AEM Cloud Service デプロイメントの品質ゲート](assets/functional-testing/quality-gates-compact.png)
...
これらの品質ゲートは、テスト戦略全体の包括的なテストフレームワークではないことに注意する必要があります。 AEM製品は、AEM Cloud Service デプロイメントプロセスに入る前に、広範なテストを受けます。 同様に、デプロイメントフェーズに達する前に、アプリケーションの品質はすでに高くなっています。 このアプローチにより、品質ゲートは、完全なテスト計画の代替として機能するのではなく、展開プロセスを保護するという主な目的に集中できます。
...
![AEM Cloud Service デプロイメントの品質ゲート](assets/functional-testing/quality-gates-overview.png)
...
すべてのテスト戦略の基盤となる、AEM アプリケーションの単体テストを実施することをお勧めします。 迅速かつ頻繁に実行するように設計されており、早期にフィードバックを提供します。 開発者ワークフロー、独自のCI/CD、AEM Cloud Service デプロイメントパイプラインに緊密に統合されます。
...
この品質ゲートは事前に設定されており、AEM アプリケーションコードに対して静的コード分析を実行します。
...
製品機能テストは、オーサリングタスクやレプリケーションタスクなど、AEM のコア機能に対する安定した HTTP 統合テスト（IT）です。 Adobeでは、デフォルトでこれらを提供および管理します。 これらは、AEM製品のコア機能が破損した場合に、カスタムアプリケーションコードの変更がデプロイされるのを防ぐことを目的としています。

実装には JUnit を使用し、Maven で実行され、公式の [AEM テストクライアント](https://github.com/adobe/aem-testing-clients)に依存します。 製品テストスイートは、次のように維持されます
[&#x200B; オープンソース プロジェクト &#x200B;](https://github.com/adobe/aem-test-samples/tree/aem-cloud/smoke)は、ベストプラクティスに従い、テストの実装の良い出発点と見なすことができます。
...
>[!NOTE]
>
>カスタム機能テストは、AEM アプリケーションの変更のデプロイとAEM製品のアップデートに使用される実稼動用パイプラインと非実稼動用（オプトイン）パイプラインの両方で実行されます。 これらは、アプリケーションが適切に機能することを保証し、リリースの安全性を高める上で重要な役割を果たします。 顧客機能テストはそれぞれの内部プレリリース検証パイプラインで実行され、早期にフィードバックを得ることができます。
...
お客様固有の開発のリスクコントロールを最大化するために、Adobeでは、AEM as a Cloud Serviceにクリティカル UI テストを含めることを推奨しています。 テストは限定し、顧客体験への影響に焦点を当てます。

テストは、Cypress、Playwright、Selenium、Java、JavaScriptをサポートする、最大限の柔軟性を備えたDocker イメージにパッケージ化されています。 これらは、カスタム機能テストと同じ特徴と目的に従います。

>[!NOTE]
>
>カスタム UI テストは、AEM アプリケーションの変更のデプロイとAEM製品の更新に使用される実稼動用パイプラインと非実稼動用（オプトイン）パイプラインの両方で実行されます。 これらのテストは、アプリケーションが適切に機能することを保証し、リリースの安全性を高めるために不可欠です。 また、顧客 UI テストは、各顧客の内部プレリリース検証パイプラインで実行され、早期にフィードバックを得るのに役立ちます。
>
>非Selenium コンテナは、[UI テスト セクション &#x200B;](/help/implementing/cloud-manager/ui-testing.md#custom-ui-testing)の環境変数に基づいて、HTTP プロキシを使用してテストを実行します。
...
エクスペリエンス監査の品質ゲートは、お客様のweb ページに対して[Google Lighthouse](https://developer.chrome.com/docs/lighthouse/overview/)監査を実行します。

AEMでは、この品質ゲートがデフォルトで提供されますが、デプロイメントパイプラインはブロックされません。 デフォルトでは、パブリッシュインスタンスのルートページ（`/`）に対して監査が実行されます。 監査対象と見なされる最大 25 個のカスタムパスを設定して貢献できます。
...
ここでは、好みのツールとフレームワークを選択できます。 お客様による機能テストやカスタム UI テストとは異なり、AEM as a Cloud Serviceに関する制限はありません。 そのため、アドビでは、長期間実行する機能テストと UI テストをここで実行することをお勧めします。
...
手動テストの場合は、追加のAEM Cloud Service開発環境を使用すると便利です。
