...
1. 変更する変数の行の最後の列にある省略記号ボタンを使用して、「 」を選択します。 **編集** または **削除**.
...
>[!TIP]
>
>複数の変数を更新する場合は、 **環境設定** タップまたはクリックする前に必要なすべての変数を一度に更新するダイアログ **保存**. これにより、1 回の更新で環境に追加できます。
