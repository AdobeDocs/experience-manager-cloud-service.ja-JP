...
* **手動** - サンドボックスプログラム環境を手動で休止状態にすることができます。 前述のとおり休止状態は自動的に発生するので、そのための要件はありません。

サンドボックスプログラム環境は、数分以内に休止状態モードになります。 休止中、データは保持されます。
...
休止状態が正常に終了すると、**Developer Console**&#x200B;画面に環境の休止状態プロセスの完了通知が表示されます。
...
AEM as a Cloud Serviceへのアクセス権を持つ製品プロファイルを持つユーザーは、誰でも&#x200B;**Developer Console**&#x200B;にアクセスできます。 これにより、環境の休止状態を解除できます。
...
* ユーザーはパイプラインを使用して、休止状態の環境にカスタムコードをデプロイします。 環境は休止状態のままとなり、休止解除すると新しいコードが環境に表示されます。
