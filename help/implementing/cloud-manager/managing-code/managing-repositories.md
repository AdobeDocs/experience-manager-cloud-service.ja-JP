...
選択したリポジトリを管理するには、リポジトリをクリックし、![詳細アイコン ](https://spectrum.adobe.com/static/icons/workflow_18/Smock_More_18_N.svg)を使用してドロップダウンメニューを開きます。 アドビが管理するリポジトリの場合は、**[分岐を確認／プロジェクトを作成](#check-branches)**&#x200B;できます。
...
Adobe以外でコードをホストする場合、使用する手順のページと所有権の検証方法は、リポジトリがホストされている場所によって異なります。 次の表を使用して、正しいオプションを選択します。
