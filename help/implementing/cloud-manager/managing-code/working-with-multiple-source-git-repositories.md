...
分岐戦略とワークフローに応じて、異なるブランチに対して同期を設定できます。 使用済みのGit リポジトリーがGitHub アクションと同様のコンセプトを提供しない場合は、Jenkins （または類似の）を介した統合も可能です。 この場合、Webhook が Jenkins ジョブをトリガーし、作業が実行されます。
