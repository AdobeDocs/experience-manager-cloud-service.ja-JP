...
1. 「**閉じる**」をクリックします。
1. Git ベンダーソリューション（GitHub Enterprise、GitLab、Bitbucket、またはAzure DevOps）に移動します。
