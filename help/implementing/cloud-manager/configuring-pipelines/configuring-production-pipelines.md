...
1. **実稼動パイプラインを追加**&#x200B;ダイアログボックスが表示されます。 パイプラインを特定するには、**パイプライン名**&#x200B;と次のオプションを指定します。 「**続行**」をクリックします。
   ...
   * **毎回確認する** - デフォルトの設定。 重要な失敗には手動での介入が必要です。
   * **直ちに失敗** - 重要なエラーが検出されると、パイプラインはキャンセルされます。 このプロセスでは、ユーザーが各エラーを手動で拒否する場合にエミュレーションを行います。
...
## 実稼動パイプラインでのスマートビルドの使用について{#about-smart-build-production-pipeline}
