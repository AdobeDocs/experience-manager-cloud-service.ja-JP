...
また、この機能のテストを開始する前に、次の既知の問題にも注意してください。

* 実稼動専用パイプラインを使用すると、最新のAEM更新のメリットを受けられない場合があります
   * 場合によっては、AEM アップデートプロセスで、フルスタックパイプラインを介して最後にデプロイされたコードにコードを戻す場合があります。
* 実稼動専用またはステージング専用のパイプラインを使用する場合、[ 環境の復元 ](/help/operations/restore.md#offsite-backup) をリクエストすることはできません。
