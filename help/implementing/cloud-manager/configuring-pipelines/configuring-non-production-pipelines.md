...
**プログラムの概要** ページの&#x200B;**パイプライン** カードのlines.md）。
...
1. 「**Source コード**」セクションで、次のオプションを定義します。
   ...
   * **Git ブランチ** - このオプションは、選択したパイプラインのどのブランチからコードを取得するかを定義します。 ブランチ名の最初の数文字と、このフィールドのオートコンプリート機能を入力します。 これにより、選択可能な一致するブランチが検索されます。
