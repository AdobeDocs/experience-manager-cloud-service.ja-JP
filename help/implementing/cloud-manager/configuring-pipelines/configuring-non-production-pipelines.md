...
| パフォーマンスが向上しない | ・ 複数の実行が発生したことを確認します（キャッシュウォームアップ）。<br> ・ほとんどのモジュールが頻繁に変更されているかどうかを確認します。 |
| 予期しないアーティファクトまたは欠落している変更 | ・ 変更がMaven依存関係トラッキング外かどうかを確認します。<br>・検証には&#x200B;**完全ビルド**&#x200B;を使用します。 |

スマートビルドを有効にするには、[実稼動以外のパイプラインを追加](#adding-non-production-pipeline)を参照してください。
