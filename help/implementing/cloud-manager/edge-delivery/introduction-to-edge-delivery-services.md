...
AEM as a Cloud ServiceのCloud Managerを使用すると、プロジェクトにEdge Delivery Servicesを有効にできます。
...
Edge Delivery Servicesのライセンスを取得している場合は、Cloud Managerで直接サイトにオンボーディングし、ガイド付きのセルフサービス体験を利用して[公開できます](/help/implementing/cloud-manager/getting-access-to-aem-in-cloud/creating-production-programs.md)。
...
Cloud Manager を通じて Edge Delivery Services ライセンスにアクセスして使用することで、アドビのメリットを最大限に活用できます。 これにより、いくつかの重要なメリットが得られます。
...
* [API-first](https://developer.adobe.com/experience-cloud/experience-manager-apis/)機能を使用して、CRUD （作成、読み取り、更新、削除）操作を実行します。
...
または、Edge Delivery Servicesのライセンスに関係なく、独自のCDN （つまり、Adobeが管理していないCDN）を使用する場合は、`aem.live` プラットフォームで設定する必要があります。 [BYO CDN 設定](https://www.aem.live/docs/byo-cdn-setup)を参照してください。


## 実稼動プログラムまたはサンドボックスプログラムへの Edge Delivery Services の追加について {#about-adding-eds-to-prod-sandbox}

Edge Delivery Servicesは、プロジェクトの開始方法やサイトを作成するタイミングに応じて、いくつかの方法で追加できます。
...
Cloud Managerの&#x200B;**Edge Delivery To Do リスト**&#x200B;は、オンボーディング タスク チェックリストです。 Edge Delivery サイトのオンボーディングと管理をガイドし、[運用開始プロセス ](/help/journey-onboarding/go-live-checklist.md)を完了するまで案内します。
