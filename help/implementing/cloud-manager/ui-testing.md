...
>
>ファイルには、「*変更しないでください*」という行が含まれている場合があります。 これは、古いテンプレートやサンプルからの従来の警告に過ぎず、Cloud Manager UI テストに必要なオプトイン編集を&#x200B;*not*&#x200B;でブロックすることはできません。 このアドバイスは安全に無視できます。オプトイン手順（例：`assembly-ui-test-docker-context.xml`を含める）に従うと、プロジェクト `pom.xml`の&#x200B;*で*&#x200B;と`testing.properties`を編集できます。
...
UI テストの実行中にテスト結果アーカイブが作成された場合は、`Download Details` カスタム UI テスト [**ステップ**&#x200B;の下にある「](/help/implementing/cloud-manager/deploy-code.md)」ボタンをクリックして、Cloud Managerからダウンロードできます。
