...
>title="UI テスト"
>abstract="カスタム UI テストは、アプリケーションの UI テストを作成して自動的に実行できるオプション機能です。 UI テストは、Docker イメージに Selenium ベースでパッケージ化され、 Java や Maven、Node や WebDriver.io、Selenium に基づいて構築されたその他のフレームワークやテクノロジーなど、幅広い言語とフレームワークに対応できます。"
...
UI テストは、言語とフレームワーク（Cypress、Selenium、Java と Maven、JavaScript など）を幅広く選択できるように、Docker イメージにパッケージ化されています。 また、UI テストプロジェクトは、[AEM プロジェクトアーキタイプ &#x200B;](https://experienceleague.adobe.com/ja/docs/experience-manager-core-components/using/developing/archetype/overview)を使用して簡単に生成できます。
...
UI テストは、[**カスタム UI テスト**](/help/implementing/cloud-manager/deploy-code.md) ステップで品質ゲートとして実行されます。[実稼動パイプライン &#x200B;](/help/implementing/cloud-manager/configuring-pipelines/configuring-production-pipelines.md)で必要です。[実稼動以外のパイプライン &#x200B;](/help/implementing/cloud-manager/configuring-pipelines/configuring-non-production-pipelines.md)ではオプションです。 リグレッションや新機能を含むUI テストでは、エラーを検出して報告できます。
...
最も簡単な方法は、[Maven Assembly Plugin](https://maven.apache.org/plugins/maven-assembly-plugin/)を設定して、Docker ビルドコンテキストアーカイブを作成し、適切な分類子を割り当てることです。
...
この実行では、Maven Assembly プラグインに対して、プラグインの専門用語の&#x200B;**アセンブリ記述子**&#x200B;と呼ばれる`assembly-ui-test-docker-context.xml`に含まれる指示に基づいてアーカイブを作成するように指示します。 アセンブリ記述子では、アーカイブに含める必要のあるすべてのファイルがリストアップされます。
...
また、アセンブリ記述子は、UI テストのローカル実行中に生成される可能性のある一部のファイルを除外します。 このプロセスにより、アーカイブを小さくし、ビルドを高速化できます。
...
>
>ファイルには、「*変更しないでください*」という行が含まれている場合があります。 これは、古いテンプレートやサンプルからの従来の警告に過ぎず、Cloud Manager UI テストに必要なオプトイン編集を&#x200B;*not*&#x200B;でブロックすることはできません。 このアドバイスは安全に無視できます。オプトイン手順（例：`testing.properties`を含める）に従うと、プロジェクト *の*&#x200B;で`assembly-ui-test-docker-context.xml`と`pom.xml`を編集できます。

アドビが指定したサンプルを使用する場合：

* [JavaScript プロジェクト アーキタイプ &#x200B;](https://github.com/adobe/aem-project-archetype/tree/master/src/main/archetype/ui.tests)から生成されたAEM ベースの`ui.tests` フォルダーの場合、次のコマンドを実行して必要な設定を追加できます。
...
* 機能テストの対象となるコンテナ化されたインフラストラクチャは、次のように制限されます。
...
* ターゲットのオーサー/パブリッシュがIP 許可リストに加えるで保護されている場合、パイプライン UI テストインフラストラクチャを許可リストに加えるする必要があります。そうしないと、UI テストが403 Forbiddenで失敗する可能性があります。
IP許可リストに加えるによるAEMaaCSの[UI テストの失敗](https://experienceleague.adobe.com/ja/docs/experience-cloud-kcs/kbarticles/ka-26654#)および[IP ガイドの概要](/help/implementing/cloud-manager/ip-allow-lists/introduction.md)も参照してください。

>[!NOTE]
>
> その他のリソースが必要な場合は、カスタマーケアケースを作成し、ユースケースを説明します。Adobeは、リクエストを確認し、適切なサポートを提供します。
...
### Seleniumの準備ができるのを待ちます {#waiting-for-selenium}
...
Docker イメージでは、追加のテスト出力（スクリーンショット、ビデオなど）を生成し、それらを環境変数 `REPORTS_PATH` で指定されたパスに保存する場合があります。 `REPORTS_PATH`より下にあるファイルは、テスト結果アーカイブに含まれます。
...
1. 次の値を使用して構築された`HTTP_PROXY`環境変数をエクスポートして、UI テストを実行するためのHTTP プロキシ接続を設定します。
   ...
   * `PROXY_HTTPS_PORT`または`PROXY_HTTP_PORT`変数によって提供されるプロキシポート（空でない値を持つ変数が使用されます）
2. HTTP プロキシへの接続時に使用されるCA証明書を設定します。 その場所は、`PROXY_CA_PATH` 変数によって提供されます。
