...
UI テストは、様々な言語やフレームワーク（Cypress、Selenium、Java、JavaScriptなど）をサポートするためにDocker イメージにパッケージ化されています。 また、UI テストプロジェクトは、[AEM プロジェクトアーキタイプ ](https://experienceleague.adobe.com/ja/docs/experience-manager-core-components/using/developing/archetype/overview)を使用して簡単に生成できます。

Adobeは、リアルタイムのリロードと自動待機を提供し、テスト中の効率性と生産性を向上させるので、Cypressの使用を推奨します。 また、Cypressはシンプルで直感的な構文を提供しており、テストを初めて行うユーザーでも簡単に学び、使用することができます。

UI テストは、[**カスタム UI テスト**](/help/implementing/cloud-manager/deploy-code.md) ステップで品質ゲートとして実行されます。[実稼動パイプライン ](/help/implementing/cloud-manager/configuring-pipelines/configuring-production-pipelines.md)で必要です。[実稼動以外のパイプライン ](/help/implementing/cloud-manager/configuring-pipelines/configuring-non-production-pipelines.md)ではオプションです。 リグレッションや新機能を含むすべてのUI テストを使用すると、エラーを検出してレポートできます。

Javaで記述されたHTTP テストであるカスタム機能テストとは異なり、UI テストはDocker イメージとして実行できます。 テストは、[UI テストの構築](#building-ui-tests)で定義されている規則に従う限り、任意の言語で記述できます。
...
## UI テストの構築 {#building-ui-tests}

Maven プロジェクトでは、Docker ビルドコンテキストを生成します。 このDocker ビルドコンテキストでは、Docker イメージの作成方法について説明します。 この画像にはUI テストが含まれており、Cloud Managerは実際のUI テストを含むDocker イメージを生成するために使用します。

この節では、UI テストプロジェクトをリポジトリに追加するために必要な手順について説明します。

>[!TIP]
>
>[AEM プロジェクト アーキタイプ ](https://github.com/adobe/aem-project-archetype)は、プログラミング言語に固有の要件がない場合に、次の説明に準拠したUI テスト プロジェクトを生成できます。
...
推奨される方法は、[Maven Assembly Plugin](https://maven.apache.org/plugins/maven-assembly-plugin/)を設定して、Docker ビルドコンテキストアーカイブを作成し、適切な分類子を割り当てることです。
...
この実行では、Maven Assembly プラグインに対して、プラグインの用語の&#x200B;**アセンブリ記述子**&#x200B;と呼ばれる`assembly-ui-test-docker-context.xml`に含まれる指示に基づいてアーカイブを作成するように指示します。 アセンブリ記述子では、アーカイブに含める必要のあるすべてのファイルがリストアップされます。
...
アセンブリ記述子は、UI テストをローカルで実行する際に生成されたファイルも除外します。 このプロセスにより、アーカイブを小さくし、ビルドを高速化できます。

Cloud Managerは、デプロイメントパイプライン中にDocker ビルドコンテキストアーカイブを自動的に取得し、テストイメージをビルドします。 最終的に、Cloud ManagerはDocker イメージを実行して、アプリケーションに対するUI テストを実行します。

ビルドでは、0または1つのアーカイブが生成されます。 アーカイブが 0 の場合、テストステップはデフォルトで通過します。 ビルドで複数のアーカイブが生成される場合、どのアーカイブが選択されるかは非決定的です。
...
>
>ファイルには、「*変更しないでください*」という行が含まれています。 古いテンプレート/サンプルからの従来の警告であり、*not*&#x200B;では、Cloud Manager UI テストに必要なオプトイン編集を行うことができません。 お知らせは無視できます。オプトイン手順（例：`testing.properties`を含める）に従うと、プロジェクト *の*&#x200B;で`assembly-ui-test-docker-context.xml`と`pom.xml`を編集できます。

アドビが指定したサンプルを使用する場合：

* [JavaScript プロジェクト アーキタイプ ](https://github.com/adobe/aem-project-archetype/tree/master/src/main/archetype/ui.tests)から生成されたAEM ベースの`ui.tests` フォルダーに必要な設定を追加するには、次のコマンドを実行します。
...
>[!NOTE]
>
> これらの値は、パイプラインの実行中に自動的に設定されます。パイプライン変数として手動で設定する必要はありません。

| 変数 | 例 | 説明 | テストフレームワーク |
|----------------------------|----------------------------------|----------------------------------------------------------------------------------------------------|---------------------|
| `SELENIUM_BASE_URL` | `http://my-ip:4444` | Selenium サーバーのURL | Selenium のみ |
| `SELENIUM_BROWSER` | `chrome` | Selenium サーバーで使用されるブラウザー実装 | Selenium のみ |
| `AEM_AUTHOR_URL` | `http://my-ip:4502/context-path` | AEM オーサーインスタンスのURL | すべて |
| `AEM_AUTHOR_USERNAME` | `admin` | AEM オーサーインスタンスにログインするユーザー名 | すべて |
| `AEM_AUTHOR_PASSWORD` | `admin` | AEM オーサーインスタンスにログインするためのパスワード | すべて |
| `AEM_PUBLISH_URL` | `http://my-ip:4503/context-path` | AEM パブリッシングインスタンスのURL | すべて * |
| `AEM_PUBLISH_USERNAME` | `admin` | AEM パブリッシュインスタンスにログインするユーザー名 | すべて * |
| `AEM_PUBLISH_PASSWORD` | `admin` | AEM パブリッシュインスタンスにログインするためのパスワード | すべて * |
| `REPORTS_PATH` | `/usr/src/app/reports` | テスト結果のXML レポートを保存するパス | すべて |
| `UPLOAD_URL` | `http://upload-host:9090/upload` | テストフレームワークからアクセスできるように、ファイルをアップロードする必要があるURL | すべて |
| `PROXY_HOST` | `proxy-host` | テストフレームワークで使用される内部HTTP プロキシのホスト名 | Selenium を除くすべて |
| `PROXY_HTTPS_PORT` | `8071` | HTTPS接続のプロキシサーバーリスニングポート （空にすることができます） | Selenium を除くすべて |
| `PROXY_HTTP_PORT` | `8070` | HTTP接続のプロキシサーバーリスニングポート （空にできます） | Selenium を除くすべて |
| `PROXY_CA_PATH` | `/path/to/root_ca.pem` | テストフレームワークで使用するCA証明書のパス | Selenium を除くすべて |
| `PROXY_OBSERVABILITY_PORT` | `8081` | プロキシサーバーのHTTP `healthcheck` ポート | Selenium を除くすべて |
...
Docker イメージは、`JUnit` XML形式でテスト レポートを生成し、環境変数`REPORTS_PATH`で指定されたパスに保存する必要があります。 `JUnit` XML形式は、テストの結果を報告するために広く使用されている形式です。 Docker イメージでJavaとMavenを使用している場合、[Maven Surefire Plugin](https://maven.apache.org/surefire/maven-surefire-plugin/)や[Maven Failsafe Plugin](https://maven.apache.org/surefire/maven-failsafe-plugin/)などの標準テストモジュールは、そのようなレポートを自動的に生成できます。
...
>
>エラーをSTDERRに記録したり、ゼロ以外の終了コードを返したりする代わりにアサーションを使用するか、デプロイメントパイプラインが正常に進行します。
>
>テストの実行中にHTTP プロキシが使用された場合、結果には`request.log` ファイルが含まれます。
...
>[!NOTE]
>
>ローカルマシンから機能テストを実行する場合と同じ動作を実現するには、管理者のような権限を持つユーザーを作成します。

* 以下は、機能テストの対象となるコンテナ化されたインフラストラクチャを制限します。

| タイプ | 値 | 説明 |
|----------------------|-------|-----------------------------------------------------------------------|
| CPU | 2.0 | テスト実行ごとに確保される CPU 時間の量です。 |
| メモリ | 1Gi | テストに割り当てられたメモリの量。 値はギガバイト単位です。 |
| タイムアウト | 30m | テストの実行時間。 |
| 推奨期間 | 15m | Adobeでは、この時間制限内にテストを保持することをお勧めします。 |

* IP許可リストに加えるがターゲットのオーサー/パブリッシュを保護する場合、パイプライン UI テストインフラストラクチャまたはUI テストを許可リストに加えるすると、403 Forbiddenで失敗する可能性があります。IP許可リストに加えるによるAEMaaCSの[UI テストの失敗](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-26654#)および[IP ガイドの概要](/help/implementing/cloud-manager/ip-allow-lists/introduction.md)も参照してください。

>[!NOTE]
>
> さらにリソースが必要な場合は、カスタマーケアケースを作成し、ユースケースを説明します。Adobeは、リクエストを確認し、適切なサポートを提供します。
...
### Seleniumの準備ができていることを確認するには、それを待ちます。 {#waiting-for-selenium}

テストを開始する前に、Selenium サーバーが実行状態にあることを Docker イメージ側で確認する必要があります。 Selenium サービスの待機は、2段階のプロセスです。
...
Selenium ステータスエンドポイントが肯定的な応答で回答すると、テストを開始できます。

AdobeのUI テスト サンプルは`wait-for-grid.sh`を使用しています。 Dockerの開始時に実行され、グリッドの準備が整った後にのみテストを開始します。


### スクリーンショットとビデオのキャプチャ {#capture-screenshots}

Docker イメージは、追加のテスト出力（スクリーンショットやビデオなど）を生成し、環境変数`REPORTS_PATH`で指定されたパスに保存します。 `REPORTS_PATH`より下にあるファイルは、テスト結果アーカイブに含まれます。

デフォルトでは、Adobeが提供するテストサンプルは、失敗したテストのスクリーンショットを作成します。
...
UI テストの実行中にテスト結果アーカイブが作成された場合は、[**カスタム UI テスト** ステップ ](/help/implementing/cloud-manager/deploy-code.md)の下にある「`Download Details`」ボタンをクリックして、Cloud Managerからダウンロードできます。

### ファイルのアップロード {#upload-files}

テストでは、場合によって、テスト中のアプリケーションにファイルをアップロードする必要があります。 テストに対してSeleniumのデプロイメントを柔軟に保つために、アセットをSeleniumに直接アップロードすることはできません。 ファイルをアップロードするには、代わりに次の手順を実行する必要があります。
...
1. `UPLOAD_URL` 環境変数で指定された URL にファイルをアップロードします。
   ...
   * `curl -X POST ${UPLOAD_URL} -F "data=@file.txt"`と同等です。
...
この値が空の場合、追加の手順は必要なく、HTTP プロキシを使用せずにテストを実行する必要があります。

空でない場合は、エントリポイントスクリプトで次の操作を実行する必要があります。

1. 次の値を使用して構築された`HTTP_PROXY`環境変数をエクスポートして、UI テストを実行するためのHTTP プロキシ接続を設定します。
   * `PROXY_HOST`変数が提供するプロキシ ホスト。
   * `PROXY_HTTPS_PORT`または`PROXY_HTTP_PORT`変数が提供するプロキシポート（空でない値を持つ変数が使用されます）。
2. HTTP プロキシへの接続時に使用されるCA証明書を設定します。 `PROXY_CA_PATH`変数は、その場所を提供します。
   * `NODE_EXTRA_CA_CERTS`環境変数をエクスポートします。
3. HTTP プロキシの準備が整うまで待機します。
   * 準備状況を確認するには、環境変数`PROXY_HOST`、`PROXY_OBSERVABILITY_PORT`、`PROXY_RETRY_ATTEMPTS`、`PROXY_RETRY_DELAY`を使用できます。
...
>[!NOTE]
>
> このセクションは、`Playwright`が選択したテスト インフラストラクチャである場合にのみ適用されます。

### HTTP プロキシの設定

>[!NOTE]
>
> ここで紹介した例では、Adobeは、Chromeがプロジェクトブラウザーとして使用されていることを前提としています。
...
`certutil.`を提供するcURLと`libnss3-tools`をインストールします
...
2. Chromium™のプロキシ CA証明書をインストールするには、`certutil`を使用します。
...
`HTTP_PROXY`環境変数が設定されている場合にプロキシを使用するには、playwright設定を変更します（例：`playwright.config.js`）。
...
## UI テストをローカルで実行する {#run-ui-tests-locally}

Cloud Manager パイプラインでUI テストをアクティブ化する前に、Adobeでは、[AEM as a Cloud Service SDK](/help/implementing/developing/introduction/aem-as-a-cloud-service-sdk.md)に対してUI テストをローカルで実行することをお勧めします。 実際のAEM as a Cloud Serviceインスタンスに対して実行することもできます。

### サイプレス試験サンプル {#cypress-sample}
...
1. Mavenを使用してテストを開始するには、次のコマンドを実行します。
...
>[!NOTE]
>
>* このコマンドは、スタンドアロンのSelenium インスタンスを開始し、そのインスタンスに対してテストを実行します。
...
1. Mavenを使用してDocker イメージをビルドするには、次のコマンドを実行します。
