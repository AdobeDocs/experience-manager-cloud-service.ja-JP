...
>[!NOTE]
>また、一部の[早期導入お客様](/help/release-notes/release-notes-cloud/release-notes-current.md#sites-early-adopter)向けに、RUM（実ユーザーモニタリング）測定を介したクライアントサイドの収集も有効になります。詳しくは、[この記事](#real-user-monitoring-for-aem-as-a-cloud-service)のドキュメントを参照してください。
...
クライアントサイドのコレクションである実ユーザーモニタリング（RUM）データサービスは、ユーザーインタラクションをより正確に反映し、web サイトのエンゲージメントの信頼性の高い測定を保証します。これにより、お客様はページのトラフィックとパフォーマンスに関する高度なインサイトを取得できます。これは、アドビが管理する CDN と アドビ以外が管理する CDN を使用する両方のお客様にとって有益です。さらに、アドビ以外が管理する CDN を使用しているお客様に対して自動トラフィックレポートを有効にできるので、トラフィックレポートをアドビと共有する必要がなくなります。
...
リアルユーザーモニタリング（RUM）は、web サイトまたはアプリケーションのデジタルユーザーエクスペリエンスをリアルタイムでキャプチャして分析する、パフォーマンス監視テクノロジの一種です。Web アプリケーションのリアルタイムのパフォーマンスを可視化し、エンドユーザーエクスペリエンスに関する正確なインサイトを提供します。

リアルユーザーモニタリング（RUM）は、URL の開始からリクエストがブラウザーに返されるまで、主要なパフォーマンス指標に対する詳細なインサイトを提供します。これはすべて、開発者がアプリケーションを強化してエンドユーザーが使いやすいようにするのに役立ちます。
...
Adobe Experience Manager のリアルユーザーモニタリングは、訪問者のプライバシーを保護し、データ収集を最小限に抑えるように設計されています。つまり、訪問者として、訪問しているサイトによって個人情報が収集されたり、アドビに提供されたりすることはありません。
...
リアルユーザーモニタリング（RUM）は、個人を特定できる情報の収集を防ぐように設計されています。Adobe Experience Manager のリアルユーザーモニタリングで収集できる完全な情報セットを以下に示します。
...
* アドビの製品チームは、クライアントサイドのリアルユーザーモニタリング（RUM）コレクションによって収集されたページビューと[コア web バイタル（CWV）](https://web.dev/vitals/)指標を表示できるドメインキーとデータダッシュボード URL を提供するためにお客様に連絡します。
...
1. **トラッカーブロッカー**
   ...
   * エンドユーザーがトラッカーブロッカーやプライバシー拡張機能を使用すると、これらのツールはトラッキングスクリプトの実行を制限するので、リアルユーザーモニタリング（RUM）のデータ収集が妨げられる可能性があります。この制限により、ページビューやユーザーインタラクションが過小レポートされる可能性があり、実際のサイトアクティビティと RUM によってキャプチャされたデータとの間に不一致が生じる可能性があります。

1. **API／JSON 呼び出しの取得に関する制限事項**

   * RUM データサービスはクライアントサイドのエクスペリエンスに重点を置いており、現時点ではバックエンド API や JSON 呼び出しをキャプチャしません。リアルユーザーモニタリング（RUM）データからこれらの呼び出しを除外すると、CDN 分析によって測定されるコンテンツリクエストとの差異が生じます。
