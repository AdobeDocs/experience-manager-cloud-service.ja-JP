...
1. パフォーマンス、アクセシビリティ、ベストプラクティス、SEO （検索エンジン最適化）のベースライン標準を満たす。
...
>* パフォーマンスに関する一般的なヒントについては、[一般的なパフォーマンスに関するヒント](#performance-tips)を参照してください。
...
デフォルトでは、**ページスコア – トレンド** 用に選択したビューは、**昨年** の **スコアの中央値** です。

凡例のカテゴリ名をクリックすると、特定の Lighthouse カテゴリのトレンドを表示するように選択できます。

![ トレンド選択可能 ](assets/experience-audit-trend-selectable.png)

グラフの上部にある **選択** ドロップダウンを使用してページ固有の詳細を選択し、下部にある **表示** および **トリガー** ドロップダウンを使用して、それぞれ異なる時間枠とトリガータイプを選択します。

**表示** ドロップダウンでは、プリセットされた時間枠や、より具体的な表示のカスタム間隔を選択できます。
...
レコメンデーションをクリックすると、そのレコメンデーションの詳細が表示されます。

![レコメンデーションの詳細](assets/experience-audit-recommendations-details.png)

利用可能な場合、展開されたレコメンデーションの詳細には、最も影響の大きい変更に焦点を当てるのに役立つ、レコメンデーションの影響の割合も含まれます。 これに加えて、推奨事項が拡張され、関連するAEM ドキュメントのリンクや、推奨される修正点の実装をガイドするヒントが含まれている場合があります。

詳細ビューの **ページを表示** リンクをクリックして、レコメンデーションが適用されるページを表示します。
...
特定のページのリンクをクリックすると、「[**ページスコア - トレンド**」セクション](#trend)の「**選択**」フィルターが更新され、選択したページの「**スコアとレコメンデーション**」タブが表示されます。
