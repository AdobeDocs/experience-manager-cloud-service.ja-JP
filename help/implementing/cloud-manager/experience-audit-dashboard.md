...
「**生レポート**」タブには、ページのすべての監査のスコアが表示されます。**Lighthouse レポート** 列のレポート日付をタップまたはクリックして、生データの JSON ファイルを取得します。
...
![ オンデマンドスキャンの実行中 ](assets/experience-audit-on-demand-running.png)
...
監査対象として設定した [ ページ ](#configuration) が使用できなかった場合や、監査で他のエラーが発生した場合は、エクスペリエンス監査にこの情報が反映されます。
...
* この監査は、パブリッシャーの [ 設定されたエクスペリエンス監査ページのパス ](#configuration) で定義されたオリジン（`.com`）ドメインをスキャンし、実際のユーザーエクスペリエンスをより正確にシミュレートし、web サイトの管理と最適化に関するより十分な情報に基づいた意思決定を支援します。
