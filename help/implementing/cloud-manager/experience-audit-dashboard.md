...
監査対象として[設定したページ](#configuration)が利用できなかった場合や、監査にその他のエラーがあった場合は、エクスペリエンス監査にこれが反映されます。
...
* 監査では、パブリッシャーの[設定済みエクスペリエンス監査ページパス](#configuration)で定義されている接触チャネル（`.com`）ドメインをスキャンして、実際のユーザーエクスペリエンスをより正確にシミュレートし、web サイトの管理と最適化についてより情報に基づいた意思決定を行うのに役立ちます。
