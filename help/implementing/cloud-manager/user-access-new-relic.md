...
Adobeでは、アプリケーションのモニタリング、可用性、パフォーマンスに重点を置きます。 AEM as a Cloud Service には、New Relic One モニタリングへのアクセスが含まれており、チームは標準製品の一部としてシステムと環境のパフォーマンス指標を包括的に可視化できます。
...
* インフラストラクチャレベルのモニタリングとアプリケーション（Adobe Experience Manager）モニタリングの主要指標を統合することで、包括的なパフォーマンス最適化を実現します。
...
新しく作成したプログラムの場合は、New Relic One サブアカウントが作成されます。 ただし、データを取り込むには、それをアクティブにする必要があります。 このアクティベーションは自動ではありません。 サブアカウントをアクティブ化するには、次の手順に従います。
...
>[!NOTE]
>
>New Relic One ユーザーを管理するには、**ビジネスオーナー**&#x200B;または&#x200B;**デプロイメントマネージャー**&#x200B;の役割のユーザーがログインする必要があります。
...
1. **New Relic ユーザーの管理** ダイアログボックスで、次の操作を行います。
   ...
   * ![閉じるアイコン &#x200B;](https://spectrum.adobe.com/static/icons/workflow_18/Smock_Close_18_N.svg)をクリックして、ユーザーを削除します。

   ![ユーザーを追加](assets/newrelic-add-users.png)

1. 「**保存**」をクリックします。

ユーザーが定義されると、New Relicは各ユーザーに確認メールを送信します。 そして、アクティベーションプロセスを完了してログインします。
...
New Relic から確認メールが届かない場合は、[トラブルシューティングの節](#troubleshooting)を参照してください。

## New Relic Oneを開く {#accessing-new-relic}

[New Relic アカウントをアクティベートしたら](#activate-account)、Cloud Managerを使用してNew Relic Oneを開くか、直接開くことができます。

**Cloud Managerを使用してNew Relic Oneを開くには：**
...
詳細については、[AEM サポートポータル &#x200B;](https://helpx.adobe.com/jp/enterprise/using/support-for-experience-cloud.html)を使用してサポートチケットを開いてください。

## 変更トラッカーの使用 {#change-tracker}

サポートされているパイプラインの実行、Cloud Managerのアップグレード、コードの復元が完了すると、AEMによってNew Relic Oneに変更トラッカーが自動的に送信されます。 これらのトラッカーは、New Relicの&#x200B;**Change Tracking** ビューで変更イベントとして表示され、デプロイメントをアプリケーションパフォーマンス、エラー率、スループットの変化と関連付けることができます。
...
| **AEMの更新** | AEMのバージョンのアップグレード： 例えば、バージョン {}からバージョン {}です。 環境変更イベントが完了すると、トラッカーが作成されます。 |
...
1. [Cloud Managerまたは直接New Relic One](#accessing-new-relic)にアクセスできます。
...
サインアッププロセスを完了し、電子メールまたはパスワードのエラーメッセージが原因でアカウントにログインできない場合は、[Admin Console](https://adminconsole.adobe.com/)を使用してサポートチケットを記録します。
...
>
>このドキュメントの「[New Relic One サブアカウントをアクティブ化](#activate-sub-account)」セクションと同じ手順に従って、New Relic One サブアカウントを再アクティブ化します。

AEM as a Cloud Service プログラムのNew Relic One製品に関する詳細なヘルプや追加ガイダンスについては、[AEM サポートポータル &#x200B;](https://helpx.adobe.com/jp/enterprise/using/support-for-experience-cloud.html)を使用してサポートチケットを開いてください。

## よくある質問 {#faqs}

+++**アドビが New Relic One でモニタリングするものは何ですか？**

Adobeは、New Relic OneのJava プラグインを使用して、AEM as a Cloud Service オーサー、パブリッシュ、プレビュー（利用可能な場合）サービスをモニタリングします。 アドビは、カスタムの New Relic One APM テレメトリと、実稼動および実稼動以外の AEM as a Cloud Service 環境でのモニタリングを有効にします。

お客様のNew Relic One アカウントは、Adobeが管理する主要なアカウントに接続され、複数のアプリケーションがレポートされます（AEM as a Cloud Serviceごとに3つ）。
...
* 両方のNew Relic One アカウントの完全なモニタリング指標とイベントは、3か月間保持されます。
...
New Relic Oneは、New Relicの新しいオブザーバビリティプラットフォームであり、Adobe サポートとチームが指標とイベントを観察、監視、表示できるようにします。

New Relic One を使用すると、ユーザーがアクセスできるすべてのアカウントを検索し、すべてのサービスとホストからのデータを 1 つのビューで視覚化できます。

Adobe サポートでは、AEM as a Cloud ServiceとNew Relic Oneおよびその他のツールを監視し、引き続きオンプレミスのサービスとインフラストラクチャにNew Relicを使用できます。 Adobe New Relic One アカウントと顧客管理のNew Relic アカウントの両方からデータを可視化できます。
...
30 日以上アクティビティが検出されなかった場合、[APM エージェントは停止します](#limitations)。 このドキュメントの「[New Relic One サブアカウントをアクティブ化](#activate-sub-account)」セクションと同じ手順に従って、New Relic One サブアカウントを再アクティブ化します。
+++
