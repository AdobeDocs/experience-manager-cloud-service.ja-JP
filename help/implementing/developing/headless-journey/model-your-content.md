---
title: コンテンツのモデリング方法
description: AEMヘッドレス開発者ジャーニーのこの部分では、コンテンツフラグメントモデルとコンテンツフラグメントを使用したコンテンツモデリングを使用して、AEMヘッドレス配信向けのコンテンツをモデル化する方法を学びます。
hide: true
hidefromtoc: true
index: false
...
* **目的**:コンテンツ構造をモデル化し、AEMコンテンツフラグメントモデルとコンテンツフラグメントを使用してその構造を実現する方法を説明します。
   * データ/コンテンツのモデリングに関連する概念と用語を紹介します。
   * ヘッドレスコンテンツ配信にコンテンツのモデリングが必要な理由を説明します。
   * AEMコンテンツフラグメントモデルを使用して（およびコンテンツフラグメントを使用してコンテンツを作成する）、この構造を実現する方法を説明します。
   * コンテンツのモデル化方法を学びます。基本的なサンプルを持つ原則
...
>[!NOTE]
>
>AEMがコンテンツを扱う際には、データモデリングをコンテンツモデリングと呼びます。
...
この情報を分析して定義するプロセスは、それらの関係と共に、**コンテンツモデリング**&#x200B;と呼ばれます。
...
## AEMヘッドレス向けコンテンツモデリング{#content-modeling-for-aem-headless}

データモデリングは確立された一連の技術であり、関係データベースの開発時によく使用されます。したがって、コンテンツモデリングはAEMヘッドレスに対して何を意味しますか。
...
1. **フラグメント参照**&#x200B;データ型を使用すると、（モデルの種類に従って）コンテンツフラグメントをネストして、複数のレベルの構造を実現できます。 これは、コンテンツのモデリングにとって重要です。
...
サイトに対してコンテンツフラグメントモデルを有効にする必要があるまさに開始時に、これを設定ブラウザーで行います。「ツール」>「一般」>「設定ブラウザ」の順に選択します。 グローバルエントリの設定を選択するか、新しい設定を作成できます。 次に例を示します。

![設定の定義](assets/cfm-configuration.png)

>[!NOTE]
>
>設定ブラウザーの「追加のリソース — コンテンツフラグメント」を参照してください。

次に、コンテンツフラグメントモデルを作成し、構造を定義します。 これは、ツール/アセット/コンテンツフラグメントモデルで行うことができます。 次に例を示します。
...
コンテンツを実際に作成する最初の手順は、コンテンツフラグメントを作成することです。 これは、アセット/ファイルの下の必要なフォルダーにある作成/コンテンツフラグメントを使用して行います。 ウィザードの指示に従って手順を進めます。

コンテンツフラグメントは、特定のコンテンツフラグメントモデルに基づいており、作成プロセスの最初の手順として選択します。
...
* [AEM Headless](https://experienceleague.adobe.com/docs/experience-manager-learn/getting-started-with-aem-headless/graphql/overview.html?lang=ja)  — はじめに — コンテンツモデリングやGraphQLなど、AEM headless機能の使い方の概要を説明する短いビデオチュートリアルシリーズ
