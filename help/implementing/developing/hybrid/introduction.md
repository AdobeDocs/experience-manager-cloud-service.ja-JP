...
* [GitHub で入手可能なサンプルの WKND SPA Project アプリ](https://github.com/adobe/aem-guides-wknd-spa)
   * をダウンロードします。 [React アプリの最新リリース](https://github.com/adobe/aem-guides-wknd-spa/releases) ～に似た名前を付ける `wknd-spa-react.all-X.Y.Z-SNAPSHOT.zip`.
   * をダウンロードします。 [アプリの最新のサンプル画像](https://github.com/adobe/aem-guides-wknd-spa/releases) ～に似た名前を付ける `wknd-spa-sample-images-X.Y.Z.zip`.
   * [パッケージマネージャーの使用](/help/implementing/developing/tools/package-manager.md) をクリックして、AEMの他の任意のパッケージと同様に、両方のパッケージをインストールします。
