...
* 明確に分離することで、システムの拡張性が向上し、フロントエンドの独立開発が可能になります。

### SPA の仕組み  {#how-does-a-spa-work}

SPAの背後にある主な考え方は、SPAがネイティブアプリケーションの応答性に近づくように、サーバーへの呼び出しと依存性を低減し、サーバーの遅延に起因する遅延を最小限に抑えることです。
...
1. `home.model.json` を新しいタブに読み込みます。
   ...
   AEM SPA Editor は、 [AEM Content Services](/help/sites-cloud/administering/content-fragments/overview.md#content-fragments-and-content-services) を使用して、ページのコンテンツ全体を JSON モデルとして配信します。
...
サンプルの WKND SPA Project アプリケーションを使用すると、JSON コンテンツ配信のコンテンツサービスやリソースの非同期読み込みを利用して、アプリの動作と公開時の読み込み方法が明確になります。
...
SPA は、AEM の外部で開発されて保守される SPA を含む、AEM 内の柔軟なレベルの統合で有効にできます。さらに、SPAはAEM内で使用できる一方、AEMを使用して追加のエンドポイントにヘッドレスにコンテンツを配信することもできます。
...
* [SPA for AEMの開発](developing.md) フロントエンド開発者とAEM向けSPAの開発を連携させる方法と、SPAとAEMアーキテクチャとのやり取りについて説明します。
