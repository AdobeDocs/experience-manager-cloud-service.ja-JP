...
このドキュメントでは、SPA内に編集可能なSPAコンポーネントを実装できるように、AEMフレームワークが満たす必要がある一般的な契約 (AEMサポートレイヤーの種類 ) について説明します。
...
ページモデルでは、JSON モデルエクスポーター (JSON モデルエクスポーター自体が [Sling モデル](https://sling.apache.org/documentation/bundles/models.html) API 書き出し可能な Sling モデルは、基になるライブラリがデータモデルを解釈できるように、次のフィールドリストを公開します。
