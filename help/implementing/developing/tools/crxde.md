...
最初に変更を保存する際には、新しく作成したノードが親ノードのノードタイプで許可されるかどうかに関する検証もリポジトリーによって実行されます。ノードの保存中にエラーメッセージが表示された場合は、コンテンツ構造が有効かどうかを確認してください ( 例えば、 `nt:unstructured` の子としてのノード `nt:folder` ノード ) で使用できます。
