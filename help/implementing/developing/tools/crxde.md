...
上部の切り替えボタンバーを使用すると、CRXDE Liteと [パッケージマネージャー。](package-manager.md)
...
The **編集ウィンドウ** リポジトリ内で現在選択されているファイルのコンテンツを表示できます。 開かれた各ファイルは、ペイン内に独自のタブとして表されます。
...
>
新しく作成されたノードが親ノードのノードタイプで許可されるかどうかに関する検証も、変更の保存時にリポジトリが実行します。 ノードの保存中にエラーメッセージが表示された場合は、コンテンツ構造が有効かどうかを確認します ( 例えば、 `nt:unstructured` の子としてのノード `nt:folder` ノード ) で使用できます。
