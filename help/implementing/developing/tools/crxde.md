...
上部のスイッチバーを使用すると、CRXDE Lite と [パッケージマネージャー](package-manager.md) をすぐに切り替えることができます。
...
また、特定の名前を持つノードの検索がサポートされます。検索するノードの名前を入力して待機します（または、右側にある検索アイコンをクリックします）。特定のノードがエクスプローラーペインにロードされると、リストが表示され、パスを選択して Enter キーを押すと、そのパスに移動できます。 この機能は、ブラウザーで現在 CRXDE クライアントアプリケーションに読み込まれているノードでしか使用できません。リポジトリー全体を検索する場合は&#x200B;**ツール**／**クエリ**&#x200B;を使用します。
...
この **編集ウィンドウ** リポジトリ内で現在選択されているファイルのコンテンツを表示できます。 開かれた各ファイルは、ペイン内に独自のタブとして表されます。
...
**「更新」ボタン**&#x200B;は、現在の選択を更新します。他のユーザーからの変更は、リポジトリのユーザービューで更新されます。 加えた変更は影響を受けません。

### 「すべて保存」ボタン {#save-all-button}

**「すべて保存」ボタン**&#x200B;は、おこなった変更をすべて保存します。保存を選択するまでは、変更は一時的なもので、コンソールを終了すると失われます。
...
>
最初に変更を保存する際には、新しく作成したノードが親ノードのノードタイプで許可されるかどうかに関する検証もリポジトリーによって実行されます。ノードの保存中にエラーメッセージが表示された場合は、コンテンツ構造が有効かどうかを確認します ( 例えば、 `nt:unstructured` の子としてのノード `nt:folder` ノード ) で使用できます。
...
1. 定義は、ブラウザーの新しいタブに CND 表記で表示されます。
1. 必要に応じて、情報を保存します。
