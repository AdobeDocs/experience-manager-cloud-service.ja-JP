...
1. コンテンツの手順を作成する場合と同様の手順に従います。 「**コンテンツセットを追加**」をクリックする代わりに、コンソールから既存のセットを選択し、省略記号メニューから「**編集**」を選択します。

![コンテンツセットの編集](assets/edit-content-set.png)

1. コンテンツセットを編集する際、設定されたパスを展開すると、除外されたサブパスを表示できます。
...
1. 「**コンテンツをコピー**」ダイアログボックスで、コンテンツコピーアクションのソースと宛先を指定します。
   ...
   * コンテンツは、上位の環境から下位の環境にコピーするか、次のような階層を持つ開発／RDE 環境間でのみ（最上位から最下位へ）コピーできます。
      ...
      * ステージング
      * 開発／RDE
   * デフォルトでは、プログラム間コンテンツのコピーは無効になっています。 ただし、お客様からのリクエストに応じて有効にすることができます。これにより、追加の&#x200B;**宛先プログラム**&#x200B;入力フィールドが使用可能になります。

1. （オプション）必要に応じて、次の値を設定します。

   * **アクセス制御リストを含める** - コンテンツのアクセス制御権限をコンテンツと共にコピーするかどうかを選択します。
   * **ワイプ** – 読み込みを開始する前に、宛先の既存のコンテンツを削除することを選択します。これにより、クリーンな状態から開始し、既存のコンテンツとの競合を回避できます。**ワイプ**を選択解除したままにすると、Cloud Managerは既存の宛先コンテンツの上に新しいコンテンツを読み込みます。ワイプを開始する前に確認プロンプトが表示され、Cloud Managerはワイプアクションと読み込みの詳細を記録してトレーサビリティを確保します。
     ![コンテンツのコピー](assets/content_copy_wipe-destination.png)
   ...
   * **パッケージ読み込み失敗ポリシー** パッケージの読み込み時に使用するポリシーを選択します。
      ...
      * **残りのパッケージで続行**：失敗した後でも、インポートプロセスを続行します。 成功しなかったパッケージ（およびそのコンテンツ）は、インポートログに記録されます
...
* コンテンツのコピーツールでは、次の 2 つのフローモードをサポートしています。
   1. 上位から下位へのフロー - コンテンツを上位環境から下位環境にコピーできます（例：実稼動 → ステージ、ステージ → 開発／RDE）。
   2. 転送フロー（新機能）- コンテンツを下位の実稼動以外の環境から上位の実稼動以外の環境にコピーすることもできます（例：開発 → ステージ、RDE → ステージ）。 この機能は明示的なリクエストがあった場合にのみ使用でき、明示的に無効にするようにリクエストされるまで有効なままです。 実稼動環境は転送フローの有効な宛先にはなりません。
