...
権限と設定方法について詳しくは、 [AEMas a Cloud Serviceのチームおよび製品プロファイル](/help/onboarding/aem-cs-team-product-profiles.md).
...
1. 次の日： **詳細** ウィザードの「 」タブで、コンテンツセットの名前と説明を入力し、「 」を選択します。 **続行**.
...
1. コンテンツセットを調整または制限する必要がある場合、サブパスを除外できます。
   ...
   1. 選択 **除外パス**.
   1. 選択 **除外サブパスを追加** 必要に応じて、除外するパスを追加します。
...
1. 選択 **作成** をクリックして、コンテンツセットを作成します。
...
* ソース上の移動または削除されたコンテンツを追跡できないので、コンテンツコピーツールをクローン作成ツールやミラーリングツールとして使用しないでください。
* コンテンツコピーツールにはバージョン管理機能がなく、最後のコンテンツコピー操作以降、変更されたコンテンツや、コンテンツセット内のソース環境で作成されたコンテンツを自動的に検出することはできません。
