...
Cloud Managerで`EXTERNALIZER`環境変数（例：`AEM_EXTERNALIZER_AUTHOR`）を定義または上書きしないでください。 代わりに、`publish`または`preview` ドメイン値を上書きする必要がある場合は、`AEM_CDN_DOMAIN_PUBLISH`および`AEM_CDN_DOMAIN_PREVIEW`環境変数を定義して使用します。 これらの変数は、起動時にExternalizer設定の対応するフィールドに自動的に割り当てられます。
