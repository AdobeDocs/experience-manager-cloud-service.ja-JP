...
パッケージフィルターが定義されるのは、ほとんどの場合、最初に[パッケージを作成](#creating-a-new-package)するときです。ただし、後で編集することもできますが、その後でパッケージを再作成して、新しいフィルター定義に基づいてその内容を更新する必要があります。
...
1. 「**編集**」をクリックして、[パッケージの内容](#package-contents)を定義します。設定の編集が完了したら、「**保存**」をクリックします。
...
>[!TIP]
>
>タイムアウトを避けるには、[サイズの大きなパッケージを作成しない](#package-size)ことをお勧めします。
...
* [cURL などの HTTP POST リクエストから](#via-post-request)。
...
パッケージの再インストールでは、既にインストールされているパッケージに対して、[最初にパッケージをインストールした](#installing-packages)時の処理と同じ手順を実行します。
