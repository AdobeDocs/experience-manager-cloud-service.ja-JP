...
>[!NOTE]
>
>パッケージには、コンテンツのみを含めることができます。任意の機能 ( 例えば、 `/apps`) は、 [Cloud Manager の CI/CD パイプラインを使用してデプロイされる](/help/implementing/cloud-manager/deploy-code.md).
...
AEMaaCS 用のパッケージを管理する方法について詳しくは、 [AEMへのデプロイ (as a Cloud Service)](/help/implementing/deploying/overview.md) （デプロイユーザーガイド）。

## パッケージサイズ {#package-size}

Adobeでは、大きなパッケージを作成しないことをお勧めします。 これは、パッケージをアップロードおよびダウンロードする際のタイムアウトの問題を回避するためです。
...
ネットワークトラフィックは可変で、常にアドバタイズされる理論上の最大値より小さいため、オンラインインターネット接続速度テストツールを使用してみてください。

アップロードとダウンロードでは、インターネットの速度はほぼ常に異なります。 パッケージのアップロードとダウンロードの両方が必要な場合は、計算に低い値（通常はアップロード速度）を使用する必要があります。

### 例 {#example}

インターネット速度テストツールを使用して、現在のアップロード速度が約 100 Mbps であることがわかります。
...
>
>したがって、提供される数式はガイドラインのみであり、実際の推奨パッケージサイズの上限は異なる場合があります。
...
| ルールタイプ | 説明 |
|---|---|
| include | ディレクトリを含めると、そのディレクトリと、そのディレクトリ内（つまりサブツリー全体）にあるすべてのファイルとフォルダーが含まれますが、 **次の条件を満たさない** 指定したルートパスの下に、の他のファイルまたはフォルダを含めます。 |
| exclude | ディレクトリを除外すると、そのディレクトリと、そのディレクトリ内のすべてのファイルとフォルダー（つまり、サブツリー全体）が除外されます。 |
...
>[!TIP]
>
>タイムアウトを避けるため、Adobeでは [サイズの大きいパッケージを作成しない。](#package-size)
...
1. 「**ビルド**」をクリックします。既存のパッケージの内容が上書きされるので、パッケージをビルドするかどうかを確認するダイアログが表示されます。
...
* [OSGi パッケージのインポート](#osgi-package-imports)
* [オーバーレイ](#overlays)
* [ACL](#acls)
...
>[!NOTE]
>
>パッケージは AEMaaCS にコードをデプロイする際には使用できないので、 **OSGi パッケージのインポート** 検証は不要です。
...
>[!NOTE]
>
>パッケージは AEMaaCS にコードをデプロイする際には使用できないので、 **オーバーレイ** 検証は不要です。
...
明示的なエラーは提供できません。 検証は、新しい ACL 権限がパッケージのインストールによって追加されたか影響を受けたかを示すだけです。
...
* [パッケージマネージャー UI を使用する](#via-package-manager)
