...
>[!NOTE]
>
>パッケージには、コンテンツのみを含めることができます。すべての機能 ( 例えば、 `/apps`) は [Cloud Manager の CI/CD パイプラインを使用してデプロイされる](/help/implementing/cloud-manager/deploy-code.md).
...
ネットワークトラフィックは可変で、常にアドバタイズされる理論上の最大値より小さいため、オンラインのインターネット接続速度テストツールを使用してみてください。
