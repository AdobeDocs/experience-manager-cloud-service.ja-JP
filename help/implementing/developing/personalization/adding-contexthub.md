...
また、[`ContextHub.Utils.JSON`](contexthub-api.md#contexthub-utils-json) クラスは、文字列にオブジェクトをシリアル化し、文字列をオブジェクトにシリアル化解除する関数を提供します。 `JSON.parse` 関数および `JSON.stringify` 関数をネイティブに含まないブラウザーをサポートするには、このクラスを使用して JSON データを処理します。
