...
<table>
  <tbody>
    ...
    <tr>
      ...
      <td>はい</td>
      <td>はい</td>
      <td style="background-color: #ffb3b3;">将来</td>
    </tr>
    <tr>
...
>[!NOTE]
>
> 今後予定されているCDN Log Technologiesについては、[aemcs-logforwarding-beta@adobe.com](mailto:aemcs-logforwarding-beta@adobe.com)にメールで関心を登録してください。
...
>
>このため、高度なネットワーク VPN設定でログ転送を使用することはできません。
...
>[!NOTE]
>AWS S3のCDN ログのサポートは、将来にわたって計画されています。 興味を登録するには、[aemcs-logforwarding-beta@adobe.com](mailto:aemcs-logforwarding-beta@adobe.com)に電子メールを送信してください。
...
* 許可されているリソース：オブジェクトとコンテナを選択する必要があります。
* 許可されている権限：読み取り、書き込み、追加、リスト、作成を選択する必要があります。
* 有効な開始日時と有効期限。
...
>
>New Relic Log APIのCDN ログのサポートは、今後も予定されています。 興味を登録するには、[aemcs-logforwarding-beta@adobe.com](mailto:aemcs-logforwarding-beta@adobe.com)に電子メールを送信してください。
>
...
>[!NOTE]
>Dynatrace Log APIのCDN ログのサポートは、今後も予定されています。 興味を登録するには、[aemcs-logforwarding-beta@adobe.com](mailto:aemcs-logforwarding-beta@adobe.com)に電子メールを送信してください。
...
Sumo Logic をデータ取り込み用に設定すると、ホスト、receiverURI、秘密鍵を単一の文字列で提供する「HTTP ソースアドレス」が表示されます。  次に例を示します。
...
>[!NOTE]
>`index` フィールドの動作は、ログの種類によって異なります。
>
>* **AEM ログ （Apache/Dispatcherを含む）**: Sumo Logic Enterprise サブスクリプションがある場合は、`index`で指定されたパーティションにルーティングされます。 エンタープライズ以外のサブスクリプションは、代わりに`sumologic_default` パーティションにルーティングされます。
>* **CDN ログ**: `index` フィールドは無視されます。これは、Sumo Logicに転送されるCDN ログでは、インデックス作成が技術的にサポートされていないためです。 CDN ログは常に`sumologic_default` パーティションにルーティングされます。
>
>詳しくは、[Sumo Logic パーティションドキュメント](https://help.sumologic.com/docs/search/optimize-search-partitions/)を参照してください。
