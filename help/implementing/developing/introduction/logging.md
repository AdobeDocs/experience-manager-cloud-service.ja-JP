...
DEBUG</td>
<td>
アプリケーションでの出来事について説明します。DEBUG ロギングがアクティブな場合は、どのアクティビティが発生するかを明確に示すステートメントと、処理に影響を与える主要なパラメーターがログに記録されます。</td>
<td>
<ul>
<li> ローカル開発</li>
<li>開発</li>
...
WARN</td>
<td>
エラーになる可能性のある条件を説明します。WARN ロギングがアクティブな場合は、サブ最適化に近づいている条件を示すステートメントのみがログに記録されます。</td>
<td>
<ul>
<li> ローカル開発</li>
<li>開発</li>
...
ERROR</td>
<td>
エラーを示す条件と解決する必要がある状態を説明します。ERROR ロギングがアクティブな場合、失敗を示す文のみがログに記録されます。 ERROR ログステートメントは、重大な問題を示しており、できるだけ早く解決する必要があります。</td>
<td>
<ul>
<li> ローカル開発</li>
<li>開発</li>
...
AEM as a Cloud Service の HTTP リクエストログは、AEM に対して行われた HTTP リクエストと、それに対する HTTP レスポンスに関する情報を時系列順に提供します。 このログは、AEM に対して行われる HTTP リクエストと、それらの要求が処理され応答される順序を理解するのに役立ちます。
...
AEM as a Cloud Service の HTTP アクセスログは、HTTP リクエストを時系列順に表示します。 各ログエントリは、AEM にアクセスする HTTP リクエストを表します。
