...
* **`self`** - サブツリーを検索しますが、パスとして指定されたベースノードが含まれます（ワイルドカードは不可）。
   * *重要な注意*:問題が `self` プロパティがクエリで使用されている場合、正しい検索結果が得られない可能性があります。 現在のの実装の変更 `self` また、プロパティに依存する既存のアプリケーションが壊れる可能性があるので、プロパティは実行できません。 このため `self` プロパティは非推奨（廃止予定）となったので、使用しないことをお勧めします。
...
下限、上限またはその両方を定義できます。また、下限と上限に対して、操作（例えば、以下）を個別に指定することもできます。
