...
>[!TIP]
>
>[AEM プロジェクトアーキタイプ](https://experienceleague.adobe.com/ja/docs/experience-manager-core-components/using/developing/archetype/overview#)の基本的な使用法と、[FileVault コンテンツパッケージ Maven プラグイン](/help/implementing/developing/tools/maven-plugin.md)について説明します。この記事では、これらの概念の理解を前提としています。
...
## リポジトリの可変エリアと不変エリア {#mutable-vs-immutable}

AEMの`/apps`および`/libs`領域は&#x200B;**不変**&#x200B;です。 AEMの開始後、実行時にこれらの領域でコンテンツを作成、更新、または削除することはできません。 実行時に不変領域を変更しようとすると失敗します。
...
AEM as a Cloud Service デプロイメントプロセスは、Oak インデックス （`/oak:index`）を管理します。 これは、新しいインデックスが展開されて完全にインデックスが再作成されるまで、Cloud Manager は新しいコードイメージに切り替わる前に待機する必要があるからです。

このため、Oak インデックスは実行時に可変ですが、可変パッケージがインストールされる前にインストールできるように、コードとしてデプロイする必要があります。したがって、`/oak:index`設定はコードパッケージの一部であり、以下の説明に従ってコンテンツパッケージ [の一部ではありません](#recommended-package-structure)。
...
+ `ui.apps` パッケージは、デプロイされるすべてのコードを含んでおり、`/apps` にのみデプロイされます。`ui.apps` パッケージの共通要素には次のものがありますが、これらに限定されるわけではありません。
   + [コンポーネントの定義と HTL](https://experienceleague.adobe.com/ja/docs/experience-manager-htl/content/overview#) スクリプト
   ...
   + [事前コンパイル済みバンドルスクリプト](https://experienceleague.adobe.com/ja/docs/experience-manager-core-components/using/developing/archetype/precompiled-bundled-scripts#)

>[!NOTE]
>
>同じコードをすべての環境にデプロイする必要があります。このコードは、ステージ環境での検証も実稼動環境にあるという信頼性を保証します。 詳しくは、「[実行モード](/help/implementing/deploying/overview.md#runmodes)」の節を参照してください。


### コンテンツパッケージ

+ `ui.content` パッケージには、すべてのコンテンツと設定が含まれています。コンテンツパッケージには、`ui.apps`または`ui.config` パッケージ以外のすべてのノード定義が含まれています。つまり、`/apps`または`/oak:index`以外のノード定義が含まれています。 `ui.content` パッケージの共通要素には次のものがありますが、これらに限定されるわけではありません。
   + コンテキスト対応の設定
      + `/conf`
   ...
   + 管理されるタグ付け分類
      + `/content/cq:tags`
   + レガシー`etc` ノード （理想的には、これらのノードを非等の場所に移行します）
      + `/etc`

### コンテナパッケージ

+ `all` パッケージはコンテナパッケージです。 デプロイ可能なアーティファクトのみを埋め込みファイルとして含めます。OSGi バンドル JAR ファイルと、`ui.apps`、`ui.config`、および`ui.content` パッケージです。 `all` パッケージにはそれ自体の&#x200B;**コンテンツやコード**&#x200B;を含めることはできず、リポジトリへのあらゆるデプロイメントをサブパッケージまたは OSGi バンドル JAR ファイルに委任します。

  `<subPackages>` 設定ではなく、[FileVault パッケージ Maven プラグインの埋め込み設定](#embeddeds)を使用して、パッケージが組み込まれるようになりました。

  Experience Manager の複雑なデプロイメントの場合は、AEM 内の具体的なサイトまたはテナントを表す複数の `ui.apps`、`ui.config` および `ui.content` プロジェクト／パッケージを作成するほうが望ましいことがあります。このアプローチを使用する場合は、可変コンテンツと不変コンテンツの分割を尊重します。 また、必要なコンテンツパッケージとOSGi バンドル JAR ファイルを`all` コンテナコンテンツパッケージのサブパッケージとして埋め込みます。
  ...
  複雑なデプロイメントコンテンツパッケージ構造は、例えば、次のようになります。
   ...
   + `all` コンテンツパッケージに次のパッケージが埋め込まれて、単一のデプロイメントアーティファクトが作成されます。
      ...
      + サイト Aで必要な`site-a.core` OSGi バンドル Jar
      ...
      + サイト Bに必要な`site-b.core` OSGi バンドル Jar
...
+ `ui.config` パッケージには、すべての [OSGi 設定](/help/implementing/deploying/configuring-osgi.md) が含まれています。
   + OSGi バンドルに属するが、通常のコンテンツノードを含まないコードと見なされます。 したがって、コンテナパッケージとしてマークされます
   + 実行モード固有の OSGi 構成定義を含む組織フォルダー
      + `/apps/my-app/osgiconfig`
   + すべてのtarget AEM as a Cloud Service デプロイメントターゲットに適用されるデフォルトのOSGi設定を含む共通のOSGi設定フォルダー
...
### 追加アプリケーションパッケージ{#extra-application-packages}
...
+ `all` コンテンツパッケージに次のパッケージが埋め込まれて、単一のデプロイメントアーティファクトが作成されます。
   + AEM アプリケーションに必要な`core`個のOSGi バンドル Jar
...
## Adobe Cloud Managerによるデプロイメント用パッケージのマーク {#marking-packages-for-deployment-by-adoube-cloud-manager}
...
Repo Initの主な利点は、スクリプトによって定義されたすべてのアクションを実行するための暗黙的な権限を持つことです。 また、このようなスクリプトはデプロイメントライフサイクルの初期段階で呼び出され、コードの実行時までにすべての必須JCR構造が存在することを確認します。
...
Repo Init スクリプトは、`RepositoryInitializer` OSGi ファクトリ設定の `scripts` エントリとして保存されます。実行モードは暗黙的にそれらをターゲットにできます。 このターゲティングにより、AEM オーサーとAEM パブリッシュサービス間、または開発、ステージ、実稼動環境の間でリポジトリ Init スクリプトを区別できます。

リポジトリ初期化OSGi設定は、複数行をサポートするため、[`.config` OSGi設定形式](https://sling.apache.org/documentation/bundles/configuration-installer-factory.html#configuration-files-config-1)で作成するのが最適です。これは、[`.cfg.json`を使用してOSGi設定](https://sling.apache.org/documentation/bundles/configuration-installer-factory.html#configuration-files-cfgjson-1)を定義するベストプラクティスの例外です。

ユーザーとグループを定義する場合、グループのみがアプリケーションの一部と見なされ、その機能に不可欠です。 実行時に AEM で組織のユーザーとグループを引き続き定義します。例えば、カスタムワークフローが名前付きのグループに作業を割り当てる場合、AEM アプリケーションで Repo Init を使用してそのグループを定義します。ただし、「Wendy&#39;s Team」や「Sean&#39;s Team」など、グループ化が単なる組織である場合は、これらのグループはAEMで実行時に定義および管理するのが最適です。

>[!TIP]
>
>Repo Init スクリプトは、インライン `scripts` フィールドで定義する&#x200B;*必要があり*、`references` 設定は機能しません。

Repo Init スクリプトの完全な語彙については、[Apache Sling Repo Init ドキュメント &#x200B;](https://sling.apache.org/documentation/bundles/repository-initialization.html#the-repoinit-repository-initialization-language)を参照してください。
...
コンテンツまたはコードパッケージは、特別なサイドカーフォルダーに配置されます。 FileVault Maven プラグインの`<embeddeds>`設定を使用して、AEM オーサー、AEM パブリッシュ、またはその両方へのインストールをターゲットにします。 `<subPackages>` 設定は使用しないでください。
...
+ 第 2 レベルのフォルダーは、フォルダー名の末尾に `-packages` が付いたアプリケーションを表します。多くの場合、すべてのサブパッケージが埋め込まれる第2 レベルのフォルダーは1つだけですが、アプリケーションの最適な論理構造を表すために、任意の数の第2 レベルのフォルダーを作成できます。
  ...
  >[!WARNING]
  >
  >慣例により、サブパッケージが埋め込まれるフォルダーの名前には、`-packages` という接尾辞が付けられます。この命名により、デプロイメントコードとコンテンツパッケージが&#x200B;**not**&#x200B;で`/apps/<app-name>/...`の下のサブパッケージのターゲットフォルダーにデプロイされていることを確認できます。 この方法は、破壊的で周期的なインストール動作を防ぎます。

+ 第 3 レベルのフォルダーは、
  `application`、`content`、`container`
   + `application` フォルダーにはコードパッケージが格納されます。
   + `content` フォルダーにはコンテンツパッケージが格納されます。
   + `container` フォルダーには、AEM アプリケーションに含まれていない[追加アプリケーションパッケージ &#x200B;](#extra-application-packages)が含まれています。
...
+ 第 4 レベルのフォルダーにはサブパッケージを格納します。このフォルダーは次のいずれかにする必要があります。
   ...
   + `install.publish`様が&#x200B;**only**&#x200B;をAEM パブリッシュにインストールしました
`install.author`と`install.publish`のみがサポート対象です。 その他の実行モードはサポートされて&#x200B;**いません**。
...
## `ui.apps` パッケージからの`ui.content`間のパッケージ依存関係 {#package-dependencies}

パッケージを適切にインストールするには、パッケージ間の依存関係を確立することをお勧めします。

一般的なルールとしては、可変コンテンツを格納したパッケージ（`ui.content`）は、可変コンテンツのレンダリングと使用をサポートする不変コード（`ui.apps`）に依存します。

この一般的なルールの例外として重要なのは、不変コードパッケージ（`ui.apps` など）に OSGi バンドル&#x200B;__のみ__&#x200B;含まれている場合です。この場合、AEM パッケージでは不変コードパッケージへの依存関係を宣言する必要はありません。その理由は、OSGi バンドル&#x200B;__のみ__&#x200B;が含まれる不変コードパッケージが、AEM [パッケージマネージャー](/help/implementing/developing/tools/package-manager.md)に登録されていないからです。したがって、依存しているAEM パッケージには依存関係がなく、インストールに失敗します。
...
### シンプルなデプロイメントパッケージ依存関係 {#simple-deployment-package-dependencies}
...
### 複雑なデプロイメントパッケージ依存関係 {#complex-deploxment-package-dependencies}

複雑なデプロイメントは、単純なケースに展開し、対応する可変コンテンツと不変コードパッケージの間の依存関係を設定します。 必要に応じて、不変コードパッケージ間にも依存関係を設定できます。
...
この記事で説明するプロジェクト構造と組織は、ローカル開発のAEM インスタンスと&#x200B;**完全に互換性があります**。
...
### Adobe Cloud Manager デプロイメント用パッケージのマーク {#cloud-manager-target}

パッケージを生成するすべてのプロジェクトで、コンテナ （**） プロジェクトの**&#x200B;を除く`all`で、`<cloudManagerTarget>none</cloudManagerTarget>` プラグイン宣言の`<properties>`設定に`filevault-package-maven-plugin`を追加して、Adobe Cloud Managerがデプロイしないように**します。 コンテナ（`all`）パッケージは、Cloud Manager を通じてデプロイされる単一のパッケージでなければなりません。このパッケージに、必要なすべてのコードパッケージとコンテンツパッケージが埋め込まれます。
...
Repo Init スクリプトを含む Repo Init スクリプトは、`scripts` プロパティを介して `RepositoryInitializer` OSGi ファクトリ設定で定義されます。これらのスクリプトはOSGi設定内で定義するため、通常の`../config.<runmode>` フォルダーのセマンティクスを使用して実行モードで簡単にスコープを設定できます。
...
### サードパーティのMaven リポジトリ {#xml-3rd-party-maven-repositories}

>[!WARNING]
>
>Maven リポジトリーを追加すると、追加のMaven リポジトリーが依存関係をチェックする際に、Mavenのビルド時間が延長される場合があります。
...
### `ui.apps` パッケージからの`ui.content`間のパッケージ依存関係 {#xml-package-dependencies}
...
### コンテナプロジェクトのターゲットフォルダーのクリーニング {#xml-clean-container-package}
