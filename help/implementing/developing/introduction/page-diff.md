...
ページのバージョンを比較する場合、差分を検出しやすくするために、比較対象となる以前のバージョンが AEM によってバックグラウンドで再作成されます。この以前のバージョンは、[並べて比較するための](/help/sites-cloud/authoring/features/page-diff.md)コンテンツのレンダリングに必要です。

この再作成操作は AEM の内部でおこなわれるもので、ユーザーに対しては透過的であり、ユーザーの介入は必要ありません。ただし、管理者がリポジトリを閲覧している場合は、再作成されたCRXDE Lite（再作成されたバージョン）がコンテンツ構造内に表示されます。
