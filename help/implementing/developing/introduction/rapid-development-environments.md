...
各RDE環境には最大7つのスナップショットがあります。 削除用にマークされているものの、まだ7日間の保持期間内にあるスナップショットは、完全に削除されるまで、その制限に向かってカウントされ続けます。 新しいスナップショットの制限に達し、容量が必要な場合は、標準の削除ではなく、[&#x200B; スナップショットの削除](#delete-snapshot)の説明に従って強制削除を使用します。
...
### スナップショットの削除の取り消し {#undelete-snapshot}
