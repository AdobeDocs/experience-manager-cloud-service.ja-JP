...
>[!NOTE]
>
>**新しい機能強化：RDE スナップショット**
>
>公開ベータ版では、RDE スナップショットを作成して、後でコードとコンテンツを復元できます。 [詳細情報](#snapshots)を以下に示します。
>
...
作成時に、RDE は使用可能な最新の Adobe Experience Manager（AEM）バージョンに設定されます。 Cloud Managerを使用して実行できるRDE リセットは、RDEをサイクルし、最新のAEM バージョンに設定します。
...
## プログラムでRDEを有効にする {#enable-rde-in-a-program}
...
作成時に、RDE は使用可能な最新の AEM バージョンに設定されます。 Cloud Managerを使用して実行できるRDE リセットでは、RDEがサイクルされ、最新のAEM バージョンに設定されます。
...
## RDE コマンドラインツールのインストール {#install-the-rde-command-line-tools}
...
1. Adobe I/O（AIO）クライアントを使用してログインします。
   ...
   特定のコンテキスト内の現在のディレクトリにある「.aio」ファイルにログイン情報をローカルに保存するには、次の手順に従います。 また、コンテキストは、CI/CD 環境またはスクリプトを設定する賢明な方法です。  この機能を利用するには、少なくともaio-cli バージョン 10.3.1を使用してください。 'npm install -g @adobe/aio-cli'を使用して更新します。
...
1. 自社の組織、プログラムおよび環境を使用するように RDE プラグインを設定します。 以下の setup コマンドでは、組織内のプログラムのリストをインタラクティブにユーザーに提示し、そのプログラム内の選択可能な RDE 環境を表示します。
   ...
   スクリプト化された環境を使用している場合は、設定手順をスキップできます。 その場合は、組織、プログラム、環境の値を各コマンドに直接含めます。 [詳しくは、以下の RDE コマンドを参照してください](#rde-command-line-tools-commands)。

### インタラクティブ設定 {#interactive-setup}
...
### 非インタラクティブ環境での手動設定手順 {#manual-setup-procedure-in-a-non-interactive-environment}
...
詳細とデモについては、ビデオ チュートリアル [RDEの設定方法（06:24） ](https://experienceleague.adobe.com/ja/docs/experience-manager-learn/cloud-service/developing/rde/how-to-setup)をご覧ください。
</details>

## 新しい機能の開発中にRDEを使用する {#use-rde-while-developing-a-new-feature}
...
* RDE が別の機能で使用されていて、[デフォルトの状態にリセット](#reset-the-rde)したい場合は、RDE をリセットします。 <!-- Alexandru: hiding for now, do not delete This can be done by way of [Cloud Manager](#reset-the-rde-in-cloud-manager) or by way of the [command line](#reset-the-rde-using-the-command-line). -->リセットには数分かかり、既存のコンテンツおよびコードはすべて削除されます。 RDE ステータスコマンドを使用すると、RDE の準備が完了したことを確認できます。 RDE は、最新の AEM リリースバージョンで復活します。
...
* RDE コマンドラインインターフェイスを使用して、ローカルコードを RDE に同期します。 以下を含む様々なタイプのファイルをインストールできます。
  ...
  リモートコンテンツパッケージの参照も可能です。 詳しくは、[RDE コマンドラインツール](/help/implementing/developing/introduction/rapid-development-environments.md#rde-command-line-tools-commands)を参照してください。 ステータスコマンドを使用すると、デプロイメントが成功したことを検証できます。 必要に応じて、パッケージマネージャーを使用してコンテンツパッケージをインストールします。
...
## RDEを使用して既存の機能をデバッグする {#use-rde-to-debug-an-existing-feature}
...
## 同じRDEで複数の開発者が共同作業する {#multiple-developers-collaborating-on-the-same-rde}
...
## RDE コマンドラインツールコマンド {#rde-command-line-tools-commands}
...
#### コンテンツパッケージのデプロイ {#deploy-a-content-package}
...
#### OSGi設定のデプロイ {#deploy-an-osgi-configuration}
...
#### コンテンツファイルのデプロイ {#deploy-a-content-file}
...
#### Apache/Dispatcher設定のデプロイ {#deploy-an-apache-dispatcher-configuration}
...
#### 設定パイプライン関連設定のデプロイ（YAML設定） {#deploy-config-pipeline-related-configuration-yaml-configurations}
...
### サイトテーマとサイトテンプレートに基づいてフロントエンドコードをデプロイする {#deploy-front-end-code-based-on-site-themes-and-site-templates}
...
### RDEのステータスの確認 {#check-the-status-of-the-rde}
...
詳細とデモについては、ビデオチュートリアル [RDE コマンドの使用方法（10:01） ](https://experienceleague.adobe.com/ja/docs/experience-manager-learn/cloud-service/developing/rde/how-to-use)を参照してください。


## 外部Git プロバイダーからのRDEへのデプロイ {#deploy-to-an-rde-from-external-git-providers}
...
* 現在、コメントを使用した RDE 環境のリセットはサポートされていません。 代わりに、[こちらで説明](/help/implementing/developing/introduction/rapid-development-environments.md#reset-the-rde-using-the-command-line)されているように、既存の AIO CLI リセットコマンドを使用する必要があります。
...
[Cloud Manager](#reset-the-rde-in-cloud-manager) または[コマンドライン](#reset-the-rde-using-the-command-line)を使用して RDE をリセットできます。 リセットには数分かかり、既存のコンテンツおよびコードはすべて RDE から削除されます。

>[!NOTE]
>
>リセット機能を使用する前に、Cloud Manager デベロッパーの役割を割り当てます。 割り当てない場合、リセットアクションはエラーで失敗します。

### コマンドラインを使用したRDEのリセット {#reset-the-rde-using-the-command-line}
...
環境の準備が再び整ったことを確認するには、[ステータスコマンド](#check-the-status-of-the-rde)を使用します。

### Cloud Manager での RDE のリセット {#reset-the-rde-in-cloud-manager}
...
>[!NOTE]
>RDE スナップショットはパブリックベータ版であるため、Adobeに連絡することなく、最新のaio プラグインにアップデートすることで有効にできます。
>
>RDE スナップショット Betaを使用することにより、まだ開発中であり、テクノロジーの正しい機能やデータの可用性に頼るべきではないことを認めます。 この機能を広範囲にテストしましたが、RDEが不安定になる可能性は少しあります。 これが発生した場合、リセットは動作状態に戻します。[aemcs-rde-support@adobe.com](mailto:aemcs-rde-support@adobe.com)にご意見をお寄せください。

RDEは、コードとコンテンツの現在の状態のスナップショットを取得することをサポートします。これは、後で復元できます。 スナップショットは、元に戻す必要があるコードを同期する場合や、異なる機能の開発を切り替える場合に便利です。 また、スナップショットの可変コンテンツのみをテストの既知の出発点として復元することも可能です。

特定のRDEのスナップショットは、別のRDEでは復元できないことに注意してください。

各RDE環境には最大7つのスナップショットがあります。 削除用にマークされているものの、まだ7日間の保持期間内にあるスナップショットは、完全に削除されるまで、その制限に向かってカウントされ続けます。 新しいスナップショットの制限に達し、容量が必要な場合は、標準の削除ではなく、[ スナップショットの削除](#delete-a-snapshot)の説明に従って強制削除を使用します。

サポートされるコマンドについては以下で説明します。 フラグとオプションの完全なリストについては、`aio aem rde snapshot --help`を使用するか、特定のサブコマンドに関するヘルプについては、`aio aem rde snapshot <subcommand> --help`を使用してください。

### スナップショットのリスト {#list-snapshots}

以下を実行することで、組織内のすべてのスナップショットを一覧表示できます。

`aio aem rde snapshot`

これにより、使用可能なスナップショットのテーブルが返されます。このテーブルは、`-s` フラグを使用して並べ替えることができます。

`aio aem rde snapshot -s <column-header>`

列ヘッダーの先頭には、逆順に並べ替えるマイナス記号を付けます。 `--json` グローバル フラグもサポートされています。

### スナップショットの作成 {#create-a-snapshot}

コンテンツとデプロイメントの両方を含むRDEの現在の状態のスナップショットを作成するには、次を実行します。

`aio aem rde snapshot create <name>`

ここで、`<name>`は、環境内のスナップショットの一意の名前です。 必要に応じて、`-d` フラグに簡単な説明を含めます。

`aio aem rde snapshot create <name> -d "description of the snapshot"`

### スナップショットの復元 {#restore-a-snapshot}

スナップショットを現在のRDEに復元するには、次を実行します。

`aio aem rde snapshot restore <name>`

（デプロイメントを復元せずに）スナップショットから可変コンテンツのみを復元するには、`--only-mutable-content` フラグを使用します。

`aio aem rde snapshot restore <name> --only-mutable-content`

### スナップショットの削除 {#delete-a-snapshot}

削除するスナップショットにマークを付けても、すぐに削除されるわけではありません。 スナップショットは7日後に削除され、必要に応じて削除を取り消す時間が与えられます。

削除するスナップショットにマークを付けるには、次を実行します。

`aio aem rde snapshot delete <name>`

すべてのスナップショットを一度に削除済みとしてマークするには、`-a` フラグを使用します。

`aio aem rde snapshot delete -a`

スナップショットをすぐに削除するには（環境スナップショットの制限にカウントされないように保持期間をスキップします）、`-f` フラグ （または`--force`）を追加します。

`aio aem rde snapshot delete <name> -f`

強制削除は`undelete`では元に戻せません。 フラグを組み合わせる場合は、オプションの完全なセットに`aio aem rde snapshot delete --help`を使用します（強制的にすべてのスナップショットを削除するなど）。

### スナップショットの削除を解除 {#undelete-a-snapshot}

保留中の削除をキャンセルしてスナップショットを保持するには、次のコマンドを実行します。

`aio aem rde snapshot undelete <name>`

## JSON出力をサポートするコマンド {#commands-that-support-json-output}
...
#### 一部のバンドルがインストールされたRDE {#an-rde-with-some-installed-bundles}
...
#### 完了を待ち、正常にリセット {#wait-for-completion-reset-successfully}
...
#### 完了を待つ、リセットに失敗しました {#wait-for-completion-reset-failed}
...
## コンテンツを設定 {#populate-with-content}
...
## RDEとクラウド開発環境の違い？ {#how-are-rdes-different-from-cloud-development-environments}
...
## 必要なRDEの数 {#how-many-rdes-do-i-need}
...
## AEM Forms Cloud Service RDEと他の環境の違い？ {#how-is-an-aem-forms-cloud-service-rde-different-from-other-environments}

Forms のデベロッパーは、AEM Forms Cloud Service の高速開発環境を使用して、アダプティブフォーム、ワークフロー、およびコアコンポーネントのカスタマイズ、サードパーティシステムとの統合などのカスタマイズを迅速に開発できます。 AEM Forms Cloud Service の高速開発環境（RDE）は、通信 API をサポートしていません。 また、アダプティブフォームの送信時にレコードのドキュメントを生成するなど、レコードのドキュメントを必要とする機能や機能もサポートしていません。 以下に示す AEM Forms の機能は、高速開発環境（RDE）では使用できません。
...
## RDE チュートリアル {#rde-tutorial}

AEM as a Cloud ServiceのRDEについて詳しくは、[設定の方法、使用方法、開発ライフサイクル（01:25） ](https://experienceleague.adobe.com/ja/docs/experience-manager-learn/cloud-service/developing/rde/overview)を示すビデオチュートリアルを参照してください。

## トラブルシューティング {#troubleshoot}

### RDEのトラブルシューティング {#rde-troubleshooting}

#### 既存のRDEの最新のAEM バージョンの取得方法 {#how-to-obtain-the-latest-aem-version-for-an-existing-rde}

作成時に、RDE は使用可能な最新の Adobe Experience Manager（AEM）バージョンに設定されます。 Cloud Managerまたは`aio aem:rde:reset` コマンドを使用して実行できる[RDE リセット ](#reset-the-rde)は、RDEをサイクルし、最新のAEM バージョンに設定します。

### AIO RDE プラグインのトラブルシューティング {#troubleshoot-aio-rde-plugin}

#### 不十分な権限に関するエラー {#errors-regarding-insufficient-permissions}
