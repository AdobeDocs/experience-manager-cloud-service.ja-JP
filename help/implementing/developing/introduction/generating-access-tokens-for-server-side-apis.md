...
AEM CS サービス認証情報は、この目的のために設計されたコードサンプルを使用して、アクセストークンと交換される場合があります。 サンプルコードは、[Adobeのパブリック GitHub リポジトリ &#x200B;](https://github.com/adobe/aemcs-api-client-lib)から入手できます。このリポジトリには、独自のプロジェクトにコピーして適用できるコード例が含まれています。 このリポジトリには参照用のサンプルコードが含まれており、実稼動用のライブラリ依存関係として維持されていないことに注意してください。
...
### 個人証明書の失効に関する注意 {#note-on-recovacting-individual-certificates}

JWT ハンドシェイク（ベアラートークンの取得に使用）の場合、次の条件を満たす必要があります。

1. その秘密鍵があります
1. Developer Consoleのそれぞれの秘密鍵の下に1つ以上のアクティブな証明書が存在します
1. トークン（JWT ハンドシェイク）の取得中に、IMSはJWT署名が、コンソールに表示されるシステムのレコード上のバインド済みおよびアクティブな（期限切れでない）証明書と一致することを確認します。

PKの下に新しい証明書を追加すると、失効した証明書が引き続き使用できる場合があります。 実際には、PKの下のすべての証明書は同等です。 1つがアクティブな場合、すべてがアクティブとみなされます。

これがセキュリティ上の問題であると考えられる場合は、別の秘密鍵を作成し、古い秘密鍵のすべての証明書を取り消す必要があります。
