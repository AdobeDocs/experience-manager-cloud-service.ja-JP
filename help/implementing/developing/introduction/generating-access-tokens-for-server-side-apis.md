...
### 個人証明書の失効に関する注意事項 {#note-on-recovacting-individual-certificates}

JWT ハンドシェイク（ベアラートークンを取得するために使用）の場合、次の条件をすべて満たす必要があります。

1. 秘密キーがあります
1. Developer Consoleの各秘密鍵の下には、アクティブな証明書が 1 つ以上存在します
1. トークン（JWT ハンドシェイク）の取得中、IMS は JWT 署名が、システム内に記録されているバインドされたアクティブな（期限切れでない）証明書と一致するかどうかをチェックします。この証明書は、コンソールに表示されます。

PK の下に新しい証明書を追加すると、失効した証明書がまだ使用可能のように見える場合があります。 実際には、PK の下のすべての証明書は同等です。 1 つのがアクティブな場合、すべてがアクティブと見なされます。

これがセキュリティ上の問題であると考える場合は、別の秘密鍵を作成し、古い秘密鍵に対するすべての証明書を失効させる必要があります。
