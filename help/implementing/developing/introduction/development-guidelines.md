...
AEMは、Cloud Serviceとして送信メールを暗号化する必要があります。 以下の節では、電子メールのリクエスト、設定、送信方法について説明します。

### アクセスの要求 {#requesting-access}

デフォルトでは、送信電子メールは無効になっています。 アクティブ化するには、次を含むサポートチケットを送信します。

1. メールサーバーの完全修飾ドメイン名(例 `smtp.sendgrid.net`)
1. 使用するポート。 メールサーバがサポートしている場合はポート465にする必要があります。それ以外の場合は、ポート587は、メールサーバが要求し、そのポートでTLSを適用する場合にのみ使用できます。
1. メールアウトする環境のプログラムIDと環境ID
1. 作成者、発行、またはその両方でSMTPアクセスが必要かどうか。

### 電子メールの送信 {#sending-emails}

[Day CQ Mail Service OSGIサービスを使用し](https://docs.adobe.com/content/help/en/experience-manager-65/administering/operations/notification.html#configuring-the-mail-service) 、受信者に直接送信するのではなく、サポート要求に示されたメールサーバーに電子メールを送信する必要があります。
...
AEM内の電子メールは、 [Day CQ Mail Service OSGiサービスを使用して送信する必要があります](https://docs.adobe.com/content/help/en/experience-manager-65/administering/operations/notification.html#configuring-the-mail-service)。

電子メール設定の設定について詳しくは、 [AEM 6.5のドキュメント](https://docs.adobe.com/content/help/en/experience-manager-65/administering/operations/notification.html) を参照してください。 AEM CSの場合、このサー `com.day.cq.mailer.DefaultMailService OSGI` ビスでは次の調整を行う必要があります。

ポート465が要求された場合：

* ～に向 `smtp.port` かう `465`
* ～に向 `smtp.ssl` かう `true`
* ～に向 `smtp.starttls` かう `false`

ポート587が要求された場合（メールサーバがポート465をサポートしていない場合のみ可）:

* ～に向 `smtp.port` かう `587`
* ～に向 `smtp.ssl` かう `false`
* ～に向 `smtp.starttls` かう `true`