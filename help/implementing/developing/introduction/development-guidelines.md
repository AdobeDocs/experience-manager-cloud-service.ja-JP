...
このドキュメントでは、AEM as a Cloud Serviceでの開発に関するガイドラインと、AMSにおけるオンプレミスのAEMおよびAEMとの違いについて説明します。
...
トラブルを最小限に抑えるためには、可能であれば長時間稼働する作業を避け、少なくとも再開する必要があります。 このようなジョブを実行するには、Sling ジョブを使用します。Sling ジョブは、少なくとも1回は保証されているため、中断された場合、できるだけ早く再実行されます。 しかし、最初から再開すべきではありません。 このようなジョブをスケジュールするには、[Sling Jobs](https://sling.apache.org/documentation/bundles/apache-sling-eventing-and-job-handling.html#jobs-guarantee-of-processing) スケジューラーを使用することをお勧めします。これにより、少なくとも1回の実行が保証されます。

実行は保証できないため、Sling Commons Schedulerをスケジュールに使用しないでください。 スケジュール設定されている可能性が高いです。

同様に、観察イベント（JCR イベントやSling リソースイベントなど）に対してアクションを実行するなど、非同期に発生するあらゆる処理では、ジョブが実行されることを保証できないため、慎重に使用する必要があります。 これは、現在AEMのデプロイメントでは既に当てはまります。
...
動作することが知られている代替案（ただし、自分で依存関係を提供する必要がある場合があります）は次のとおりです。
...
タイムアウトを提供することに加えて、これらのタイムアウトと予期しないHTTP ステータスコードの適切な処理も実装する必要があります。
...
開発環境と迅速な開発環境は、開発、エラー分析、機能テストに限定する必要があり、高いワークロードや大量のコンテンツを処理するように設計されていません。
