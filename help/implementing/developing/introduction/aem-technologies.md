...
AEM は、拡張性が高く柔軟なテクノロジーに基づいて構築された実証済みの堅牢なプラットフォームです。このドキュメントでは、AEMを構成する様々な部分の詳細な概要を説明し、フルスタックAEM開発者向けの技術的な付録として用意されています。 はじめる前のガイドとしての意図はありません。AEM開発を初めておこなう場合は、 [AEM Sitesの開発の手引き — WKND チュートリアル](develop-wknd-tutorial.md) を最初のステップとして使用します。
