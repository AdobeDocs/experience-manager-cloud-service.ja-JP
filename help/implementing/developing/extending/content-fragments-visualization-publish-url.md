...
1つまたは複数のアタッチされたHTML テンプレートを持つモデルに基づくコンテンツフラグメントが公開されると、そのフラグメントのレンダリングされたHTMLは、次の構造を持つURLのAdobe Experience Manager（AEM）as a Cloud Service パブリッシュ層を介して使用可能になります。
