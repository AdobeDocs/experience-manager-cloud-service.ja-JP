...
作成者は、スタイルを適用して、テキストの一部の外観を変更できます。 スタイルは、CSS スタイルシートで事前に定義した CSS クラスに基づいています。 スタイル設定したコンテンツは、CSS クラスを参照する `span` 属性を使用して `class` タグで囲まれます。次に例を示します。
...
1. プロパティテキストを同じノードに追加します。 値は、スタイルを選択する際に作成者に表示されるスタイルの名前です。
   * 名前： `text`
*タイプ： `String`
...
段落書式プラグインを初めて有効にした場合、デフォルトの段落書式は使用できません。 ポップアップリストが空です。 段落書式を使用できるようにするには、次の操作をおこないます。
...
>[!CAUTION]
>
>RTE の段落書式を設定する際に、書式オプションとしての段落タグ &lt;p> を削除しないでください。次の場合、 `<p>` タグが削除された場合、コンテンツ作成者は [!UICONTROL 段落書式] オプションを使用できます。
...
CRXDE でプロパティを保存すると、HTML 表現された文字が表示されます。半分の例を以下に示します。 上記の手順を繰り返して、作成者がさらに特殊文字を使用できるようにします。
...
1. これらを参照する CSS スタイルシートの場所を定義します。 詳しくは、 [スタイルシートの場所の指定](#locationofstylesheet) これは、 [テキストのスタイル](#textstyles). 他のスタイルを定義した場合は、場所を定義できます。
...
辞書を追加する場合は、必要に応じて次の手順に従います。

1. ページ [https://extensions.openoffice.org/](https://extensions.openoffice.org/) に移動します。
1. 必要な言語を選択し、スペル定義を含む ZIP ファイルをダウンロードします。 アーカイブの内容をファイルシステム上に抽出します。
...
>[!NOTE]
>
RTE スペルチェッカーは、オンデマンドで使用できます。テキストの入力を開始しても、自動的には実行されません。
>
スペルチェッカーを実行するには、ツールバーの「スペルチェッカー」ボタンを選択します。 RTE は単語のスペルをチェックし、スペルミスのある単語をハイライトします。
>
スペルチェッカーが提案する変更を組み込むと、テキストの状態が変更され、スペルミスのある単語はハイライト表示されなくなります。 スペルチェッカーを実行するには、「スペルチェッカー」ボタンを再度選択します。
