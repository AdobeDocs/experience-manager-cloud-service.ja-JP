...
### テーブルの非表示のヘッダーをアクセシビリティ用に設定 {#hiddenheader}
...
スペルチェッカーを実行するには、ツールバーの「スペルチェッカー」ボタンをタップまたはクリックします。 RTE は単語のスペルをチェックし、スペルミスのある単語をハイライトします。
...
任意のテキスト内でタブ文字を押すと、事前に定義された数のスペースが挿入されます。デフォルトでは、これは 3 つの改行なしスペースと 1 つのスペースです。
