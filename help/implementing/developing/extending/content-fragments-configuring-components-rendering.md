...
* 公開中に依存関係を正しく判断する（前回の公開以降に変更が加えられた場合は、フラグメントとモデルをページと共に自動的に公開できるようにする）。
...
一部の機能については、コンポーネントは定義済みの規則に従う必要があります。次の表に、各段落に対してコンポーネントごとに定義する必要があるプロパティ ( `jcr:paragraph` 各コンポーネントインスタンスの ) を確認し、サービスが正しく検出して処理できるようにする必要があります。
