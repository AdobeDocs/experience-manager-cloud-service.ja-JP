---
title: Cloud Service [!DNL Adobe Experience Manager] 内にコンテンツをオーサリングするようにリッチテキストエディタを設定します。
description: Cloud Service [!DNL Adobe Experience Manager] 内にコンテンツを作成するようにリッチテキストエディターを設定します。
contentOwner: AG
...
ツールバーには、WYSIWYGオーサリング環境で使用できるオプションが用意されています。 [!DNL Experience Manager] 管理者は、インターフェイスのツールバーで使用できるオプションを設定できます。 では、包括的な編集オプションのセットをデフォルトで使用でき [!DNL Experience Manager]ます。 開発者は、カスタマイズ [!DNL Experience Manager] してさらに編集オプションを追加できます。
...
| ダイアログ | ページコンテンツの上面にダイアログボックスが表示されるが、ページ全体に広がらない。 | 機能は慎重に有効にしてください。 |
...
[!DNL Experience Manager] コンポーネントは、ページコンテンツを非表示にして、利用可能な画面を占めるフルスクリーン表示で開くことができます。 最も多くの編集オプションをオファーするので、インライン編集の詳細バージョンはフルスクリーン編集にすることを検討してください。 インライン編集モードを使用する場合は、 ![アイコンをクリックしてコンパクトツールバーからフルスクリーンでRTEを開く](assets/rte_fullscreen.png)。
...
| [画像](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/widgets-api/index.html?class=CQ.form.rte.plugins.ImagePlugin) | `image` | 基本的な画像のサポート（コンテンツまたはコンテンツファインダーからのドラッグ） ブラウザーによって、サポートされる動作が作成者によって異なります |
...
* RTE capabilities are supported only in [!DNL Experience Manager] component dialogs. RTEは、ウィザードまたはFoundation-formsではサポートされていません。
