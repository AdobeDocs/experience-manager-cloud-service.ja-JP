...
#### 標準アセットの場合、コンテンツフラグメントは次の場所に保持されます。{#asset-location}
...
#### Assets コアと統合するには：{#feature-integration}
...
>[コンテンツフラグメントコンポーネントは、コアコンポーネントの一部です[#$tu71]。詳しくは、[コアコンポーネントの開発[#$tu72]を参照してください。
>
>



* このコンポーネントを使用して、表示するバリエーションを選択できます。`fragmentPath``fragmentPath`
...
### コンテンツフラグメントは、次のフレームワークと統合できます。{#integration-with-other-frameworks}
...
* **コンテンツフラグメントは、AEM の翻訳ワークフローと完全に統合されています。つまり、アーキテクチャレベルでは以下を意味します。**
   ...
   * それぞれ異なる言語のルートの下に位置していますが、関連する言語ルートの下の同じ相対パスを共有します。
      ...
      * `/content/dam/<path>/en/<to>/<fragment>`
         ...
         `/content/dam/<path>/en/<to>/<fragment>`上記に対して次のようになります。

         `/content/dam/<path>/de/<to>/<fragment>`

         `/content/dam/<path>/de/<to>/<fragment>`ルールベースのパスを除き、コンテンツフラグメントの様々な言語バージョンの間にそれ以上のつながりはありません。UI には言語バリアント間を移動する手段が用意されていますが、それらは 2 つの別々のフラグメントとして扱われます。
   * [!NOTE]
...
* コンテンツフラグメントは、[メタデータのスキーマ](/help/assets/metadata-schemas.md)を（再）利用します。メタデータのスキーマは、標準アセットを使用して定義できます。
   ...
   * CFM には、次のような独自の固有のスキーマがあります。[](/help/assets/metadata-schemas.md)

   * `/libs/dam/content/schemaeditors/forms/contentfragment`

      `/libs/dam/content/schemaeditors/forms/contentfragment`必要に応じて拡張できます。

      各スキーマフォームは、フラグメントエディターと統合されています。

   * コンテンツフラグメント管理 API - サーバー側 {#the-content-fragment-management-api-server-side}

## サーバー側 API を使用して、コンテンツフラグメントにアクセスできます。以下を参照してください。{#the-content-fragment-management-api-server-side}

[com.adobe.cq.dam.cfm[#$tu111]



>[!CAUTION]コンテンツ構造に直接アクセスする代わりに、サーバー側 API を使用することを強くお勧めします。
>
>主要インターフェイス {#key-interfaces}

### 次の 3 つのインターフェイスが、入口の役割を果たします。{#key-interfaces}

**コンテンツフラグメント**（[ContentFragment[#$tu117]）
...
* 
   ...
   * 要素を一覧表示する
      ...
      * 要素データにアクセスする（`ContentElement` を参照）](#caveats)

      * そのフラグメントに対して定義されているバリエーションを一覧表示する`ContentElement`
   ...
   * 
      ...
      * バリエーションを一覧表示する
         ...
         * バリエーションを解決するためのショートカット（要素に指定されたバリエーションを使用できない場合は実装固有の追加のフォールバックロジックを適用）`ContentVariation`
      * **コンテンツバリエーション**（[ContentVariation[#$tu147]）
   * 
   ...
   要素の新しいバージョンを作成する`ContentFragment``ContentElement``ContentVariation``Versionable`
...
### 次のものを適応させることができます。{#adapting-using-adaptto}
...
* `Resource` - 基になる Sling リソース。基になる `Resource` を直接更新するには `ContentFragment` オブジェクトを再構築します。
   ...
   * `ContentElement` は、次のものに適応させることができます。`Asset``Asset``ContentFragment`
...
* データ構造を更新するには、`ContentFragment` から新しいバリエーションを作成します。
   ...
   * コンテンツフラグメント管理 API - クライアント側 {#the-content-fragment-management-api-client-side}`ContentFragment.removeVariation()`

## [!CAUTION]

>[!CAUTION]クライアントサイド API は内部用です。
>
>追加情報 {#additional-information}

### 次のファイルを参照してください。{#additional-information}
...
### 既存のコンテンツフラグメントにアクセスするには、API を表すリソースを以下に適応させます。{#example-accessing-an-existing-content-fragment}

`com.adobe.cq.dam.cfm.ContentFragment`

`com.adobe.cq.dam.cfm.ContentFragment`次に例を示します。
...
ノード：`<conf-root>/settings/dam/cfm/jcr:content`](/help/assets/content-fragments/content-fragments-managing.md#save-cancel-and-versions)

* プロパティ名：`autoSaveInterval`
* タイプ：`Long`
* デフォルト：`600`（10 分）。`/libs/settings/dam/cfm/jcr:content` で定義されています

* 自動保存間隔を 5 分に設定する場合は、次のようにノードにプロパティを定義する必要があります。`600``/libs/settings/dam/cfm/jcr:content`
...
## 詳しくは、次を参照してください。{#components-for-page-authoring}

[コアコンポーネント - コンテンツフラグメントコンポーネント[#$tu206]（推奨）
