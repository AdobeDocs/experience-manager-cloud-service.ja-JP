...
### 制限：マルチフィールドコンテナ内の条件 {#conditions-multi-field-limitation}

条件`var`の解決は絶対であり、現在の行に対する相対ではありません。 `var`は、ルートレベル `fieldName`か、コンテナ内のフィールドの先頭にコンテナ名`containerName|fieldName`を付ける必要があります。

同じアプローチは、複数のコンテナ（反復可能な行）に対しては機能しません。 行コンテンツは、実行時に`containerName/0|fieldName`、`containerName/1|fieldName`などとしてインデックスが作成されますが、そのインデックスは条件のオーサリング時には不明で、行が追加、削除、または並べ替えられるにつれて移動します。 その結果、作成者が同じ行内の特定のフィールドをターゲットにする方法がないので、マルチコンテナ内の条件はサポートされていません。
