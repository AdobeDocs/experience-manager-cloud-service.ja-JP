...
| `value` | `FieldValue` | これはデフォルト値で、プレースホルダーとして機能します。 値が設定されていない場合、ユニバーサルエディターは、モデル定義で`value`として定義されたものを保持します。 これにより、表示される内容が、バックエンドに保持される内容と一致することが保証されます。 | いいえ |
...
>[!NOTE]
>
>`_`または[`aem` プラグインを使用する場合、フィールド名にアンダースコア （`xwalk`）を使用することはできません。](/help/implementing/universal-editor/component-definition.md#plugins)
...
| 設定 | 値タイプ | 説明 | 必須 |
|---|---|---|---|
| `valueFormat` | `string` | 保存する数値の種類は`long` （既定値）または`double`です。 | いいえ |
...
![textarea コンポーネントタイプ ](assets/component-types/textarea.png)のスクリーンショット
