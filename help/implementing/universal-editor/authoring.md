...
* 以下が可能です。 [プレーンテキストを編集](#editing-content) 適切な場所に
...
When in [コンポーネントモード](#component-mode) を選択し、 [コンテンツフラグメント](/help/assets/content-fragments/content-fragments.md) 編集オプションがコンポーネントレールに表示されます。
...
ワークフローのニーズに応じて、コンポーネントパネル内でコンテンツフラグメントの詳細を編集することもできます。
