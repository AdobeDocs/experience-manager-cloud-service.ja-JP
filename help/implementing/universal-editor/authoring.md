...
コンポーネントモードでは、コンテンツ作成者は、次のようなコンポーネントを編集するために、クリックして選択できます。
...
また、テキストの詳細はプロパティパネルで使用できます。 変更は、フォーカスが編集されたフィールドをプロパティパネルに残すと、自動的に保存されます。
...
をタップまたはクリックします。 **置換** ボタンをクリックして、アセットライブラリ内の別の画像に置き換えます。
