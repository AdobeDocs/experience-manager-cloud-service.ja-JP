...
これをおこなうために、ユニバーサルエディターを使用すると、コンテンツ作成者は直感的な UI を使用できます。この UI では、最小限のトレーニングで、単にコンテンツをジャンプして編集を開始できます。
...
>[!NOTE]
>
>コンポーネントモードは現在開発中で、現在はコンポーネントの選択に制限されています。
...
選択したコンポーネントの詳細がパネルに表示されます。 表示する必要のある詳細が含まれていないコンポーネントもあることに注意してください。
...
現在のモードと選択したコンテンツに応じて、異なるインプレース編集オプションが表示される場合があります。 さらに、 [コンポーネントレール。](#component-rail)
