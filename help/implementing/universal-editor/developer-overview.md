...
1. ユニバーサルエディターのプロパティパネルにある **コンテンツツリー** アイコンをクリックすると、インストルメントを行ったページ上のすべてのティーザーが、エディターに認識されていることがわかります。 選択したティーザーがハイライト表示されています。
...
1. プロパティパネルをクリックして「プロパティ」タブを表示し、実装したフィールドを確認します。
   ...
   ![ 実装されたプロパティパネル ](assets/dev-properties-rail-instrumented.png)
...
1. プロパティパネルをクリックし、コンポーネントのスタイルを調整する新しいフィールドがあることを確認します。

   ![ 「スタイル」フィールドを含む実装されたプロパティパネル ](assets/dev-style-instrumented.png)
...
1. [プロパティパネルを使用するために、ティーザーコンポーネントのインストルメントを行いました。](#properties-rail-component)
