...
ユニバーサルエディターを探索し、独自のプロジェクトで実装を進める際は、次の制限に注意してください。

* 25 個を超えるAEM リソース（コンテンツフラグメント、ページ、エクスペリエンスフラグメント、アセットなど） 単一ページの実装として参照する必要があります。
* サポートされているAEM バックエンドはAEMas a Cloud Serviceのみです。
* AEMas a Cloud Serviceリリース `2023.8.13099` 以上が必要です。
* コンテンツ作成者は、独自のExperience Cloudアカウントを持つ必要があります。
* サポートされているブラウザーは Chrome と Edge です
