...
別のオプションとして、別々に設定された 2 つの異なる環境にアプリをデプロイし、オーサー層とパブリッシュ層それぞれからコンテンツを取得する方法があります。作成者がユニバーサルエディターで公開済み URL を開くことを許可するには、小さなスクリプトを作成して、パブリッシュ側 URL をオーサー環境で同等の URL に「変換」できます ( 例： `author` サブドメイン ) で分類されるので、作成者は自動的にリダイレクトされます。
