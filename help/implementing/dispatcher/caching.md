...
AEM as a Cloud Service CDN での HTTP 応答のキャッシュは、オリジンからの HTTP 応答ヘッダー `Cache-Control`、`Surrogate-Control` または `Expires` によって制御されます。
...
`Cache-Control` に `private`、`no-cache` または `no-store` が含まれるオリジン応答は、AEM as a Cloud Service の CDN によってキャッシュされません（詳しくは、[CDN キャッシュを無効にする方法](https://experienceleague.adobe.com/ja/docs/experience-manager-learn/cloud-service/caching/how-to/disable-caching)を参照してください）。また、Cookie を設定する応答、つまり `Set-Cookie` 応答ヘッダーを持つ応答は、CDN によってキャッシュされません。
...
厳密な clientlib のバージョン管理が有効な場合、クライアントライブラリに長期ハッシュキーがセレクターとして追加されます。結果として、clientlib リファレンスは次のようになります。

```
<link rel="stylesheet" href="/etc.clientlibs/wkndapp/clientlibs/clientlib-base.lc-7c8c5d228445ff48ab49a8e3c865c562-lc.css" type="text/css">
```

厳密なclientlib バージョン管理は、AEM as a Cloud Serviceでデフォルトで有効になっています。
...
### ロングキャッシュのクライアントライブラリが利用できない場合は、ショートキャッシュ URLにフォールバックします {#clientlib-shortcache-fallback}

中間キャッシュ（Adobe CDNやブラウザーキャッシュなど）は、新しいコンテンツが公開された後、しばらくの間、**古いHTML**&#x200B;応答を引き続き提供できます。 キャッシュされたHTMLは、デプロイメントがそれらのアーティファクトを置き換えたため、パブリッシュ層に存在しなくなったロングキャッシュ clientlib URL （`lc-` セレクターパターン）を引き続き参照できます。

この場合、AEM as a Cloud Serviceは&#x200B;**HTTP リダイレクト**&#x200B;を&#x200B;**ショートキャッシュ**&#x200B;のclientlib URLに発行します。 ショートキャッシュ URLでは、`sc-` セレクターパターンと時間ベースのセグメントを使用して、リクエストが現在利用可能なクライアントライブラリの&#x200B;**最新** バージョンに解決できるようにします。
...
ショートキャッシュ URLへの&#x200B;**リダイレクト**&#x200B;と&#x200B;**ショートキャッシュ clientlib応答**&#x200B;自体の両方が短期間有効です。応答では、最大キャッシュ時間が&#x200B;**60秒** （`max-age=60`）です。 **ダウンストリームキャッシュ** （顧客管理のCDN、プロキシなど）は、これらの応答を&#x200B;**not** キャッシュする必要があります。 TTLを増やしたり、短キャッシュのclientlib URLを不変の長期アセットとして扱ったりするカスタムキャッシュルールを使用しないでください。これにより、古いJSやCSSでユーザーがストランドされる可能性があります。

この動作は、**クライアントライブラリがHTMLと後方互換性を維持する必要があることを意味します** HTMLがキャッシュされている限り、キャッシュから引き続き提供される可能性があります。 実際には、HTML キャッシュで適度なTTLが使用されている場合（上記[のデフォルトを含む](#html-text)）、この問題が発生することはほとんどありません。 HTML キャッシュの有効期間を&#x200B;**非常に長く**&#x200B;設定する場合、ユーザーがショート キャッシュのフォールバックを通じて配信される新しいJSおよびCSSと共に古いマークアップを読み込む可能性があることを考慮してください。
