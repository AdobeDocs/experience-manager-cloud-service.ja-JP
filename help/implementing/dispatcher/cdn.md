...
1. `X-AEM-Edge-Key` を設定します。 値は、最初にCloud Manager設定パイプラインを使用して設定し、次に[この記事](/help/implementing/dispatcher/cdn-credentials-authentication.md#CDN-HTTP-value)で説明されているように、お客様のCDNで同じエッジキーを設定する必要があります。

   * これは、Adobe CDN でリクエストのソースを検証し、`X-Forwarded-*` ヘッダーを AEM アプリケーションに渡すために必要です。 例えば、`X-Forwarded-For` を使用してクライアント IP を決定します。 したがって、`X-Forwarded-*` ヘッダーが正しいことを確認するのは、信頼できる呼び出し元（顧客管理 CDN）の責任となります（以下のメモを参照）。 `x-aem-debug`](#test-forwarded-headers)を使用して転送されたヘッダーをテストする方法については、[を参照してください。
...
この顧客CDN設定は、パブリッシュ層とプレビュー層でサポートされていますが、オーサー層の前にはサポートされていません。
...
#### x-aem-debugを使用した転送ヘッダーのテスト {#test-forwarded-headers}

訪問者が転送ヘッダー（`X-Forwarded-For`、`X-Forwarded-Host`、`Forwarded`）を制御できないことをテストするために、AEMで管理されるCDNは、訪問者指定の値をクリアし、信頼できる値を設定します。 ランダムな値でサイトを呼び出し、`x-aem-debug`応答ヘッダーを調べます。
...
`www.example.com`をサイトのドメインに置き換えます。 `x-aem-debug`応答ヘッダーは、サイトのホストとクライアント IPを反映する必要があります。送信した値は表示されません。 次に例を示します。
...
#### エラー421のリダイレクトが正しくありません {#error-421}
