...
>title="お客様の CDN （またはプロキシ）がAEMの管理による CDN を指している"
...
お客様が既存の CDN （またはロードバランサーやWAFなどの任意の種類のリバースプロキシ）を使用する必要がある場合は、それを管理してAEMが管理する CDN を参照できます。ただし、次の条件を満たす必要があります。
