...
## 秘密鍵のデプロイ：環境変数とパイプライン変数 {#deploying-secrets}

CDN 設定で使用するシークレットは、次の 2 つの方法でデプロイできます。

* **パイプラインシークレット変数** - Cloud Managerでタイプ [&#x200B; シークレット &#x200B;](/help/implementing/cloud-manager/configuring-pipelines/pipeline-variables.md) の **パイプライン変数** として設定され、**適用されたステップ** が **デプロイ** に設定されています。 これらは、設定パイプラインレベルの設定として使用できます。

* **環境シークレット変数** - Cloud Managerでタイプ [&#x200B; シークレット &#x200B;](/help/implementing/cloud-manager/environment-variables.md) の **環境変数** として設定され、**サービスが適用** が **すべて** に設定されています。 これらは、環境レベルの設定として使用できます。

**推奨：パイプラインのシークレット変数。** 可能な場合は、パイプラインのシークレット変数を使用します。これは、同じパイプライン実行で設定と共にデプロイされるからです。 これにより、シークレットと設定を同期させ、ロールアウトを簡素化できます。

同じ設定に対して、パイプラインシークレットと環境シークレットを混在させることはできません。 パイプラインシークレットの変数がデプロイステップに対して定義されている場合、それらは環境設定で使用されます。

次の画像は、Cloud Managerでパイプラインシークレット変数を設定する方法を示しています。

![&#x200B; パイプライン秘密鍵変数の設定 &#x200B;](/help/implementing/dispatcher/assets/pipeline-secrets-configuration.png)

パイプライン変数（シークレットを含む）の追加、編集、管理について詳しくは、[Cloud Managerのパイプライン変数 &#x200B;](/help/implementing/cloud-manager/configuring-pipelines/pipeline-variables.md) を参照してください。

## 秘密鍵の操作に関するガイドライン {#secrets-guidelines}
