...
1. ブラウザーに入力されたURL
2. DNSにマッピングされたCDNに対する要求がそのドメインに対して行われました
3. コンテンツがCDN上で完全にキャッシュされている場合、CDNはコンテンツをブラウザーに提供します
4. コンテンツが完全にキャッシュされていない場合、CDNはディスパッチャーに（リバースプロキシ）を呼び出します
5. コンテンツがディスパッチャー上で完全にキャッシュされている場合、ディスパッチャーはそのコンテンツをCDNに提供します
6. コンテンツが完全にキャッシュされていない場合、ディスパッチャーはAEM発行に対して呼び出し（リバースプロキシ）を行います
7. ブラウザーでレンダリングされたコンテンツ

### CDN {#cdn}

AEMには次の2つのオプションがあります。

1. 管理CDN - AEMの標準搭載CDN。
2. 管理対象外のCDN — お客様は独自のCDNを提供し、その管理を完全に担当します。

1つ目のオプションは強くお勧めします。2つ目のオプションを使用する場合、設定ミスの結果はアドビでは責任を負いません。

#### 管理CDN {#managed-cdn}

アドビがCDNをプロビジョニングした後、CNAMEを作成し、管理対象CDNドメインでホストされるアドビ制御ドメインにアプリケーションのドメインをマッピングする必要があります。

CDNはレイヤ7 webアプリケーションファイアウォールの役割を果たし、SSLの終了が必要なので、顧客が署名したSSL証明書が必要です。 プレリリース段階では、手動で証明書をアドビに提供する必要があります。

GA時に、証明書をCloud Managerにアップロードし、CDNにアップロードする必要があります。 SSL証明書の有効期限が近づくと、Cloud Managerで証明書を更新できるように通知されます。

#### 非管理CDN {#unmanaged-cdn}

以下の場合は、独自のCDNを管理できます。

1. 既存のCDNがある。
2. 君がやる
3. AEMアーキテクチャにまだ対応していないCDNに対する広範なAPI呼び出しは、アプリケーションで行われません。
4. クラウドサービスとしてのAEMでは、エンドツーエンドのシステムが正しく機能することを確立できます。
5. 許可するCDN URLのホワイトリストをアドビに提供する必要があります。

アドビは、CDNの元のAEM cloudのURLを提供します。
...
AEMをクラウドサービスとして使用する前は、ディスパッチャーキャッシュを無効にする方法が2通りありました。

1. 発行ディスパッチャーフラッシュエージェントを指定して、複製APIを呼び出します
...
代わりに、レプリケーションAPIアプローチを使用する必要があります。 [APIのドキュメントが用意され](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/com/day/cq/replication/Replicator.html) 、キャッシュのフラッシュの例については、 [](https://helpx.adobe.com/experience-manager/using/aem64_replication_api.html) APIの例ページを参照し、特に、使用可能なすべてのエージェントに対してACTIVATEタイプのレプリケーションアクションを発行するCustomStepの例を参照してください。
...
この公開トリガーによる無効化は、クイックスタートと同じです。発行インスタンスは、（複製とパイプラインキューを介して）作成者から新しいバージョンのページまたはアセットを受け取ると、フラッシュエージェントを使用してディスパッチャー上の適切なパスを無効にします。 更新されたパスは、親と共に、 [statfilelevelで設定できるレベルまで、ディスパッチャーキャッシュから削除さ](https://docs.adobe.com/content/help/en/experience-manager-dispatcher/using/configuring/dispatcher-configuration.html#invalidating-files-by-folder-level)れます。
