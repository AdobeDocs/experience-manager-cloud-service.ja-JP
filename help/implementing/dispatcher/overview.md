...
デフォルトでは、コンテンツタイプHTML/textは、ディスパッチャーレイヤーで300秒（5分）後に有効期限切れに設定されます。これは、ディスパッチャーキャッシュとCDNの両方が優先するしきい値です。 パブリッシュサービスの再デプロイメント中に、Dispatcher のキャッシュがクリアされ、その後、新しいパブリッシュノードがトラフィックを受け入れる前にウォームアップされます。
