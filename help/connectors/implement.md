...
以下はAEM コネクタの構築に役立つリファレンスであり、コネクタの [ 送信 ](submit.md) および [ 保守 ](maintain.md) に関するガイダンスと共にお読みください。
...
* AEM から外部システムにデータを書き出す。例えば、AEMを利用した web サイトでニュースレターの購読設定が CRM に送信されるとします。
* AEM からアセットを取得する。例えば、AEM Assets に保存されているアセットを外部のコンテンツ管理システム（CMS）で参照するような場合です。または、別の例として、AEM Assets内の画像にリンクする PIM システムを使用する場合もあります。
...
* AEM Admin Consoleでのサイト、ページ、アセットの分析。 例えば、既存ページまたは未公開ページの SEO のレコメンデーションを提示するような場合です。
...
ローリングデプロイメントを容易にするために、コネクタなどのAEM as a Cloud Service パッケージでは、「不変」コンテンツと「可変」コンテンツを厳密に分けています。 パッケージは、次の内容を含むように明確に編成する必要があります。
...
Adobeは、特に複数のコネクタを使用しているお客様がクリーンなリポジトリー構造を維持できるように、ほとんどのコネクタコードを `/apps/connectors/<vendor>` の下に配置することをお勧めします。

クラウドサービスの設定
-----------------------------

コネクタ実装の 1 つの側面は、コネクタの設定を支えるコードです。 このコードにより、ツール／操作／Cloud Services で、コネクタ名の付いたカードが表示されるようになります。カードをクリックすると、[設定ブラウザー](/help/implementing/developing/introduction/configurations.md#using-configuration-browser)がポップアップ表示され、コネクタ設定を含んだ親フォルダーをユーザーが選択できます。コネクタのコードによって、設定が必要なすべてのプロパティを含んだフォームが生成され、最終的に、`/conf` の配下の設定フォルダーにプロパティ値が格納されます。このフォルダーは、後ほど「サイトのプロパティ」タブまたは「アセットのプロパティ」タブで選択できます。


コンテキスト対応の設定
-----------------------------

[ コンテキスト対応設定 ](https://sling.apache.org/documentation/bundles/context-aware-configuration/context-aware-configuration.html) を使用すると、`/conf` の下の `/libs`、`/apps`、`/conf`、サブフォルダーなど、様々なフォルダーをまたいで設定をレイヤー化できます。 継承をサポートしているので、ユーザーがグローバル設定をおこないながら、マイクロサイトごとに個別に変更を加えることができます。この機能をCloud Service設定に使用できるので、コネクタコードは、特定の設定ノードを参照するのではなく、コンテキスト対応の設定 API を使用して設定を参照する必要があります。

変更した設定がコネクタで使用される場合は、コネクタ提供のデフォルト設定に対する今後の更新をユーザー側のあらゆる設定と結合できるようにコネクタを設計します。顧客がカスタマイズしたコンテンツや設定を事前の通知や同意なしに変更すると、そのコネクタで予期しない動作が混乱したり引き起こしたりする可能性があることに注意してください。

コーディングのベストプラクティス
----------------------

AEM as a Cloud Serviceはクラウドネイティブなソリューションなので、コネクタのコード戦略に影響を与える可能性のあるガイドラインがいくつかあります。 詳しくは、[AEM as a Cloud Service の開発ガイドライン](/help/implementing/developing/introduction/development-guidelines.md)を参照してください。

AEM コネクタのテスト
-------------------------

ローカル環境の開発テクニックを使用して新しいコネクタを作成（または既存のコネクタを変更）する必要があります。パートナーチームは、ISV パートナーにサンドボックス環境を提供し、AEM コネクタをバニラアプリケーションにデプロイして、確実に機能するようにします。
